...
- [ ] **2.22d — сигналы, которые никому не доходят** (D8 + D10) · оператор + chart + CLI
  - [ ] D8: `CapacityWarning` — это диск НОДЫ (`node_free_fraction` = `/node/fs/*`), но
  ...
  - [ ] D8: предупреждать на КАЖДОЙ команде, жёлтым, по форме
  ...
  - [ ] D8: отпорожить по-томовый семпл, который УЖЕ берётся (`pvc_usage` → `status.capacity`,
        но `is_capacity_warning` принимает только node-долю) и показать в `volume status`.
  - [ ] D8: колонка состояния в таблице клеймов `app status` (`print_resource_claims`,
  ...
  - [ ] D10: сделать walk способным упасть — засеять приложение ВЫШЕ пола, утверждать на
        `status.containerStatuses[].resources` (не на `spec`), hard-fail вместо лога.
  - [ ] D10: оживить `notApplied` (единственный прод-вызов хардкодит `infeasible: false`,
        `lib.rs:870`; CRD-поле, тип и рендер CLI построены и недостижимы).
  - [ ] D10: преflight k8s-пререквизита так же, как это делает путь смены ноды
