...
- [x] **2.22f — долговечность ACL Dragonfly** (D5) · ADR-first
  ...
  > чем закрывает §11.
- [ ] **2.22g — расписание бэкапа: заменить поверхность, а не валидировать** (D2) · CLI + schema + chart
  ...
  - [ ] `--at 03:00` вместо `--cron`. Таймзона по умолчанию — **машины оператора**, `--timezone`
  ...
  - [ ] `backup status` печатает расписание обратно **в той же зоне**, в которой его задали.
  - [ ] Частота — продуктовое решение (ежедневно бэкап, еженедельно проверка). Если понадобится
        второе значение, это `--every day|week`, а не поле cron.
  - [ ] `--check off` настоящей веткой (`checkSchedule: ""` → omit в чарте), чтобы отключение
  ...
  - [ ] **Таймзона сейчас не выставлена и не названа нигде** — ни в чарте, ни в схеме, ни в CLI,
  ...
  - [ ] Сырой cron как escape hatch допустим позже, но не единственной поверхностью и не той,
        которой учит гайд.
