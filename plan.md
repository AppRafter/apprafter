...
- [ ] **2.23k — порядок операторского nav и распил target-store** · docs-only
  - [ ] Порядок подтверждён неверным по всем трём пунктам. `Secrets` **вверх с 16 на 9**:
  ...
  - [ ] `Target store reference` не одна вещь: примерно 40% — механика (раскладка на диске,
        сериализация, цепочка разрешения кредов), 60% — рецепт (add / rotate / show / rename /
        remove). Переносить целиком нельзя, надо **делить**.
  - [ ] **Рецептная половина сохраняет имя файла** `operator-guide/target-store.md` (заголовок
  ...
  - [ ] Механическая половина → `how-it-works/the-target-store.md`, вместе с обеими цитатами
