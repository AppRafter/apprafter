...
- [ ] **c. Рантайм.** pg-клиент в провижионере (исполнить готовые строители), ACL redis для общего
      `$N` (общий канальный префикс + явные `-publish -spublish` у ro), контроллер `SharedDatabase`
      (провижининг + **выводимый** `refCount` + отказ `rm` при ненулевом), условие
      `ExtensionUnavailable` поверх `pg_available_extensions`, CLI `apprafter db`, триггер ADR 0052
