...
- [ ] **2.23d — правда на страницах** · docs-only, без переездов файлов; **после 2.23a**
  - [ ] `migration-plans.md` — admonition (`:67-76`, считая пустую строку на `:67`: именно она
  ...
  - [ ] Тот же «Known gap: approving a `needs.*` removal does not release the data»
  ...
  - [ ] Блоки про более ранние билды не нужны до 1.0 (`:16`): `quickstart.md:305`
  ...
  - [ ] Фраза, дезавуирующая Amazon упоминанием Amazon (`backup-restore.md:568`, повтор `:614`).
  ...
  - [ ] `Infrastructure.cue` в quickstart (`:126-151`): убрать 20-строчный `infra.cue` и
  ...
  - [ ] Пикер машины: quickstart подаёт `--server-type` обязательным флагом и шлёт за выбором в
  ...
  - [ ] `doctor` — в преквизиты, до шага 1. Он **уже** годится: после 2.22a (`18d378d`)
  ...
  - [ ] Проход по корпусу `bootstrap-all` → `up` и снятие ложного утверждения про
