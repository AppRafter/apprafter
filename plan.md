...
> 🚧 **В РАБОТЕ 2026-08-08** (ветка `feat/2.16e-vpa-autoscaling`; спек `docs/superpowers/specs/2026-08-08-2.16e-vpa-design.md` (4 раунда внешнего ревью) + план `docs/superpowers/plans/2026-08-08-2.16e-vpa-autoscaling.md`, **ADR 0054**). Уточнения от ревью, отменяющие исходные формулировки ниже: **`updateMode: InPlace`** (не `InPlaceOrRecreate` — тот evict'ит single-replica под при infeasible resize, ломая RESTARTS=0); **трёхзначный кноб `PlatformStack.spec.resources.autoscale.mode ∈ {full, up-only, off}`, ДЕФОЛТ `full`** (двунаправленный enforcing — НЕ «старт в Off»; `up-only` = только вверх, `off` = observe+mirror); оператор эмитит **один** `VerticalPodAutoscaler` на managed CR-env (`containerName:"*"`, RequestsOnly, `minReplicas:1`, minAllowed=seed 32Mi/25m, maxAllowed.memory=512Mi), prune на managed→pro; зеркало `status.recommendation`→`Application.status.recommendedResources` (+`app status`); чарт `vertical-pod-autoscaler` 0.11.0 (VPA 1.7.1), self-managed cert (certGen, без cert-manager), `failurePolicy:Ignore`, feature-gate `InPlaceOrRecreate=true` (alpha), metrics-server = hard-prereq. Память корректируется только в `[32Mi,512Mi]` (RequestsOnly cap на seed-лимите). Обесценивает D2-budget-close 2.16d (перемер headroom в walk). op v0.2.39 / ps 0.2.49 / cue-cmp 0.1.20 / cli 0.2.39.

**Поставка:**
- [ ] VPA в platform-stack (recommender + updater + admission-controller). Mode `InPlace` (см. 🚧-заметку выше: `InPlaceOrRecreate` отвергнут, трёхзначный кноб, дефолт `full`).
