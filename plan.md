...
- [ ] **2.22f — долговечность ACL Dragonfly** (D5) · ADR-first
  - [ ] **Сначала поправка к ADR 0042**: он отклонил файловые ACL с условием «revisit if
        ...
        `&user:*` работают).
  - [ ] Билдер файловой формы + байтовое сравнение с `SETUSER`-argv. Осторожно:
  ...
  - [ ] Per-instance Secret + `spec.aclFromSecret`. **Ловушка:** `optional` принимается схемой
  ...
  - [ ] Строка `default` обязательна и её ошибка запирает провизионер снаружи его же инстанса
        — единственное здесь, что нельзя выпускать без живой проверки.
  - [ ] **Один писатель**: файлом владеет петля ресинка; `provision`/`gc` продолжают
  ...
  - [ ] Ужесточить walk: Phase 9/10 `needs-redis-walk.sh` УЖЕ рестартят инстанс (`:1203`,
  ...
  - [ ] Отдельно: образ сервера Dragonfly платформой НЕ пинится (тег берётся из
