...
- [ ] **2.23e — дока проще: порядок под флоу и распил разросшегося** · docs-only
  - [ ] `mkdocs-redirects` включается **первым** и отдельно. Плагин лежит в `flake.nix:75`
        именно под это, но в `mkdocs.yml` `plugins:` его нет — сегодня любой переезд страницы
        молча 404-ит опубликованный URL.
  - [ ] `backup-restore.md` — 989 строк, вдвое длиннее следующей операторской страницы. Оба
  ...
  - [ ] **Решение владельца (2026-09-05): смена машины — отдельная страница.** Операция редкая и
  ...
  - [ ] Заодно, раз эти секции всё равно переписываются: `backup enable --endpoint/--prefix`
  ...
  - [ ] `Node preparation` — из setup-пути в day-2. На хэппи-пасе не нужен: резервирования и
  ...
  - [ ] `Publish the documentation site` — из операторского гайда в Contributing. Это внутренний
  ...
  - [ ] nav обоих гайдов под флоу владельца. Сегодня он упорядочен по семействам артефактов:
