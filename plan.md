...
- [ ] **`tracing` logs идут в stdout вместо stderr.** Любая команда, чей output читается машинно (`kubeconfig | kubectl --kubeconfig=/dev/stdin …`, `kubeconfig > /tmp/kc`, `argocd-password | …`), ломается, потому что первая строка stdout это `2026-…Z INFO command invoked …` от tracing-subscriber, а не валидный YAML/пароль. Чинится в `platform-cli/src/main.rs` (или где `tracing_subscriber::fmt::init()` зовётся): `tracing_subscriber::fmt().with_writer(std::io::stderr).init()`. Один-line фикс + smoke-тест в `tests/cli_smoke.rs`, что `kubeconfig` stdout начинается с `apiVersion:` (не с timestamp). Affected commands: `apply`, `destroy`, `import`, `kubeconfig`, `argocd-password` — все, у кого machine-readable stdout.
