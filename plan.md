...
- [ ] **c. Живой прогон трека B.** Цепочка «манифест → claim → NACK → NATS → DLQ наполнился» не
      пройдена ни разу. Нужен полный `cluster-bootstrap`, а он ставит Cilium, который не поднимется
      под rootless podman (известный memlock-блокер). Три части — имена полей против CRD, поведение
      сервера, эмиссия провижионером — покрывают цепочку по кускам, но не связку. Строка в
      `docs/status.md` до прогона — `🚧`.
