...
- [ ] **2.23e — дока проще: порядок под флоу и распил разросшегося** · docs-only
  ...
  - [~] nav обоих гайдов под флоу владельца. **Операторский — сделан**; гайд разработчика
        ждёт распила `image-iteration.md` и страницы «исходник → образ → тег». Сегодня он упорядочен по семействам артефактов:
  ...
  - [ ] `image-iteration.md` несёт две разные точки чтения: «что платформа делает с моим тегом»
  ...
  - [ ] Две дыры флоу разработчика, где содержимого нет **вообще**: докеризация (шаг 3) и рецепт
  ...
  - [ ] **Блокер, который надо снять ДО первой такой страницы:**
  ...
  - [ ] Всё, что решено **не** писать сейчас, уезжает в честный список «Not documented yet»
