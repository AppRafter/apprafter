...
доки, не выполняет задачу. Счёт по 2.23e: −3 страницы из операторского setup-пути, три новых
файла (два из них — распил существующих), суммарно **минус ~450 строк**.
...
- [ ] **2.23b — установка, которая работает** · лендинг + дока, без bump CLI
  ...
  - [ ] **Резолв версии — не деталь реализации, а суть.** Монорепо публикует пять серий
        ...
        `operator/v0.1.134`. `README.md:97` и оба quickstart (`operator-guide/quickstart.md:41`,
        `dev-guide/quickstart.md:38`) берут версию именно так. CLI уже умеет правильно —
        `version_check.rs:147` `pick_canonical_cli_tag`; `install.sh` повторяет ту же фильтрацию,
        три сниппета в доке чинятся тем же коммитом.
...
- [ ] **2.23c — лендинг: упоминания фаз** · один контент-проход + один гейт
  ...
  - [ ] **Знак `+` убирается.** Он значил «эта фаза или позже» и приехал из внутреннего
        ...
        (у 3 и 4 его нет, у 5/6/8 есть, а `Phase 4.5+` — третий смысл и вне реестра). 18 строк:
...
- [ ] **2.23d — правда на страницах** · docs-only, без переездов файлов; **после 2.23a**
  - [ ] `migration-plans.md` — admonition (`:68-76`) вставлен **внутрь** таблицы: она
  ...
  - [ ] Тот же «Known gap: approving a `needs.*` removal does not release the data»
        **неактуален**: 2.22b (`a4cc7ef`, operator/v0.2.45) научил оператор удалять незаявленные
        ...
        `redis.md:152`, `persistent-disk.md:136`. Проверить заодно ADR 0051 и `spec.md`:
        коммит `651c0db` правил их в ту же сторону.
...
- [ ] **2.23f — хром сайта документации** · docs-only
  ...
  - [ ] 23 ссылки на 12 страницах показывают имя файла с `.md` как видимый текст, хотя href
...
- [ ] **2.23g — CI следит за версиями внешних компонентов** · CI-only, без bump
  - [ ] Автоматики нет никакой: ни `dependabot.yml`, ни Renovate, ни одной из 29 workflow,
        проверяющей апстрим. ~60 внешних пинов в семи синтаксисах, большая часть — осень 2024
        в дереве от 2026-09.
  - [ ] Dependabot закрывает 4 класса из 8 и **не видит** ни CUE-пины helm-чартов, ни входы
        flake, ни `with: version:` в workflow, ни `curl .../releases/download/vX/` — то есть
        большую часть риска. Renovate закрывает те же 4 плюс `nix`/`helmv3` плюс
        `customManagers` по регуляркам.
