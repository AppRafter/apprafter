...
- [x] **(f) Информационная архитектура** *(= W5)*. Ветка `feat/2.19f-information-architecture`, план `docs/superpowers/plans/2026-08-19-docs-2.19f-information-architecture.md`; ADR 0057 получил amendment по факту поставки. **Ни релиза, ни монорепо-тега:** `cli/platform-cli` не тронут ВООБЩЕ, поэтому `apprafter --help` и байт-сверяемый `commands.json` не двигались; единственные правки Rust — комментарии в `cli/docsgen`, называвшие переименованные страницы (комментарий, указывающий на несуществующий файл, — ровно тот класс дефекта, ради которого весь этот гейт и строился).
  **1. Доки перестали говорить с инсайдерами.** Внутренние координаты роадмапа («в фазе 8.2»), коды подфаз и указатели в `spec.md` переписаны в терминах читателя либо удалены там, где предложение и без них полное; где спецификация действительно и есть ответ — абсолютный GitHub-URL, и **само предложение говорит, что это roadmap репозитория**, чтобы читатель понимал, что уходит из документации. Число не записываем — записываем команду: `git ls-files -- 'docs/*.md' | grep -vE '^docs/(adr|changelog|measurements|superpowers)/' | xargs grep -nEi '\bphase [0-9]|\b[0-9]+\.[0-9]+[a-z]\b|\bplan\.md\b|\bspec\.md\b'` даёт **25** на `fa3472e` и **7** на закрытии. Эти семь — осознанные и все разные: четыре абсолютных URL-цитаты спецификации; одна — `contributing/documentation-gate.md`, где `spec.md` назван **путём репозитория** в таблице путей, которые гейт исключает (https-ссылка заставила бы страницу описывать собственный скоуп в терминах, которых она не использует); и две — link-таргеты сгенерированного референса на страницу команды `apprafter plan`, то есть **ложное срабатывание паттерна, а не утечка**. Команда осознанно ограничена `docs/`: `README.md` входит в корпус гейта, но не является страницей сайта, и ссылки на роадмап там для читателя РЕЗОЛВЯТСЯ — человек на титульной странице репозитория может открыть `plan.md`.
  ...
  **5. Карта редиректов НЕ построена — решение с замером, а не недосмотр.** Редиректить не от чего: ни один URL этого сайта никогда не был публичным (сайт не опубликован, ни один workflow его не публикует, `docs.apprafter.dev` не встречается ни в одном деплой-исходнике, лендинг рендерит «Soon» из пустого `docsUrl`). Запись отображала бы URL, которого никто никогда не держал, в URL, до которого пока никто не может дойти. **Триггер назван явно: первая страница, переехавшая ПОСЛЕ публикации (2.19g), требует записи**, а `mkdocs-redirects` уже лежит в python-окружении флейка — это блок конфига, а не смена зависимости. Один класс URL публичным БЫЛ и никакой картой редиректов не обслуживается в принципе: относительные ссылки `README.md` в `docs/` — это GitHub-blob-URL'ы, куда плагину MkDocs не дотянуться; единственное лекарство — правка README тем же коммитом, что и переименование (сделано).
