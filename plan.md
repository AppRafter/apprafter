...
- [ ] **2.22h — отказ реконсиляции должен быть виден без чтения логов** (D16) · оператор + CLI
  ...
  - [ ] Важное различие: **спроектированные** отказы surface'ятся хорошо
  ...
  - [ ] `error_policy` (`application/src/lib.rs:1018-1031`) целиком: `warn!` + два счётчика +
  ...
  - [ ] Событие из `error_policy` — это **единственное узкое место**, одно изменение покрывает
        все 48 путей (то же плечо, что классификаторы в 2.22a). Даром достаются dedup, `count`,
        `firstTimestamp`/`lastTimestamp` и TTL на стороне apiserver — «предусмотреть
        устаревание» не требует нового механизма. **RBAC:** у оператора только `create` на
        events (`rbac.yaml:521-526`), чтение требует `get`/`list` — глагол едет с кодом.
  - [ ] Ограниченный `status.recentProblems[]` рядом: TTL событий (час по умолчанию) короче
  ...
  - [ ] Печатать в `app status` и `platform status`. **Ни одна CLI-поверхность сегодня не читает
        события вообще** — платформа эмитит два, которых никто никогда не показывал.
  - [ ] Две ловушки: повторяющаяся ошибка обязана **дедуплицироваться, а не спамить** (иначе
  ...
  - [ ] Walk: намеренно вызванный отказ (403 — очевидная фикстура, воспроизводится удержанием
