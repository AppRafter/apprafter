...
- [ ] **2.23o — два класса гейта, на разных расписаниях** · после 2.23n
  - [ ] Измерено: 89 цитат, из них **49 ссылок, 37 голым текстом, 3 в бэктиках**, ноль внутри
        ...
        расширением `adr-reference` не делается.
  - [ ] `adr-citation-form` — **рождается зелёным**, одним коммитом: на гайдах голых цитат всего 14,
        а после снятия тринадцати «упоминаний» из 2.23n останется около пяти.
  - [ ] `adr-citation-placement` — **рождается красным**, по образцу recipe-purity: класс есть,
        но включается в `recipe_findings` и выводится отдельной подкомандой отчёта, чтобы lefthook
        и `just lint` оставались зелёными, пока пишется слой механики. Зелёным его сделать
        невозможно: 29 цитат некуда переносить, пока страниц нет.
