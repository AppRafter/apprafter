...
- [ ] **e. Рантайм — код закрыт, прогон не пройден.** Контроллер `SharedDatabase` (обе ветки: pg через
      ...
      регрессия `crd-validate`. Написан `e2e/shared-database-walk.sh` — **не пройден**, и пункт
      остаётся открытым именно поэтому: «код готов» это не «работает», и в этом же треке уже был
      ключ, который принимался всеми гейтами и не делал ничего. Строка в `docs/status.md` — `🚧`.
