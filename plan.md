...
> 🚧 **В РАБОТЕ — из десяти подфаз закрыты ДВЕ (2.19a + 2.19b).** Дизайн: `docs/superpowers/specs/2026-08-17-documentation-system-design.md` (rev 2 — переписан после 4-линзового adversarial-ревью: 8 P0 + 40+ P1, три собственных фактических утверждения спека оказались неверны и исправлены). Ветка `feat/2.19-docs-system`. **ADR ратифицирован постфактум:** `docs/adr/0057-documentation-system.md` (`Accepted`, 2026-08-18) — долг трека закрыт с опозданием (спек §8 требовал ADR перед 2.19b, а он написан после неё; сам ADR это фиксирует и помечает каждое ещё не реализованное решение). **Ни релиза, ни монорепо-тега:** поведение CLI не изменилось (lib-таргет + правки doc-comment'ов), чарты/оператор не тронуты, сайт всё ещё не публикуется (2.19g).
...
- [x] **(b) Generated CLI reference** *(= W1)*. У `cli/platform-cli` появился lib-таргет: `dispatch` уехал в `src/dispatch.rs`, `main.rs` — тонкий шим над `apprafter::run()`; публичная поверхность = `run()` + фасад `docs_api` из двух пунктов (осознанное расширение API, а не побочный эффект рефактора). Новый крейт `cli/docsgen` генерирует `docs/reference/cli/**` — 26 страниц по top-level командам, индекс, literate-nav `SUMMARY.md` и машиночитаемый `commands.json` на все 95 command-path'ов — а `docsgen check` **байт-в-байт** сверяет их с clap-деревом в `just lint`, lefthook и CI: переименованный флаг теперь роняет сборку. Рукописный `docs/reference/cli.md` (15 из 27 команд, при этом заявлявший полноту) удалён; новый `docs/reference/environment.md` описывает 20+ переменных окружения, которых генератор не видит (их читают `std::env::var` или зависимости). Генератор сработал как аудит doc-comment'ов: `status`/`login`/`upgrade-tier` теперь **первым предложением** признаются скелетами, документированы семь ранее безымянных флагов `backup enable` и `platform upgrade`, у `--no-ping` убрано ложное «любое непустое значение», а внутреннее Rust-имя `nodes[0].kind` заменено на реальный ключ манифеста `type` в трёх help-строках и в `operator-guide/troubleshooting.md` (там оно учило операторов писать непарсящийся ключ).
- [ ] **(c) Drift-гейт по гайдам** *(= W2)*. Fence/inline/table/prose-гейт (`check=cue|cli|shell`, включение реальных файлов вместо копий, forbidden-claims lint) + фикстуры, воспроизводящие все шесть исторических дрифтов §1.3 как падения; затем починка всех действующих нарушений.
...
**Acceptance закрытых подфаз:** *(a)* `just docs-check` и `just docs-build` зелёные в чистом checkout'е через `nix develop`; ни одной страницы вне nav без записи в allow-list; внутренние страницы не в `site/`. *(b)* `docsgen check` роняет `just lint` на любой правке clap-дерева без `just docsgen-generate`; `commands.json` покрывает все 95 путей; `reference/cli.md` удалён вместе с четырьмя входящими ссылками.
