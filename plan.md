...
- [x] **(g) Публикация** *(= W6)*. Ветка `feat/2.19g-publication`, план `docs/superpowers/plans/2026-08-19-docs-2.19g-publication.md`; ADR 0057 получил amendment по факту поставки, решение 8 перестало быть «decided, not yet implemented». **Ни релиза, ни монорепо-тега, и ответ выведен, а не предположен:** `git diff --name-only <base>..HEAD -- cli/ operator/ schemas/ platform-stack/ argocd-cue-cmp/` пуст, то есть `apprafter --help` байт-идентичен, байт-сверяемый `commands.json` не двигался и ни один чартовый артефакт не менялся; `cli/Cargo.toml` бампается под РЕЛИЗНЫЙ тег в том же коммите, что его режет, а docs/CI-работа тега не режет. Новый образ `ghcr.io/apprafter/docs` версионируется SHA коммита, а не git-тегом, так что публикация не заводит и собственного тег-стрима.
  ...
  **6. Лендинг-переключатель — отдельный коммит, и порядок в нём назван.** `landing/web/src/data/fallback/siteSettings.json` — это РЕЛИЗНЫЙ путь (образ собирается с `LANDING_USE_FALLBACK=1`), а глобал Payload — превью-путь; `Header.astro` подставляет `<githubUrl>#readme` и вешает бейдж «Soon», только пока `docsUrl` пуст. Значение не набрано по памяти: оно байт-идентично `site_url` из `mkdocs.yml`, а его хост равен `expose.hostname` в `docs-site/apprafter/Application.cue`. Проверено самим релизным путём — `astro build` под `LANDING_USE_FALLBACK=1`, якорь nav вычитан из `dist/index.html`: с пустым значением рендерится `#readme` + бейдж, с заполненным — живая ссылка без бейджа (A/B на одном и том же дереве, чтобы проверка отличала состояния, а не подтверждала себя). Коммит держится отдельно, чтобы его можно было НЕ пушить: приземлённый раньше, чем резолвится DNS, он направляет парадную дверь лендинга в несуществующий хост — отказ хуже бейджа, потому что выглядит как зашипленное.

  **Находки, о которых сообщено и которые НЕ правились здесь:** (а) лендинговый `try_files … /404.html` переписывает неизвестный URL в существующий файл, так что `file_server` не на чем упасть и **любой** неизвестный URL отвечает **200** с телом 404-страницы — перемерено на реальном `landing/web/Caddyfile` поверх реально собранного `dist`; docs-Caddyfile fallback не несёт и отдаёт настоящий 404. (б) `scripts/lint-cue.sh` вет'ит `schemas/`, `platform-stack/cue/` и `examples/` — и НЕ трогает `docs-site/` и `landing/`, то есть оба деплоимых манифеста не валидирует никто; это не гипотеза: `cue vet ./landing/web/apprafter` проходит, а `cue fmt --check` на той же директории падает, называя оба файла, — так выглядит непокрытый путь через год. (в) SPDX-проверка не достаёт ни до `docs-site/**`, ни до `landing/**` (в `PATTERNS` нет ни глоба под эти деревья, ни общего `**/*.cue`), так что заголовки на новых файлах верные и необеспеченные. (г) `scripts/docs-artefacts-check.py` падает трейсбеком на НОВОЙ странице, которая написана, но ещё не застейджена (`source` строится из `git ls-files`, `published` — из сборки), причём собственный страж `orphan_pages` под этот же случай стоит НИЖЕ строки, которая первой разыменовывает недостающий ключ.
