...
- [ ] **2.23e — дока проще: порядок под флоу и распил разросшегося** · docs-only
  ...
  - [ ] `backup-restore.md` — 989 строк, вдвое длиннее следующей операторской страницы. Оба
        ...
        restore и DR уезжают в `restore.md`; ранбук смены машины — в отдельную страницу рядом с
        `choosing-the-machine.md`. `### Two defects the validation runs found` (`:461-489`)
        удаляется: это запись changelog, она уже есть в `plan-history.md:232`.
...
- [ ] **2.23f — хром сайта документации** · docs-only
  ...
  - [ ] Plausible как на лендинге. Штатный путь material — партиал
        `overrides/partials/integrations/analytics/` + `extra.analytics.provider`. Ловушка:
        `navigation.instant` (`mkdocs.yml:85`) переключает страницы через `pushState`, поэтому
        либо подписка на `location$`, либо history-режим в самом Plausible, **но не оба** —
...
- [ ] **2.23g — CI следит за версиями внешних компонентов** · CI-only, без bump
  ...
  - [ ] Dependabot закрывает 4 класса из восьми и **не видит** ни CUE-пины helm-чартов, ни входы
        flake, ни `with: version:` в workflow, ни `curl .../releases/download/vX/` — то есть
        большую часть риска. Renovate закрывает те же 4 плюс `nix` плюс `customManagers` по
        регуляркам. `helmv3` в этот счёт НЕ идёт: он читает блок `dependencies:` в `Chart.yaml`,
        а такого блока в репозитории нет — апстримные helm-пины живут в CUE и требуют регулярок
        в любом случае.
  - [ ] Начать с `dependencyDashboard: true` и **без единого PR**: один issue со списком всех
        доступных апгрейдов — это ровно то, что просит находка, и это ноль шума.
  - [ ] Обязательное к учёту: `semanticCommitType: "chore"` + scope `deps` (дефолтный
        `build(deps):` завернёт `check-commit-msg.sh`), `rangeStrategy: "bump"` для cargo
        (чтобы `Cargo.toml` и `Cargo.lock` ехали одним коммитом), и **явный `enabled: false`**
        на первопартийные версии в `platform-stack/cue/component_*.cue` — бот там подерётся
        с publish-workflow.
  - [ ] **Не заводить override версий через env.** `cli-providers/build.rs` осознанно делает CUE
        единственным источником правды на этапе сборки; правильная форма — бот правит пин в
        дереве, существующие path-фильтрованные гейты гоняются по правленому дереву.
