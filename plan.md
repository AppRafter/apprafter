...
- [ ] **2.22b — владение: оператор удаляет то, что породил** (D4 + D12 + D13) · оператор + RBAC + chart
  ...
  - [ ] LIST детей этого Application + удаление тех, кого нет в желаемом наборе. Предикат —
  ...
  - [ ] **НЕ «третья prune-ветка в той же форме»**: `has_needs` (`lib.rs:504`) гейтит весь блок
  ...
  - [ ] Глагол `delete` на `resourceclaims` в `rbac.yaml` — **в том же изменении**, что и код
        (повторный урок: ADR 0048 и фикс GC 0.2.31).
  - [ ] Порядок несущий: удаление только ПОСЛЕ апрува плана и применения новой спеки, и
        финализатор провизионера обязан отработать → снапшот пишется, 7-дневное окно
        открывается как задокументировано.
  - [ ] D12: `else`-ветка для Service (`lib.rs:726`) и CNP (`:736-740`). У Service `delete` в
        RBAC есть с самого начала. Проверить `is_destructive` для снятия `expose`.
  - [ ] D13: решение per-app-копии vs sweep. ownerRef убрали обоснованно (копия названа по
        креденшлу и разделяется), но владельца не назначили никакого. Per-app-копии складывают
        дефект в тот же обобщённый диff.
