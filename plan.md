...
- [x] **`HetznerCloudProvider::destroy()` race-condition.** ✅ закрыто `v0.1.47` 2026-05-10. `wait_for_server_gone()` poll'ит `GET /v1/servers` после `delete_server`, exponential back-off (500ms → 5s cap), 60s deadline. Один sync-point для всей async-cleanup contract, вместо per-call retry. Регресс-guard `destroy_waits_for_server_async_cleanup_before_deleting_network` симулирует один poll-iteration с `status=deleting` до vanish.
