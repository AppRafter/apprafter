...
- [x] **2.22h — отказ реконсиляции должен быть виден без чтения логов** (D16) · оператор + CLI
  ...
  >
  > **Третье отклонение, не закрытое: `platform status` проблемы НЕ печатает.** Реестр
  > per-Application, у платформенного контроллера своя поверхность; бить одно о другое не стал.
  > Долг явный.
  >
