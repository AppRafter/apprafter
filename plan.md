...
- [x] **F2 (operability, 0.2.27):** cilium-operator `operator.podAnnotations.apprafter.io/cilium-config-rev` → Argo роллит оператор на cilium-config change. Оператор читает cilium-config только на старте → gateway-api включился, а оператор крутился на старом конфиге → gateway-контроллер не стартовал, ошибка **молчала** до ручного `rollout restart`. (component-only → loader байт-идентичен, no CLI.)
