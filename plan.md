...
- [ ] **c. `app restart`** (ADR 0064). Прежний отказ был рамкой объёма подфазы 2.20 и уже отозван
...
- [ ] **d. Namespace.** `app add` поднимает его сам; `secret seal` предлагает создать с
      подтверждением (молча — никогда: блоб привязан к namespace необратимо); `secret remove`
      перестаёт печатать `✓ Removed` для того, чего не было; `secret list -n` и `volume *`
      перестают читать отсутствующий namespace как пустой.
- [ ] **e. `apprafter top`** — capacity / allocatable / requested / schedulable / in-use / free по
      памяти, CPU и диску, с разбивкой на платформу, интегрированные сервисы и приложения.
- [ ] **f. Слой отображения.** Восемь мест, печатающих сырой RFC3339, сводятся в
      `cli_core::timefmt`; шесть пунктов из `cli-fixes.md` (домен в `app status`, байты в VPA reco,
      namespace в `app list`, сортировка и рендер тега в `backup list`).
