...
- [ ] **2.22c — секреты: раскрытие, индекс, распространение** (D14 + D7 + D6)
  - [ ] **Сначала бесплатное (docs-only):** три поставки из поправки к ADR 0007 —
  ...
  - [ ] Оператор, две строки: `check_env_secret_refs` (`lib.rs:1553-1578`) РАЗЛИЧАЕТ «нет
        Secret'а» и «нет ключа» и схлопывает в один `bool` — назвать реальную причину и
        namespace (он в скоупе строкой выше).
  - [ ] `apprafter secret list` — имя, namespace, **имена ключей, никогда значения**, флаг
  ...
  - [ ] Индекс в ОБЕ стороны из одного прохода (LIST Applications + скан `env` на
  ...
  - [ ] Хеш разрешённого конфига на pod-темплейте + показ дрейфа в `app status`.
  ...
  - [ ] Аннотация автора/времени на сеал — для инсайдерской ошибки и форензики, не как контроль.
