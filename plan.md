...
- [ ] **2.21a** — пункт ледгера в ADR 0058; решение по `Post-launch` в реестре; чистка
  утечки `Product 1` / `Product 2` в `roadmap.json`.
- [ ] **2.21b** — переписывание страницы: убрать колонки `plan.md` и `order/CP`, продуктовые
  группы вместо `## order N`, схлопнуть 1 380 слов блокквот-прозы, колонка «Документация»,
  расширенный словарь отметок для политик и архитектурных свойств.
- [ ] **2.21c** — перенос 23 фич матрицы killer features под правилами редактуры; девять из
  них в ледгере вообще отсутствуют; каждое `done` проверяется против кода.
- [ ] **2.21d** — соединение: ячейки Phase становятся якорями реестра; гейт на href;
  ссылка лендинг ↔ `/status/`.
