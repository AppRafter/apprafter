...
- [x] **F1 (BLOCKER, 0.2.27):** `gateway-api-crds` → **experimental** канал (`config/crd/experimental`), не standard — cilium 1.16.5 gateway-контроллер на старте требует `tlsroutes.gateway.networking.k8s.io` (v1alpha2, только в experimental); без него fatal required-resources-check → GatewayClass `cilium` не Accepts → ни один Gateway не Programmed. + регресс-гард в `check-gateway-render.sh`. Не ловилось в kind (там Gateway не поднимали).
