...
- [ ] **F3 (cosmetic):** `ignoreDifferences` на root `platform` Application — Gateway `/metadata/finalizers` (cilium) + HTTPRoute API-defaulting (parentRefs `group`/`kind`, matches `path.type`, …) — иначе Gateway+HTTPRoute вечно OutOfSync в Argo. CLI (`render_root_application`; только новые bootstrap'ы — existing manual-patch). **Pending:** точный jsonPointers-набор по живому Argo DIFF.
