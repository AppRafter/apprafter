...
- [ ] **b. CLI: бандл-модель.** `parse_applications`, плюральный резолвер вместо
      `find_apprafter_app_name`, `app_index`, переработка `list`/`status`/`remove`/`logs`/`open`/
      `rollback`, кардинальная pin-aware ячейка HEALTH, чтение `status.conditions[]` Argo в
      `app status`, локальный двойник проверок бандла в `app validate`, отказ на `--path` в файл.
