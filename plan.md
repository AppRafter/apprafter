...
- [x] **2.22b — владение: оператор удаляет то, что породил** (D4 + D12 + D13) · оператор + RBAC + chart
  ...
  - [ ] Walk: приложение с ДВУМЯ needs, убрать ОДИН → RetainedClaim появился, второй клейм не
        тронут. Ни один из четырёх needs-walk'ов этого не делает — все удаляют приложение
        целиком, поэтому все гейты и проходили.
