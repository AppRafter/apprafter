...
- [ ] **c. Живой прогон трека B.** Цепочка «манифест → claim → NACK → NATS → DLQ наполнился» не
      пройдена ни разу. Три части — имена полей против CRD, поведение сервера, эмиссия
      провижионером — покрывают её по кускам, но не связку. Строка в `docs/status.md` до прогона —
      `🚧`.
...
- [ ] **e. Остаток рантайма.** Контроллер `SharedDatabase` (провижининг pg/redis + **выводимый**
      `refCount` + отказ `rm` при ненулевом), путь привязки claim'а по `sharedRef` (исполнить
      `bind_consumer`, записать коннекшн-секрет потребителя), CLI `apprafter db`, триггер ADR 0052
      на первый бинд пары, фикстуры, регрессия `crd-validate` и живой прогон.
