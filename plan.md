...
- [ ] **(h) CLI UX** *(= W7)*. `EXAMPLES` + `after_help` на каждой команде, `secret list` + namespace-мастер для `secret seal`, `status.conditions[]` с remediation в `app status`, `requires="select"` на `--namespace`, `app open --yes`, shell-completions, help-coverage-тест. Идёт ДО контента (i): гейт `check=cli` резолвит примеры против ТЕКУЩЕГО clap-дерева.
