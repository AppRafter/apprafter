...
| 2026-05-24 | M1.5 walk-fix #7 post-B.1.79a — migration helper observability + cross-mount safe rename + richer-legacy preference. Two operator-impacting bugs surfaced on the v0.1.155 walk: (a) operator ran `apprafter app add` from `cli/`, hit `no cached kubeconfig in state` — inspection found `cli/.apprafter/state.json` (5375 bytes, real hetzner_cloud + kubeconfig) AND `~/.config/apprafter/state/dev/.apprafter/state.json` (412 bytes, placeholder with round-number IDs server_id=42 firewall_id=200 floating_ip_ids=[300], kubeconfig null). v0.1.154-155 migration helper's silent "new-wins by existence" branch preserved the placeholder и left the rich legacy file unmigrated, with no stderr signal — operator had no idea why their real state wasn't loading. (b) After manually deleting the placeholder per-target file to force migration, hit `Invalid cross-device link (os error 18)`. Initial "different physical partitions" framing was inaccurate: operator's `/projects/` и `$HOME` were on the SAME btrfs filesystem, just different subvolumes mounted at separate paths (common NixOS layout). EXDEV fires on ANY mount-point boundary crossing: btrfs subvolumes, ZFS datasets in one pool, bind mounts, overlay/fuse mounts — all return EXDEV on rename across the boundary regardless of whether the bytes physically share а block device. `std::fs::rename` is single-syscall с no fallback. Three coordinated fixes: (1) richer-legacy preference — when both files exist, parse both и compare hetzner_cloud presence via new pure `read_has_hetzner_cloud(path)`; if legacy is richer than per-target (legacy has hetzner_cloud, target doesn't), back placeholder up to `state.json.bak` и migrate legacy as authoritative. (2) Loud stderr notice on the genuine skip path (both files carry hetzner_cloud — two real clusters case); silent skip removed entirely. (3) New `cross_fs_safe_rename(src, dst)` helper tries `fs::rename` first, falls back to `fs::copy` + `fs::remove_file` on EXDEV (os error 18 on Linux + macOS); all three rename callsites в the migration helper (state move, known_hosts move, backup rename) routed through it. +2 unit tests: `migrate_legacy_prefers_richer_legacy_when_per_target_lacks_hetzner_cloud` (exact v0.1.155 walk scenario), `migrate_legacy_keeps_per_target_when_both_carry_hetzner_cloud` (defensive — two real clusters case). Total cli-state migration tests: 5 → 7. CLI 0.1.155 → 0.1.156. Chart unchanged | v0.1.156 |
| 2026-05-24 | M1.5 post-B.1.79a #7 — `apprafter app add` wizard defaults path prompt to cwd-relative-to-repo-root. Operator follow-up after the v0.1.154 state migration: registering landing manifests from `/projects/.../apprafter/landing/cms/` had wizard suggest `/` (clap default) — operator had to manually type `landing/cms`, then was unsure whether to suffix `apprafter` (convention dir name) or not. Fix lands two coordinated UX bits: (1) new pure helper `detect_path_relative_to_repo_root()` in `app_wizard.rs` shells out to `git rev-parse --show-toplevel`, computes cwd relative to result, canonicalises both ends (dodges macOS `/var → /private/var` symlinks), returns `Some("landing/cms")` for subdirs, `Some("/")` when cwd IS repo root, `None` when git missing or cwd outside work tree. `WizardInputs.detected_path` field carries it; `app::add_via_wizard` populates via the new detector. Wizard's `run` chooses path default by precedence: explicit non-default `--path X` → cwd-detect → clap default `/`. (2) Documentation + verification confirms `apprafter/` subdirectory is implicit — operators can specify project root (`landing/cms`) instead of convention dir (`landing/cms/apprafter`); the CMP plugin's discovery glob (`{**/apprafter*.cue,**/apprafter/**/*.cue}` from chart 0.1.42) matches files at any depth, и `cue export ./...` recurses through subdirectories. Local `bash argocd-cue-cmp/entrypoint.sh` against both `landing/cms` and `landing/cms/apprafter` produces identical YAML doc streams. CLI 0.1.154 → 0.1.155. Chart unchanged | v0.1.155 |
| 2026-05-24 | M1.5 polish post-B.1.79a #6 — state migrates from per-cwd `<cwd>/.apprafter/state.json` to per-target `<config>/state/<active-target>/.apprafter/state.json`. Operator complaint: ran `apprafter apply` from project root then opened a second shell at `landing/cms/` to register the first Application via `apprafter app add` — hit `Error: state has no hetzner_cloud section; run apprafter apply first` even though apply had just succeeded. Root cause: `StatePaths::for_root(&cwd)` was the only constructor; every operational command opened with `std::env::current_dir()? → for_root(&cwd) → State::load_or_default(&paths)`, so the second invocation's cwd had its own (empty) `.apprafter/` directory rather than the one created at the project root. The per-target store scaffolding existed at `cli-core/src/target.rs:186` (`TargetStorePaths::state_dir(name)` returning `<store>/state/<target>/`) since Track A.2 — that landed Track A.8's "wire the existing per-CWD flow over here" comment but the wiring itself was deferred and never revisited. Fix: new `StatePaths::for_active_target(&TargetStorePaths, &str)` factory points at `<store>/state/<target>/.apprafter/` (intermediate `.apprafter` segment kept so on-disk shape mirrors legacy layout operators grep for); new `migrate_legacy_state_if_present(legacy_root, target_paths)` helper moves the v0.1.153 file (and sibling `known_hosts`) into the new per-target slot on first run after upgrade, printing one stderr notice; idempotent + new-wins (legacy file left alone when per-target slot already populated). New shared `commands::state_paths::resolve_state_paths(target_override)` threads the three steps every command repeated: resolve `TargetStorePaths` via `default_config_root()`, resolve active target name (with `--target` override), build per-target `StatePaths`, run migration. Returns a bundle carrying the `TargetStorePaths` too so credential resolution downstream skips a redundant env probe. Eager existence check when `--target <name>` supplied — surfaces `target X not found (available: …)` instead of the much later generic "no provider configured" when operators typo a name. Callsites updated: `apply.rs`, `destroy.rs`, `import.rs`, `kubeconfig.rs`, `argocd_password.rs`, `cluster_bootstrap.rs`, `init.rs`, `status.rs`, `plan.rs`, `k8s_helpers.rs` (the latter fixes `platform` / `migration` / `app` / `repo creds` / `open` thin wrappers transparently). Dead code removed: `discover_from` / `discover_or_root` helpers added speculatively for a walk-up-from-cwd approach were never adopted (walk-up can't help when cwd is `landing/cms/` and state is per-target, not per-project-root). +5 unit tests on the migration helper (`for_active_target_lays_out_state_under_store_state_target`, `migrate_legacy_no_op_when_legacy_absent`, `migrate_legacy_moves_state_when_only_legacy_exists`, `migrate_legacy_leaves_existing_target_state_untouched`, `migrate_legacy_moves_known_hosts_alongside_state`). Integration test fixtures across 6 suites (`import_test`, `kubeconfig_test`, `argocd_password_test`, `aliases_test`, `cli_smoke`, `miette_render_test`) updated to seed `APPRAFTER_CONFIG_DIR` + an active target before running `init` so the per-target state slot is writable; state-file paths in assertions switched from `<cwd>/.apprafter/state.json` to `<cfg_dir>/state/default/.apprafter/state.json`. CLI 0.1.153 → 0.1.154. Chart unchanged. Operator / admission-webhook unchanged | v0.1.154 |
| 2026-05-24 | M1.5 walk-fix #5b post-B.1.79a — cue-cmp entrypoint accepts both unwrapped и named-wrapper source layouts. v0.1.152 push triggered `argocd-cue-cmp-check` CI workflow; entrypoint smoke step failed `entrypoint did not render Application kind`. Root cause: walk-fix #5 entrypoint enumerated `to_entries[]` looking for NESTED objects-with-apiVersion+kind (style B — `landingWeb: v1alpha1.#Application & {...}`); the smoke fixture writes the manifest UNWRAPPED at package scope (style A — `apiVersion: ... kind: ... metadata: ... spec: ...` directly), so top-level `apiVersion` is а string, filter rejected everything, loop exited without emitting. Two legitimate CUE source layouts exist (style A single-resource files, style B multi-resource files); walk-fix #5 handled only B. Fix: dispatch before enumeration via jq check `(type == "object" and has("apiVersion") and has("kind"))` — style A emits `cue export ./... --out yaml` verbatim, style B falls through к existing per-key logic. Verified locally against both `/tmp/cue-smoke/apprafter/Application.cue` (style A, single YAML doc) и `landing/web/apprafter/` (style B, two docs с `---` separator). argocd-cue-cmp v0.1.2 → v0.1.3 (publish workflow auto-fires on new tag); platform-stack chart 0.1.42 → 0.1.43 picks up new image tag via CUE import; CLI 0.1.152 → 0.1.153 | v0.1.153 |
| 2026-05-24 | M1.5 walk-fix #5 post-B.1.79a — argocd-cue-cmp emits flat manifest stream + accepts apprafter/ directory layout. Operator asked how to register AppRafter landing manifests (`landing/web/apprafter/Application.cue`, `landing/cms/apprafter/Application.cue`) via `apprafter app add`. Investigation surfaced два unsurfaced bugs в cue-cmp plugin path (CMP path не был exercised end-to-end до этой попытки регистрации первого user app'а): (1) CMP discovery glob `**/apprafter*.cue` matches только files starting с `apprafter` basename — landing manifests named `Application.cue` inside `apprafter/` directory не matched, Argo CD fall through to default raw-YAML, syncs zero rendered manifests; (2) entrypoint runs `cue export ./... --out yaml` raw — для typical source files (`landingWeb: v1alpha1.#Application & { ... }`) produces NESTED YAML с manifest под field key, Argo CD treats как ONE invalid document (no apiVersion at top level) → sync fails. Fix landed across three artefacts: (a) `argocd-cue-cmp/entrypoint.sh` rewritten — enumerates top-level k8s-shaped values (objects с `apiVersion` + `kind`) via `cue export --out json | jq`, re-exports each via `cue export -e <key> --out yaml`, emits `---` separator per doc; helper top-level values (shared constants, library defs) silently skipped through the `apiVersion+kind` filter. (b) `argocd-cue-cmp/Dockerfile` adds `jq` к Alpine base (~600 KiB) для the enumeration step. (c) `argocd-cue-cmp/plugin.yaml` + chart's embedded plugin.yaml в `component_argocd.cue`'s extraObjects ConfigMap extend glob к `{**/apprafter*.cue,**/apprafter/**/*.cue}` via doublestar v4 brace alternation — matches both filename-prefix convention (`apprafter-web.cue`) и directory-marker convention (`landing/web/apprafter/Application.cue`). Verification: local `bash entrypoint.sh` against `landing/web/apprafter/` emits two well-formed YAML docs (landing-web + landing-web-preview) с `apiVersion: apprafter.io/v1alpha1` на верхнем уровне каждого + `---` between them. argocd-cue-cmp image v0.1.1 → v0.1.2 (publish workflow auto-fires на new tag); platform-stack chart 0.1.41 → 0.1.42 picks up new image tag через `argocd-cue-cmp/version.cue` import + extends embedded glob; CLI 0.1.151 → 0.1.152 picks chart pin via `RELEASED_PLATFORM_STACK_VERSION`. Operator workflow post-upgrade: `apprafter platform upgrade --to 0.1.42` then `apprafter app add <repo-url> --path landing/web/apprafter --name landing-web` works | v0.1.152 |
| 2026-05-24 | M1.5 walk-fix #4 post-B.1.79a — version_check picks wrong tag series + 24h → 6h TTL. User ran v0.1.142 binary after v0.1.150 release went live; banner stayed silent. Cache inspection: `{"latest_tag":"platform-stack/v0.1.38","fetched_at_secs":...}` — а chart-stream tag, not а CLI version. Root cause: the monorepo publishes four release streams (canonical CLI `vX.Y.Z`, chart `platform-stack/vX.Y.Z`, landing `landing-vX.Y.Z`, argocd-cmp `argocd-cue-cmp/vX.Y.Z`), и the `/releases/latest` API endpoint returns the most recently CREATED release across all four. v0.1.149's `version_check` took whatever came back; `trim_start_matches('v')` didn't strip the `platform-stack/` prefix; `semver::parse` failed silently; banner suppressed. Fix: switch endpoint к `/releases?per_page=30` (newest-first array). New pure helper `pick_canonical_cli_tag(releases)` walks the array и returns the first `tag_name` whose stripped form parses as canonical `semver::Version` — all four prefixed streams fail `Version::parse` naturally without explicit denylist. Cache also gains defensive validation в `read_fresh_cache`: non-semver `latest_tag` treated as cache miss, clears leftover garbage from pre-fix caches without operators deleting the file manually. TTL 24h → 6h (project in active daily development; 6h dial keeps operators current без burning GitHub API quota). +8 unit tests на pure helpers: canonicalise_cli_tag для each of 4 prefix permutations + empty/garbage + pre-release suffix happy path; pick_canonical_cli_tag для mixed-stream window happy path + no-CLI-tag-in-window pathological + malformed release-object skip + non-array defensive. Total version_check module: 4 → 13 tests. CLI 0.1.150 → 0.1.151. Chart unchanged | v0.1.151 |
| 2026-05-24 | M1.5 walk-fix #3 post-B.1.79a — release-cli workflow missing CUE install. First push of v0.1.149 triggered the new `release-cli` workflow; all three matrix builds (linux-x86_64, darwin-x86_64, darwin-aarch64) failed identically с `cli-providers/build.rs panicked: neither cue nor nix run nixpkgs#cue is on PATH`. Root cause: `cli-providers/build.rs` shells out to `cue export` at compile time to extract `_loaderValues` + chart versions from `platform-stack/cue/`. GitHub-hosted runners (ubuntu-latest, macos-latest) don't ship cue by default. The fix was already shipping in `lint.yml` + `nightly.yml` — both install cue via `cue-lang/setup-cue@v1.0.1` action — but the new release-cli workflow forgot к copy that step. Fix: add `Install CUE` step between Rust toolchain setup и `cargo build` в the matrix `build` job, pinned к `v0.10.0` so all three CI workflows share one cue version. CLI 0.1.149 → 0.1.150. Chart unchanged. Strictly а CI fix — Rust code unchanged, CLI binary byte-identical to 0.1.149 | v0.1.150 |
| 2026-05-24 | M1.5 polish post-B.1.79a #5 — CLI release workflow. Walk feedback on v0.1.135's npm-style newer-release banner: banner never fires even when CLI is many tags behind tip — root cause: `version_check::resolve_latest_tag` polls `https://api.github.com/repos/apprafter/apprafter/releases/latest` but monorepo had no workflow creating GitHub Releases on `v0.*` tag pushes. Releases page empty → API 404 → fail-quiet banner stays silent. Fix: new `.github/workflows/release-cli.yml` triggers on `v0.*` / `v1.*` tag pushes (плюс manual `workflow_dispatch` для re-builds), builds `apprafter` CLI binary for 3 targets (x86_64-unknown-linux-gnu native on ubuntu-latest, x86_64-apple-darwin cross-compiled on macos-latest, aarch64-apple-darwin native on macos-latest), packages each into `apprafter-<tag>-<target>.tar.gz` (stripped binary + README + LICENSE) с sha256 sidecar, creates GitHub Release with assets uploaded. Verify step runs `apprafter --version` against fresh binary и asserts output matches tag minus `v` prefix — catches forgotten Cargo.toml bumps before they ship а broken courtesy banner. Release body autogenerated by pulling first `##` changelog section that mentions the tag from UNRELEASED.md; falls back к а short default for chart-only / landing-stream tags without dedicated entries. Security: every potentially-tainted input (inputs.tag, github.ref_name, extracted changelog text) flows through `env:` binding before reaching shell `run:` — no `${{ }}` interpolation inside body, eliminates action-injection class. Tag-glob trigger constrains ref_name к known-safe shape. Manual re-run path deletes existing release first to avoid `gh release create` errors on duplicate asset names. Linux aarch64 deliberately absent — tier-1 ships on x86 only; cross-compile via `cross`/QEMU а separate complication для Phase 2+. CLI 0.1.148 → 0.1.149. Chart unchanged. First release to land на GitHub Releases page once tagged + pushed → makes newer-release banner functional for all future tags | v0.1.149 |
| 2026-05-24 | M1.5 polish post-B.1.79a #4 — `apprafter platform freeze` fallback to Argo CD Application's targetRevision. Walk feedback from v0.1.142 closure: `apprafter platform freeze cilium` without `--version` errored с "No effective version found for component 'cilium' in status.componentVersions" because PlatformController ships in M1.5 without populating `PlatformStack.status.componentVersions` (that field is reserved for а Phase 2 enhancement) — the verb was unusable without manually looking up the version. Fix: new pure `resolve_effective_pin(stack, argocd_app, component)` helper walks а 3-step fallback chain — (1) `PlatformStack.status.componentVersions.<component>` when present, (2) Argo CD `Application argocd/<component>.spec.source.targetRevision` (always present for а chart-managed component since umbrella's `templates/applications.yaml` emits it), (3) hard error pointing at `--version <v>` + `apprafter platform status`. `freeze()` reads PlatformStack CR, fetches Argo CD child Application (404 = `None` treated as "no signal"), then resolves. Source preference order: PlatformStack.status wins over Argo CD targetRevision because during а bump cycle the Argo CD Application's targetRevision may briefly lag behind the umbrella's reconciled state — the umbrella patches its children's targetRevision on each component cycle, and mid-flight the value reflects the OLD version until the patch lands; PlatformStack.status, when populated, surfaces the operator's intended version. +4 unit tests на pure resolver: `_prefers_platformstack_status` (both sources present, status wins), `_falls_back_to_argocd_target_revision` (M1.5-typical state succeeds via Argo CD fallback instead of erroring — load-bearing for the operator-facing fix), `_errors_when_both_sources_empty` (clean error message with --version hint), `_handles_argocd_app_without_target_revision` (defensive: malformed CR с empty spec.source treated like 404). Total platform module: 9 → 13 tests. CLI 0.1.146 → 0.1.148 (v0.1.147 taken by landing-stream release `558acad`; CLI version skips ahead к 0.1.148 to avoid а tag collision). Chart unchanged | v0.1.148 |
| 2026-05-24 | M1.5 walk-fix #2 post-B.1.79a — AppProjects as standalone umbrella manifests at sync-wave -30. Operator upgrade 0.1.39 → 0.1.40 hit `Unable to refresh admission-webhook: app is not allowed in project "platform", or the project does not exist`. Root cause: chart 0.1.40 (part 1) shipped 4 AppProject definitions via `_loaderValues.argocd.values.configs.projects` — the argocd subchart renders those as AppProject CRs only when it syncs (sync-wave -15). Child Applications at sync-wave 0 (admission-webhook, apprafter-operator, network-policies, backstage, argocd-cue-cmp) reference `spec.project: platform` (new default из part 1). **Argo CD's app-of-applications model does NOT strictly serialise sync-waves between separate Applications** — only within а single Application's child-resource sync. Across multiple umbrella-managed Applications, Argo CD treats each as an independent reconciler и starts them roughly together. Race: admission-webhook (wave 0) sometimes tries to refresh BEFORE AppProject `platform` has landed via argocd (wave -15). Fix: ship AppProject CRs as **standalone manifests in the umbrella chart** at sync-wave -30 (earlier than even Cilium at -20). New shared `_appProjects` map в `platform-stack/cue/app_projects.cue` carries the 4 definitions; two sites consume it — new `templates/appprojects.yaml` template (umbrella) emits one `kind: AppProject` per entry at sync-wave -30 on every umbrella sync; `_loaderValues.argocd.values.configs.projects` keeps the same definitions для initial `apprafter cluster-bootstrap` install before any umbrella sync has run. Both sites describe identical resources (same group/kind/name/namespace); Argo CD reconciler treats them as one logical object — duplication is idempotent. New CUE shape `#AppProjectSpec` constrains entries к AppProject fields umbrella actually sets (description, sourceRepos, destinations, cluster/namespace resource whitelists). `#PlatformValues` gains top-level `appProjects: [string]: #AppProjectSpec`; tier overlays (tier_solo + tier_team) wire `appProjects: _appProjects`. Fix is structural — fresh installs на 0.1.41+ и 0.1.40 → 0.1.41 upgrades both work without manual intervention. Existing 0.1.40 operators who want immediate refresh-error clearance (instead of waiting for the chart-pull + reconcile cycle) can apply the 4 AppProjects inline via `kubectl apply -f` — optional unblocker, not а workaround. CUE renders cleanly; `helm template` against rendered chart emits 4 AppProject CRs at sync-wave -30 alongside existing child Applications. Chart 0.1.40 → 0.1.41. CLI 0.1.145 → 0.1.146 (CLI picks up chart bump via `RELEASED_PLATFORM_STACK_VERSION`). Operator chart unchanged | v0.1.146 |
| 2026-05-24 | M1.5 polish post-B.1.79a #3 — wizards for `apprafter app add` and `apprafter repo creds add`. Same gate as `target add` wizard from v0.1.76: fires when stdin + stdout are TTYs AND `--no-interactive` not set; pre-fills every prompt from flag value or auto-detected context; falls through into the non-interactive code path with `no_interactive=true` to avoid recursion. `app add` wizard prompt order: Git URL (Text, default = `--git-url` flag or cwd's `git remote get-url <--remote>` normalised through `normalise_git_url`), Application name (Text, DNS-1123 validated inline, default = `--name` or `derive_app_name(url)`), Branch / revision (Text, default = `--branch` or `git symbolic-ref --short HEAD` or `main`), Path (Text, default `/`), AppProject (Select: apps/platform/platform-providers, default = `--project`). Two pure helpers `detect_git_origin(remote)` / `detect_git_branch()` shell out to `git` non-fatally — missing binary / non-repo cwd / unconfigured remote returns `None` so the wizard works from any cwd. `repo creds add` wizard prompt order: Friendly name (Text, DNS-1123), URL prefix (Text, validated for scheme+host shape — accepts `https://...` / `git@host:...`, rejects bare paths that could never prefix-match an Argo CD repoURL), Auth type (Select: pat/basic), Username (Text, default `git`), Token (Password masked via `PasswordDisplayMode::Masked`, `without_confirmation`); inline provider-aware format validation runs as operator types unless `--no-validate` flowed through. New CLI flags `apprafter app add --no-interactive` and `apprafter repo creds add --no-interactive`. Visibility bumps: `repo_creds::AuthType` and `validate_token_format` promoted to `pub` so wizard can reuse rules without duplication. +11 unit tests across `app_wizard` (5) + `repo_creds_wizard` (6); coverage: TTY gate, DNS-1123 happy/sad, URL prefix happy/sad (bare-path rejection load-bearing), non-empty trim, parse_auth_type_safe garbage fallback. CLI 0.1.144 → 0.1.145. Chart unchanged | v0.1.145 |
| 2026-05-24 | M1.5 polish post-B.1.79a #2 — `apprafter platform status` UX (dates / sort / width). Walk feedback: raw RFC3339 timestamps unreadable at a glance ("when did that check happen?"), `versionHistory` table ordered by JSON document position rather than `appliedAt` desc (brittle across server-side re-sorts), Conditions table sprawled past 80-col terminals because the old heuristic wrapped MESSAGE to a flat 60 chars without accounting for total layout width. Three coordinated fixes: (1) new pure `format_timestamp_with_relative(raw, now)` helper parses RFC3339 via `chrono`, renders as `"2026-05-24 14:30 UTC (2 hours ago)"`; humanise scale uses operator-relevant granularity (under 45s → "just now", under 1h → minutes, under 1d → hours, under 30d → days, under 1y → months, else years); unparseable input surfaces verbatim so audit data is never lost. (2) new `collect_history_rows(status, now, take)` sorts by parsed `appliedAt` desc using `DateTime::parse_from_rfc3339`; entries with unparseable timestamps sink to the bottom so corrupt / mid-write CRs don't dominate the visible head. (3) new `render_conditions_table(conditions)` queries `terminal_size::terminal_size()` for TTY width, computes a budget by subtracting non-MESSAGE column max widths + separator overhead, wraps MESSAGE to the remainder; falls back to 100 columns when stdout is piped. New workspace deps: `chrono` (clock+serde, no default features) and `terminal_size`. `print_status` signature gains a `now: DateTime<Utc>` parameter so tests can pin a deterministic reference moment for relative-date assertions. +7 unit tests on pure helpers (format_timestamp_renders_absolute_and_relative, format_timestamp_handles_unparseable_input, humanise_relative_uses_just_now_under_45_seconds, humanise_relative_handles_minutes_hours_days_months_years span coverage, collect_history_rows_sorts_by_applied_at_desc, collect_history_rows_puts_unparseable_timestamps_last, collect_history_rows_caps_at_take). Total platform module: 2 → 9 tests. CLI 0.1.143 → 0.1.144. Chart unchanged | v0.1.144 |
| 2026-05-24 | M1.5 polish post-B.1.79a #1 — CLI Cyrillic → English sweep. Walk feedback на B.1.79a closures pointed out что CLI output, comments, и error messages в new modules (`app.rs`, `repo_creds.rs`, `platform.rs`, `open.rs`, и т.д.) contained mixed Cyrillic/English text: both fully-Russian sentences (e.g. "Не нашёл effective version для component 'cilium' в status.componentVersions") и Cyrillic letters used as Latin look-alikes inside English text (`к`-Cyrillic for `to`, `и`-Cyrillic for `and`, `с`-Cyrillic for `with`, `а`-Cyrillic for `a`, etc.). Some shells в CI / container environments fail to render Cyrillic correctly, и mixed-language strings break grep filters used by `apprafter doctor`/log scraping tools. Mass translation across 10 CLI files (cli.rs 101 → 0 Cyrillic occurrences, main.rs 2 → 0, commands/app.rs 121 → 0, commands/repo_creds.rs 74 → 0, commands/open.rs 35 → 0, commands/platform.rs 31 → 0, commands/cluster_bootstrap.rs 9 → 0, commands/version_check.rs 7 → 0, commands/migration.rs 6 → 0, commands/k8s_helpers.rs 4 → 0) + bonus single Cyrillic word stripped from cluster_smoke_test.rs. Repo-wide `grep -cE '[А-Яа-яЁё]'` returns 0 across entire `cli/` tree. Standardised common phrases: confirmation prompts read `Confirm?`, cancellation prints `Cancelled.`, hints prefixed `Hint:`. Test names and assertion messages translated alongside production code so test bodies match new English error strings. CLI 0.1.142 → 0.1.143. Chart unchanged | v0.1.143 |
| 2026-05-22 | M1.5 Track B.1.79a **closure** + остатки от 1.79 — `apprafter platform freeze/unfreeze/rescue`. `freeze <component> [--version <v>]` patches `PlatformStack.spec.overrides.<component>.pin`; без `--version` reads effective version из `status.componentVersions.<component>` и locks that. CRD schema (`schemas/v1alpha1/platformstack.cue`) already supports `overrides` map (B.1.74 era), so chart-side change не required. `unfreeze <component>` — RFC 7396 merge-patch с null strips `overrides.<component>` entry; component falls back к chart's curated pin (per umbrella). `rescue [--yes]` — thin wrapper over `apprafter cluster-bootstrap` с recovery confirmation banner ("re-apply Cilium / Argo CD / CRDs / operator chain как initial bootstrap — все Applications потеряют Sync/Healthy на несколько reconcile cycles"); use case Argo CD self-adopt stuck. Non-interactive shell без `--yes` → refuses. **1.79a closure**: AppProjects + project field (part 1), `open argocd` polish (part 2), `app add/list/status/remove` (part 3), `app logs/rollback` (part 4), `repo creds add/list/show/rotate/remove` (part 5), `platform freeze/unfreeze/rescue` + context-aware error hints + plan.md acceptance boxes (this commit). Deferred с rationale: `platform channel` (Phase 2 multi-channel), interactive wizards (batched с future `target add` wizard polish), inline PAT prompt (hint sufficient), CMP plugin user-app project default (Phase 2 concern), `open backstage/grafana/hubble` (Tier 2+ services). Per CLAUDE.md phase-closure discipline plan.md acceptance boxes отражают что shipped vs deferred; manual walk против real Hetzner cluster confirmed (см. 0.1.136 walk-fix). CLI 0.1.141 → 0.1.142. Chart unchanged | v0.1.142 |
| 2026-05-22 | M1.5 Track B.1.79a part 5 — `apprafter repo creds` subcommand family (add/list/show/rotate/remove) с alias-free top-level `apprafter repo creds`. Closes private-repo onboarding loop — operators больше не составляют вручную YAML для Argo CD repo-creds Secret. Per Argo CD declarative-setup contract, writes Secret в `argocd` namespace с labels `argocd.argoproj.io/secret-type: repo-creds` (load-bearing — repo-server filters by it) + `apprafter.io/managed-by: apprafter` + `apprafter.io/cred-name: <name>`, annotations `apprafter.io/auth-type: <pat|basic>` (consumed by `rotate` для re-validation) + `apprafter.io/source: cli`, `stringData` (НЕ `data` — kubectl base64-encodes server-side) с `url` / `username` / `password`. Argo CD repo-server prefix-matches Application's `spec.source.repoURL` against the registered `url` — register `https://github.com/myorg` → every `myorg/*` repo inherits creds automatically. `add <name>`: `--url-prefix` required, `--type pat\|basic` (`ssh` rejected с Phase 2 deferral hint), `--username git` default, `--token` via flag/env/masked-stdin-prompt; provider-aware regex validation (`github_pat_*` 80+, `ghp_*` exactly 40, `glpat-*` 20+, generic 20+ fallback для Gitea/Forgejo), `--no-validate` skip. Pre-flight refuses на name collision. `list`: table NAME/URL PREFIX/TYPE/USERNAME. `show <name>`: detail с masked password + pointer к `kubectl get secret -o jsonpath ...` для plaintext decode. `rotate <name>`: in-place merge-patch на `data.password` (base64-encoded since patching `data` не `stringData`); re-validates against recorded auth-type annotation; repo-server подхватит на следующем pull cycle. `remove <name>`: dependency check by default через pure helper `find_apps_matching_prefix(applications_json, url_prefix)` (refuses если Application(s) have prefix-matching `spec.source.repoURL`); `--force` skip dependency check, `--yes` skip только confirmation prompt. Inline PAT prompt в `apprafter app add` остаётся deferred (sufficient surface hint pointing на `repo creds add` уже в v0.1.139's auth-failure message). +18 unit tests на pure helpers: parse_auth_type happy/ssh-deferral/unknown, validate_token_format для каждого provider format (github fine-grained 80+, ghp_ exactly 40 reject both shorter/longer, glpat-, generic 20+, short refuse с --no-validate hint, basic accepts any non-empty), validate_dns_1123 для creds name, build_repo_creds_secret carries secret-type label + managed-by + cred-name labels + routes к argocd namespace + uses stringData без data field, find_apps_matching_prefix filters by repo URL prefix + skips helm-only apps без repoURL. CLI 0.1.140 → 0.1.141. Chart unchanged | v0.1.141 |
| 2026-05-22 | M1.5 Track B.1.79a part 4 — `apprafter app logs/rollback`. Closes the delete-debug-revert loop без operator detour к raw kubectl / Argo CD UI. `app logs <name>`: wrap над `kubectl logs` scoped к the workload namespace из Application CR's `spec.destination.namespace`. Default — selector mode `-l app.kubernetes.io/instance=<app-name>` (Argo CD's documented standard label stamped на every child resource); `--pod <name>` overrides selector с direct pod name. Selector mode additionally emits `--prefix=true` (distinguish multi-pod stream lines) + `--max-log-requests=10` (cap fan-out). Pod mode drops both для clean single-pod output. Flags: `-f/--follow`, `--tail <N>` (`-1` default = no limit), `--container <c>` (disambiguate multi-container pods — kubectl's error message surfaces verbatim когда required). Pure helpers `build_kubectl_logs_target(app_name, pod)` + `build_kubectl_logs_args(target, namespace, follow, tail, container)` separate arg-construction из process spawn для exhaustive testability. `app rollback <name> [--to <rev>]`: reads `status.history` (Argo CD's chronologically-ordered list, newest last); без `--to` picks second-to-last entry's `revision` через pure `pick_previous_revision(app)` helper, с `--to <rev>` uses explicit value verbatim. Pre-flight refuses когда target revision matches current `spec.source.targetRevision` (no-op). Interactive `inquire::Confirm` (default No) или `--yes` non-interactive. Patches `spec.source.targetRevision` через JSON merge-patch; Argo CD automated sync rolls forward на reconcile cycle. +6 unit tests (target selector default, target pod override, args selector form с prefix+max-requests, args pod form drops prefix, pick_previous_revision happy 3-entry history, pick_previous_revision errors на 0/1/missing history с pointer к --to flag). Total app crate: 12 → 18 tests. CLI 0.1.139 → 0.1.140. Chart unchanged | v0.1.140 |
