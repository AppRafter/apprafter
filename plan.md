...
- [ ] **2.23j — точечные правки, ни от чего не зависящие** · docs-only + три комментария в коде
  - [ ] Дубль в руководстве по бэкапу: фраза «The backup distinguishes user material from platform
  ...
  - [ ] `## What a backup captures` перечисляет объекты Kubernetes и redis и **не говорит, что
  ...
  - [ ] `image-iteration.md` → `## Private images` объясняет, что нужен `SourceCredential`, и
  ...
  - [ ] `dev-guide/application-cue.md` — последняя ссылка подписана «Operator quickstart», а ведёт
        на quickstart разработчика в том же каталоге.
  - [ ] Три протухших комментария, все три — запись, а не сборка: обоснование в `gate.rs` для
