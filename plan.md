...
- [ ] **2.23c — лендинг: упоминания фаз** · один контент-проход + один гейт
  - [ ] `PhaseChip.astro:30` — `width: fit-content` + `justify-self: start`. Чип `inline-flex`
  ...
  - [ ] Ссылка на фазу в turnkey-ячейке (`comparison.json:60`) не выделена ничем: `global.css:25`
  ...
  - [ ] Sticky-заголовок наезжает на кнопку подписки: `.roadmap-waitlist` авторазмещается в
  ...
  - [ ] Контент-проход: лейблами оформлены 3 упоминания фазы из ~20. Остальные — проза в CMS-полях.
  ...
  - [ ] **Знак `+` убирается.** Он значил «эта фаза или позже» и приехал из внутреннего
  ...
  - [ ] **Правка контента лендинга — правка в двух местах.** Прод рендерится из Payload,
