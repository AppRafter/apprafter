...
**Поставка:**
- [ ] `LICENSE` в корне с текстом FSL-1.1-MIT (из официального шаблона sentry/license-templates).
- [ ] `LICENSE-MIT` для будущей конверсии (для прозрачности).
- [ ] `NOTICE` с описанием модели (2-летнее окно → MIT) на русском и английском.
- [ ] SPDX-заголовок-шаблон в `docs/contributing/license-headers.md`.
- [ ] Подпапки плагинов (`providers/`, `backstage-plugins/`) — отдельный `LICENSE` MIT (см. §7).

**Acceptance:** GitHub распознаёт лицензию; SPDX-header применим ко всем будущим исходникам.
