...
- [ ] **Слой механики недостроен.** Пять гайдов не имеют куда сослаться, потому что страницы
      механики для них не существует: `secrets.md`, `private-repos-and-registries.md`,
      `backup-maintenance.md`, `cloudflare-origin-cert.md`, `recovery.md`. Нужны
      `sealing-a-secret.md` (ADR 0007), `source-credentials.md` (ADR 0039), `the-public-route.md`
      (зона → пара листенеров → HTTPRoute → `PublicRouteReady` + origin-firewall),
      `what-cluster-bootstrap-installs.md` (ADR 0025/0028). Это прямое продолжение свипа 2.23n.
