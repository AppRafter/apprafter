...
- [ ] **(i) Контент** *(= W8)*. ~15 недостающих гайдов волнами (env value references, sealed secrets, per-env deploy, machine picker, `spec.resources` + VPA, портал) — каждый с маркерами, frontmatter, биндингами и рецептами.
...
**Размер:** XL · a–f (закрыты) — L; масса впереди в i (контент) и j (прогулка по каждому гайду).
