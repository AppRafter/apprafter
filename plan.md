...
- [ ] **(f) Информационная архитектура** *(= W5)*. Перестройка nav, разбиение `needs-*`-walk'ов, карта редиректов, переприцеливание входящих ссылок. Идёт ДО публикации (g), чтобы редиректы существовали раньше публичных URL.
...
**Размер:** XL · a+b (закрыты) — L; масса впереди в c (гейт вне fence'ей — главный дизайн-риск), i (контент) и j (прогулка по каждому гайду).
