...
- [x] `docs/changelog/history.md` — Keep a Changelog v1.1 формат, секция Phase 0 (v0.0.1 → v0.0.8) с Added/Changed.
...
- [x] Release notes (v0.1.41 — `docs/changelog/history.md` graduates the Phase 1 section into a `v0.1.0-mvp` release block).
...
- [x] `docs/changelog/history.md` — new `v0.1.69` block с Changed/Added/Docs/Backwards-compatibility секциями; historic v0.1.x entries сохранены as-is (no rewriting of past system state).
...
> 🔁 **ПИВОТ при реализации (2026-06-11): host-network, НЕ LB-IPAM+L2.** Cilium L2-announcement — bare-metal-only (ARP на общем L2-сегменте, IPv6/NDP не поддерживает); на Hetzner Cloud публичный IP узла маршрутизируется SDN провайдера, не через ARP → LB-IPAM/L2 не назначат и не анонсируют адрес. Правильный механизм для single-node-cloud (у узла уже есть публичный IP) — Gateway API **host-network mode**: Envoy биндит host-netns порты 80/443 прямо на узле, `node-IP:443` → Gateway, БЕЗ LoadBalancer-Service / LB-IPAM / L2. Релиз **platform-stack 0.2.25** (chart-only, БЕЗ CLI — cilium loader-values байт-идентичны). Build отгружен на master; **1.83a ЗАКРЫТ 2026-06-13** — чистый T8 live-Hetzner-ретест зелёный (F1–F4, мульти-домен, teardown, без ручных рестартов) после фиксов 0.2.27 (F1/F2-v1) → 0.2.32 (F2-rollOutPods весь cilium-стек) + cli v0.2.14 (F3 ignoreDifferences) + F4-резолв через F3-Gateway.
...
**T8 walk-fixes (live-Hetzner 2026-06-12 — known gotchas cilium host-network Gateway on GitOps):**
- [x] **F1 (BLOCKER, 0.2.27):** `gateway-api-crds` → **experimental** канал (`config/crd/experimental`), не standard — cilium 1.16.5 gateway-контроллер на старте требует `tlsroutes.gateway.networking.k8s.io` (v1alpha2, только в experimental); без него контроллер падает на required-resources-check → **Gateway никогда не Programs** (NB: `GatewayClass.Accepted` на cilium 1.16.5 остаётся `Unknown` даже при здоровом Programmed Gateway — несущий сигнал = `Gateway.Programmed`, НЕ GatewayClass.Accepted; мой прежний runbook-чек на GatewayClass был неверным). + регресс-гард в `check-gateway-render.sh`.
- [x] **Gateway-walk регресс-гард (sandbox-run — дыра, что kind не закрыл):** новый `e2e/gateway-walk.sh` — реальный Cilium 1.16.5 + gateway-контроллер под `sandbox-run` (rootful microVM, memlock unlimited; в kind под rootless podman cilium-gateway не живёт) → experimental CRD + наш отрендеренный `gateway.yaml` → assert `Gateway.Programmed=True` + listeners. **Прогнан ЗЕЛЁНЫМ:** positive (experimental → Programmed) + negative (`APPRAFTER_GW_WALK_NEGATIVE=1`: standard → Gateway никогда не Programs — F1-симптом воспроизведён). + nightly workflow. Закрывает test-gap: контроллерный уровень F1 был sandbox-валидируем (а я пунтанул всё в Hetzner).
- [x] **F2 (operability — РАСШИРЕН в Run 2, 0.2.27→0.2.32):** cilium читает config-ФЛАГИ только на старте → включение gateway-api/host-network на ЖИВОМ кластере требует рестарта ВСЕГО стека: **agent (`ds/cilium`** — отдаёт CiliumEnvoyConfig в Envoy по xDS), **operator** (Gateway→CEC), **standalone envoy (`ds/cilium-envoy`** — биндит host-netns 80/443). Run-2 walk поймал: Gateway `Programmed=True`, но Envoy не биндит 80/443, т.к. agent на старом конфиге не пушит CEC-listener → только ручной `rollout restart` всех трёх анблокировал. Прежний 0.2.27-фикс (`operator.podAnnotations.cilium-config-rev`) катил ТОЛЬКО оператора. **Заменён на штатные cilium-values `rollOutCiliumPods`/`operator.rollOutPods`/`envoy.rollOutPods: true`** (config-checksum на pod-template → авто-ролл каждого пода при изменении cilium-config) — 0.2.32, chart-only (loader байт-идентичен, no operator/CLI). Fresh-инсталлы не затронуты (upgrade-корректность).
- [x] **F3 (cosmetic, CLI v0.2.14):** `ignoreDifferences` на root `platform` Application — иначе Gateway+HTTPRoute вечно OutOfSync в Argo (selfHeal зря churn'ит). **Поля по ЖИВОМУ Argo DIFF (T8 Run 2 — apiserver-дефолтинг, НЕ финалайзеры):** Gateway `.spec.listeners[].tls.certificateRefs[].group` (apiserver дозаполняет пустой core-group `''`); HTTPRoute `.spec.parentRefs[].group` (`gateway.networking.k8s.io`) + `.spec.parentRefs[].kind` (`Gateway`) + `.spec.rules[].matches` (apiserver дефолтит match-all `PathPrefix /`, наш redirect-rule без matches). В `render_root_application` (новые bootstrap'ы) + регресс-тест `render_root_application_ignores_gateway_api_defaulting`; existing-кластер — одноразовый `kubectl patch application platform ... spec.ignoreDifferences` (поле оператор НЕ владеет → переживает его SSA-патчи).
- [x] **T8 — live Hetzner host-network smoke (приёмка) — ✅ ВЗЯТ (чистый ретест 2026-06-13, F1–F4 все зелёные вживую, БЕЗ ручных рестартов):** Gateway `Programmed=True` за 1s, 3 listener'а Accepted/Programmed/ResolvedRefs, `curl -I http://demo.apprafter.dev` → **301** → https, :443 TLS-терминируется (404 = нет app-route); **мульти-домен** (2-я entry demo2 → +2 listener'а apex+wildcard, curl 301); **teardown** (`allowedDomains:[]`) → Gateway+HTTPRoute сами `NotFound`, root App `Synced`; F2-rollOutPods отработал (Programmed без ручного restart'а cilium-стека), F3-ignoreDifferences → root Synced, F4 → проекция+prune сами. GatewayClass `Accepted=Unknown` — informational cilium-quirk, не гейтили. Gateway `Programmed=True`, Envoy биндит ОБА порта 80/443, `curl -I http://<zone>` → 301. F1+F2 разблокируют Programmed (0.2.27). Острые углы: cilium [#42786](https://github.com/cilium/cilium/issues/42786) (PROGRAMMED:False)=F2; [#39422](https://github.com/cilium/cilium/issues/39422) (80+443) — по `ss -tlnp`. Домен/серт вручную (1.83e/1.83f дальше). **Runbook (исправлен R1-R4):** `NODE_IP` из `InternalIP`+`awk '{print $1}'` (Hetzner без CCM не вешает ExternalIP; InternalIP отдаёт v4+v6); проверять cilium-config, НЕ Argo-`application` (CRD-коллизия с `applications.apprafter.io`); openssl через `nix run nixpkgs#openssl req --`; без `yq`. Полные команды — в отчёте/ответе ассистента 2026-06-12.
...
- [ ] Migration лендинг-манифеста: убрать label `apprafter.io/hostname` (см. PART 0.6 чек 2 — если label не используется, пропустить), добавить `spec.base.expose.hostname: "apprafter.dev"`.
  > 🐞 **Довесок к этому же пункту — мягкий 404 лендинга (найдено в 2.19g, НЕ правилось там).** в `landing/web/Caddyfile` директива `try_files` имеет хвост `/404.html` (`grep -n try_files landing/web/Caddyfile`): неизвестный URL переписывается в существующий файл, `file_server` падать не на чем, и **любой** неизвестный URL отвечает **200** с телом 404-страницы. Краулеры индексируют такой ответ как настоящую страницу. Перемерено A/B на реальном `landing/web/Caddyfile` поверх реально собранного `dist`; правильная форма — убрать хвост и ловить в `handle_errors`, как сделано в `docs-site/Caddyfile`.
  > **Почему это ждёт здесь, а не было починено на месте:** `.github/workflows/landing-autotag.yml` срабатывает на любом пуше в дефолтную ветку с дифом по `landing/**` → автотег `landing-v*` → `release-landing.yml` → `landing-web:latest` → оператор перерезолвит дайджест и выкатит. Любая правка под `landing/**` = прод-деплой лендинга, поэтому она делается ОСОЗНАННО и вместе с остальными изменениями лендинга, а не мимоходом из чужой подфазы. Тот же механизм — причина, по которой переключатель `docsUrl` в 2.19g стал шагом оператора, а не придержанным коммитом.
...
- [x] **2.4g** ✅ — `e2e/needs-pg-walk.sh` (k3d, подключён в Justfile + nightly `e2e-pg-nightly.yml`) + **подробный ручной walk на реальном Tier-1** (scaffold→add→provision→DSN→delete→recover→GC). Walk выявил два 2.4f GC-бага → пофикшены walk-fixами (operator v0.2.11 hardening) и **live-подтверждены** на перепрогоне 2026-06-05: (A) recovery time-bomb — ре-провижен отменяет `RetainedClaim` + GC live-guard пропускает дроп живого claim'а (лог `live ResourceClaim present (recovery) … skipping drop`); (B) role-leak — phased drop (`ensure:absent` после DB-дропа → CNPG дропает роль → прун после подтверждения), `pg_roles` пуст после полного GC. Walk также вскрыл отдельный thread platform-version-resolver'а (PlatformController): OCI-пагинация + ghcr `tags:null` (v0.2.12), channel-tag resolver O(1) + publish-side `oras tag` (ADR 0041, v0.2.13), pin-enforcement-survives-poll-failure + condition `UpstreamReachable` (v0.2.14) — каждый adversarial-verified. CLI walk-fixes (app logs inner-name, app status pods+services+claims, scaffold --needs, app add publicness probe, skip-if-current loader, Tier-1 autoUpgrade opt-out, group-qualified Argo wait — 8 коммитов) собраны в **platform-cli v0.2.5** (монорепо-тег v0.2.5). Гейт зелёный (cli build/clippy -D warnings/test + cue vet -c). **NOTE: 2.4 ≠ закрытие Фазы 2 — §6 M2 НЕ флипать.** Ждёт пуша.
...
**Декомпозиция:**
- [x] **2.4h-a** — OCI-registry-клиент (новый модуль): manifest HEAD/GET → `Docker-Content-Digest`, Bearer-token-флоу (`WWW-Authenticate` realm/service/scope для ghcr/dockerhub), auth из `dockerconfigjson`, анонимный путь. Pure-парсеры + мокнутый HTTP. Основная масса.
- [x] **2.4h-b** — схема: `spec.base.imagePolicy.resolve` + `status.image.{tag,resolved,resolvedAt}` + condition `ImageResolved` в 4 зеркала (CUE `application.cue` + kube-rs `operator-core` + OpenAPI `crd-application.yaml` + webhook). cue-vet пример.
- [x] **2.4h-c** — `operator-rendering`: принимает резолвнутый digest-параметр, рендерит `image:` как digest (или verbatim при `off`/fallback). Остаётся чистой функцией.
- [x] **2.4h-d** — интеграция в контроллер application: resolve (2.4h-a + `pick_pull_credential`) → requeue ~60с conditional → status-write (свой field-manager, SSA-split) → render-with-digest → graceful-fallback + `ImageResolved=False`. Метрика `apprafter_image_resolve_total{result}`; image-change мимо MigrationPlan.
- [x] **2.4h-e** — `apprafter app status` показывает running-digest (`status.image.resolved` + tag + resolvedAt-age).
- [x] **2.4h-f** — dev-guide про цикл итерации (push→auto-deploy, opt-out, escape-hatch `rollout restart`); shipped-пример + CMS-манифест с явным `imagePolicy`. Координированный operator+platform-stack release bump.

> ✅ **2.4h закрыт (2026-06-05).** Subagent-driven (12 TDD-тасков) на ветке `feat/2.4h-image-digest`: hand-rolled OCI-resolver (manifest HEAD → `Docker-Content-Digest`, bearer/dockerconfigjson/anon, инжектируемый `RegistryHttp`-seam; `oci_distribution::Reference` для парса — корректнее hand-rolled), 4 зеркала схемы, чистый renderer-параметр, контроллер-интеграция (throttle ~60с на `status.image.resolvedAt` как у PlatformController; record-tag-on-failure; dedup cred-list; graceful-fallback никогда не блокирует rollout), `app status` running-digest, dev-guide. Whole-feature adversarial-review (6 dimensions × 3-skeptic) нашёл 14 подтверждённых (critical: no-throttle storm; majors: tag-not-in-status, digest-validation, per-env merge, cred-dedup) → все пофикшены sweep'ом + verified. Метрика `apprafter_image_resolve_total{result}`. Релиз: operator+platform-stack **v0.2.15** (CRD-additive, change=safe, default-ON all tiers) + cli **v0.2.6**. Acceptance ниже — на real-cluster walk (после merge+push).
...
- ✅ **ЗАКРЫТО** (2026-09-12, follow-up-пасс; строка в `plan-history.md`): арм `jetstream` → `nats-system:4222` (селектор `app.kubernetes.io/name=nats` + `.../component=nats`, снят с живого `nats-0`), census-тест на весь класс (перечисляет ключи из схемы самого `Needs`) и acceptance #15 в walk'е (14 → 15), который читает применённый CNP и проверяет селектор против живого пода. Enforcement на настоящем Cilium по-прежнему не доказан — это единственное, что осталось. Исходная запись сохранена как история: ~~**Egress-правила для NATS нет.**~~ ADR 0061 §1 обещает, что координаты сервера попадают в needs-derived egress-политику (ADR 0045) как тройка `(namespace, podSelector, port)` — но `default_target` в `operator-rendering/src/egress.rs` арма `jetstream` не имеет. CNP рендерится всегда, egress становится default-deny, `world` не покрывает in-cluster endpoints — то есть под приложения с `needs.jetstream` НЕ дотягивается до `nats-system:4222` на любом профиле. Walk этого показать не мог: он поднимается без Cilium (`k3d_up`, не `kind_up_cilium`), а весь NATS-трафик в нём идёт из `nats-box`, который не Application и никаким CNP не селектится. Доки говорят об этом прямо (guide + how-it-works), и `docsgen behaviour::CLAIMS` заставит их отозвать фразу в тот день, когда арм появится.
- ✅ **ЗАКРЫТО** (2026-09-12, тот же пасс): `validate_jetstream_env_overrides` отклоняет override, который опускает `streams`/`consume`, перечисляя теряемое; триггер — отсутствие КЛЮЧА, а не пустота, так что явный `streams: []` остаётся способом сказать «в этом окружении их нет». Исходная запись как история: ~~**Вебхук не отклоняет per-env override, который роняет `streams`/`consume`.**~~ ADR 0061 §6 обещает отказ с указанием того, что теряется; в `validate_jetstream_need` такой проверки нет, а merge в `effective_spec` остаётся per-key whole-object replace. Значит `environments.prod.needs.jetstream: {size: …}` молча сносит весь контракт продьюсера/консьюмера для этого окружения. Документировано как поведение, не как баг — но обещание ADR не выполнено.
...
- ✅ **ЗАКРЫТО** (2026-09-12, тот же пасс): колонка заполняется из `status.streams` (declared + dynamic, `unattributed` не считается — это capture, а не ёмкость; consume-only читается как `no streams`). Попутно выяснилось, что `pg` печатает `—` ровно так же и это честно: CNPG не пишет ни `instance`, ни `volumeClaimRef`. Исходная запись как история: ~~**`backing_resource` в `app status` для jetstream-claim'а печатает `—`**~~ (нет `instance`/`dbnum`/`volumeClaimRef`). Косметика.
...
- ✅ **ЗАКРЫТО** (2026-09-12, walk-пасс; строка в `plan-history.md`): acceptance **#16** в `needs-jetstream-walk.sh` (15 → 16). Фикстура `feeder` (объявляет стрим `inbox` над `["feeder.own.>", "arrival.>"]`) + `arrival` + `bystander`. Позитив: на claim'е `arrival` появляется `PrefixPreCaptured`/`PrefixDeclaredElsewhere` с СОСТАВНЫМ именем `feeder_inbox`, и claim при этом **доходит до Ready** — это отчёт, а не гейт; `ForeignSubjectCapture` на нём молчит (за объявленный fan-in namespace поручился). `feeder.own.>` не декорация: без него у self-фильтра `p.app != me.app` нечего было бы отфильтровывать и негатив проходил бы по неправильной причине. Оба негатива гейтятся на наличии `status.streams.observedAt` — инвентарь и условия едут ОДНИМ SSA-apply, так что «условия нет» отличимо от «правила не отрабатывали». Живая мутация: снимаем `arrival.>` с объявления — условие ГАСНЕТ (сужение; арм 1 триггера #16 срабатывает только на ДОБАВЛЕНИЕ чужого сабджекта, так что аппрува не нужно — проверено отсутствием MigrationPlan). Исходная запись как история: ~~**`PrefixPreCaptured` покрыт только юнит-тестами**~~ — на живом кластере не наблюдался ни разу.
- **Сумма `max_mem` аккаунтов не зажата, и переполнение НЕ падает громко** (найдено при написании #16; измерено на пиннутом `nats:2.14.3-alpine` в podman). Каждый namespace получает `max(file_quota/10, 64Mi)` памяти (`account_max_mem_bytes` + `ACCOUNT_MAX_MEM_FLOOR_BYTES`), `component_nats.cue` пинит `memoryStore.maxSize: 192Mi` — под это влезает ~три namespace'а. Комментарий в `component_nats.cue` предсказывает, что на переполнении сервер «fail to (re)start … cluster-wide»; **на пути SIGHUP-релоада это не так и хуже**: сервер логирует `Error enabling jetstream on configured accounts: insufficient memory resources available (10028)`, следом печатает `Reloaded server configuration` и продолжает работать — старые аккаунты живы, а НОВЫЙ остаётся вообще без JetStream, при этом его юзеры нормально аутентифицируются. Провижионер вечно крутится на `AwaitingNatsReady` («the server may not have reloaded the accounts file yet») — ровно то, что как раз произошло успешно. Два следствия: (1) зажать сумму `max_mem` там же, где ADR 0061 §6 зажимает сумму файловой квоты; (2) сообщение `AwaitingNatsReady` вводит в заблуждение — стоит различать «не перезагрузился» и «перезагрузился, но у аккаунта нет JetStream». Именно поэтому #16 живёт в `demo`, а не в своём namespace: четвёртый аккаунт туда уже не влезал.
...
**Зафиксированные решения (с пользователем, 2026-06-05; ADR 0042 — переписан на per-DB `$N`):**
- **Deployment = dragonfly-operator always-on + ПУЛ shared-инстансов LAZY** (как CNPG). platform-stack ставит dragonfly-operator + сид `redis-integrated` ServiceProvider (`type: redis`, `backend: dragonfly`). Provisioner лениво создаёт shared `Dragonfly` CR по классу persistence (`platform-redis-ephemeral-<NNN>` / `platform-redis-persistent-<NNN>` со snapshot→PVC). Solo без redis-приложений не платит за под. **Инстанс-инварианты (ИЗМЕРЕНО 2026-06-05):** `--dbnum=1024` (жёсткий max `kMaxDbId`; пустые БД бесплатны — ленивая аллокация, idle-RSS плоский по dbnum), `--num_shards=1` (**дефолт, ВСЕ тиры**) через `spec.args` — оператор НЕ ставит ни dbnum, ни num_shards (иначе авто-шардинг по vCPU ноды раздувает память на tenant'а: floor = 280 кБ × shards). Число шардов — **operator-override на platform-манифесте** (seed `redis-integrated` ServiceProvider / platform values — там же tier-aware sizing типа CNPG `instances`): хочешь больше throughput — поднимаешь руками без правки кода. `maxmemory-policy noeviction`. dbnum **иммутабелен в рантайме** (restart-only) → рост ёмкости только ГОРИЗОНТАЛЬНО (новый инстанс пула) при 1024 claim'ах/инстанс.
- **Изоляция = своя нумерованная БД + `$N`-pinned ACL-user** (НЕ key-prefix; НЕ requirepass — server-global; НЕ голый logical-DB — `SELECT` не граница). Provisioner ИМПЕРАТИВНО (новый redis-клиент, admin-кред инстанса) создаёт `claim_<ns>_<app>_redis` с `$<N> resetkeys ~* resetchannels &claim_<ns>_<app>_redis:* +@all -@admin -@dangerous +info +client|… +sort_ro`. `$N` пинит юзера к БД N (`SELECT` чужой → NOPERM) — **жёсткая граница БЕЗ key-prefix на стороне приложения**, `SCAN`/`RANDOMKEY` заперты в БД N (нет утечки имён ключей). Канал-prefix `&claim_x:*` — только для pub/sub (каналы не DB-scoped). Скриптинг разрешён (declared-keys default Dragonfly + `$N` запирают Lua) — queue/lock (BullMQ/Redlock) — основной кейс.
- **Аллокация DB-номера = claim → `(instance, dbnum)` в `ResourceClaim.status`** (k8s-объект, переживает рестарт инстанса, GC'ится RetainedClaim-машинерией). Lowest-free `dbnum<1024`; free/used индекс — кэш из живых claim-статусов. Нет слота → новый инстанс пула (или `InsufficientCapacity` до auto-grow). **Recycle-safety:** `FLUSHDB` на аллокации (не только teardown) + всегда свежий ACL-user — чтобы новый tenant не унаследовал ключи мёртвого при сбойной очистке.
- **ACL-reconcile loop** (обязателен): per-claim юзеры — runtime-state (in-memory, теряются на reload пода). Луп смотрит readiness инстансов пула и на (ре)старте ре-применяет `ACL SETUSER` всем живым claim'ам инстанса (re-pin к `dbnum` из status), + периодический resync. Расширяет `resourceclaim-provisioner` (он уже владеет redis-клиентом/секретами/индексом).
- **Persistence = per-claim opt-in → placement.** `#ServiceNeed` получает `persistent?: bool` (дефолт false) → 4 зеркала. Persistence в Dragonfly — **на уровне инстанса** (RDB/AOF — весь датасет, нет per-DB snapshot/restore): `{}` → ephemeral-инстанс пула; `{persistent:true}` → persistent-инстанс. Изолированная durability (свой RPO/restore) → per-claim-instance tier.
- **DSN = DB-pinned URL + channel-only prefix.** connection-Secret: `REDIS_URL = redis://claim_x:<pass>@<instance>.dragonfly-system.svc:6379/<N>` (`/<N>` + `$N` ACL → жёсткий пин к БД N, **ключи без prefix**) + `REDIS_CHANNEL_PREFIX = claim_x:` (только для имён pub/sub-каналов). Таблица needs→env рендерера (2.4e) обобщается до СПИСКА пар: pg→[(DATABASE_URL,DATABASE_URL)]; redis→[(REDIS_URL,REDIS_URL),(REDIS_CHANNEL_PREFIX,REDIS_CHANNEL_PREFIX)]. webhook reserved-env guard отклоняет литеральные `REDIS_URL`/`REDIS_CHANNEL_PREFIX` при `needs.redis`.
- **GC = per-DB `FLUSHDB`.** Dragonfly-путь в 2.4f GC: после grace — `SELECT N; FLUSHDB ASYNC` (один командой, не `SCAN MATCH|UNLINK`) + `ACL DELUSER claim_x` (рвёт живые коннекты юзера) + вернуть N в free-pool + удалить connection-Secret. Shared-инстансы остаются.
...
**Размер:** L (per-DB аллокация + ACL-reconcile loop сверх 2.4-паттерна)
...
> ✅ **ЗАКРЫТО (launch-срез, 2026-06-06).** Subagent-driven по декомпозиции 2.6b-1…2.6b-6 (ветка `feat/2.6b-needs-disk`). `needs` обобщён до **(type, name)** скаляр\|массив (закрытая структура + `OneOrMany<T>`); Application генерирует один `ResourceClaim` на entry (`<app>-<type>[-<name>]`), рендерер дизамбигуирует инъекцию (`DATABASE_URL_<NAME>` для named pg; unnamed остаётся `DATABASE_URL`). Диск — **через ResourceClaim**: `Backend::Disk` создаёт **unowned** RWO PVC (matched к сиду `disk-local`; class local / local-path на T1), рендерер монтирует в `mountPath` + пивотит Deployment в `strategy:Recreate`, GC снапшотит disk-RetainedClaim (volumeClaimRef+namespace) → unowned PVC переживает grace → reattach при redeploy (cancel-on-reprovision) → force-GC дропает PVC+снапшот. **Live-walk `e2e/needs-disk-walk.sh` GREEN end-to-end** (10 фаз, kind+podman, LOCAL_OPERATOR, ~3.5 мин): generate→schedule→provision(unowned RWO PVC)→resume+mount+Recreate→data-durability(переживает рестарт пода)→delete+snapshot+PVC-survives→reattach(data intact)→force-GC(PVC дропнут) + multi-pg-массив assertion (2 CNPG DB + `DATABASE_URL_PRIMARY`/`_ANALYTICS`). **Walk вскрыл баги — все пофикшены + re-walk GREEN:** (operator) gate-readiness требовал `connectionSecretRef`, которого у disk-claim нет → принимает `volumeClaimRef`; ServiceProvider CRD `type`-enum без `disk`; ResourceClaim `spec.size` t-shirt-enum → per-type (quantity для disk, t-shirt для service); (harness) PVC RBAC additive, ожидание терминации старого webhook-пода (rollout-race), **Phase-4 transient-gate**: диск провижится мгновенно (single-pass local-path + pre-warmed CNPG) → `AwaitingResourceClaim` ненаблюдаем поллингом → гейт доказывается ДЕТЕРМИНИРОВАННО (claim-генерация + Phase-7 mount/env; pause-логика — юнит-тесты). **scaffold `--needs` расширен на `redis`+`disk`** (решение пользователя — закрыл pre-existing mismatch: redis-walk-док уже документировал `--needs redis`, которого не было). **Релиз:** operator+platform-stack **v0.2.21** (CRD-additive, change=safe, CRD-compatible 0.2.20) + cli **v0.2.7** (монорепо-тег `v0.2.7` локально; binary изменился — scaffold). Гейты: cargo fmt/clippy -D warnings/test --workspace (cli+operator) + just lint + cue vet + `just crd-validate` (7 CRD Established) + `just platform-stack-check` + strict mkdocs. Доки: `operator-guide/needs-disk-walk.md` (ручной Tier-1 walk) + dev-guide needs-reference + nightly `e2e-disk-nightly.yml` (07:00 UTC). **NOTE: 2.6b ≠ закрытие Фазы 2 — §6 M2 НЕ флипать.** **Запушено + опубликовано** (0.2.21; теги `operator/v0.2.21` + `platform-stack/v0.2.21` на origin, монорепо `v0.2.7`). **Валидировано на published-релизе 2026-06-07:** все 4 e2e-walk'а зелёные на kind+podman пользователя (disk/gitops/pg/redis); non-LOCAL disk-walk бутстрапит platform-stack 0.2.21 из ghcr, пропускает сборку и зелёный за **2m52s** → релизный v0.2.21 реализует весь needs.disk-чейн для реальных юзеров (disk-local seed, 2.6b CRD, provision+GC PVC в опубликованном чарте; ClusterRole `rbac.yaml` даёт PVC create/delete). **2 follow-up'а (на origin/master, post-release):** `de943fc` (operator/webhook Dockerfile `FROM rust:` → `docker.io/library/rust:` — иначе podman short-name prompt вешает LOCAL-сборку) + `9f2293f` (`just e2e-disk`/nightly по умолчанию на published-образ, как pg/redis; LOCAL_OPERATOR за явным флагом для pre-release-валидации). Отложенный full-vision (StatefulSet/per-replica/replicated/shared/snapshots/auto-expand) — чекбоксы ниже, per ADR 0043 §Decision, НЕ входит в launch.

**Source:** design session 2026-05-27; **ADR 0043** (needs.disk + named multi-claim needs) + design `docs/superpowers/specs/2026-06-06-needs-disk-named-claims-design.md`.
...
> Это **T2 CSI-комплемент** к 2.6d: 2.6d поставил T1-путь (restic pull через ephemeral RO-helper — работает и на single-node без CSI-snapshot-capable драйвера); per-claim `backup.enabled` CSI-snapshot ниже — его T2+ дополнение (VolumeSnapshot вместо restic-pull, когда драйвер поддерживает снапшоты). Оба сходятся в один `restore`-опыт.
- [ ] При `backup.enabled: true` operator создаёт `VolumeSnapshotClass` reference + AppRafter CronJob per claim (Velero отклонён, ADR 0050; CSI-VolumeSnapshot заменяет restic-pull только на T2+ где драйвер поддерживает снапшоты).
...
> ✅ **CLOSED 2026-06-19.** Координированный CRD-additive release **operator v0.2.31 / platform-stack 0.2.37 / cue-cmp 0.1.13 / cli v0.2.28** (monorepo tag v0.2.28, локально). Что зашипилось: новый namespaced **`SharedVolume` CRD** (`spec:{size,class}`, `status:{ready,pvcRef,refCount}`) с explicit lifecycle (`apprafter volume create/rm/list/status`); cross-app `needs.disk.ref` (referenced `{ref,mountPath,readOnly?}` vs owned `{name,size,mountPath}`, webhook разводит по shape; reference-arm биндит существующий PVC, не провижинит/не GC'ит backing; renderer mount БЕЗ форса `Recreate`); новый backend-тип **`shared-disk`** (не кросс-матчится на owned-`disk`) + `shared-local` ServiceProvider seed (T1 local-path); **capacity-signal** через kubelet Summary API (RBAC `nodes/stats`) → condition + Warning Event при node-free<threshold, surface в `app status`/`volume status`. ADR 0049 + `operator-guide/shared-volumes.md`. **kind+podman live walk GREEN end-to-end** (2 app в одном ns ref один SharedVolume читают/пишут общий dir, rolling update не роняет mount, `volume rm` отказывает пока референсится). **node-free capacity live-read** — best-effort, **SOFT-skipped на kind** (kubelet `nodes/stats` reachability cluster-зависим — валидируется на реальном кластере позже). 2.6c ≠ Phase closure (это order-3.5 pulled-up, §6/spec.md не трогаются). spec.md актуализация не требуется. Следующее по SR-порядку: 2.6d (export/backup) / далее order 4.
...
- [x] **2.6c-1 — capacity-signal (owned + shared).** Оператор читает per-PVC `usedBytes`/`capacityBytes` + node-level `fsAvailableBytes` через kubelet Summary API (`/stats/summary`, RBAC `nodes/stats`) — **без зависимости от full OTel** (он order 5). Condition в `status` + Warning Event при `node-free < threshold` (default 15%). Surface в `apprafter app status` / `apprafter volume status`. Defer: Grafana/Alertmanager → order 5.
- [x] **2.6c-2 — SharedVolume backend + `shared-local` provider + объект lifecycle.** Новый backend-тип **`shared-disk`** (НЕ `disk`) → scheduler не кросс-матчит на owned-`disk` линейку. `shared-local` ServiceProvider (T1): `storageClass: local-path`, провижинит ОДИН unowned PVC, `accessMode: ReadWriteOnce` (node-level — на single-node N подов монтируют корректно). T2-расширение свопает `shared-local`→`shared-nfs` (RWX), не трогая манифесты. SharedVolume — **выделенный namespaced CRD `apprafter.io/SharedVolume`** (`spec:{size,class}`, `status:{ready,pvcRef,refCount}`) с explicit lifecycle (`apprafter volume create/rm`), независим от приложений. Объектом, не «ref на голый PVC» — ради portability 0.2 (`ref` стабилен, backing меняется local-path→NFS).
- [x] **2.6c-3 — `ref`-грамматика + reference-arm + renderer + CLI.**
    - [x] `needs.disk: { ref, mountPath, readOnly? }` (referenced) vs `{ name, size, mountPath }` (owned); webhook разводит по shape.
    - [x] Webhook: reject `ref` на несуществующий SharedVolume; **reject cross-ns ref** на T1 (single-node same-ns инвариант) с hint «cross-namespace → T2 (NFS)».
    - [x] Provisioner reference-arm: биндит существующий PVC, не провижинит backing, **не GC'ит** при удалении app.
    - [x] Renderer: mount, **БЕЗ форса `strategy: Recreate`** (owned-arm форсит из-за single-PVC-cross-node; reference-arm на single-node RWO-multi-pod терпит RollingUpdate).
    - [x] CLI: `apprafter volume create/rm/list/status`; `rm` отказывает пока есть референсы. Refcount-GC с grace — defer.
- [x] **2.6c-4 — walk + docs + ADR 0049 + release.** `e2e/shared-volume-walk.sh` (2 app один ns ref один SharedVolume → оба видят writes; rolling update не роняет mount второго; `volume rm` отказывает пока референсится). Docs `operator-guide/shared-volumes.md`. ADR 0049 (cross-app SharedVolume, reference-концепт в claim-грамматике — cross-cutting как `(type,name)` в 0043). Координированный release operator+platform-stack+cli (CRD-additive).
...
> ✅ **CORE CLOSED 2026-07-15** (order-3.5 pulled-up, sub-subphases 2.6d-1…2.6d-3 core + 2.6d-5). **CLI-only release** (`cli/` + `e2e/` + `docs/` — НИ ОДНОГО operator/schema/chart-изменения → нет operator/platform-stack/cue-cmp bump). Что зашипилось: три cluster-scope команды поверх одной extraction — `apprafter export <ns>` (Kind 1: native `pg_dump -Fc` + деревья файлов через ephemeral RO-helper-pod → плоская самодостаточная папка, ноль restore-логики), `apprafter backup` (restic-обёртка тех же dumps + сериализованные AppRafter+Argo CR + расшифрованные секреты + manifest → зашифрованный restic-репо; local-filesystem default, ноль форса бакета), и `apprafter restore --into <fresh-cluster>` (Kind 2 restore-ordering apply-CR→provision→`pg_restore`/restic-restore→re-seal секретов в target'а SealedSecrets, auto-register всех приложений без ручного re-add) + `--data-only` (recover тома/БД в существующий кластер без пересоздания app). **Два-кластерный kind+podman walk (`e2e/backup-restore-walk.sh`) GREEN end-to-end** (export → зашифрованный restic backup → restore-into-fresh → `--data-only`, прогнан дважды). Walk вскрыл + пофиксил **3 реальных product-бага** (все прошли unit/review: pg_dump/pg_restore major-version-match helper-image selection + claim-ready-vs-DB-ready race — commit `3d22d92` с regression-тестом; image-name/pg18/target-registry walk-fixes — `f170d6a`) и **2 platform-фикса surface'нул**, поставленных отдельными релизами: sealed-secrets chart-source repoint (**platform-stack 0.2.38**, `564cba0`) + 2.4h `imagePolicy.resolve: "off"` CRD-YAML фикс (operator v0.2.32 / **platform-stack 0.2.39**). ADR 0050 + `operator-guide/backup-restore.md`. Release: **cli v0.2.29** (monorepo tag v0.2.29, локально). **`--reprovision` (mode a, T13) — SHIPPED + real-Hetzner-validated 2026-07-16 (cli v0.2.31)** (clone-to-new: destroy source → restore --reprovision → fresh box + data intact; walk `e2e/restore-reprovision-hetzner.sh` GREEN); **automated S3 push (2.6d-4) — SHIPPED 2026-07-17** (cli/runner v0.2.33 / operator v0.2.33 / platform-stack 0.2.42 / cue-cmp v0.1.14 / runner-образ `apprafter-backup:v0.2.33`; 0.2.40+0.2.41 yanked; AppRafter CronJob-restic, opt-in `PlatformStack.spec.backup`; ✅ **файловая + S3 backup/restore GREEN на real Hetzner** после 2 раундов live-фиксов — 1 release-coordination + 7 runner-багов, все прошли unit/CRD/review; ⏳ push 1 коммита за юзером); полная DR-валидация (`restore` за <1ч на реальном кластере) едет на real-Hetzner manual run. 2.6d ≠ Phase closure (order-3.5 pulled-up, §6/spec.md не трогаются). Следующее по SR-порядку (speedrun §4.2): **3.7 / 2.16b** (App-scope migration: auto-detect destructive app-ops + авто-app-scope-MigrationPlan + in-Argo аппрув), затем **4** (T2 substrate). (`--reprovision` T13 + полная DR-валидация на real-Hetzner — DONE 2026-07-16, cli v0.2.31.) **T12 (persistent-redis backup+restore) — SHIPPED 2026-08-28 (cli v0.2.51):** закрыл redis-ногу 2.6d, которая шипилась deferred (backup-таблица обещала redis-снапшот, которого никакой код не делал — 2.19j это назвал). `export`/`backup` захватывают `persistent:true` redis как whole-instance Dragonfly-снапшот (`SAVE`+tar → `redis/<ns>/<claim>/dump.tar`, дедуп по инстансу); `restore` (`--into` + `--data-only`) заливает обратно live-загрузкой в РАБОТАЮЩИЙ инстанс через `DFLY LOAD` (без scale → провизионер не видит исчезновения, не ре-провижнит/не FLUSH'ит). Первый cut скейлил CR 0→1 (освободить RWO-PVC под helper) → провизионер FLUSH'ил восстановленное на same-cluster `--data-only` — заменено на no-scale DFLY LOAD. `e2e/backup-restore-walk.sh` GREEN на ОБОИХ restore-путях (SET t12key → dump.tar → recovery на fresh + `--data-only`). CLI-only (нет operator/chart/cue-cmp).

> ⚠️ **Known wart / техдолг (2026-07-16, deferred → backlog):** upgrade ЖИВОГО кластера ЧЕРЕЗ границу, где новый platform-stack сид зависит от нового CRD-enum (напр. 2.6c: `shared-local` ServiceProvider `type: shared-disk` + расширенный ServiceProvider CRD enum), ловит transient **seed-vs-CRD-discovery гонку** — сид применяется, пока apiserver ещё не прогнал structural-schema discovery обновлённого CRD (CRD на sync-wave `-5`, но discovery-refresh отстаёт) → seed reject (`Unsupported value "shared-disk"`) → Argo исчерпывает 5 ретраев → **parent platform Application залипает degraded**, что даёт ДВА симптома: (1) stale seed-sync ошибку в Argo, (2) блок дальнейших авто-апгрейдов (PlatformController не продвигает `target`, пока parent не чистый Synced+Healthy — отсюда `available>current` при `UpgradeAvailable=False`). **Наблюдалось на реальном кластере (2026-07-16):** живой апгрейд-through-2.6c залип на 0.2.37 27 дней; ручной форс-синк platform Argo-app разрулил ОБА симптома → кластер доехал до channel-latest 0.2.39. Свежий kind-walk это структурно НЕ ловит (CRD ставится с нуля, гонки нет) — класс «ловит только живой апгрейд» (ср. 2.9 deployment-selector-immutable). **Fix (правильный, при upgrade-robustness / T2 работе):** усилить Argo retry/backoff на platform app-of-apps (перекрыть discovery-лаг) ИЛИ развести sync-wave «сид строго-после-CRD» с запасом, чтобы CRD-enum+сид апгрейды не залипали и не требовали ручного форс-синка. **Deferred** — не срочно (recoverable форс-синком, не data-loss).
...
- **Scheduled-runner — РЕШЕНО (2.6d-4, 2026-07-17): AppRafter CronJob-restic** (`apprafter-backup` runner переиспользует тот же `backup-core` движок — один backup-путь, ноль нового in-cluster operator'а). K8up отклонён (добавил бы CNCF-operator + его CRD + дублирующий движок; см. ADR 0050 Alternatives).
...
- [x] **2.6d-1 — `export` (Kind 1).** Per-resource native pull на машину оператора: `apprafter export <ns>` энумерит stateful-claim'ы → `pg_dump -Fc` (НЕ CNPG barman — он in-cluster PITR), дерево файлов (volume/SharedVolume), redis-snapshot (если persistent). Плоская самодостаточная папка (`pg/*.dump`, `volumes/<name>/...`, `manifest.json`). Цель: изучить локально + migrate-out. **Не backup.** Транспорт вниз — ephemeral RO-helper-pod → `tar`/stream (kubectl-cp семантика; на single-node RWO node-level → helper монтирует RO рядом с работающим app без contention).
- [x] **2.6d-2 — restic-движок + local-pull backup (T1 default).** AppRafter оркестрирует restic против extraction'а. **Default backend = local-filesystem на машине оператора** (через тот же ephemeral RO-helper → restic в локальный репо). Артефакт = `data (native dumps) + сериализованные AppRafter+Argo CR + секреты (расшифрованные) + manifest`, restic-wrapped (dedup/incremental). Репо шифруется (содержит секреты; ключ у оператора). Ноль форса бакета.
- [x] **2.6d-3 — `restore` с local pull (Kind 2) — restore-into-running.** Единственный restore-путь, в **аналогичный свежий кластер** (DR «source мёртв»): apply сериализованных CR → оператор провижинит свежие claim'ы → restic restore данных / `pg_restore` в них → **re-seal секретов** в SealedSecrets/OpenBao target'а. **Auto-register всех приложений** из бэкапа (Argo Application CR пересоздаются → ворклоады из Git); **ручного re-add приложений нет.** Restore-ordering (apply→provision→load→re-seal) — основная сложность блока. Также data-restore в существующий кластер (`--data-only`: recover тома/БД без пересоздания app). **Restore-into-running (`--into <running-target>` / `--data-only`) — DONE, walk-GREEN.** ✅ **`--reprovision` (mode a — provision свежего кластера как часть restore, T13) — SHIPPED + real-Hetzner-validated 2026-07-16** (cli v0.2.31). `bootstrap_all::run(target)` провижинит свежий кластер в таргете ПЕРЕД replay'ем (топология+токен из local target-config, R2; version-alignment через существующий version_warning + ApplyPlatformStack); ленивый kc резолвится после reprovision-шага; `--reprovision`+`--data-only` reject up front. Walk `e2e/restore-reprovision-hetzner.sh` GREEN: provision→CMS+seed→backup→**destroy (source dead)**→restore --reprovision→verify (server id сменился 151688298→151689614, CMS auto-Ready, секрет+маркер целы).
- [x] **2.6d-4 — automated S3 backup (opt-in) — SHIPPED 2026-07-17 (cli/runner v0.2.33 / operator v0.2.33 / platform-stack 0.2.42 / cue-cmp v0.1.14 / runner-образ `apprafter-backup:v0.2.33`; 0.2.40+0.2.41 yanked).** Scheduled push того же restic-репо в remote S3-бакет юзера. **Два раунда фиксов на живых Hetzner-walk'ах:** (1) release-coordination — operatorVersion сначала оставили на уже-опубликованной v0.2.32 (2.4h imagePolicy) → gate release-operator скипнул ребилд → operator-сторона не зашипилась + ps 0.2.40 запинил stale operator; + cue-cmp drift-guard упал → operator v0.2.33 + cue-cmp v0.1.14 + ps 0.2.41, 0.2.40 YANKED. (2) **runner-фиксы (7 багов, все прошли unit/CRD/review, вскрыл только live S3-walk):** RBAC — pods `patch` (SSA helper-pod), pods/exec `get` (kube-rs WebSocket exec = GET, не kubectl-POST), secrets `list`, `sourcecredentials`, configmaps `create` split (нельзя ограничить resourceNames); код раннера — singular `secret`→plural (kube-rs discovery) + `eprintln!` фатальной ошибки в stderr. **Validated end-to-end на real Hetzner** (fixtest-образ + RBAC: полный backup в Hetzner Object Storage → snapshot закоммичен + lastSuccess → restic restore из S3 чистый) → ps 0.2.42 + runner v0.2.33, 0.2.41 YANKED. ✅ **файловая + S3 backup/restore регрессии GREEN на real Hetzner.** ⏳ push (1 коммит) + опциональный финальный walk на опубликованном 0.2.42 — за юзером. **Runner = AppRafter CronJob-restic** (`apprafter-backup` бинарь поверх общего `backup-core` движка; K8up отклонён — см. ADR 0050). Chart-owned delivery (`templates/backup.yaml`, guard `{{- if .Values.backup.enabled }}` → SA/scoped-RBAC + nightly backup CronJob + weekly `restic check` CronJob + CiliumNetworkPolicy egress-pin). Config на `PlatformStack.spec.backup` (opt-in, default off, GitOps-native). CLI: `apprafter backup enable/disable/status/prune/check/unlock` (enable/disable/status — декларативные merge-patch spec.backup после fail-closed preflight; prune/check/unlock — operator-side restic с полными creds юзера) + `--staging-mode monolithic|sequential` на local-pull backup + `restore s3:… --credential-file`. **StagingMode**: monolithic (default, 1 snapshot/run, manifest v1) / sequential (per-claim snapshot-set, manifest-snapshot ПОСЛЕДНИМ как commit-point, peak-disk = max(claim)). **Retention**: host+format-aware `plan_prune` (фиксированный `--host apprafter-backup`, keep 7/4/6 на manifest-представителях, forget по explicit-id), `enforce: operator` (default, prune вне кластера) / `cluster`. **Two-tier creds**: оператор владеет полными creds вне кластера; cluster Secret scoped (Put/Get/List + Delete только `locks/*` → append-only history). Creds НИКОГДА в chart values (только `credentialRef.name` через `envFrom: secretRef`). Status в non-chart-owned `apprafter-backup-status` CM. **Не дефолт, opt-in.** Walks: `e2e/backup-s3-hetzner.sh` (real-Hetzner monolithic, user-gated на бакет юзера) + `e2e/backup-s3-sequential-kind.sh` (kind+MinIO sequential). ⏳ real-Hetzner walk-run + push — за юзером.
- [x] **2.6d-5 — walk + docs + ADR 0050 + release.** `e2e/backup-restore-walk.sh`: `export` даёт открываемые dumps; local-pull backup → restore в свежий kind-кластер поднимает app'ы (auto-register) + данные + секреты; data-restore в существующий кластер. Docs `operator-guide/backup-restore.md` (local-pull default, S3 opt-in, restore-into-fresh, restic repo portability). ADR 0050 (backup/restore + data portability: restic-движок, Kind1/Kind2, local-default+S3-opt-in, restore-ordering, re-seal, Velero-rejected rationale). **Два-кластерный kind+podman walk GREEN дважды** (см. CORE-closed note выше).
- [ ] **2.6d-6 — захват данных JetStream (закрывает вторую половину A6).** 2.6d закрылся 2026-07-15, `needs.jetstream` — 2026-09-12: тип claim'а, которого не существовало, в объём попасть не мог, и ADR 0061 `## Scope` держит «backup and restore of JetStream stores» вне 2.5 явным пунктом. Первая половина A6 (снапшот выглядел полным) закрыта `0608e9e` — маркер `no_data` в манифесте + предупреждение в `backup create`/`backup show`/`export`; вторая — сам захват — этот пункт. **Три неизвестные сняты замерами до написания кода** (2026-09-19): (1) `mgr_<ns>` уже рендерится с `publish/subscribe: allow [">"]` (`nats_accounts.rs::render_account`) — прав на `$JS.API.STREAM.SNAPSHOT`/`RESTORE` и на delivery-subject хватает, **operator не трогаем**, и это ровно та идентичность, которую ADR 0061 §4.2 зарезервировал за ADR 0050 («Snapshot is `mgr_<ns>`'s job»); (2) на пиннутом `nats:2.14.3-alpine` + `nats` CLI v0.2.3: `nats stream backup <stream> <dir>` кладёт `backup.json` + `stream.tar.s2`, консьюмеры включены по умолчанию, `nats stream restore <dir>` **поверх существующего стрима отказывает** (`Stream "X" already exist`, exit 1), а после `stream rm -f` восстанавливает и сообщения, и консьюмеров с их состоянием; (3) права in-cluster-раннера уже cluster-wide (`pods` create/get/patch/delete + `pods/exec` + `secrets` get/list), default-deny NetworkPolicy висит только на `default`, helper-под живёт в `nats-system` рядом с сервером → **ни чарта, ни CNP, ни RBAC не трогаем**. Итог: нога **CLI + один пин в чарте**. Код — только `cli/` (operator, RBAC, CNP не трогаются), но scheduled-раннер запускается ОДНИМ образом, чей тег — литерал в `platform.cue` (`#BackupValues.image`), и ничто его не выводит: без бампа пина ночной CronJob продолжил бы писать снапшоты без jetstream-данных, а `apprafter backup` с ноутбука — с ними. Поймал `scripts/check-backup-runner-pin.sh` (гейт, написанный ровно после того, как пин протух на 15 образов). Пин `v0.2.66` → `v0.2.76` едет чартом **platform-stack 0.2.78**.
  - [x] **Захват.** `DataKind::JetStream`; арм `"jetstream"` в `plan_extraction` порождает элемент на КАЖДЫЙ стрим из `status.streams.declared + dynamic` (`unattributed` не берём — claim за них не отвечает, ADR 0061 §9), claim без инвентаря пропускается как непровизионенный. Координаты сервера — из connection-Secret'а самого claim'а (`host`/`port`), namespace NATS выводится из host'а, креды — Secret `nats-mgr-<ns>` (ключи `user`/`password`) в нём же. Helper-под в namespace NATS на пиннутом `natsio/nats-box` (четвёртый образ в `images.rs`), `nats stream backup … >/dev/null 2>&1 && tar c -C /tmp/bk .` → `jetstream/<ns>/<claim>/<stream>.tar` (stdout несёт ТОЛЬКО tar).
  - [x] **Восстановление.** Шаг в `restore.rs` после провижининга claim'ов: untar → `nats stream rm -f <stream>` → `nats stream restore` с ретраем на `already exist` (NACK пересоздаёт объявленный стрим пустым по своему циклу — это гонка, а не ошибка; конфиг после restore доводит NACK, Git остаётся источником правды для конфигурации, снапшот — только для данных). Работает и для `--data-only`; удаление живого стрима перед заливкой — разрушительно по определению, говорится вслух.
  - [x] **Снятие предупреждения.** `jetstream` уходит из `CLAIM_TYPES_WITHOUT_DATA_CAPTURE`; тест на дизъюнктность двух таблиц уже стоит и заставит это сделать в одном коммите с армом планировщика. Механика A6 остаётся живой (следующий нешипнутый тип), её тесты переезжают на синтетический тип.
  - [x] **Доки + релиз.** Строка `jetstream` в таблице `## What a backup captures` переехала из «captured as configuration only» в захватываемые (с оговоркой про `unattributed` и про снапшоты старше 0.2.76); `operator-guide/jetstream.md` переписан с «a backup does not carry the messages» на «what a backup carries»; `--help` у `export`/`restore` больше не перечисляет только pg/volumes/redis. **Запись в `docsgen behaviour::CLAIMS`** на фразу «no jetstream data is in this» с анкером на арм планировщика — гейт мутационно проверен (возврат фразы в доки роняет `docsgen gate`). CLI-релиз v0.2.76 (он же версия раннера) + platform-stack **0.2.78** с бампом пина раннера; operator и cue-cmp не трогаются.
  - [x] **Walk — GREEN 2026-09-19 (10 ассерций 2.6d-6).** `e2e/backup-restore-walk.sh` вырос на jetstream-фикстуру (стрим + сообщения + консьюмер → backup → restore-into-fresh → сообщения и консьюмер на месте). Блокер снят: `platform-stack/v0.2.77` (фикс nack) уже на origin. **Прогоны 2026-09-19 нашли две поломки, обе НЕ в предмете подпункта:** (1) `maxBytes` у объявленного стрима обязателен намеренно («a stream without it silently claims the whole account quota») — apiserver отклонял Application целиком, дефект был в моей фикстуре; (2) **walk сломан с 2.28**: с ADR 0065 §1.3 `expose.port` без объявленной readiness получает TCP-пробу по умолчанию, а приложение-фикстура (мигратор + sleep) на 5432 не слушает — deployment не мог стать Available. Никакой CI этот walk не гоняет, поэтому поломка жила молча с 2026-09-15. Декоративный `expose` из фикстуры убран. (3) NATS-имя стрима СОСТАВНОЕ (`nats_stream_name` = `<app>_<declared>` → `shop_orders`), и фикстура звала его по объявленному имени; попутно вскрылось, что публикация ДО создания стрима уходит в core NATS и пропадает, а `nats pub` всё равно рапортует успех — поэтому walk теперь сначала дожидается, что `status.streams.declared` перечислил стрим (это ровно то поле, из которого планирует захват, так что пустой инвентарь больше не может дать «зелёный бэкап ни о чём»). (4) PURGE приложению запрещён (`allowPurge` по умолчанию false), состояние цели готовит менеджерская идентичность — отказ закреплён отдельной ассерцией.

    **Что доказано на кластере:** инвентарь → 3 сообщения + durable → артефакт `jetstream/demo/shop-jetstream/shop_orders.tar` (5120 B, `backup.json` + `stream.tar.s2`, снят helper-подом в `nats-system` под `nats-mgr-demo`) → **Phase 3b**: стрим приведён в состояние цели (существует и ПУСТ, durable снят) → `restore --data-only` → 3 сообщения и durable вернулись. Phase 3b НЕ гейтится вторым кластером специально: двухкластерные фазы на этой машине дважды ушли в SOFT-SKIP (корневое Argo-приложение свежего кластера не доходит до `Synced` за 600 с — предсуществующее ограничение харнесса, не связанное с 2.6d-6), и без неё restore остался бы непроверенным. До прогона захват доказан на пиннутом `nats:2.14.3-alpine`: оба шипящихся скрипта прогнаны дословно против настоящего сервера в podman — дамп 5.6 KB tar, цель с пересозданным ПУСТЫМ стримом (0 сообщений/0 консьюмеров) → после restore 13 сообщений и консьюмер `worker` со своим pending. Это доказывает механизм, но не k8s-путь (helper-под, RBAC, координаты из Secret'ов).


> **Попутная находка 2.6d-6 — ИСПРАВЛЕНО (cli v0.2.76):** `find_claim_data_dir` (`restore.rs`)
> распознавал per-claim-снапшот по каталогам `pg` / `redis` / `disk`, а экстрактор пишет тома в
> `volumes/` — и `disk/` не писала НИ ОДНА версия (`git log -S 'join("disk")'` пуст). То есть
> sequential-бэкап claim'а `needs.disk` давал снапшот, который restore пропускал с note'ом и
> рапортовал успех: том не восстанавливал никто. Класс тот же, что у D26 (`a75f0dc`, который эту
> функцию и завёл), на один каталог позже; sequential-walk (`backup-s3-sequential-kind.sh`)
> поднимает pg+redis и disk-claim'а не имеет, поэтому не мог этого увидеть. **Фикс:** раскладка
> стала ОДНИМ утверждением — `DataKind::payload_dir` (исчерпывающий match: новый kind не
> компилируется, пока не назовёт каталог) + `DataKind::ALL`, который проверяется тестом через
> планировщик. Обе стороны бэкапа читают одну таблицу вместо двух литералов.
...
**Open decisions:** ~~scheduled-runner K8up vs CronJob-restic~~ — РЕШЕНО в 2.6d-4 (AppRafter CronJob-restic; ADR 0050 Alternatives). Ещё открыто (T2): T2-default local-pull+warning vs internal-redundant-S3; нужен ли отдельный шифрованный-бандл формат vs «папка + restic-репо».
...
> 🎨 **Дизайн (2026-06-07, ADR 0044 + spec `docs/superpowers/specs/2026-06-07-2.9-per-env-deploy-design.md`):** env — **deploy-time** селектор per-Application, НЕ cluster-global (инертный `APPRAFTER_ENV` выпиливается). Модель **scalar**: один env-агностичный манифест → **один Argo-апп на env-деплой** (`<name>-<env>`), self-contained в своём namespace (ownerRef-каскад как есть), группировка лейблами `apprafter.io/application` + `apprafter.io/environment` (ключи **уже устоявшиеся** — migration-strategy ставит их на **MigrationPlan'ы** как discoverability-теги, `strategy.rs:163-175`; 2.9 переиспользует те же ключи на **Argo-аппе** (CLI `app add`) + **children** (рендерер) — та же семантика «принадлежит app X / env Y»; migration-гейт матчит по spec-полям `scope.application.ref`, НЕ по лейблам). `Application.spec.environment` (инжектит CMP) выбирает override; `effective_spec(app, spec.environment)` сливает; webhook `env ∈ environments`. Namespace **ортогонален** (выбор юзера на `app add`: existing/new). Мягкий дефолт `PlatformStack.spec.defaultEnvironment` (пред-выбор в CLI, не гейт). Single-CR fan-out (cross-ns ownerRef нельзя), dev-mode форсинг (1.9/2.9/3.9 dev-слайсы), Backstage-вкладки, `apprafter promote` (= cross-CR копирование image между `<name>-<src>`/`<name>-<dst>`) + env-миграция — **отложены** (ADR 0044 §Decision/§Non-goals). Удаление `Context.env_name` ре-вайрит и MigrationPlan-гейт (`find_blocking_migration_plan` ← `app.spec.environment`). Поставка ниже = доуточняется планом. **NB: `### 2.9 Per-environment overrides` (это) ≠ `## Фаза 2.9 — Dev Mode` (стр. ~3190 + roadmap-таблица «2.9 Dev Mode») — разные 2.9; dev-mode отложен.**

> ✅ **ЗАКРЫТО (2026-06-07).** Subagent-driven (T1–T12, ветка `feat/2.9-per-env-deploy`). env — **deploy-time per-CR** `Application.spec.environment` (инжектит cue-cmp из Argo `plugin.env APPRAFTER_APP_ENV`), `effective_spec` сливает override на base; инертный cluster-wide `APPRAFTER_ENV` + `Context.env_name` выпилены (MigrationPlan-гейт ре-вайрен на `app.spec.environment`). Scalar: `app add --env` → один Argo-апп `<name>-<env>` self-contained в выбранном namespace, группировка существующими лейблами `apprafter.io/application` + `apprafter.io/environment`; `app status`/`remove` агрегируют по лейблу; webhook `env ∈ environments`; `PlatformStack.defaultEnvironment` — мягкий дефолт. **Live-walk `e2e/gitops-walk-per-env.sh` GREEN end-to-end** (Approach A — side-load operator+webhook+cue-cmp; 9 фаз, ~3.5 мин): app add --env → plugin env → cue-cmp инжектит → operator env-резолвит → два namespace'а (replicas 1 vs 2, TIER dev/prod) → status агрегирует → remove --env хирургичен. **Walk вскрыл критический баг (был бы инертен в проде):** Argo CD префиксует `plugin.env` переменные `ARGOCD_ENV_`, поэтому cue-cmp читал голый `$APPRAFTER_APP_ENV` и НИКОГДА не инжектил `spec.environment` (всё рендерилось base, `--env` молча игнорился; host-тест это маскировал) → фикс `f91bdf0` (читает префиксную форму) + регресс-тест (test-inject 15/15). **Релиз:** operator+platform-stack **v0.2.22** + cli **v0.2.8** (монорепо-тег v0.2.8) + **argocd-cue-cmp v0.1.8** (фикс инъекции; platform-stack деривит тег из `version.cue`). Гейты: cargo fmt/clippy -D/test --workspace (cli+operator) + just lint + cue vet + `just crd-validate` (CRD-additive, change=safe) + `just platform-stack-check` (рендерит 0.2.22). **NB: 2.9 ≠ закрытие Фазы 2 — §6 M2 НЕ флипать** (M2 закрывается после 2.10–2.12). spec.md §3.1 reconciliation (unification→override-wins; substrate→scalar) — на phase-close актуализации (трекнуто в ADR 0044 §References). **Findings:** `parse_application` распознаёт только Style-B манифесты → CLI-side env-валидация для Style-A скипается (webhook бэкстопит) — non-blocking follow-up. **Ждёт пуша** (push policy).
...
**Поставка — ✅ ЗАКРЫТО (launch-срез), кроме явно отложенного:**
- [x] Renderer учитывает `environments.<env>` (override-wins merge) per-CR — `effective_spec(app, app.spec.environment)` вместо инертного cluster-wide `APPRAFTER_ENV`.
- [x] Каждый env-деплой = **отдельный `app add --env <env>`** → один Argo-апп `<name>-<env>` → один namespace (**выбор юзера**, ортогонален env — НЕ авто-суффикс). Несколько env со-резидентно = несколько `app add` в разные namespace'ы; группируются лейблом `apprafter.io/application`. Заменяет cluster-wide `APPRAFTER_ENV` (scalar, ADR 0044 — НЕ operator fan-out).
- [x] **Конфиг/selection-поверхность:** `app add --env` (+ интерактив: env из declared, дефолт из `PlatformStack.defaultEnvironment`; namespace existing/new); env **виден** (`app status <name>` агрегирует по лейблу + `status.environment`). Закрывает gap инертного `APPRAFTER_ENV`.
- [x] Selector ServiceProvider различен по env — **выпадает из override-механизма** (блок `environments.prod` сам задаёт `needs.*.selector`), отдельного кода нет (managed-тиров на launch нет).
- [ ] ~~Backstage показывает вкладки по env'ам~~ — **отложено** (Phase 3 Backstage, ADR 0044 §Non-goals).
...
### 2.16a — Per-claim password rotation
> 🏁 SR: A — автоматическая ротация кредлов всех claim-ролей (security baseline); притянут в launch, surfaced на 2.4g walk

**Контекст (находка 2.4g walk 2026-06-04):** пароль per-claim роли генерится ТОЛЬКО на (ре)провижне (claim Scheduled→ready) — первый провижн + восстановление; на reconcile готового claim'а и на рестартах (под/оператор) НЕ ротируется (`should_provision`=false когда ready). **Периодической/плановой ротации НЕТ** — кредл статичен на всю жизнь claim'а. CNPG умеет ротировать через basic-auth Secret, но никто это не автоматизирует.

**Цель:** автоматическая периодическая ротация паролей всех claim-ролей (pg сейчас; обобщить на jetstream/redis по мере появления).

**Поставка:**
- [ ] Политика ротации (интервал, per-tier дефолт — Regulated короче; opt-out/override на claim).
- [ ] Контроллер/таймер регенерит пароль роли → обновляет basic-auth Secret → CNPG ALTER ROLE → пересобирает connection Secret; rolling, без даунтайма (под перечитывает Secret).
- [ ] Метрика last-rotated + surface в `app status`; ручная ротация по запросу.

**Acceptance:** пароль claim-роли ротируется по интервалу автоматически; приложение продолжает коннектиться (перечитывает обновлённый Secret).

**Зависит от:** 2.4 (claim-провижн), 2.8/2.11 (secret-инфра)

**Размер:** M · топически ближе к 2.8/2.11 — номер 2.16a по просьбе (slot перед закрытием M2); перенести при желании
...
> ✅ **ЗАКРЫТО 2026-07-18** (order-3.7, SR:A; ADR 0051 + spec `docs/superpowers/specs/2026-07-17-2.16b-app-scope-migration-design.md`, T1–T16 subagent-driven, ветка `feat/2.16b-app-scope-migration`). App-scope деструктив-детект ВКЛЮЧЁН: классификатор `detect_destructive` по **эффективной per-env** спеке vs застампленный `status.lastAppliedSpec` baseline. Gated — `needs-removal`→data-migration; `scale-to-zero`/`image-path-change`/`domain-change`/`network-visibility-change`/`env-ref-removal`→requires-restart (ОДИН primary детерминированно: severity↓, trigger↑, field↑). Soft (не гейт, шлёт `SoftDestructiveChange` Event) — env-литерал removal, `needs.*.selector`/`.size`, image-tag, scale-down N→M. App-scope `MigrationPlan` в **namespace приложения** + controlling ownerRef→Application CR (каскад на delete, RBAC ns-scoped); approve через Argo-узел / `apprafter migration approve` (auto-resolve ns; `list` cluster-wide) → render new spec → re-stamp baseline → consume (delete plan) + anti-loop + `.owns(MigrationPlan)` instant-consume; **reject только через Git** (revert → план само-удаляется; approve-only, spec.md §3.8). **Two-env kind+Argo walk (`e2e/app-migration-walk.sh`) GREEN end-to-end** (baseline-stamp → effect-per-env gate [dev gated, prod untouched] → plan shape+ownerRef → self-heal → CLI approve→consume→anti-loop → soft-Event → revert=reject-via-Git). Walk вскрыл **2 реальных product-бага** (оба прошли unit/CRD/review): (1) `status.lastAppliedSpec` пруннился в `{}` — bare `type:object` под status-preserve-unknown = structural pruning boundary → явный preserve-unknown statusSchemaPatch (`a6f7b0c`, regression-guard fails-when-reverted); (2) pause-статусы теряли baseline — `apply_status` это SSA под одним field-manager, пропуск поля ПРУННИТ его → гейт само-отменялся за ~200мс → все 4 pause-builder'а пере-слают baseline (`a40bdf7`, 4-builder regression-тест); + 1 walk-harness fix (progress→stderr в plan-name capture). **Продуктовый нюанс:** `network-visibility-change` никогда не standalone-primary для webhook-валидной правки (webhook связывает hostname↔public двусторонне → флип public→internal со-удаляет hostname → tie-break даёт `domain-change`) — корректная defense-in-depth. ADR 0051 + spec.md §3.8/§3.1 (Revision 11→12) актуализированы; `operator-guide/migration-plans.md` расширен; корневая таблица `2.16b-destructive-ops.md`. Гейты зелёные: cargo fmt/clippy -D/test --workspace + just crd-check/crd-validate + just platform-stack-check (0.2.43) + final code-review SHIP. **Релиз:** operator+webhook charts v0.2.33→**v0.2.34**, cue-cmp 0.1.14→**0.1.15** (`schemas/v1alpha1/application.cue` изменилась), platform-stack currentVersion 0.2.42→**0.2.43** (compatibility change=safe, operatorVersion v0.2.34), cli 0.2.33→**v0.2.34** (Task 13 тронул бинарь; monorepo tag v0.2.34 local). 2.16b ≠ Phase closure (order-3.5/3.7 pulled-up; §6-бокс не трогается — но spec.md §3.8/§3.1 актуализированы как описание shipped-поведения). Post-gate по SR: **2.16b-sc** (SourceCredential-scope, тот же флип), затем **4** (T2 substrate).
...
**Поставка:**
- [x] Классификатор `detect_destructive(old, new) -> Option<DestructiveChange>` по таксономии выше (per-op класс); юнит-таблица всех случаев (вкл. env-edge-кейсы и image tag-vs-path).
- [x] Врезка детекта в Application reconcile loop (сейчас не вызывается) + авто-создание app-плана через `create_plan_for` (не только из тестов).
- [x] **Безопасная Argo-обвязка (ОТЛИЧИЕ от ADR 0048):** app-план + ownerRef **в ОДНОМ namespace** — либо план в namespace приложения + ownerRef на Application CR (он managed-узел в дереве Argo-апа юзера), либо per-app anchor в namespace приложения. **НЕ** план в `apprafter-system` + cross-ns ownerRef на Application CR — k8s GC тихо удалит план (тот же foot-gun, что обошли для платформы одной-ns anchor'ом). Тогда existing approve/reject-action (`apprafter.io_MigrationPlan`) загорается на узле плана под приложением юзера.
- [x] Health-Lua узла плана (как `apprafter.io_MigrationPlan` для платформы — детали «from→to / op / awaiting approval»). Пауза на Application CR (AwaitingMigrationApproval health) — уже есть, интегрировать.
- [x] Live kind+Argo walk (per `feedback_phase_closure_validation`): деструктивная правка апа → план создан + видим на апе + approve в дереве кликается → flip → reconcile применяет; same-ns ownerRef не удаляется GC (assert «план жив через 60с»).
...
#### CLI-hardening A — эфемерный kubeconfig (`apprafter kc` TTL)
> 🏁 SR: B (pull-up, ДО 1.0 / публичного OSS-релиза) — **гигиена креденшла, НЕ agent-defense.** Как только `kc` начинают вызывать чужие люди на чужих машинах, вечный cluster-admin-kubeconfig в профиле становится их проблемой (забытый файл) — без всякого агента. Откладывать до MCP неправильно. (Происхождение: разбор в ходе ревью S3, вышло за scope таблицы деструктивных операций.)

**Контекст:** всё в 2.16b/2.16b-sec защищает путь **git→cluster** (`spec.md` §3.8). Локальный слой «рабочая станция + артефакты CLI» сейчас без владельца. `apprafter kubeconfig` (alias `kc`, `cli.rs:133`) SSH-фетчит `/etc/rancher/k3s/k3s.yaml` (кешируется age-шифрованно, 1.3).

**A.0 (УСТАНОВЛЕНО в S3 — природа креденшла, делать выводы отсюда):** это **статический k3s admin CLIENT-СЕРТИФИКАТ** (`client-certificate-data`), НЕ SA-JWT (подтверждено `cli-providers/hetzner_cloud/kubeconfig.rs`; тест-фикстура `client-certificate-data: …`). ⇒ короткий TTL — НЕ параметр `expirationSeconds`, а **выбор ветки**:
- (a) ре-платформить `kc` на **TokenRequest API**: создать (один раз) cluster-admin SA + ClusterRoleBinding, на каждый `kc` — TokenRequest (`spec.expirationSeconds`) → собрать kubeconfig вокруг короткого токена + cluster CA + server URL. Размер **S–M** (меняет природу вывода: static-cert → эфемерный token; потребители kubeconfig с token работают). **Рекомендация — эта ветка, «do it right».**
- (b) короткоживущие client-cert'ы из k3s CA — отдельный процесс выпуска. Размер **M–L**.

Из-за (a)/(b) TTL «качественно + быстро» inline НЕ делается → пункт до 1.0, не сразу.

**Поставка:**
- [ ] A.1 — короткий TTL по умолчанию (ориентир 1–4ч; точное значение — по реальной длительности сессий при имплементации; хуже всего «бессрочно»).
- [ ] A.2 — флаг `--duration` (длинная сессия; верхняя граница есть, но не жёсткая). **Флаг НЕ ослабляет модель:** кто может вызвать `kc`, продлевает повторным вызовом, а не флагом — влияет только на удобство человека.
- [ ] A.4 — acceptance: `kc` без флагов → короткий TTL, факт истечения проверяется в тесте (после истечения запрос **отвергается API-сервером**, не «поле выставлено»); `--duration` работает; `spec.md`/docs фиксируют эфемерность + как получить новый.

**Побочный эффект (важно для Item B):** короткий TTL закрывает и «забытый kubeconfig», и бОльшую часть «агент утащил артефакт из профиля» (протухший kubeconfig неопасен независимо от того, кто прочитал) ⇒ если `kc` эфемерен, шифрование профиля (B.4) перестаёт быть приоритетом.

**Зависит от:** нет. **Размер:** S–M (ветка a) / M–L (ветка b) — см. A.0. **Открытый вопрос:** JWT-vs-client-cert РЕШЁН (client-cert); конкретный дефолтный TTL — по реальным сессиям.

---

#### CLI-hardening B — agent-aware CLI hardening
> 🏁 SR: C (managed / **MCP** — заявляется, когда заявляется агентная поверхность; тогда же и защиты). Дифференциатор #3 = **MCP-native agentic safety** ⇒ «как CLI ведёт себя под агентом» — часть продукта (показуемая в README/лендинге), а не внутренняя практика. (Происхождение: ревью S3.)

**Контекст:** локальный агент (Claude Code на машине оператора) в путь git→cluster НЕ входит: тот же UID, тот же kubeconfig, за спиной у всех кластерных контролов — для API-сервера это оператор. Ни один admission-webhook его не видит.

**B.0 (модель противника — записать ДО имплементации, иначе бесконечное упрочнение):**
- **Non-goal:** решительный противник с исполнением кода под UID оператора (`kc` даёт cluster-admin, keyring разблокирован, PATH пишется, PTY аллоцируется) — не сдерживаем.
- **Цель:** агент, делающий ОЧЕВИДНОЕ — зовёт `apprafter migration approve`, т.к. это следующий разумный шаг. Против него дешёвые контролы работают почти идеально.
- **Явный non-goal:** шифрование артефактов профиля с ключом, доступным тому же UID (обфускация, не защита; имеет смысл ТОЛЬКО через системный keyring — B.4).

**Поставка:**
- [ ] B.1 — TTY-жест на деструктивных командах: промпт из **`/dev/tty`** (не stdin → отсекает пайп/редирект; обход = PTY-обвязка = вне B.0); **дефолта нет** (пустой Enter = отказ, т.к. ввод не совпал, а не «дефолт no»); `--yes`/`--force` НЕ существуют; ввод простой (`yes`), одинаковый для всех scope. **Защита — в КАНАЛЕ (`/dev/tty`), не в содержании ввода** (уникальная строка/хеш копируется процессом дешевле, чем набрать `yes`, + создаёт ложное свидетельство «аппрувящий видел план»). Дешёвая проверка: поведение `inquire` при не-TTY stdin (возможно, половина уже есть).
- [ ] B.2 — `migration approve`: весь `spec.changes[]` (не только primary — иначе rollup §1.2 не доезжает до места решения) + `classifications[]` видимо+различимо; TTY-жест; показать хеш аппрува (для ОТОБРАЖЕНИЯ, не ввода — «что показали»=«что аппрувили»). Зависимость (зафиксировать явно): промпт рендерит план, доверяет ПЛАНУ а не манифесту (закрыто approvedSpecHash+F-1b).
- [ ] B.3 — `apprafter kc`: TTY-жест (kc даёт cluster-admin → обходит approve; замок на approve при свободном kc бессмыслен). Ортогонален Item A: TTL ограничивает срок артефакта, жест — его выдачу; оба нужны.
- [ ] B.4 — профиль + keyring: оценивать ТОЛЬКО после Item A (если kc эфемерен — ценной цели в профиле нет). Если долгоживущее осталось — шифровать ТОЛЬКО с ключом в системном keyring (обход → keyring API → возможен GUI-промпт, B.7). Ключ рядом с файлом / под тем же UID вне keyring — НЕ делать.
- [ ] B.5 — ревизия деструктивных команд: один проход по всему CLI (`app remove`, claim-операции, всё сносящее/переконфигурирующее) → список → решение по каждой (жест/оставить). Иначе гейт на approve, а рядом команда без плана.
- [ ] B.6 — `approvedVia`: `cli-tty` (жест был) / `cli-noninteractive` (жеста не было — ЭТО сигнал «процесс, не человек») / `argo`. `cli-forced` не нужен (нет `--yes`). Проверить: если FSM уже пишет `conditions[].lastTransitionTime` на переход в `approved` — «когда» уже есть, остаётся «каким путём».
- [ ] B.7 (горизонт, НЕ в этом item'е) — GUI-жест: текстовый канал имеет потолок (любая копируемая строка копируется процессом); выше — жест мимо stdout/stdin (GUI keyring-промпт, focus-loss lock, hardware touch). Ложится на `apprafter-core` + Tauri-портал (уже спланировано), новой архитектуры не тянет.
- [ ] B.8 — acceptance: `echo yes | apprafter migration approve <plan>` НЕ аппрувит; approve без TTY = отказ (не зависание); полный `changes[]`+классы до промпта; `--yes`/`--force` отсутствуют (тест на surface команд); `kc` требует жеста; `approvedVia` различает `cli-tty`/`cli-noninteractive`.

**Зависит от:** Item A (объём B.4), `apprafter-core` (общий домен CLI/GUI/портал), 2.16b-sec (rollup/approvedSpecHash — B.2), ADR 0036 (MCP agentic-safety). **Размер:** M. **Продуктовое:** довести до показуемого (README/лендинг) — конкуренты этот слой не ставят (у imperative-PaaS агент дёргает API напрямую, вопрос «а если агент решил лишнего» там не поставлен).
...
> ✅ **ЗАКРЫТО 2026-08-07** (order-3.7, SR:A; spec `docs/superpowers/specs/2026-08-07-2.16b-sc-design.md`, ветка `feat/2.16b-sc`). Live-создание MigrationPlan при деструктивной правке `SourceCredential` ВКЛЮЧЕНО — «тот же флип, другой scope» поверх 2.16b (ADR 0051). Добавлен **новый scope-вариант `sourcecredential`** дискриминатора MigrationPlan (kube-rs тип + OpenAPI CRD + CUE schema + webhook-валидатор + approve-only phase-rule), `SourceCredentialStatus.lastAppliedSpec` (preserve-unknown baseline для diff'а), и **общая consume-ticket state machine хойстнута в `operator-core`** (`decide`/`plan_state`/`MigrationDecision`/`PlanState`/`plan_hash_matches` — реюз verbatim обоими контроллерами, ноль клонирования; app-scope тесты прошли без правок). Coverage-narrowing (удаление `git.repoPrefixes` / `registry.hosts` при матчащемся приложении, `detect_destructive`→`coverage-removal`/`breaking`) → `create_plan_for` создаёт `sourcecredential`-scope план в **namespace самого credential'а** + controlling ownerRef→SourceCredential CR (каскад GC), **паузит ОБЕ половины derivation** (старые wider-coverage repo-creds + pull-secret не трогаются → in-flight аппы сохраняют доступ), consume на approve (re-derive narrowed + GC непокрытого). **Approve-only** (reject через webhook заблокирован; откат = re-widen спеки → нет деструктив-дельты → `DeleteThenRender` GC'ит устаревший план; `previous_spec_snapshot=None`). Actor-agnostic (raw `kubectl edit` тоже триггерит). CLI `migration list`/`approve` scope-agnostic (без hard-assume `scope.application`); `repo creds remove` reverse-dep gate оставлен как комплементарный best-effort UX. **kind+podman walk (`e2e/sourcecredential-migration-walk.sh`) GREEN end-to-end** (register SC → app матчится → удалить covering host → авто-план в cred-ns + ОБЕ derivation паузятся [старые Secret'ы целы] + Argo-clickable + `migration approve` резюмит+GC'ит; + kubectl-edit actor-agnostic вариант). **Закрывает 1.79c acceptance #4.** **Релиз:** operator+webhook charts v0.2.35→**v0.2.36**, cue-cmp 0.1.16→**0.1.17** (`schemas/v1alpha1/{migrationplan,sourcecredential}.cue` изменились), platform-stack currentVersion 0.2.44→**0.2.45** (compatibility change=safe, operatorVersion v0.2.36), cli 0.2.34→**v0.2.36** (Task 8 тронул `migration`/`repo creds` бинарь; 0.2.35 был operator-only → пропущен в cli-стриме; monorepo tag v0.2.36 local). 2.16b-sc ≠ Phase closure (order-3.7 pulled-up; §6-бокс не трогается).
...
**Поставка:**
- [x] Scope-вариант `sourcecredential` в MigrationPlan (kube-rs тип + OpenAPI CRD + CUE schema + admission `validator_migrationplan.rs` — required-fields per scope, как application/platform).
- [x] `create_plan_for` для SourceCredential: план в namespace SourceCredential CR (или release-ns) + controlling ownerRef на SourceCredential CR (same-ns GC-safe, паттерн ADR 0051); webhook-valid form; `previous_spec_snapshot` по семантике reject (approve-only vs revert — решить, зеркалит ли application-scope). → approve-only (mirror app-scope), `previous_spec_snapshot=None`.
- [x] Врезка `detect_destructive` (уже есть) в SourceCredential reconcile loop → авто-создание плана + пауза derivation derived Secret'ов до `completed`; consume-ticket по образцу 2.16b (approve → apply → delete plan). Паузятся ОБЕ половины; state machine хойстнута в operator-core.
- [x] CLI `migration list` уже cluster-wide (2.16b) — подтвердить, что sourcecredential-план видим; approve-путь тот же.
- [x] Live walk: убрать registry-host из покрытия CR при матчащемся приложении → авто-план + derived pull-secret не трогается до approve (закрывает 1.79c acceptance #4).
...
> ✅ **ЗАКРЫТО 2026-08-08** (order-3.7, SR:A; spec `docs/superpowers/specs/2026-08-07-2.16c-deep-merge-struct-fields-design.md`, ветка `feat/2.16c-deep-merge-struct`). Per-environment override `expose`/`imagePolicy` теперь **частичный** (deep-merge по подполям, override-wins): env несёт только diff, env-инвариантные подполя (`expose.port`) наследуются из `base` — дублирование порта в env-override больше не требуется. Новый all-optional тип **`#ApplicationEnvOverride`** бэкает `environments[*]` (CUE + Rust `ApplicationEnvOverride`/`ExposeOverride`); `base` остаётся `#ApplicationSpec` с обязательным-when-present `expose.port`, поэтому «забыл порт в base» ловится локально `cue vet`. Оператор `effective_spec` мёржит по подполям и теперь **fallible** (base-отсутствует + env-only `expose` без порта → `Ready=False`/`InvalidEffectiveSpec`, ничего не применяется — `Err` обрабатывается ДО `detect_all`, не пропускает миграционный гейт «применив что-то»); webhook инфорсит «env-only `expose` (нет `base.expose`) обязан нести `port`»; `crd-check` получил рекурсивный `#ApplicationEnvOverride ⊇ #ApplicationSpec` drift-gate + assertion «в сгенерированных CRD нет OpenAPI `default:`». Унаследованные `expose.hostname`/`tls` потребляются только под `network: "public"` (иначе inert). **`needs.*` deep-merge ВЫДЕЛЕН в новый item 2.16i** (OneOrMany-merge undefined + partial-override меняет имя ResourceClaim → GC → пустая БД = data-loss + reopen 2.4d selector-gate — своя миграционная аналитика). Self-correction: тест `effective_spec_env_override_replaces_expose_block` инвертирован под deep-merge. **kind LOCAL-BUILD walk `e2e/expose-deep-merge-walk.sh` GREEN.** change=requires-restart (на апгрейде partial-`public`-override без hostname наследует base-hostname → live 2.16b destructive-gate фаерит → app паузится на `AwaitingMigrationApproval` до аппрува; две compat-заметки: hop-через-0.2.34 не стампит baseline → гейт не фаерит на первом reconcile; НЕТ даунгрейда ниже этого релиза после записи partial `environments[*].expose`). spec.md §3.1/§4.4 (Revision 13→14) актуализированы. **Релиз:** operator + admission-webhook charts v0.2.36→**v0.2.37**, cue-cmp 0.1.17→**0.1.18** (`schemas/v1alpha1/application.cue` изменилась), platform-stack currentVersion 0.2.46→**0.2.47** (compatibility change=requires-restart, operatorVersion v0.2.37); НЕ CLI, нет monorepo tag (манифест-семантика чисто in-cluster). 2.16c ≠ Phase closure (order-3.7 pulled-up; §6-бокс не трогается).
...
- [x] **Оператор:** `effective_spec` мёржит `expose` по подполям (`port`/`network`/`hostname`/`tls` — override-wins per field), а не целиком; тест `effective_spec_env_override_replaces_expose_block` → deep-merge-тест; решить то же для `imagePolicy`. (Реализовано через all-optional `#ApplicationEnvOverride`/`ExposeOverride` + per-field merge fns; `effective_spec` теперь fallible — env-only `expose` без порта → `InvalidEffectiveSpec`; `imagePolicy` тоже deep-merge.)
- [x] **CUE:** `expose.port` → `port?` (опциональный) в `#ApplicationSpec`, чтобы частичный `environments[*].expose` экспортировался (`cue export` больше не требует конкретный port в env-override). Schema-change ⇒ cue-cmp пересобирает (bundle `schemas/v1alpha1`). (Финально: `base` держит `#ApplicationSpec` с обязательным `expose.port`; частичность несёт отдельный all-optional `#ApplicationEnvOverride` — «забыл порт в base» ловится `cue vet`; cue-cmp 0.1.18.)
- [x] **Webhook:** кросс-полевой инвариант «у **эффективного** expose есть `port`» — `port` присутствует в `base.expose` ИЛИ в каждом переопределяющем `environments[*].expose` (ровно как существующее правило «image в base ИЛИ в каждом env»). Не даём смёржиться в expose-без-порта.
- [x] **Walk:** манифест с `base.expose:{port,network:public,hostname}` + `environments.dev.expose:{network:"internal"}` (только diff) → cue-cmp экспортит, оператор рендерит dev с `port` из base + internal (без публичного route), prod наследует публичный expose. Подтвердить на kind. (`e2e/expose-deep-merge-walk.sh` GREEN на kind LOCAL-BUILD.)
...
> ✅ **ЗАКРЫТО 2026-08-08** (order-3.7, SR:A; ADR **0053** + re-spec `docs/superpowers/specs/2026-08-08-2.16d-resource-baseline-respec.md` + замер `docs/measurements/2.16d-baseline-2026-08-08.md`, ветка `feat/2.16d-resource-baseline`). Ни один под (платформа или приложение) не BestEffort — QoS выбран по роли из **реального замера solo-узла** (Hetzner solo-tier, ~4GB, published ps 0.2.47; не cpx22/8GB — замер поправил допущение спеки). **Поставка:** (1) новое поле **`resources`** на `Application` (base + per-env на `#ApplicationEnvOverride` из 2.16c) с **deep-merge MAP-of-map** — `requests`/`limits` мёржатся независимо по ключу (env-override `limits.memory` НЕ роняет base `limits.cpu`; образец — merge `env`, не struct); (2) **app-seed Burstable** измеренный (`requests {cpu:25m, memory:32Mi}`, `limits {memory:512Mi}`, БЕЗ cpu-limit) — щедрый-но-**присутствующий** memory-limit (отсутствующий ломает render-детерминизм 2.4e / T2-well-formedness / local↔in-cluster parity 2.12; низкий 128Mi детерминированно OOMKill'ит Node/JVM/Python); явный `spec.resources` уважается дословно (override-wins, seed только когда опущено); (3) **backends Guaranteed** (`requests==limits`) с когерентными DB-параметрами — CNPG 256Mi + `shared_buffers=32MB` + ephemeral-storage; Dragonfly 320Mi + `--maxmemory=256mb` (оба ниже cgroup-limit); замер ассертит `status.qosClass==Guaranteed` на живом поде (init-контейнер без resources молча демоутит); (4) **platform-stack requests/limits** на всех компонентах (argocd/cilium/cert-manager/cnpg-op/dragonfly-op/cue-cmp-sidecar + добавлен отсутствовавший limit на sealed-secrets → был requests-only BestEffort) из таблицы замера; (5) **node-резервации** (`system-reserved=1500Mi` под out-of-`kubepods` `k3s.service` в `system.slice` / `kube-reserved=256Mi,100m` / `eviction-hard<100Mi` / `OOMScoreAdjust=-999`) в CLI/k3s-installer на bootstrap + **retrofit-команда `apprafter node reserve-headroom`** (SSH + config + `systemctl restart k3s`, ~30s API-outage за confirmation-prompt); (6) **webhook** `validate_resources` (валидность quantity через `is_k8s_quantity` + `request <= limit` по ключу с реальным сравнением quantity). **No PriorityClass** (ядро OOM-score QoS-derived, не priority-derived); **LimitRange ОТЛОЖЕН** до Capsule policy-layer (ResourceQuota делает его обязательным) — deviation от исходного deliverable, рендерер+provisioner покрывают каждый AppRafter-owned под и так удовлетворяют acceptance; **VPA-vs-backends non-goal** записан в ADR (VPA реверсит pod-level правки на CNPG/Dragonfly CR → backend right-sizing = recommendation-only в `ServiceProvider.spec.config`, тема 2.16e). **D2 budget-close ЗАКРЫВАЕТСЯ** на ~4GB solo-узле с запасом (allocatable после резерваций ~1958Mi > platform-requests ~800Mi + backend-Guaranteed 576Mi + app-seed 32Mi ≈ 1408Mi) — ladder не понадобился (без pull-forward 2.16f, без подъёма T1-минимума, без trim компонентов). spec.md §3.1/§4.5/§4.12 (Revision 14→15) актуализированы; ADR 0053 записан. **Релиз:** operator + admission-webhook charts + schema + platform-stack + **CLI** (installer-половина = node-резервации → CLI bump + monorepo tag, в отличие от 2.16c) — версии: op/wh **v0.2.38** / cue-cmp **0.1.19** / ps **0.2.48** / cli **0.2.38**; compatibility change=`requires-restart` (retrofit = k3s restart, re-render добавляет limits → роллит поды). 2.16d ≠ Phase closure (order-3.7 pulled-up; §6-бокс не трогается).
...
- [x] `operator-rendering` эмитит блок `resources` на контейнере приложения. Default seed = **Burstable: low memory/cpu request + memory limit**. NB: «limit-only» НЕ вариант — limit без request авто-выставляет request=limit (это уже не BestEffort и резервирует весь limit); нужен явный низкий request + потолок-limit. Asymmetry: memory request щедро (incompressible → не OOM), CPU скромно (compressible → throttle, не kill). Seed **runtime-agnostic** консервативный (runtime-detector отвергнут как хрупкий для прода; наблюдение из 2.16e поправит точнее, чем угадывание по рантайму).
- [x] **Pro-mode:** если `Application.spec.*.resources` задан юзером — берётся дословно (override-wins); seed применяется ТОЛЬКО когда поле опущено. Per-env merge (как `expose`/`needs`).
- [x] **Backends:** provisioner проставляет `resources` в CNPG `Cluster` CR и Dragonfly CR (они тоже BestEffort; stateful Postgres — самый опасный). Tier-aware дефолт (как CNPG `instances`).
- [~] **LimitRange** per tenant namespace (`defaultRequest`/`default`/`min`/`max`) — catch-all backstop для всего, что рендерер не владеет (jobs, raw-поды). Платформенный chart. **→ ОТЛОЖЕН до Capsule policy-layer** (ADR 0053: LimitRange обязателен только рядом с ResourceQuota; рендерер+provisioner покрывают каждый AppRafter-owned под и так удовлетворяют acceptance «ни один под не BestEffort»).
- [x] CRD-additive: опциональное `resources` на Application spec; `just crd-validate`.
- [x] Live walk: апп без `resources` → под **Burstable** (assert `kubectl get pod -o jsonpath='{.status.qosClass}'` ≠ BestEffort); апп с явным `resources` → дословно; CNPG/Dragonfly поды не BestEffort.
...
> ✅ **ЗАКРЫТО 2026-08-10** (order-3.7, SR:A; спек `docs/superpowers/specs/2026-08-08-2.16e-vpa-design.md` (4 раунда внешнего ревью + adversarial), план `docs/superpowers/plans/2026-08-08-2.16e-vpa-autoscaling.md`, **ADR 0054**; собрано subagent-driven, FF-merge в master). Оператор right-size'ит app-requests **in-place** через upstream VPA: эмитит **один** `VerticalPodAutoscaler` на managed CR-env (**`updateMode: InPlace`** — не InPlaceOrRecreate, тот evict'ит single-replica; `containerName:"*"`, RequestsOnly, `minReplicas:1`, minAllowed=seed 32Mi/25m, maxAllowed.memory=512Mi), prune на managed→pro, зеркалит `VPA.status.recommendation`→`Application.status.recommendedResources` (+`app status`). Трёхзначный кноб `PlatformStack.spec.resources.autoscale.mode ∈ {full(default), up-only, off}`; `apprafter platform autoscale set/show`. Чарт `vertical-pod-autoscaler` 0.11.0 (VPA 1.7.1), self-managed certGen (без cert-manager), `failurePolicy:Ignore`, feature-gate `InPlaceOrRecreate=true` (alpha, на updater+admission), metrics-server = hard-prereq. Память корректируется только в `[32Mi,512Mi]` (RequestsOnly cap на seed-лимите). GitOps: **Option A** (VPA владеет живым подом in-place, оператор — константным seed-шаблоном; НЕ writeback — Phase 3). Backend-recommendation + KEDA-exclusion guard — вынесены в follow-up (ADR 0054; KEDA-guard → 2.6a). Обесценивает D2-budget-close 2.16d. **Live-walk на реальном Hetzner GREEN end-to-end** (все сценарии, оба проекта = 0) — и он ОКУПИЛСЯ: поймал **3 бага, невидимых unit/CRD/review**: (1) зеркало не персистило — `recommendedResources` объявлено голым `{...}` → CRD `type:object`-без-preserve-unknown → apiserver пруна́л вложенные поля на SSA (фикс: структурная схема); (2) **`vpa_available` startup-race** — one-shot проба проиграла CRD-инсталляции 0.06с → VPA молча выключен на всю жизнь пода (фикс: ленивая перепроба `Arc<AtomicBool>`); (3) VPA certgen Job был BestEffort (фикс: resources). Все диагностированы детерминированно на локальном kind. **Релиз (несколько итераций):** op/wh **v0.2.41** / cue-cmp **0.1.21** / ps **0.2.52** / cli **v0.2.39** (монорепо-тег v0.2.39); op/ps/cue-cmp v* — workflow-made. 2.16e ≠ Phase closure (order-3.7 pulled-up; §6-бокс не трогается; spec.md актуализируется на закрытии фазы).

**Поставка:**
- [x] VPA в platform-stack (recommender + updater + admission-controller). Mode `InPlace` (см. 🚧-заметку выше: `InPlaceOrRecreate` отвергнут, трёхзначный кноб, дефолт `full`).
- [x] **Recommender tuning под недельные пики:** `--memory-aggregation-interval=24h` + `--memory-aggregation-interval-count=14` (14д окно) + `--memory-histogram-decay-half-life=168h` (7д half-life). NB: дефолтный 24h half-life стирает недельный пик (2⁻⁷≈0.8%) → недопровижн cyclic-аппов. Trade-off: длиннее окно = медленнее реакция на снижения (консервативнее) — на узком узле в правильную сторону. `--memory-saver=true` (рекоммендер держит state только для VPA-таргетнутых подов → меньше его RAM на тесном T1).
- [x] **`minAllowed.memory` floor — обязателен** per-VPA. Defense: checkpoint float→int rounding bug может выдать 0-memory rec после рестарта рекоммендера → OOM-loop; плюс incompressible memory. `maxAllowed` тоже.
- [x] **GitOps-reconciliation** (VPA мутирует live spec out-of-band → Argo видит drift на `resources`): решить в ADR — либо Argo `ignoreDifferences` на `/spec/template/spec/containers/*/resources` для VPA-managed, либо writeback рекомендации в манифест. Writeback чище под GitOps-as-sole-control-surface + ложится на MCP-agent approval-gate.
- [x] **Pro-mode opt-out:** аппы с явным `resources` (2.16d) → VPA `Off`/не таргетятся (юзер владеет; advisory-нотификация «actual=X» в портал — единственный осмысленный сток в pro-режиме).
- [x] **KEDA-эксклюзия:** VPA НЕ на одном CPU/mem-метрике с KEDA-скейлингом (ADR 0019) — death-spiral (VPA↓request → HPA-математика едет → реплики взрываются → гистограммы скашиваются). Гард + докум.
- [x] VPACheckpoint персист (CRD в datastore — на single-node k3s это kine/sqlite); пережить рестарт рекоммендера.
- [x] Live walk: апп без `resources` → VPA набирает историю → `InPlaceOrRecreate` поднимает/опускает request на живом поде **без рестарта** (assert `RESTARTS=0`, под не пересоздан); minAllowed держит пол.
...
> ✅ **ЗАКРЫТО 2026-08-11** (SR:A; спек `docs/superpowers/specs/2026-08-10-2.16f-argocd-footprint-design.md` (4 раунда внешнего ревью R1–R4 + независимая перепроверка несущих механик), план `.../plans/2026-08-10-2.16f-argocd-footprint.md`, замер `docs/measurements/2.16f-argocd-footprint-2026-08-11.md`; на master, config-only). Тюнинг argocd-компонента через chart-values: `GOMEMLIMIT`/`GOGC` env на app-controller (256MiB) + repo-server (128MiB, M2 — форкает helm/git), `resource.exclusions` в `argocd-cm` (Endpoints/Event/events.k8s.io-Event/EndpointSlice/Lease/metrics.*/Cilium identity+endpoint/VPACheckpoint — kind-scoped, `CiliumNetworkPolicy` остаётся), `applicationSet.replicas:0` (в 7.7.7 НЕТ `enabled`-гейта → replicas:0 под prune:false, без orphan). Релиз **ps 0.2.53** (+ `0.2.53-rc.1` walk-vehicle prerelease), change=safe, без operator/cue-cmp/CLI, без монорепо-тега; M1(b) rc-as-walk-vehicle, F2 dotted `-rc.N`, F4 no-CLI-release-в-rc-окне. **REFRAME (live-driven, user-approved):** свежий throwaway app-controller RSS шумный (359/206/211/144, ~206 БЕЗ тюнинга — холодный старт) → GOMEMLIMIT=256 не связывает на свежем; польза (кэп роста worn-in + −CPU) видна только на обжитом кластере. Walk гейтит **delivery+no-reg** (env применён, Synced/Healthy, appset 0/0, exclusions, no OOM/restart), RSS/CPU **recorded не gated**; RSS-снижение валидируется на dogfood. **ПОДТВЕРЖДЕНО на реальном dogfood (0.2.46→0.2.53):** app-controller **344→208Mi (−40%)**, CPU **131→15m (−88%)**, appset-под убран, 208<256-кэп → кэп связывает и работает. **F1 fresh-install SSA GREEN** (throwaway MODE=stable walk, реальный Hetzner: appset сходится к 0/0 на свежем bootstrap'е — cross-manager SSA helm-replicas:1↔Argo-0 разрешается; + delivery/no-reg/exclusions/no-OOM на обоих легах Leg3-fresh+Leg2-upgrade; cleanup оба проекта=0). Walk также поймал **harness-баг** (`set -o pipefail` + `kubectl -o json | grep -q` на большом STS-JSON → grep-q матчит рано → SIGPIPE kubectl → pipefail ложно роняет env-assert; DIAG показывал env ВСЕГДА → продукт исправен; фикс = capture-в-переменную + bash-match, детерминистически доказан локально). **Единственный follow-up:** request re-pin из осевшего dogfood-замера (208→~180-200 vs 288, ~90-110Mi allocatable) — минорная allocatable-оптимизация, отдельный ps-бамп когда dogfood устаканится. Пред-апгрейд backup dogfood'а снят (`apprafter backup create`, native local restic). 2.16f ≠ Phase closure (order-3.7 pulled-up; §6-бокс не трогается).
...
- [x] `GOMEMLIMIT` на app-controller (256MiB) + `GOGC=50` + repo-server (128MiB — M2, форк-headroom helm/git под 256Mi-cap). Доставлено (env живьём на обоих workload'ах, verified).
- [x] `resource.exclusions` в `argocd-cm`: `Event` + `events.k8s.io/Event`, `Endpoints`/`EndpointSlice`, `Lease`, `metrics.k8s.io/*`, `cilium.io CiliumIdentity/CiliumEndpoint` (+ `autoscaling.k8s.io/VerticalPodAutoscalerCheckpoint` — 2.16e). Kind-scoped (`CiliumNetworkPolicy` остаётся). Доставлено.
- [x] Отключён `applicationset-controller` — через **`applicationSet.replicas:0`** (в 7.7.7 нет `enabled`-гейта; replicas:0 держит объект в desired под prune:false, без orphan). Под убран (verified на dogfood).
- [x] Замер до/после — в `docs/measurements/2.16f-argocd-footprint-2026-08-11.md` (не PROJECT_STATS): dogfood app-controller **344→208Mi**, CPU 131→15m; throwaway RSS recorded (шумный, reframe).
- [x] Skip: sharding + Kyverno-exclusions (как планировалось).
...
> ✅ **ЗАКРЫТО 2026-08-11** (order-3.7, SR:A; спек `docs/superpowers/specs/2026-08-11-2.16g-node-swap-design.md` — **4 раунда внешнего ревью R1-R4 + upstream/working-tree верификация**, план `.../plans/2026-08-11-2.16g-node-swap.md`, **ADR 0055**; ветка `feat/2.16g-node-swap`, subagent-driven). Новая **`apprafter node prep`** — идемпотентный umbrella (2.16d reservations + host-swap `min(RAM,8Gi)` + `GOMEMLIMIT=2GiB` на k3s), **hard-remove `reserve-headroom`**; + **`apprafter node status`** (live swap-posture, [api]/[ssh]-labels, graceful degradation). **Поставка:** (1) swap = **`min(RAM,8Gi)`** swapfile (dd `oflag=direct`+ionice, fstab `sw,nofail`) + `swappiness=10` + kubelet **drop-in** (`failSwapOn:false` + `memorySwap.swapBehavior:NoSwap`) — **не kubelet-arg** (B1: swapBehavior это KubeletConfiguration-поле, не флаг), Option-A managed-dir (Q4); (2) **`GOMEMLIMIT=2GiB`** на `k3s.service` (R3 — k3s это Go, своп без GOMEMLIMIT = thrash; бьёт Go-heap-компонент, residual покрывает своп — P6); (3) **bootstrap: `INSTALL_K3S_SKIP_START`** → бинарь ставится не стартует → `k3s --version`+cgroup2-гейт → пишем конфиг → `systemctl start k3s` (**fail-soft**: сбой swap = рабочая нода без подушки, никогда не незапущенная — Q2/P2); (4) retrofit-гейт `kubeletVersion` ≥1.34 (P3, не configz) + cgroup2, на <1.34 **REFUSE+upgrade-hint** (не silent-skip); (5) **atomic apply + whole-step rollback** (trait-backed unit-стейт-машина: `failSwapOn:false` убирается ПОСЛЕДНИМ и никогда при живом свопе — P1; `swapoff` с таймаутом; `rm` недостижим из swapoff-fail-ветки); (6) **inline `memory.swap.max=0`** на существующих kubepods-cgroup'ах (depth-agnostic `find` — NoSwap применяется при СОЗДАНИИ контейнера, N18/Q3); (7) **encrypted-swap residual-risk** = Hetzner-снапшот (N11), записан в ADR 0055; (8) **T1-only** (swap-шаг tier-gated; T2 etcd/`swappiness=1` deferred @3.1, T3/T4 deferred). **Real-Hetzner walk GREEN end-to-end** (`e2e/node-swap-walk.sh`, 8 шагов, 2 ноды в 2 проектах): configz NoSwap + swap active + GOMEMLIMIT + fstab-nofail; pod-NoSwap; **подушка сработала под давлением** (pswpout+2.2M, k3s.service выжил, /readyz up); **SKIP-gate + no-swap→swap transition + Q7** (inline-set пережил VPA-resize → роллить workload'ы НЕ обязательно) + idempotent; reboot-persistence + post-boot-self-healing. Walk окупился — **4 итерации harness-багов** (pod-OOM-ассерт противоречил «NoSwap защищает НОДУ не workload'ы»; node-selection: label-anchor project-wide → 2-я нода в OTHER-проекте; env HCLOUD_TOKEN перебивал stored-token), **продукт ни разу не был причиной**. dogfood k3s v1.35.5 ≥1.34 (N1 clear). **Релиз:** CLI-only (node prep/status + installer) → **cli v0.2.40 + monorepo-тег v0.2.40**; ps/operator/cue-cmp не трогаются. spec.md не трогается (Rev 15). **Follow-up'ы ЗАКРЫТЫ в cli v0.2.41 (real-Hetzner STEP6-walk GREEN; tag v0.2.41 supersedes сломанный v0.2.40 — его stale `Cargo.lock` не собирался под `release-cli --locked`):** Q10 — `node status` читает API-поля через кэш-kubeconfig (SSH-down больше не гасит их); STEP6 — undocumented `APPRAFTER_NODE_SWAP_FORCE_INVALID_DROPIN` fault-hook + разводка walk-STEP6 на retrofit-ноду. **STEP6-walk поймал реальный продуктовый баг (unit/review пропустили):** `wait_for_recovery` пробил только apiserver `/readyz`, а битый kubelet-drop-in кладёт **kubelet** при (недетерминированно) живом apiserver → rollback НЕ срабатывал, забрикованная нода жила (и на реальном failSwapOn-брике так же) → фикс `bb20d00`: probe требует ещё и kubelet через node-proxy configz + regression-guard. **dogfood-retrofit `apprafter node prep` на `platform-1` ВЫПОЛНЕН пользователем 2026-08-11** (swap 3.7G active, NoSwap, swappiness=10, swapBehavior=NoSwap) — и вскрыл 2 бага ОТОБРАЖЕНИЯ `node status` (не свопа): (1) `GOMEMLIMIT: unknown` на всех нодах — `probe_gomemlimit` делал `strip_prefix("GOMEMLIMIT=")` на токене `Environment=GOMEMLIMIT=2GiB` (не срезал `Environment=`); (2) `provision: unknown` на ретрофит-нодах — breadcrumb писался только на bootstrap-пути. Фикс **cli v0.2.42**: чистый `parse_gomemlimit` + retrofit пишет provision-breadcrumb + walk теперь зовёт `node status` (walk маскировал баг: читал GOMEMLIMIT через `--value`+substring, `node status` не звал). Девиации: swap=`min(RAM,8Gi)` (не «2-4G скромный» plan.md — R4), drop-in (не kubelet-arg), T2 deferred. 2.16g ≠ Phase closure (order-3.7 pulled-up; §6-бокс не трогается).
...
- [x] Swapfile при node-setup (`dd oflag=direct`+ionice, fstab `sw,nofail`, disk-precheck vs 2.6c-15%). **Размер = `min(RAM,8Gi)`** — DEVIATION от «2-4 ГБ скромный» (R4: на T1-4GB это 4G = RAM, кап на бо́льшие ноды; bigger swap = дольше thrash-runway перед OOM, не безопаснее).
- [x] `vm.swappiness=10` на T1 (sysctl drop-in, переживает reboot — verified). T2=1 — deferred @3.1.
- [x] `failSwapOn:false` + `memorySwap.swapBehavior:NoSwap` — через KubeletConfiguration **drop-in** (Option-A managed-dir), НЕ kubelet-arg (B1: swapBehavior — поле KubeletConfiguration, не флаг); version-gated ≥1.34 (B2, `INSTALL_K3S_SKIP_START` → `k3s --version`); configz-verified в walk.
- [~] T2 etcd-swap-protection (mlock / `swappiness=1`) — **deferred @3.1** (open research, НЕ блокер T1; как и планировалось).
- [~] encrypted swap — **residual-risk записан в ADR 0055** (Hetzner-снапшот `/swapfile` с apiserver-секретами — N11; `swapoff+rm` перед любым snapshot-flow); random-key encryption deferred (R5).
- [x] **Real-Hetzner walk GREEN** (`e2e/node-swap-walk.sh`, 8 шагов, 2 ноды): подушка свопит спайк (pswpout>0, k3s.service выжил, /readyz up), Postgres NoSwap (`VmSwap≈0` + cgroup `swap.max=0`), SKIP-gate + no-swap→swap transition + Q7-survives-resize + idempotent + reboot-persistence.

**Acceptance (T1 launch — DONE):** T1-ноды провижатся со свопом `min(RAM,8Gi)` + NoSwap (`swappiness=10`); спайк control plane поглощается свопом вместо OOM (k3s.service выживает, /readyz up); поды (особенно Postgres) в RAM (`swap.max=0`); `apprafter node prep` идемпотентен + gate-first + whole-step-rollback + refuse-<1.34; `reserve-headroom` убран. **(T2 acceptance — `swappiness=1` + etcd swap-protection — deferred @3.1.)**
...
> ✅ **ЗАКРЫТО 2026-08-12** (cli **v0.2.43**, монорепо-тег `v0.2.43`, CLI-only; ADR **0056** + spec `docs/superpowers/specs/2026-08-12-2.16h-machine-picker-design.md` rev 6; ветка `feat/2.16h-machine-picker`, subagent-driven через Workflow-разведку). Единый `(region × SKU)` machine-picker (single-endpoint из `/v1/server_types` — план-овый `/v1/datacenters` **мёртв** после 2026-10-01, вскрыто разведкой; `with_scorer` а не `with_filter`) + **Decision 0 (2.16h-a, BREAKING): убран implicit `cpx22`-дефолт** — провижн без явного типа → `server_type_not_selected`, existing-кластеры self-heal через backfill. **Real-Hetzner walk GREEN** (`e2e/machine-picker-walk.sh`, non-interactive плечи: no-type→error+0-ресурсов / explicit-type→точный SKU+факт / legacy-backfill / `target machine` refuse-on-provisioned; sweep→0). Спека прошла **5 раундов внешнего ревью** (R1 вскрыл мёртвый API+scorer, R2–R4 достроили блёст-радиус Decision 0, R5 затяжка). Ручная приёмка владельца → **3 UX-коррекции:** `target machine` refuse на запровиженном кластере (смена машины = `backup`+`restore --reprovision`, не in-place resize), мёртвый deferred-intent-гард убран, stale-подсказка убрана. Девиации от плана: single-endpoint (не datacenters), sort-Select не `sort:`-токен (discoverability), matrix-filter не переживает sold-out-toggle (inquire 0.7.5 не отдаёт набранный фильтр). 2.16h ≠ Phase closure (order-3.7 pulled-up; §6-бокс не трогается — Phase 2 закрывается на 2.11–2.12).
...
> ✅ **ЗАКРЫТО 2026-08-12** вместе с 2.16h в ОДНОМ релизе (cli **v0.2.43**). Real-Hetzner walk GREEN. `DEFAULT_SERVER_TYPE` убран; create-путь без типа → `server_type_not_selected`; existing-кластеры self-heal (backfill). `target machine` refuse-on-provisioned. См. ADR 0056 §(d),(g).
...
### 2.16i — Per-environment deep-merge для `needs.*` (split-out из 2.16c)
> 🏁 SR: A · order 3.7 — **ВЫДЕЛЕНО из 2.16c** (design R3-H3/R4-N2): `needs.*` deep-merge — provisioning-layer data-loss путь, требует своей миграционной аналитики, поэтому 2.16c оставил `needs` на сегодняшнем per-key wholesale-replace, а этот item владеет всеми тремя под-проблемами. НЕ закрыто.

**Контекст:** 2.16c сделал per-environment override struct-полей `expose`/`imagePolicy` частичным (deep-merge по подполям). `needs.*` намеренно НЕ тронут: в отличие от `expose`, частичный override `needs.<type>` пересекает провижионер и может уничтожить данные, а форма `OneOrMany` делает сам merge неоднозначным. Пока `needs` остаётся на per-key wholesale-replace (env-ключ целиком заменяет base-ключ) — новой data-loss-поверхности 2.16c не добавил.

**Три под-проблемы (каждая — свой дизайн-вопрос, все три должны быть решены вместе):**
- [ ] **(a) `merge_needs` для `OneOrMany` (scalar|array) не определён.** `needs.<type>` это `#ServiceNeed | [...#ServiceNeed]` (`OneOrMany`) — что значит «deep-merge» массива под-нужд? Решить: match-by-name (мёржить одноимённые записи) / wholesale-on-array (массив = замена целиком, deep-merge только для скалярной формы) / reject (запретить partial override для array-формы на webhook). Определить семантику ДО реализации.
- [ ] **(b) partial `needs.<type>` override, наследующий `name`, меняет имя ResourceClaim.** Имя claim'а — `{app}-{type}[-{name}]`; наследование `name` из base под изменённым override сдвигает имя → GC старого claim (7-дневный RetainedClaim grace) → пустая БД (**data-loss**). Нужна своя миграционная аналитика: как ведёт себя upgrade-originated мутация имени, гейтить ли её как destructive (2.16b machinery), защищать ли данные.
- [ ] **(c) re-open 2.4d «`needs.*.selector` changes — NON-destructive».** Решение 2.4d («смена `needs.*.selector` не деструктивна») принято для **юзерской** правки; upgrade-originated мутация selector'а через deep-merge — другой контекст, требует пере-рассмотрения (может ре-таргетнуть claim на другого провайдера → тот же data-loss-класс).

**Acceptance:** per-environment override `needs.*` становится частичным без риска потери данных — либо через match-by-name merge с явной защитой claim-name/GC-пути, либо через осознанное решение оставить array-форму wholesale + гейтить опасные мутации. Все три под-проблемы закрыты с regression-guard'ами.

**Зависит от:** 2.16c (struct-deep-merge machinery + `#ApplicationEnvOverride`), 2.4d (`needs.*.selector` decision — re-open), 2.4f (RetainedClaim/GC), 2.4 (provisioner + claim-naming), 2.9 (per-env модель). **Связан с** 2.16b (destructive-gate — если мутацию имени/selector'а гейтить).

**Размер:** M · сам merge-код мелкий; масса — определение семантики `OneOrMany`-merge + миграционная аналитика claim-name/GC-пути + re-open 2.4d selector-решения.
...
### 2.19 Documentation system — сайт, drift-гейты, LLM-артефакты, CLI-UX
> 🏁 SR: A · order 3.7 — публичные доки как продуктовая поверхность (`docs.apprafter.dev`). Всё, что зашипилось в 2.9–2.16h, сегодня существует только в `--help` и в plan.md: сайт не опубликован, лендинг рендерит «Docs — Soon», а единственная CLI-референс-страница описывает 15 из 27 top-level команд. Трек **широкий** (машинерия + контент + CLI-UX, одна закрывающая ручная прогулка), поэтому разложен на подфазы **a–j**; закрывается целиком только на 2.19j.

> ✅ **ВСЕ ДЕСЯТЬ ПОДФАЗ ЗАКРЫТЫ (2.19a–2.19j).** Но строка в `FEATURE_TRACKER.md` остаётся 🚧, и это осознанное чтение её же правила: правило говорило «флип на 2.19j», а ОБОСНОВАНИЕМ давало «сайт, который никто не может открыть, — не поставленная возможность». Обоснование переживает свой триггер: между веткой и читателем стоят пять ручных шагов владельца (DNS, видимость пакета в реестре, регистрация приложения), и `https://docs.apprafter.dev/` пока не отвечает. Строка флипнется, когда сайт вернёт страницу. **2.19j несёт CLI-патч `v0.2.48`** — см. пункт (j). **Ни релиза, ни монорепо-тега** несут a–f; **2.19g несёт CLI-патч `v0.2.45`** — см. пункт (g), реверс первоначального ответа зафиксирован там, а не подправлен; **2.19h несёт CLI-патч `v0.2.46`** (новая команда `completion` + секция `Examples:` в `--help` у 75 команд), и её запись говорит вслух, что дефект, с которого начался трек, — неописанный `--namespace` у `secret seal` — был закрыт ДО неё ранними walk-фиксами, а вклад подфазы другой; **2.19i несёт CLI-патч `v0.2.47`** — не ради контента, а ради ДВУХ `about`: у `apprafter plan`, обещавшего диф, которого команда не считает, и у `apprafter destroy`, чья справка утверждала узкую область («infrastructure managed by this state»), тогда как удаляется всё помеченное `apprafter=true` в проекте токена; вторую нашли ревью — зачистка прозы поправила восемь страниц и пропустила предложение, которое читатель разрушительной команды встречает первым, и её запись фиксирует главную поправку трека: «~15 гайдов» из спека — оценка, которую семь подфаз никто не перемерял, а по дереву команд их **пять** (плюс три команды, осознанно оставленные без гайда, потому что не реализованы). Дизайн: `docs/superpowers/specs/2026-08-17-documentation-system-design.md` (rev 3, актуализирован под 2.19c — переписан после 4-линзового adversarial-ревью: 8 P0 + 40+ P1, три собственных фактических утверждения спека оказались неверны и исправлены). Ветка `feat/2.19-docs-system` (2.19c — на `feat/2.19c-drift-gate`, план `docs/superpowers/plans/2026-08-18-docs-2.19c-drift-gate.md`). **ADR ратифицирован постфактум:** `docs/adr/0057-documentation-system.md` (`Accepted`, 2026-08-18) — долг трека закрыт с опозданием (спек §8 требовал ADR перед 2.19b, а он написан после неё; сам ADR это фиксирует и помечает каждое ещё не реализованное решение). **Ни релиза, ни монорепо-тега:** чарты/оператор/cue-cmp не тронуты, `docsgen` — dev/CI-only крейт, сайт по-прежнему не отдаётся — 2.19g поставила ВСЮ машинерию публикации (образ, Caddyfile, `Application.cue`, `release-docs.yml`), но между веткой и читателем стоят ШЕСТЬ ручных шагов оператора, перечисленных в записи (g) и в раннбуке (2.19g при этом режет CLI-патч 0.2.45 — см. (g)). Но **бинарь CLI изменился** (утверждение «поведение CLI не изменилось» верно только для 2.19a/b — там это были lib-таргет + правки doc-comment'ов): 2.19c встроил в `apprafter app validate` все четырнадцать файлов `schemas/v1alpha1` вместо `types` + `application`, так что манифест, называющий любой другой kind пакета (`#SharedVolume`, `#SourceCredential`, ещё десять), больше не падает с «undefined field» — ошибкой про наш временный workspace, а не про файл пользователя. Тот же бандл использует CUE-чек гейта, так что встраивание одно, а не два. Бампа версии это не требует (никакой команды/флага/дефолта не двигали), но молчать об этом нельзя. **2.19d** (ветка `feat/2.19d-docs-xref-health`) шипнутый бинарь не трогает вовсе — вся его машинерия живёт в `cli/docsgen`, — так что «ни релиза, ни тега» для неё верно без оговорок. **2.19e** (ветка `feat/2.19e-hooks-llm`) — тоже: два build-хука `mkdocs` + `description` во frontmatter + четвёртый проход `scripts/docs-check.sh`; единственная правка Rust — docstring в `cli/docsgen/src/scan.rs`, который её же замер и опроверг. ADR 0057 получил amendment по 2.19e (что поставлено и что осознанно НЕ построено). **2.19f** (ветка `feat/2.19f-information-architecture`) шипнутый бинарь не трогает вовсе — `cli/platform-cli` не изменён ни на строку, так что `apprafter --help` и `commands.json` байт-идентичны, а единственные правки Rust — комментарии в `cli/docsgen`, называвшие переименованные страницы. Её amendment в ADR 0057 фиксирует и то, чего она НЕ построила: карты редиректов нет, потому что редиректить не от чего (см. (f) ниже).

**Контекст (замеры спека §1, до трека):** 93 tracked-страницы (58 ADR + 35 не-ADR); `mkdocs build --strict` проходил, но **не валидировал ничего** — 64 страницы висели вне nav под `validation.nav.omitted_files: info`, битые ссылки и якоря не проверялись; Nix-fallback в `Justfile` **никогда не работал** (`nix shell nixpkgs#python3Packages.mkdocs-material` не даёт бинаря `mkdocs`), и `CLAUDE.md:18` документировал сломанный путь как рабочий; **docs-only PR гонял ноль CI-джобов** (в `lint.yml` нет docs-фильтра); `edit_uri` указывал на несуществующий `main`. Шесть подтверждённых дрифтов код↔доки, **пять из шести живут вне code-fence'ей** (таблицы и проза) — отсюда вся форма гейта 2.19c. CLI: 27 top-level + 68 вложенных = **95 command-path'ов**, ни одного usage-примера, ни completions, ни snapshot-тестов.

**Цель:** довести доки до состояния продуктовой поверхности — опубликованный сайт на своих финальных URL'ах, машинно-проверяемая связь код↔доки (drift ломает `just lint`, а не всплывает на аудите), LLM-артефакты, и CLI, который сам себя объясняет.

**Поставка (подфазы; закрыты a–f):**
- [x] **(a) Foundations** *(= W0 спека)*. Один `python3.withPackages` env в `flake.nix` (mkdocs-material + literate-nav + redirects), docs-таргеты `Justfile` ходят через `nix develop`. Строгая сборка впервые **валидирует**: `not_in_nav` allow-list, `validation.links.{not_found,anchors,unrecognized_links}`, `nav.omitted_files: warn` — `operator-guide/node-prep.md` оказался невидимым юзер-гайдом и вернулся в nav. Внутренние страницы (`changelog/history.md`, `changelog/plan-history.md`, `measurements/`) больше не публикуются на сайт (`history.md` **остаётся на своём пути** — его парсит `release-cli.yml`). Лицензия доков названа: проза **CC-BY-4.0** (`LICENSE-CC-BY-4.0`), сэмплы кода **Apache-2.0** — в `docs/license.md`, `README.md`, `NOTICE` и футере сайта; пять залётных `FSL-1.1-Apache-2.0` SPDX-заголовков на Markdown под `docs/` убраны (противоречили новой лицензии), `contributing/license-headers.md` теперь говорит, что Markdown под `docs/` per-file-заголовка не несёт. Новый гейт `just docs-check` (strict build + reference drift) вкручен в `just lint`, lefthook-хук и `.github/workflows/docs.yml`.
- [x] **(b) Generated CLI reference** *(= W1)*. У `cli/platform-cli` появился lib-таргет: `dispatch` уехал в `src/dispatch.rs`, `main.rs` — тонкий шим над `apprafter::run()`; публичная поверхность = `run()` + фасад `docs_api` из двух пунктов (осознанное расширение API, а не побочный эффект рефактора). Новый крейт `cli/docsgen` генерирует `docs/reference/cli/**` — 26 страниц по top-level командам, индекс, literate-nav `SUMMARY.md` и машиночитаемый `commands.json` на все 95 command-path'ов — а `docsgen check` **байт-в-байт** сверяет их с clap-деревом в `just lint`, lefthook и CI: переименованный флаг теперь роняет сборку. Рукописный `docs/reference/cli.md` (15 из 27 команд, при этом заявлявший полноту) удалён; новый `docs/reference/environment.md` описывает 25 переменных окружения, которых генератор не видит (их читают `std::env::var` или зависимости). Генератор сработал как аудит doc-comment'ов: `status`/`login`/`upgrade-tier` теперь **первым предложением** признаются скелетами, документированы семь ранее безымянных флагов `backup enable` и `platform upgrade`, у `--no-ping` убрано ложное «любое непустое значение», а внутреннее Rust-имя `nodes[0].kind` заменено на реальный ключ манифеста `type` в трёх help-строках и в `operator-guide/troubleshooting.md` (там оно учило операторов писать непарсящийся ключ).
- [x] **(c) Drift-гейт по гайдам** *(= W2)*. Три чек-гейта в `cli/docsgen`, вкрученные в `just lint`, pre-commit-хук и CI (`scripts/docs-check.sh`) над **33** гайдами в скоупе (`spec.md` вне скоупа осознанно — это roadmap, не описание того, что поставлено; сгенерированный `docs/reference/cli/**` тоже вне скоупа — он уже байт-сверяется отдельным гейтом 2.19b): **CLI-инвокации** — путь команды + имена флагов (никогда не значения, никогда обязательность аргумента) резолвятся против `commands.json`, и во fence'ах, и в инлайн-спанах — **384** вызова; строгий резолвер, парсящий значения/`required`, ронял бы валидную документацию массово — 38 инвокаций в скоупе это usage-синопсис (`[--namespace <ns> ...]`), `\|` в половине случаев альтернация в скобках, а не пайп, и 161 из 213 инлайн-спанов — голые command-path без единого аргумента (корпус КАК НАЙДЕН, на `dc4c5de`; после правок этой подфазы — 165 из 219, 75 %). **Schema-идентификаторы** — backtick-пути резолвятся против CRD-полей плюс пяти CUE-only kind'ов; `Kind.spec.…` резолвится только против ЭТОГО kind'а, что закрывает silent-pass плоского union'а — **236** резолвится, из них **115** (чуть меньше половины) — только под узлом `x-kubernetes-preserve-unknown-fields`, то есть «резолвится» ≠ «проверено», и гейт печатает это соотношение каждый прогон, а не прячет его в общий счётчик. **Полные CUE-документы** — батчевым `cue vet -c -i` (**2** документа этим прогоном; фрагменты — вне скоупа, см. ниже). Обязательства выводятся из **содержимого** fence'а, никогда из его языкового тега: у ручного shell четыре разных тега и 14 fence'ов вовсе без тега, так что гейт на теге тривиально глушится удалением тега. Маркеры-исключения типизированы (закрытый словарь причин), датированы, стареют против РЕАЛЬНОЙ даты git-тега (а не даты коммита), недействительны без даты и репортятся, если не совпали ни с чем — молчаливое неиспользуемое исключение тоже находка.
  Гейт нашёл **33** проблемы, все починены: `apprafter promote` и другие непоставленные обещания убраны из `dev-guide/index.md`; примеры `--env`/`--namespace` сверены с реальным размещением флагов в `cli.rs`; `expose.public` → `expose.network` в восьми местах (два — внутри `kubectl apply`-heredoc'ов, где apiserver молча дропает неизвестный ключ, и читатель получает workload без экспозиции и без единой ошибки, объясняющей почему); 14 fence'ам проставлены info-строки; модель `DATABASE_URL`/`REDIS_URL` (снятая ADR 0046 в пользу decomposed-ключей `url`/`user`/`pass`/`host`/`port`/`db`) поправлена на 25 из 30 строк в трёх файлах (`application-cue.md` 5, `needs-pg-walk.md` 11, `needs-redis-walk.md` 14 — замер по in-scope-корпусу на `dc4c5de`) — оставшиеся пять уже были верны (они называют переменную окружения, которую читает приложение, а не ключ Secret'а); достроена таблица полей (`resources`, `imagePolicy`, `needs`, `environments`, `expose.hostname`, `expose.tls`). Два места исключены маркером, а не правкой, каждое с типизированной датированной причиной: `operator-guide/node-prep.md` документирует СНЯТУЮ команду (`reason=historical` — новая причина в словаре маркера ровно под этот случай) и `dev-guide/application-cue.md:36` корректно документирует схему **Argo CD**, а не нашу (`reason=external-tool`). 20 фикстур с парными негативными контролями (`cli/docsgen/testdata/drift/`) пинят каждый найденный дрифт и доказывают, что чек не срабатывает на лексически похожем, но корректном соседе.
  **Сознательно не покрыто:** CUE **фрагменты** не валидируются — 15 из 17 `cue`-fence'ов в скоупе это фрагменты, а не полные документы (один скелет не обслужит семь разных anchor-глубин; батч-валидатор проверяет только полные документы). `requires`/`conflicts` (класс «`--namespace` без `--select`») **нереализуем**: clap 4.6 держит их `pub(crate)` без геттера — пункт СНЯТ из дизайна, а не отложен на будущее. Протухший ключ Secret'а внутри `jsonpath` (ADR 0046 переименовал ключи pg-коннекшн-Secret'а, `{.data.DATABASE_URL}` теперь читает пустоту на живом кластере) — известная дыра: ни один существующий чек её не видит (строка не называет `apprafter`-подкоманду, не содержит dotted schema-путь, не является CUE-документом), зафиксирована фикстурой с парным негативным контролем и комментарием, что понадобится для закрытия (новый чек, резолвящий документированные ключи Secret'а против того, что реально пишет provisioner — а не расширение identifier-чека, который резолвит против CRD, а ключ Secret'а не CRD-поле). `spec.md` вне скоупа по решению — доки описывают то, что поставлено СЕГОДНЯ, а `spec.md` — это roadmap. Заодно ADR 0042 §7 и ADR 0043 §1 помечены superseded-in-part ADR 0046 (composed `REDIS_URL`/`REDIS_CHANNEL_PREFIX` и авто-инжект `<VAR>_<NAME>`, которые каждый из них описывал).
- [x] **(d) Xref + health-метрики** *(= W3)*. Ветка `feat/2.19d-docs-xref-health`. **Ни релиза, ни монорепо-тега:** `cli/Cargo.toml` не тронут, чарт/оператор/cue-cmp не двигались, `docsgen` — build-time-крейт, не входящий ни в один релизный артефакт. Поставлено четыре механизма плюс канал исключений; два пункта исходного списка СНЯТЫ по замеру, один ОТЛОЖЕН по замеру, ещё один оказался не нужен.
  **1. Обязательства переживают расфенсивание блока.** Отступ в четыре пробела на верхнем уровне и HTML-`pre` теперь несут ровно то же, что и fence. Закрыт обход, с которым гейт зашипился: контрибьютору, которому сказали «пометь fence», дешевле снести делимитеры и сдвинуть тело — рендерится идентично как код, а для всех чеков блок исчезал, и в диффе это не читается как подавление. Трекинг контейнеров тут load-bearing, а не косметика: `pymdownx.tabbed`, admonition'ы и list item'ы отступают теми же четырьмя пробелами, и без него чек даёт **шесть** ложных срабатываний на одном только `docs/dev-guide/quickstart.md`. Незакрытый `pre` — свой класс (`unclosed-pre`), потому что чинится другой правкой, чем незакрытый fence.
  **2. `code-path`** — путь репозитория, названный в backtick-спане или в относительной ссылке, которого нет. Две поверхности резолвятся по-разному: спан — от корня репозитория (читатель грепает его оттуда), таргет ссылки — относительно каталога страницы (как это делает MkDocs), а спан, являющийся ТЕКСТОМ ссылки, своего утверждения не делает — проверяется таргет. Глобы душатся на резолве, а не на извлечении, и набор паттерн-символов — `*` `?` `{` `}`, **но НЕ** `[`/`]`: три tracked-файла несут скобки в именах (динамические роуты Next.js), и считать их паттерном значило бы душить ровно эти ссылки жёстче всего именно в тот момент, когда один из файлов переезжает. У класса **нет ignore-ключа**: чужой путь называется чужим в предложении, а не исключается, иначе читатель будет грепать его в НАШЕМ репозитории.
  **3. `adr-reference`** — цитата, не называющая ни одного ADR, либо называющая решение со статусом `Superseded` / `Deprecated` / `Unused` (зарезервированный и брошенный слот). Читаются обе орфографии (`ADR 0046`, `ADR-0027`), множественное число, цитата, **перенесённая через разрыв строки**, и номер, зашитый в таргет ссылки или в путь файла. Вердикт берётся из **ведущего токена** тела `## Status`, поэтому частичное вытеснение («§7 superseded in part by …», которое несут четыре ADR) корректно НЕ находка; подстрочное правило на том же корпусе давало только ложные срабатывания.
  **4. `adr-check-ignore`** — канал исключений для этого класса, на той же машинерии, что `cli-check-ignore` и `schema-check-ignore`: те же типизированные причины, тот же `since=`, стареющий против РЕАЛЬНЫХ дат тегов, то же «без даты — недействительно» и тот же аудит неиспользованных. Ключ — канонический `ADR NNNN`, поэтому один ключ покрывает все написания одной цитаты. Осознанно цитировать снятое решение легитимно (это наша же идиома — предложение, которое объявляет вытеснение тем же вздохом), а класс зашипился вообще без способа это сказать.
  **5. Census + два health-класса.** `docs/measurements/docs-health.json` коммитит **семь** чисел, `docsgen gate` сверяет их **по значению**: шесть obligation-счётчиков не могут уменьшаться (рост проходит молча и намеренно — гейт, заставляющий перезаписывать файл ради написания страницы, это гейт, вокруг которого ходят), а `exemptions` сверяется на **равенство в обе стороны**, так что и объявление, и снятие исключения дают ровно одну ревьюабельную строку диффа. Перезапись — `docsgen metrics`. Отсутствующий или непарсящийся census — **exit 2** (сломан гейт), никогда exit 1.
  **СНЯТО/ОТЛОЖЕНО по замеру (не по вкусу):** **staleness-гейт СНЯТ** — нежизнеспособен ни на каком пороге: самой старой странице корпуса 104 дня, так что правило на 180 дней не может сработать ещё месяцами; правило на 90 дней вечно бьёт по одним и тем же четырём абстрактным страницам; 24 из 33 страниц делят две даты последнего касания и пересекают ЛЮБОЙ порог одновременно; обе стрелки сбрасываются на не-событиях (прогон SPDX-заголовков, прогон форматтера); `git log -1 -- <path>` не ходит через переименования, а одну страницу корпуса уже переименовывали; и из последних 100 коммитов, тронувших `schemas/**` или `cli.rs`, страницу в скоупе тронули **три**. Если staleness вернётся — его форма это дифф против опубликованного тега с явным pathspec (ровно то, что уже делает `scripts/check-operator-version-bump.sh`), а не прошедшие дни. **Типизированные `verified-by`-биндинги ОТЛОЖЕНЫ** — 17 из 28 tracked-скриптов `e2e/*.sh` не запускаются ничем автоматическим (ни workflow, ни `Justfile`-таргет), и в этом множестве `backup-restore-walk.sh`, который страница корпуса УЖЕ называет как своё подтверждение; требовать поле на ~20 dev/operator-страницах значит изготовить ~15 утверждений, которые читаются как проверенные и не являются таковыми. **Отдельный anchor-чекер НЕ НУЖЕН** — `validation.links.anchors: warn` под `--strict` уже роняет сборку на мёртвом якоре, и все 28 якорных ссылок в скоупе резолвятся. **Три ранее предложенных ратчета ЗАМЕНЕНЫ:** `blocks_executed` нереализуем (ключ `run=` принимает только `local`, а гейт репортит `run=local` как находку, потому что документированный блок никто не исполняет ⇒ ратчет вечно читает `0 >= 0`); `check_none` на нуле запрещает, а не ратчетит — на этом значении он банит тот самый типизированный датированный протухающий канал исключений, который решение и построило; а **байт-сверяемый** census покраснел бы на следующий день после коммита без единого коммита между, потому что три изначально предложенных поля — функции `now` (семь оставленных — нет).
  **Честный заголовок: все три новых чека находят на корпусе НОЛЬ дефектов сегодня.** Это замер, а не надежда, и это причина такого порядка работ: 2.19i добавляет ~15 гайдов, которые будут густо цитировать ADR'ы и пути репозитория, а страж, поставленный ПОСЛЕ контента, лишь узаконивает то, что контент уже сделал неправильно. Гейт, который в день поставки не репортит ничего, — честен; описывать его так, будто он что-то нашёл, — нет.
  **Предел, названный, а не замазанный:** счётчики меряют кардинальность, никогда специфичность — обязательство, ЗАМЕНЁННОЕ (а не удалённое), не стоит ничего: rich-инвокация на голую, глубокий путь на мелкий, конкретный файл на его каталог — все семь чисел держатся; проза, не несущая ни одного считаемого утверждения, невидима для census'а целиком. Эти ратчеты защищают от небрежного, не от мотивированного — от мотивированного защищает ревью.
- [x] **(e) mkdocs-хуки + LLM-слой** *(= W4)*. Ветка `feat/2.19e-hooks-llm`, план `docs/superpowers/plans/2026-08-19-docs-2.19e-hooks-and-llm-artefacts.md`; ADR 0057 получил amendment по факту поставки. **Ни релиза, ни монорепо-тега:** `cli/Cargo.toml` не тронут, чарт/оператор/cue-cmp не двигались, единственная правка Rust — docstring в `cli/docsgen/src/scan.rs`, который замер этой же подфазы и опроверг. Два хука в `docs/hooks/`, зарегистрированных ключом `hooks:` в `mkdocs.yml`; **ни один артефакт не коммитится** — все выводятся из закоммиченного контента на сборке, поэтому дрифтовать им не с чем (ровно поэтому пакет и мог приехать в любой точке трека).
  **1. CUE-лексер, но поставка тут — самопроверка, а не лексер.** pygments 2.20.0 лексера для CUE не несёт, а корпус пишет **37** `cue`-fence'ей на 20 tracked-страницах, **31** из них доезжает до сайта (остальные шесть — в changelog-рабочем файле, исключённом из сайта). Без хука все 31 рендерятся простым текстом, и сборка при этом **зелёная**: `pymdownx.highlight` глотает исключение неудавшегося lookup'а и молча откатывается на `text`. Поэтому `on_config` не просто регистрирует: он резолвит алиас (ловя `ClassNotFound` и объясняя последствие), проверяет, что алиас указывает **на НАШ** лексер (а не на чужой, занявший имя), и требует от сэмпла шесть семейств токенов (comment/keyword/string/name/number/operator) — через иерархию токенов pygments, так что перетокенизация в дочерний подтип ложно НЕ срабатывает, а потеря конструкции срабатывает. Оба режима отказа **пронаблюдены падающими** до коммита: самопроверка, падения которой никто не видел, — это самопроверка, про которую никто не знает, работает ли она. R1 в ADR называл митигацией «обязательный unit-тест»; зашипилось строже — проверка живёт в `on_config` и гоняется на КАЖДОЙ сборке (превью, `just lint`, CI), а не в наборе, который надо позвать.
  **Замер плана не воспроизвёлся и исправлен, а не подогнан:** в плане стояло 34 fence'а. Реально **37** tracked / **31** на сайте; счёт, требующий, чтобы fence открывал строку, даёт 33 — молча теряя четыре (три внутри blockquote'ов, один с отступом под list item'ом). Сборка подтверждает независимо: число блоков с токенами на сайте выросло ровно на 31.
  **2. `description` на каждой странице, которую индексирует `llms.txt`.** 31 рукописная страница получила строку. Из **34** страниц в скоупе гейта 33 несут frontmatter с описанием; единственное исключение — корневой `README.md`, который не является страницей сайта, так что ему нечего заполнять (ни записи в индексе, ни meta-тега). До подфазы описание было ровно у ОДНОЙ рукописной страницы — сгенерированные командные страницы несли `title`/`description`/`audience`/`status` изначально, их пишет генератор. На собранном сайте страниц, отдающих **своё** HTML-`meta`-описание, стало 28 → **60**; оставшиеся 60 — это 59 ADR'ов и 404, они падают на `site_description`, то есть на одно и то же предложение везде. Цена поля она же и его предел: три черновика переписаны ДО коммита, потому что каждый утверждал то, чего страница не поддерживает (секция enforcement'а в будущем времени, описанная как настоящее; команда, посчитанная в группу, к которой не относится; platform-scope-глагол, поданный как доступный на приложение). Описание — проза, и не проверяет его ничто, кроме ревью.
  **3. `llms.txt` / `llms-full.txt` / markdown-двойник рядом с каждой страницей.** Замер на собранном сайте: **119** опубликованных страниц, 59 из них под `adr/`; **60** в индексе, **119** в бандле, **119** двойников. Раскол по ADR'ам: **в бандл и в двойники — да, в индекс — нет.** Каждый ADR описывает мир на день ратификации (ровно поэтому drift-гейт держит их вне скоупа), и это половина всего, что публикует сайт, — индексировать их значило бы похоронить гайды: модели, которой отдали весь корпус, они нужны; модели, которой отдали карту, — не должны попадаться по дороге. Две формы, которых дизайн не предвидел: индексу понадобилась секция **`Other pages`** (две опубликованные страницы лежат в `not_in_nav`, и чисто nav-овый индекс молча бы их потерял; с секцией индекс ПОЛОН по опубликованным не-ADR-страницам — это и позволило написать сторожа как полноту, а не как переписанный список имён групп), и вложенные nav-секции несутся **префиксом ярлыка**, а не расплющиваются, потому что расплющенная *Reference* показывала две разные страницы как «Overview». Хук **роняет сборку**, если у индексируемой страницы нет `description`: план называл это контрактом, но не обеспечивал ничем.
  **4. Четвёртый проход в `scripts/docs-check.sh`.** mkdocs не знает, что эти три артефакта должны были появиться, так что переставший писать хук оставляет зелёную сборку и опубликованный сайт со сломанным машиночитаемым слоем. Проход делает **десять** утверждений: индекс есть и называет лицензию, его ссылки абсолютны, каждая резолвится в страницу, которая на сайте ЕСТЬ, каждая опубликованная не-ADR-страница в индексе ПРИСУТСТВУЕТ, бандл есть и держит по записи на страницу, и запись эта с телом, а не голый заголовок, названный двойник существует, и число двойников равно числу страниц. Все ожидания **выводятся из только что собранного сайта** — ни одного переписанного списка страниц, поэтому завтрашняя страница покрыта без правки скрипта.
  **СНЯТО/ОТЛОЖЕНО — оба пункта той же формы, на которой трек уже обжигался:** **`since` во frontmatter СНЯТ.** Это ровно то, что провалилось у `verified-by` подфазой раньше: per-page-утверждение о версии, которое НЕЧЕМ проверить — его не читает ни один гейт, на нём не падает сборка, и читатель не отличит актуальный `since` от верного два релиза назад; пишется один раз при авторстве и тухнет молча. Требовать его = изготовить по одному непроверяемому утверждению на страницу, то есть ровно тот класс дефекта, ради удаления которого вся система и строится. Контраст, который вопрос и решает, живёт в этом же репозитории: `since=` в ИСКЛЮЧЕНИИ остаётся, потому что гейт резолвит тег в дату коммита и обнуляет исключение через 180 дней. Дело не в имени поля, а в том, может ли что-то в дереве назвать утверждение неверным; версию, если она нужна странице, называют в ПРОЗЕ, где её видит читатель и спрашивает ревьюер. `audience` выживает по тому же тесту с другой стороны — он ВЫВОДИМ (frontmatter → nav-группа → группа ближайшего каталога-предка → `general`), поэтому опционален, а не обязателен. **`<llm-only>` / `<llm-exclude>` ОТЛОЖЕНЫ** — замер: **ноль мест использования** во всём репозитории. Естественный дом для них — проход markdown-двойника, а он теперь существует, так что добавить их позже = горсть строк против РЕАЛЬНОГО места использования, а не механизм, угаданный заранее (именно так система, разобранная в §Context ADR'а, и набрала машинерию, которую не гонял ни один CI-джоб). То же самое записано в самом хуке про `--8<--`-инклюды (двойник несёт строку инклюда, а не содержимое): тоже ноль мест использования сегодня.
- [x] **(f) Информационная архитектура** *(= W5)*. Ветка `feat/2.19f-information-architecture`, план `docs/superpowers/plans/2026-08-19-docs-2.19f-information-architecture.md`; ADR 0057 получил amendment по факту поставки. **Ни релиза, ни монорепо-тега:** `cli/Cargo.toml` не тронут (0.2.44), чарт/оператор/cue-cmp не двигались. Rust всё же правился, и это стоит назвать точно, потому что первая редакция этой строки утверждала обратное и была опровергнута собственной веткой: doc-комментарий в `cli/platform-cli/src/cli.rs` говорил «later sub-phases» — внутренняя координата, которую пользователь читает в `apprafter --help`, — и его переписали, отчего байт-сверяемый `commands.json` сдвинулся на строку (гейт этого и требует, регенерация в том же коммите). Плюс комментарии в `cli/docsgen`, называвшие переименованные страницы (комментарий, указывающий на несуществующий файл, — ровно тот класс дефекта, ради которого весь этот гейт и строился).
  **1. Доки перестали говорить с инсайдерами.** Внутренние координаты роадмапа («в фазе 8.2»), коды подфаз и указатели в `spec.md` переписаны в терминах читателя либо удалены там, где предложение и без них полное; где спецификация действительно и есть ответ — абсолютный GitHub-URL, и **само предложение говорит, что это roadmap репозитория**, чтобы читатель понимал, что уходит из документации. Число не записываем — записываем команду: `git ls-files -- 'docs/*.md' | grep -vE '^docs/(adr|changelog|measurements|superpowers)/' | xargs grep -nEi 'phase[- ]?[0-9]|\b[0-9]+\.[0-9]+[a-z]\b|\bplan\.md\b|\bspec\.md\b|\bM[0-9]|track [ab]\b|sub-?phase|plan item'` даёт **36** на `fa3472e` и **7** после корректировок ревью. **Паттерн РАСШИРЕН по итогам ревью** — первая его версия (`\bphase [0-9]` + три литерала) давала 25 → 7 и пропускала мимо себя десять инсайдерских координат, записанных иначе (см. блок корректировок ниже). Эти семь — осознанные и все разные: четыре абсолютных URL-цитаты спецификации; одна — `contributing/documentation-gate.md`, где `spec.md` назван **путём репозитория** в таблице путей, которые гейт исключает (https-ссылка заставила бы страницу описывать собственный скоуп в терминах, которых она не использует); и две — link-таргеты сгенерированного референса на страницу команды `apprafter plan`, то есть **ложное срабатывание паттерна, а не утечка**. Команда осознанно ограничена `docs/`: `README.md` входит в корпус гейта, но не является страницей сайта, и ссылки на роадмап там для читателя РЕЗОЛВЯТСЯ — человек на титульной странице репозитория может открыть `plan.md`.
  **2. Две страницы-заглушки удалены, а не «разрешены на месте».** `architecture/index.md` и `concepts/index.md` открывались «**Status:** stub», отправляли читателя в файл, которого сайт не несёт, и занимали два из девяти top-level-слотов nav. Страница, существующая только чтобы сказать «меня ещё нет», хуже 404: 404 хотя бы не стоит клика. Nav: **9 top-level → 6**.
  **3. Единственную ценную таблицу ПЕРЕСОБРАЛИ, а не перенесли.** План велел перенести CRD-таблицу концептов в `docs/reference/index.md` на основании «она верна сегодня». Она не была верна: против CRD, которые реально ставит Helm-чарт оператора, она называла четыре объекта, не имеющих CRD вовсе (`AccessGrant`, `ExternalSurface`, `ServiceProviderPlugin` и `Infrastructure` — последний вообще не кластерный объект ни в одной фазе, а локальный манифест, который парсит CLI), и теряла четыре поставленных (`PlatformStack`, `RetainedClaim`, `SharedVolume`, `SourceCredential`). Перенесена ФОРМА таблицы; строки выведены из шаблонов чарта, назначение каждого — из docstring'а его CUE-схемы. Перенести как есть значило бы повысить ошибку заглушки до статуса референса.
  **4. Семь переименований через `git mv` + `Public ingress` свёрнут в operator-секцию.** «walk» — слово ЭТОГО репозитория для e2e-скрипта (`e2e/needs-pg-walk.sh`); на сайте это был заголовок и URL, которые читателю нечем интерпретировать. Страницы названы по тому, что читатель получает (`postgres.md`, `redis.md`, `persistent-disk.md`, `egress-policy.md`, `connect-a-git-repository.md`). Две страницы public-ingress переехали КАТАЛОГОМ, а не только nav-строкой: оставить `/public-ingress/` под секцией «Operator Guide» — ровно то расхождение, ради устранения которого подфаза и существует. `license.md` и `contributing/README.md` сняты с `not_in_nav` в реальные позиции nav (лицензию посетитель ищет по имени). **Имена `e2e/*-walk.sh` сохранены** — это то, что поставлено, `code_paths`-флор гейта их резолвит, и там слово верное; неверным было одолжить его читателю.
  **5. Карта редиректов НЕ построена — решение с замером, а не недосмотр.** Редиректить не от чего: ни один URL этого сайта никогда не был публичным (сайт не опубликован, ни один workflow его не публикует, лендинг рендерит «Soon» из пустого `docsUrl`; `docs.apprafter.dev` теперь стоит в `mkdocs.yml` — см. корректировку `site_url` ниже — но никто его не отдаёт). Запись отображала бы URL, которого никто никогда не держал, в URL, до которого пока никто не может дойти. **Триггер назван явно: первая страница, переехавшая ПОСЛЕ публикации (2.19g), требует записи**, а `mkdocs-redirects` уже лежит в python-окружении флейка — это блок конфига, а не смена зависимости. Один класс URL публичным БЫЛ и никакой картой редиректов не обслуживается в принципе: относительные ссылки `README.md` в `docs/` — это GitHub-blob-URL'ы, куда плагину MkDocs не дотянуться; единственное лекарство — правка README тем же коммитом, что и переименование (сделано).
  **И безопасными переименования делает НЕ census.** Все его счётчики — тоталы, поэтому семь переименований оставили `pages` неподвижным: для census'а переименование и удаление неразличимы. Различает их `mkdocs build --strict` с валидацией ссылок **и якорей**, который падает на первой же входящей ссылке или заголовочном якоре, названном с обеих сторон. Ревьюабельным перенос URL делает линк-валидатор, а не census.
  **Census:** `pages` 34 → 32 (ровно два удаления), `identifiers` 236 → **238**; остальные шесть счётчиков не двинулись. Гейт поймал РЕАЛЬНУЮ потерю: переписывание списка ссылок в operator-индексе снесло идентификатор `needs.pg` вместе с текстом ссылки, где он сидел, и `identifiers` вернулся 235 против коммитнутых 236 — утверждение вернули (и лучше поставили: список теперь называет поле за каждым из трёх гайдов), а не занизили число под факт.
  **Из формулировки ADR НЕ сделан раскол «contributor walks vs user guides» — раскалывать нечего.** Каждая из этих страниц описывает задачу владельца кластера, то есть оси contributor/user, по которой их можно было бы разрезать, там просто нет. Контрибьюторскими были СЛОВАРЬ и несколько секций, и убран/понижен именно он. (Замер, которым это обосновывалось раньше, был НЕВЕРЕН — считался `apprafter\s+[a-z]`, то есть и упоминания в прозе; внутри fence'ов четыре dependency-гайда доминированы `kubectl`, а не `apprafter`. См. блок корректировок ниже.) Ось operator/developer (кто владеет кластером vs кто шипит приложение) замером держится и сохранена.
  **Три per-item-замера плана НЕ воспроизвелись** (корпусные счётчики воспроизводились точно — не воспроизводились именно утверждения о содержимом): (1) «две ссылки на `plan.md` — в СГЕНЕРИРОВАННОМ референсе, значит это clap-doc-comment, и его читают в `--help`» — такого doc-comment'а нет: `grep -c 'plan.md' docs/reference/cli/commands.json` даёт 0, `apprafter --help` не содержит строки вовсе, а обе строки — markdown-link-таргеты на реальную страницу `docs/reference/cli/plan.md`; следование букве плана заставило бы гоняться за фантомом и, возможно, править посторонний внутренний комментарий ради движения числа; (2) CRD-таблица «верна сегодня» — см. пункт 3; (3) «12 внутренних кросс-ссылок + 1 README» — сильно занижено; число не записываем (оно уже было записано неверно один раз), записываем команду: `git grep -nE '\]\((\./)?(\.\./)*(docs/operator-guide/|operator-guide/|public-ingress/)?(gitops-walk|needs-pg-walk|needs-redis-walk|needs-disk-walk|needs-networkpolicy-walk|cloudflare-origin-cert|connect-a-domain)\.md' fa3472e -- . | grep -v '^fa3472e:docs/changelog/'`. Греп самого плана не видел ссылок в том же каталоге (`](./gitops-walk.md)`, `](needs-pg-walk.md)`) и вовсе не покрывал `public-ingress/`.
  **Известные дыры, названные, а не замазанные:** концептуальной страницы на сайте больше нет вообще (вся концептуальная часть — таблица на референс-странице; это честнее двух извинений в лучших слотах nav, но меньше того, что подразумевает ссылка с лендинга в 2.19g — «что это такое» пишется в 2.19i, третья заглушка ответом не является); `Infrastructure` теперь НАЗВАН на референс-странице и не документирован нигде (ближайшая поверхность — строка `APPRAFTER_MANIFEST` в `docs/reference/environment.md`) — гайд в 2.19i; **nav** operator-guide остался плоским списком из 17 записей (осознанно: группировка nav — это поведение `navigation.sections` и, возможно, сегменты URL, а ремит подфазы — устаканить URL'ы, а не импортировать docs-фреймворк); при этом ИНДЕКС секции по итогам ревью переписан в карту с группировкой по задачам, потому что как карта он не работал — линковал 11 страниц из 16.
  **КОРРЕКТИРОВКИ ПО ИТОГАМ ТРЁХ РЕВЬЮ (first-time visitor / honesty / adversarial).** Общая форма всех находок одна: **паттерн переписи — не чтение**. Регексп свипа сам себе задал критерий успеха, и четыре документа записали его результат как полноту. Это ровно P1 — ожидание, выведенное из той же стороны, которую проверяют. **(1) Десять инсайдерских координат уцелели**, каждая записана иначе, чем видит паттерн: `Phase-4`/`Phase-8.5` (через дефис — `phase\s+[0-9]` их не берёт), `M3 target`, `M2+`, `Track A`, `Track B 1.70`, `plan item 1.80`, «later sub-phases» в clap-doc-комментарии (а он уходит в `apprafter --help`, `commands.json` и две сгенерированные страницы) и две строки с «walk» как существительным про документ, о котором читатель не слышал. Одна из них стояла тремя строками ниже blockquote'а, который этот же свип и почистил, — страница противоречила собственной уборке. Все переписаны в терминах читателя (не подменены другой внутренней координатой); паттерн расширен ПОСЛЕ чтения, а не вместо него; `docsgen-generate` прогнан тем же коммитом; **бампа версии и тега по-прежнему нет** — `cli/Cargo.toml` бампается под РЕЛИЗНЫЙ тег в том коммите, что его режет, а правка help-строки требует регенерации `commands.json`, а не бампа. **Чтение нашло то, чего не берёт и расширенный паттерн:** `DoD checklist` как пользовательский заголовок на четырёх страницах, `MANDATORY: psql DROP assertion` и два его брата, и `## Platform-CLI coverage` с «this guide exercises every shipped `platform-cli` subcommand … `kubectl` are sanity-only supplements» — имя КРЕЙТА вместо имени продукта и словарь walk-дисциплины в заголовках трёх гайдов. **(2) Замер command-dominance был ложным** и был ЕДИНСТВЕННЫМ заявленным основанием решения: считался `apprafter\s+[a-z]`, то есть и упоминания в прозе. Внутри fence'ов (то, что читатель реально выполняет) `postgres.md` — 40 `kubectl` против 10 `apprafter`, `redis.md` 39/10, `persistent-disk.md` 36/10, `egress-policy.md` 17/5. Вывод (не раскалывать) устоял на ДРУГОМ основании — все эти страницы про задачу владельца кластера; ложное доказательство удалено, истинная форма записана: dependency-гайды `kubectl`-доминированы, потому что наблюдения, которым они учат, — это `kubectl`-чтения. **(3) `ServiceProvider` помечен «пишет Operator»** — ни одна подкоманда `apprafter` его не создаёт, все сидированы umbrella'ой (`platform-stack/cue/service_providers.cue`), а четыре другие страницы сайта уже говорят «the **seeded** `pg-integrated`». Ячейка перенесена из заглушки дословно, то есть «пересборка таблицы» покрыла набор СТРОК, но не ячейки; остальные четыре «Operator» пересобраны поиском писателя в дереве, каждая теперь называет команду/компонент. **(4) `site_url` кодировал дизайн, который ADR 0057 (решение 8) ОТВЕРГ** — `https://apprafter.dev/docs`, путь на origin'е лендинга. Это не косметика: значение запекается в абсолютные URL'ы `llms.txt`, `llms-full.txt`, 117 markdown-двойников и `sitemap.xml`, то есть отвергнутый дизайн публиковался в четырёх классах артефактов. «URL'ы устаканены» было НЕВЕРНО, пока это стояло. Теперь `https://docs.apprafter.dev/`; пересобрано, артефакты последовали. **(5) Три ссылки на переименованные страницы уцелели ВНЕ `docs/`** — живая markdown-ссылка в `e2e/README.md`, комментарий в `cli/docsgen/tests/invocation_test.rs` и — худшее — `e2e/lib.sh`, печатавший мёртвое имя страницы оператору, у которого только что упал memlock-preflight, прямо перед `exit 2`. Это опровергает правило, которое ADR институционализировал («переименование — проблема `mkdocs --strict`, а не грепа; доверяй сборке»): корпус гейта — `git ls-files -- docs README.md`, `--strict` не видит ничего вне `docs_dir`, в `docs.yml` нет триггера на `e2e/**`. Правило исправлено на верное, каталоги названы поимённо, команда свипа записана. **(6) Два входа не выполняли работу, ради которой существуют:** operator-индекс линковал 11 страниц из 16 (в т.ч. `backup-restore.md` — поставленный гайд на 650 строк — не линковался вовсе, а слово «backups» встречалось дважды и оба раза внутри списков непостроенного) → переписан в карту по задачам с одним блоком «not documented yet»; operator-quickstart отвечал на «поставь это на мой сервер» через `nix develop` + `cargo install --path cli/platform-cli`, т.е. предполагал checkout репозитория → теперь ведёт релизным бинарником. Туда же: `## Step 0 — run the k3d e2e first` открывал четыре dependency-гайда, предписывая читателю сайта прогнать наш собственный e2e как обязательный гейт → перенесён в подвал как опциональная заметка для контрибьюторов. **(7) Мелочи:** `docs/index.md` заявлял «generated reference for every command and **object**» (сгенерированного object-референса нет — это net-new оверклейм этой же ветки); `VERSION=v0.2.27` в dev-quickstart отставал на 17 релизов → заменён командой, которая резолвит последний релиз (P2: удаляем цифру, оставляем команду); `docs/adr/README.md` рендерил `# docs/adr/` как заголовок вкладки и открывался контрибьюторским процессом → «Architecture Decision Records» + `description` + индекс под лидом; контрибьюторская инструкция «не редактируй `docs/reference/cli/` руками» уехала с пользовательской референс-страницы в `docs/contributing/documentation.md`; секционные индексы используют заголовки страниц вместо голых имён файлов; `contributing/README.md` стал списком ссылок вместо таблицы неликабельных code-span'ов; замер в докстринге `cli/docsgen/src/health.rs` перевыведен (точнее — цифры удалены, свойство оставлено), там же исправлена ссылка на страницу, с которой уехал `cli/docsgen/src/render.rs`.
  **Старые пути остаются названными в записях закрытых подфаз** — §1.15 и §1.82 (`gitops-walk.md`), §1.83e (`docs/public-ingress/cloudflare-origin-cert.md`), §1.83g (`docs/public-ingress/connect-a-domain.md`) и запись самой §2.19c (`needs-pg-walk.md` / `needs-redis-walk.md`): это исторические записи о том, что было поставлено ТОГДА, и они не переписываются (то же правило, по которому не переименовывается зашипленное). Актуальные пути перечислены здесь; переименования полностью — в `git log --follow` и в записи `plan-history.md`.
- [x] **(g) Публикация** *(= W6)*. Ветка `feat/2.19g-publication`, план `docs/superpowers/plans/2026-08-19-docs-2.19g-publication.md`; ADR 0057 получил amendment по факту поставки, решение 8 перестало быть «decided, not yet implemented». **CLI-патч-релиз; ни чарта, ни оператора, ни cue-cmp — и ответ ПЕРЕВЕРНУЛСЯ по ходу ревью, что записано в ту сторону, в которую он двигался.** Первая редакция утверждала «ни релиза, ни тега» и была верна для дерева без правок в `cli/`; ревью нашло, что шипнутый манифест не читается парсером самого CLI, и фикс лёг в `cli-core`. Перевывести: `git diff --name-only <base>..HEAD -- cli/ operator/ schemas/ platform-stack/ argocd-cue-cmp/` — всё, кроме `cli/`, пусто; `cli/` не пусто (`ApplicationExpose.port` стал `Option<u16>`), поведение `apprafter app add` меняется, поэтому `cli/Cargo.toml` бампнут 0.2.44 → 0.2.45 в том же коммите, а ветка несёт монорепо-патч-тег. `docs/reference/cli/commands.json` сдвинулся ровно на строку `cli_version` — это проекция бампа, а не движение поверхности команд; регенерирован тем же коммитом. Новый образ `ghcr.io/apprafter/docs` версионируется SHA коммита, а не git-тегом, так что публикация не заводит и собственного тег-стрима.

  **Софт готов, сайт НЕ отдаётся.** Это главное, что должна сказать эта запись: между веткой и читателем стоят пять ручных шагов, которых репозиторий сделать не может (пуш; проверка зоны; DNS-запись в Cloudflare; однократная регистрация приложения; `docsUrl` в CMS + придержанный лендинг-коммит). Раннбук `docs/operator-guide/publish-the-docs-site.md` написан так, чтобы средние три делались по нему без плана и без ADR.

  **1. Сборка живёт в workflow, а не в Dockerfile — решение с замером.** `docs-site/Dockerfile` — одна стадия над `caddy:2-alpine`, копирующая ГОТОВЫЙ `site/`; build-стадии в нём нет, и причина записана в самом файле, потому что это ровно то «улучшение», которое сделает следующий контрибьютор. `pip install mkdocs-material` внутри образа завёл бы вторую, незапиненную тулчейн-цепочку, которая производит опубликованный сайт, пока гейт валидирует собранный ИНАЧЕ; решение 2 ADR'а ввело единое `python3.withPackages` ровно против этого, и дрифт был бы невидим — обе сборки зелёные, различаются только опубликованные байты. Поэтому `release-docs.yml` гоняет `mkdocs build --strict` под `nix develop` и отдаёт результат в `docker/build-push-action`.

  **2. Релизный workflow гейтит СЕБЯ, а не полагается на `docs.yml`.** Workflow'ы, поднятые одним событием, независимы: `needs:` между ними нет, вывод чужой джобы никто не читает, так что пуш, роняющий гейт, доводит этот workflow до конца и публикует. Пути тоже расходятся именно в публикующую сторону — `docs-site/**` роняет релиз и в фильтре `docs.yml` не встречается вовсе. И `--strict` заменой не является: он ловит мёртвые ссылки, якоря, страницы вне nav и недостающие include'ы, но не ловит устаревший сгенерированный CLI-референс, утверждение, переставшее резолвиться против продукта, битый `llms.txt`/двойник и ссылку в `exclude_docs`'нутую страницу (mkdocs репортит её на INFO — ниже досягаемости `--strict`, и `scripts/docs-check.sh` грепает её отдельно). Поэтому релизная джоба гоняет `scripts/docs-check.sh` сама, делит rust-cache-ключ с `docs.yml` и чекаутится на `fetch-depth: 0` — `docsgen gate` старит исключения против реальных дат тегов.

  **3. Path-фильтр срабатывает на изменение ОПУБЛИКОВАННЫХ байтов, и это НЕ копия `exclude_docs`.** Список исключений перечитывается командой `sed -n '/^exclude_docs:/,/^\$/p' mkdocs.yml` и раскладывается натрое: внутренний changelog и `measurements/` — в негации (это и есть разряженный пункт Re-evaluation: они трогают `docs/` почти каждым коммитом и не меняют ничего, что видит читатель); `superpowers/` нет ни в одном списке, потому что дерево в gitignore и в диффе пуша появиться не может (паттерн под недостижимый случай — мёртвый конфиг); а `hooks/` и `reference/cli/SUMMARY.md` из сайта исключены, но релиз ронять ОБЯЗАНЫ — хуки на сборке ИСПОЛНЯЮТСЯ и формируют каждую страницу, а `SUMMARY.md` и есть nav всех опубликованных страниц. Фильтр, транслитерированный из `exclude_docs`, сделал бы правку хука и правку nav непубликуемыми молча. `mkdocs.yml` и `flake.nix`/`flake.lock` ронят релиз (бамп темы переписывает HTML каждой страницы и меняет отпечатки ассетов, пока в `docs/` не двигается ничего); `docs-site/**` ронит (Caddyfile запечён в образ — иначе правка кэш-заголовка доехала бы до прода лишь тогда, когда кто-то следующий раз тронет страницу), а `docs-site/apprafter/**` вырезан обратно (манифест Argo CD читает из конфиг-источника кластера; он не вход образа, и его правка не должна чеканить новый digest). Продуктовый исходник отсутствует намеренно: он меняет то, что гейт ПРОВЕРЯЕТ, а не то, что сайт СОДЕРЖИТ, а обязательная следом регенерация трогает `docs/` и релиз роняет. **Связка `exclude_docs` ↔ фильтр названа и НЕ обеспечена гейтом:** удаление записи из `exclude_docs` публикует страницу, правки которой больше не режут релиз, и это не проверяет ничто.

  **4. Кэш-правила выведены из того, что mkdocs РЕАЛЬНО кладёт.** Отпечатками помечены ровно бандл, search-воркер и две таблицы стилей (`<name>.<8-hex>.min.<css|js>` плюс `.map` к каждому), и это **не весь** `assets/`: фавиконка и языковые стеммеры lunr отпечатка не несут — перепроверяется `find site/assets -type f | grep -vE '\.[0-9a-f]{8}\.min\.'`. Поэтому лендинговое сплошное `header /_astro/* … immutable` не переносится, хотя ТАМ оно верно (Astro хеширует каждое имя, которое отдаёт): копирование запинило бы мутабельные URL'ы на год, и апгрейд темы, поменявший стеммер, не доехал бы до читателя, у которого закеширован старый. Вместо этого три непересекающихся матчера: `path_regexp` на форму отпечатка → immutable; два неотпечатанных поддерева → сутки; всё остальное → 300 с. Якорь длины `{8}` несущий, а не декоративный: под свободное `[0-9a-f]+` подпадают `lunr.da.min.js` и `lunr.de.min.js` (`d`, `a`, `e` — hex-цифры). Всё под `assets/`, не попавшее ни в одно правило, намеренно проваливается БЕЗ `Cache-Control`: недокэшировать — поправимо, запинить мутабельный URL на год — нет. HTML держит 300 с, а не лендинговый час, потому что `search/search_index.json` отпечатка не несёт и не должен переживать страницы, которые индексирует. `.md` принудительно отдаётся как `text/plain`: базовый образ через `mailcap` подсовывает `text/markdown`, который браузер предлагает СКАЧАТЬ, обессмысливая двойники, — вход, который этот репозиторий не пинит и не тестирует, поэтому заголовок ставится явно.

  **5. Новому поддомену не нужна ни новая зона, ни новый сертификат — перевыведено, а не процитировано.** `platform-stack/cue/render_tool.cue` на каждую запись `gateway.allowedDomains` печатает ДВА HTTPS-листенера, `https-apex-<san>` и `https-wild-<san>`, и оба называют один и тот же секрет `importedCertRef`: одна регистрация, один сертификат, оба хоста. `render_httproute` оператора ставит `parentRefs[0]` на Gateway `platform` порт 443 БЕЗ `sectionName`, так что маршрут цепляется по совпадению hostname и никому не надо сообщать, что wildcard-листенер существует. Предел стоит знать: правило покрытия — РОВНО одна метка в обоих слоях (`hostname_covered_by_zone` принимает апекс или префикс без точки, `hostname_matches_domain` в CLI согласен), так что `docs.apprafter.dev` покрыт зоной `apprafter.dev`, а вложенный хост — уже нет. Поддомен на один уровень бесплатен, глубже — новая зона.

  **6. Лендинг-переключатель — ШАГ ОПЕРАТОРА, а НЕ придержанный коммит.** Сначала он был сделан отдельным коммитом — по посылке «коммит можно не пушить». Посылка неверна: `.github/workflows/landing-autotag.yml` срабатывает на ЛЮБОМ пуше в дефолтную ветку, чей диф трогает `landing/**`, бампает патч свежайшего `landing-v*`, пушит тег и диспатчит `release-landing.yml`, который публикует `ghcr.io/apprafter/landing-web` и `:<tag>`, и `:latest` — а манифест лендинга смотрит именно на `:latest`, который оператор перерезолвит в дайджест на каждом reconcile. То есть смёржить такой коммит = задеплоить его, без отдельного одобрения; плюс он лежал НЕ на конце ветки, так что «выкинуть» его значило бы ребейзить закрывающий коммит поверх. Коммит удалён с ветки (`git rebase --onto`), `docsUrl` вернулся в `""`, а порядок (сначала сайт отвечает — потом ссылка) записан шагом 5 раннбука. Содержательная часть, которая от этого не изменилась: `landing/web/src/data/fallback/siteSettings.json` — это РЕЛИЗНЫЙ путь (образ собирается с `LANDING_USE_FALLBACK=1`), а глобал Payload — превью-путь, значит ставить надо ОБА; `Header.astro` подставляет `<githubUrl>#readme` и вешает бейдж «Soon», только пока `docsUrl` пуст; значение байт-идентично `site_url` из `mkdocs.yml`, а его хост равен `expose.hostname` в `docs-site/apprafter/Application.cue`.

  **Находки, о которых сообщено и которые НЕ правились здесь:** (а) лендинговый `try_files … /404.html` переписывает неизвестный URL в существующий файл, так что `file_server` не на чем упасть и **любой** неизвестный URL отвечает **200** с телом 404-страницы (мягкий 404 — краулеры индексируют его как настоящую страницу) — перемерено на реальном `landing/web/Caddyfile` поверх реально собранного `dist`; docs-Caddyfile fallback не несёт и отдаёт настоящий 404. **Почему не правится здесь — тот же механизм, что и в пункте 6:** любой коммит, трогающий `landing/**` и доехавший до дефолтной ветки, автотегает и публикует лендинг, так что однострочный фикс мимоходом в докерской подфазе означал бы незапланированный прод-деплой ДРУГОГО сайта. Это работа того, кто следующим осознанно меняет лендинг — записано прямо в открытом пункте «Migration лендинг-манифеста» (§1.83b, PART 0.6), чтобы находка ждала там, куда этот человек придёт. (б) `scripts/lint-cue.sh` вет'ит `schemas/`, `platform-stack/cue/` и `examples/` — и НЕ трогает `docs-site/` и `landing/`, то есть оба деплоимых манифеста не валидирует никто; это не гипотеза: `cue vet ./landing/web/apprafter` проходит, а `cue fmt --check` на той же директории падает, называя оба файла, — так выглядит непокрытый путь через год. (в) SPDX-проверка не достаёт ни до `docs-site/**`, ни до `landing/**` (в `PATTERNS` нет ни глоба под эти деревья, ни общего `**/*.cue`), так что заголовки на новых файлах верные и необеспеченные. (г) `scripts/docs-artefacts-check.py` падает трейсбеком на НОВОЙ странице, которая написана, но ещё не застейджена (`source` строится из `git ls-files`, `published` — из сборки), причём собственный страж `orphan_pages` под этот же случай стоит НИЖЕ строки, которая первой разыменовывает недостающий ключ.
- [x] **(h) CLI UX** *(= W7)*. Ветка `feat/2.19h-cli-ux`, план `docs/superpowers/plans/2026-08-19-docs-2.19h-cli-ux.md`; ADR 0057 получил amendment по факту поставки. **CLI-патч-релиз `v0.2.46`; ни чарта, ни оператора, ни cue-cmp** — `git diff --name-only <base>..HEAD -- cli/ operator/ schemas/ platform-stack/ argocd-cue-cmp/` пуст везде, кроме `cli/`, а шипнутый бинарь меняется дважды: новая команда `completion` и секция `Examples:` в `--help` у 75 команд. `cli/Cargo.toml` бампнут 0.2.45 → 0.2.46 тем же коммитом, `commands.json` сдвинулся ровно на строку `cli_version` (регенерация тем же коммитом).

  **Дефект, с которого начался весь трек, был закрыт ДО этой подфазы — и это первое, что должна сказать запись.** Исходный запрос называл `apprafter secret seal` командой, у которой не описан `--namespace`. Он описан: `apprafter secret seal --help` печатает `-n, --namespace <NAMESPACE>` вместе с дефолтом, а doc-комментарий несёт ещё и последствие («the sealed blob only unseals as `<namespace>/<name>`» — ровно то, что кусается, см. память про namespace-футган). По всему дереву — **187 аргументов, 0 без описания, 0 команд без `about`**, и это перемерено на закрытии тем, что печатает САМ clap (свип по `-h` всех 97 help-страниц, скрипт целиком — в amendment'е ADR), а не проекцией `commands.json`; два счётчика сошлись с разных сторон. Закрыли это ранние walk-фиксы, а не трек. **Вклад этой подфазы — вторая половина запроса: примеры, и машинерия, которая не даёт сгнить обеим половинам.** У свипа есть тонкость, стоящая записи: clap печатает описание в ДВУХ раскладках (в той же строке или с отступом на следующей, когда help длинный), и парсер, знающий только первую, репортит на этом дереве пять ложных пустых — первая редакция свипа так и делала.

  **1. Страж — ПЕРВЫМ, примеры — вторым, потому что примеры жили в слепом пятне между двумя гейтами.** `docsgen check` байт-сверяет `docs/reference/cli/**` со свежим рендером: он доказывает, что страницы соответствуют исходнику, но НЕ что исходник верен — пример с опечаткой во флаге рендерится одинаково оба раза и проходит. `docsgen gate` это дерево осознанно исключает (им владеет `check`), так что чек, резолвящий `apprafter`-вызовы, ни одного примера не читает. Каждый гейт выводит ожидание из той же стороны, которую проверяет. 124 примера, написанные в эту щель, были бы 124 непроверенными утверждениями в артефакте, весь смысл которого — быть правдой. Страж (assertion C в `cli/docsgen/src/examples.rs`) показан падающим на заведомо неверных примерах ДО того, как написан первый настоящий, причём демонстрация шла с `docsgen generate` ПЕРВЫМ — байт-сверка была чистой, пока `docs/reference/cli/secret.md` публиковал `apprafter secret seal db-url --namesapce web`. Это и есть слепое пятно, показанное, а не описанное. **Попутная корректировка цитаты:** план этой подфазы ссылался на `scan::EXCLUDED`; там лежит `["docs/adr/", "docs/changelog/", "docs/measurements/"]` и CLI-референса нет — исключение сделано отдельным правилом по префиксу `render::DIR` внутри `scan::is_in_scope`. Вывод не изменился, цитата была неверна.

  **2. Примеры — структурная таблица, а не разделитель в `after_help`.** `pub const EXAMPLES: &[CommandExamples]` в `cli/platform-cli/src/examples.rs`, реэкспорт через фасад `docs_api`; бинарь рендерит её в `after_help`, `docsgen` читает структурно. Решающая причина не в том, что парсить прозу некрасиво: **парс, который ничего не нашёл, ПРОХОДИТ.** Поменяй разделитель, оберни блок, покрась его или напиши пример, содержащий разделитель, — и страж молча судит ноль записей, продолжая печатать OK; та же форма вакуумности, что и у чека, выводящего ожидание из проверяемой стороны. Отсюда же следствие: запись, которую страж прочитать не смог, — **находка, а не пропуск**. Пять классов: неразрешимый путь, неразрешимый флаг, запись без единого `apprafter`-вызова, пустая запись и пример, подшитый не к той команде (`apprafter app list` под `backup enable` — настоящий вызов и ложная страница; копирование соседа — самый вероятный способ такое написать). Алиасы резолвятся в канонический путь.

  **3. Покрытие: 124 строки примеров на ВСЕХ 75 видимых листьях.** Корень, 18 групповых узлов и 3 скрытых листа `auth` не несут ни одного — по правилу. Второй редакторский проход удалил 32 комментария и одну запись целиком: они пересказывали help флага, напечатанный шестью строками выше. Правило, которое выжило и записано на самой константе: комментарий остаётся только там, где делает читаемой голую или неоднозначную строку (`apprafter app add` без аргумента, `apprafter volume list` без `--namespace`) либо называет сценарий для комбинации флагов. Формы значений сверены с кодом, который их парсит, а не с памятью: `up-only`/`off` (`platform.rs:881`), `internet|internal|strict` (`platform.rs:673`), `pg|redis|disk` (`app_scaffold.rs:353`), `monolithic|sequential`, `solo|team|prod|regulated` (`cli-core/src/tier.rs`), а два примера `backup enable` — это РОВНО две легальные формы `construct_repo_url` (`backup.rs:224`), взаимоисключающие, поэтому ни один не смешивает `--endpoint` со схемным bucket'ом. SKU (`cx22`/`cx32`) взяты из собственного help'а CLI, а не выдуманы; `cpx*` из юнит-фикстур `apply.rs` НЕ подняты — это фикстуры строкового парсинга, а не утверждение о каталоге Hetzner.

  **4. Completions берут `clap_complete::Shell` как ТИП, а не локальный enum поверх него.** Локальный enum был бы вторым, руками поддерживаемым утверждением о множестве, которым владеет чужой крейт, — а `Shell` помечен `#[non_exhaustive]`, так что этот второй список молча замёрз бы на сегодняшних пяти. Все пять шеллов отдают скрипт, и каждый АССЕРТИТСЯ в `cargo test` через шипнутый бинарь (итерация по `Shell::value_variants()`, не по списку в тесте). Рецепты установки покрывают **bash, zsh, fish** — рецепт под конвенции, по которым никто здесь не ходил, был бы выдуман, а не написан; страница называет эти три и отсылает к `apprafter completion --help` за полным списком, чтобы документированное множество не разошлось с аргументом. Глагол `completion` в единственном числе — как у `kubectl`/`helm`/`argocd`/`k9s`; алиаса нет (did-you-mean clap'а сам отвечает на `completions`). Рецепты пройдены руками под изолированным `HOME` (bash 5.3.9 — `_apprafter` прогнан аргументами, которые bash даёт `-F`-функции; zsh 5.9 — `fpath`+`compinit`; fish 4.6.0), и прогулка нашла два апстримных поведения, записанных, а не обойдённых: **(а)** `#[command(hide = true)]` убирает `auth` из `--help`, но НЕ из completion-скриптов — `clap_complete` фильтрует только скрытые possible-values; не чинится, потому что clap не даёт удалить сабкоманду из собранного дерева, а пересобранный руками `Command` молча теряет то, что пересборка забыла; **(б)** значения ПОЗИЦИОНАЛА дополняют только bash и zsh — `apprafter completion <TAB>` под fish/elvish/PowerShell падает в дополнение по файлам.

  **5. Страж от гниения help'а пинит СВОЙСТВА, и один закоммиченный список — намеренно.** Полный снапшот каждой help-страницы падает на любом улучшении формулировки и регенерируется не глядя — хуже, чем никакого теста. Планка: **ревьюер диффа не заметит, что свойство сломано, а ломается при этом что-то вне диффа.** Прошли семь: у каждого видимого листа есть пример; у скрытой команды примера НЕТ (иначе `auth` возвращается и в `--help`, и в референс — отмена решения с дальней стороны); ни один пример не пуст и ни одна строка таблицы не обещает строк, которых у неё нет; у каждой команды есть `about`; у каждого флага и позиционала есть короткий help; алиасы, которые ПЕЧАТАЮТ гайды, доезжают до своей команды; и гайды всё ещё печатают каждый алиас, который таблица заявляет. Шесть из семи выводятся из clap-дерева на момент теста. **Седьмой — закоммиченный список, и это дизайн, а не срез угла:** вывести «какими алиасами пользуются гайды» из clap-дерева — ровно циклично (удали алиас `t`, и список из дерева просто перестанет его содержать, то есть чек пройдёт на несуществующем алиасе). Список — другая сторона, а седьмое свойство держит его честным против корпуса. Он ВЫВЕДЕН, а не угадан: alias-aware индекс из `commands.json` + обход всех `apprafter …`-токенов в `docs/**` даёт семь вызовов на пяти страницах и семь токенов — `t`, `ls`, `info`, `rm`, `kc`, `cb`, `up` (план называл пять — `t`, `kc`, `up`, `ls`, `rm`; два, `cb` и `info`, план не называл, и `apprafter volume rm` правильно НЕ попал: там `rm` — каноническое имя). Непокрытое названо: гайд, который НАЧНЁТ печатать новый алиас, в список сам не добавится — это ловит `docsgen gate`, резолвящий все вызовы в скоупе, включая алиасы; перевыводить множество внутри теста значило бы завести второй токенайзер вызовов, а этот трек уже видел, как два токенайзера расходятся.

  **6. ПЯТЬ пунктов исходного списка (h) НЕ построены, и ни один не снимается замером — каждый пропробован и является реальной дырой.** Список (h) называл восемь вещей; поставлены три (`EXAMPLES` + `after_help`, shell-completions, help-coverage-тест) плюс страж, которого список не называл. Не поставлены: **`secret list`** (`apprafter secret --help` показывает `seal`, `remove`, `help`); **namespace-мастер для `secret seal`** (в `commands/secret.rs` пикера нет, `--namespace` дефолтится в `apprafter-system` — тот самый футган); **`status.conditions[]` с remediation в `app status`** (`commands/app.rs` читает `/status/phase` и `Scheduled` у клейма, но `conditions[]` самой Application CR не читает НИКОГДА, хотя `operator-core/src/application.rs` их эмитит); **`requires = "select"` на `--namespace`** (атрибута `requires` в `cli/platform-cli/src/cli.rs` нет вообще, `export` и `backup create` до сих пор пишут «Ignored unless `--select` is also passed» прозой); **`app open --yes`** (`commands/app_open.rs` спрашивает `Continue anyway?` на нездоровом приложении и в неинтерактивном шелле ЖЁСТКО отказывает — выхода для CI нет). У первых двух есть триггер внутри трека: гайд по sealed-secrets в 2.19i не сможет рассказать, как посмотреть, что засилено и в каком namespace, не имея `secret list` и мастера; гайд по `app status` не покажет remediation, которую команда не печатает. Оставшиеся два — изменения СЕМАНТИКИ команд, требующие собственного решения, а не докерской подфазы: `requires = "select"` превращает молча игнорируемый флаг в ошибку парсинга у `export` и `backup create` (ломает любой скрипт, передающий `--namespace` в одиночку), а `app open --yes` меняет то, что позволено неинтерактивному шеллу.

  **Находка, о которой сообщено повторно и которая НЕ правилась:** `scripts/docs-artefacts-check.py` снова упал трейсбеком (`KeyError: 'reference/cli/completion/'`) на НОВОЙ странице, написанной, но ещё не застейдженной, — тот же дефект, что записан находкой (г) в 2.19g. Стейджинг лечит; репортится второй раз, по-прежнему не починено.
- [x] **(i) Контент** *(= W8)*. Ветка `feat/2.19i-missing-guides`, план `docs/superpowers/plans/2026-08-20-docs-2.19i-the-missing-guides.md`; ADR 0057 получил amendment по факту поставки. **CLI-патч-релиз `v0.2.47`** — не ради гайдов, а ради одного `about` (см. ниже): шипнутый бинарь меняется, `cli/Cargo.toml` бампнут 0.2.46 → 0.2.47 тем же коммитом, `commands.json` сдвинулся ровно на строку `cli_version`. Ни чарта, ни оператора, ни cue-cmp.

  **«~15 гайдов» — оценка, которую семь подфаз никто не перемерял, и она НЕВЕРНА: гайдов ПЯТЬ.** Перемерено по дереву команд, а не по оценке (команда — в amendment'е ADR 0057). **Сама команда из плана подфазы тоже была дефектной и давала ТРИ разных ответа на один вопрос:** корпус ключевался по `p.name`, поэтому страницы с одинаковым базовым именем затирали друг друга (`docs/index.md` / `dev-guide/index.md` / `operator-guide/index.md` схлопываются в одну запись, как и два `quickstart.md`), а какая уцелеет — порядок вставки в dict. План намерил **12** на `0a2ba48`, повторный прогон его же команды на том же дереве дал **11**, по полному пути — стабильные **10**. Двое лишних у плана — `apprafter target show` и `apprafter upgrade-tier`, оба уже упомянуты на страницах, проигравших коллизию базового имени. Из стабильных 10 трое — осознанные отсутствия (ниже), так что реальной работы было **семь неописанных листьев**, и они ложатся на пять поверхностей: `dev-guide/resources-and-autoscaling.md` (`platform autoscale set|show`), `dev-guide/secrets.md` (`secret remove`), `dev-guide/environments.md` (`platform env set|show`), `operator-guide/choosing-the-machine.md` (поднимает capability с одного упоминания до страницы) и — двумя расширениями, а не новой страницей — `operator-guide/target-store.md` (`target show|rename|remove`) + `dev-guide/image-iteration.md` (`app rollback`).

  **ТРИ команды остаются недокументированными ОСОЗНАННО, и это находка, а не дыра:** `apprafter login` и `apprafter upgrade-tier` — обе «NOT IMPLEMENTED» в собственном `about`, `apprafter plan` — скелет над `DryRunProvider`. Гайд на любую из них описывал бы будущее. Они СНОВА всплывут в любом будущем прогоне инвентарной команды — не писать. `upgrade-tier` при этом из вывода команды выпал, потому что теперь НАЗВАН отсутствующим в `docs/operator-guide/index.md` (там же исправлена формулировка «`apprafter upgrade-tier` exists и safety-семантика идёт через `MigrationPlan`», читавшаяся как «capability есть, нет только гайда»): чек — тест на УПОМИНАНИЕ, а не на обучение, и ровно здесь эта разница кусается.

  **Один дефект, который инвентаризация вскрыла и который починен, а не задокументирован:** `about` у `apprafter plan` обещал «Show the diff between the desired state and what is live», тогда как `commands/plan.rs` строит `DryRunProvider` и провайдера не читает вовсе. Переписан caveat-first (генерируемый референс поднимает первое предложение в overview-таблицу и в `description` страницы, так что оговорка во втором предложении невидима ровно там, где читатель решает); указывает на `apprafter apply` как на место, где сравнение реально живёт, и на `target show` / `platform status` / `app status` как на read-only-виды. `apprafter status` НАМЕРЕННО не назван — его собственный `about` говорит, что он скелет и кластера не трогает.

  **Четыре расхождения между гайдами, написанными параллельно, найдены и сведены на этапе разводки:** (1) `--env` на неоднозначном приложении — таблица в `environments.md` говорила «asks», её же проза двумя секциями ниже — «errors»; `single_deployment_or_guidance` возвращает `Err`, таблица исправлена; (2) `metadata.namespace`, запиненный в мультиэнвовом примере, — и `application-cue.md`, и `resources-and-autoscaling.md` демонстрировали ровно ту ловушку, о которой предупреждает новая страница (запиненный namespace рендерит один и тот же объект на каждый env, CMP штампует `spec.environment` и лейбл, но CR НЕ переименовывает), у обоих убран; (3) пять правил per-field merge были выписаны дважды почти дословно — канон оставлен в `application-cue.md` (это семантика поля, а страница — филд-референс), `environments.md` сжат до одного предложения + ссылка; (4) `examples/applications/parser.cue` описан в индексе как «worked multi-environment manifest», хотя пинит `metadata.namespace: "demo"`, а его `dev`-оверрайд ставит `expose.network: "vpn"`, который вебхук отвергает — указатель убран (сам файл — `cue vet`-фикстура, не тронут).

  **Оба индекса секций перестроены в карты:** `dev-guide/index.md` был плоским списком из шести пунктов, девять бы никто не читал — сгруппирован по задаче тем же приёмом, каким 2.19f перестроила индекс operator-guide.

  **Census вырос по каждой оси и не упал ни по одной** (`0a2ba48` → закрытие): 33 → 37 страниц, 427 → 566 инвокаций, 248 → 322 идентификаторов, 2 → 5 полных CUE-документов в 17 → 22 `cue`-fence'ах, 104 → 104 путей репозитория, 70 → 76 цитат ADR; LLM-артефакты 60 → 64 / 119 → 123 / 119 → 123. **Exemptions остались 2** — пяти новым страницам не понадобилось ни одного эскейпа. `code_paths` не сдвинулся ВООБЩЕ — это правило «читатель без чекаута» в виде числа: где нужен файл репозитория, страницы дают абсолютный GitHub-URL. Коммитнутый пол при этом успел отстать от живого замера (`32/398/246/2/17/91/68/2`, писался до 2.19h) — он ратчетится только на падение, — поэтому перезаписан `docsgen metrics` тем же коммитом: без этого новые страницы можно было выпотрошить бесплатно.

  **Записано, но НЕ починено (каждое — с пробой, не списано):** help у `ServerTypeNotSelected` называет несуществующее поле манифеста `nodes[0].kind` (ключ — `type`; `kind` — только имя Rust-поля под `serde(rename)`) — правка в `cli/cli-core/src/error.rs` сдвинула бы `commands.json` второй раз в подфазе, чья правка CLI должна быть одним `about`; `cli/platform-cli/src/examples.rs` в пяти местах использует `cx22`/`cx32`, которые собственный changelog репозитория объявляет снятыми, и без токена провайдера вопрос не решается — поэтому `choosing-the-machine.md` не называет НИ ОДНОГО типа машины и учит пикеру как списку; `docs/operator-guide/troubleshooting.md` под `server_type_unavailable` советует передать `--server-type` «вместе с `--region`» на команде провижининга, где `--region` не существует (гейт это пропускает, потому что два флага нигде не написаны в одном вызове — живой пример «гейт проверяет имена, а не истину»).

  **Триггер двух из пяти непостроенных пунктов 2.19h сработал не так, как та запись предсказывала:** она ждала, что гайд по sealed-secrets не сможет рассказать, что засилено и в каком namespace, без `secret list` и мастера. Гайд отвечает на оба вопроса — через `kubectl get sealedsecrets --all-namespaces` и листинг ключей, — что честно, но это `kubectl`-ответ на странице, весь предмет которой — first-class CLI-задача. `secret list` и мастер по-прежнему не построены, и гайд теперь не блокер, а доказательство спроса.
- [x] **(j) Закрытие** *(= W9)*. Прогулка по КАЖДОМУ гайду релизным бинарём 0.2.47: четыре ходока по непересекающимся наборам страниц, пятый читает собранный сайт как посторонний, шестой сводит и **перепроверяет каждую блокирующую находку сам**. Живой кластер не трогали (решение владельца): мутирующие шаги проверялись чтением кода, который их реализует. **35 находок, 30 блокирующих, 23 из них гейт поймать СТРУКТУРНО не мог** — девять подфаз зелёных гейтов о них не говорили, потому что гейт резолвит имена и никогда не проверяет истину. **Две инструкции вели к потере данных:** `apprafter destroy --target <старый>` (область — весь проект токена по метке `apprafter=true`, имя кластера не читается никогда, то есть снесло бы и новый кластер) и смена окружения через `app remove` без предупреждения, что `ResourceClaim` уходит вместе с развёртыванием, база живёт 7 дней в `RetainedClaim` и потом удаляется, а новое окружение получает пустую. **Одна страница на 298 строк** (`connect-a-git-repository.md`) описывала возможность, удалённую в v0.1.97, и проходила гейт девять подфаз, потому что вестигиальное `spec.argocd.bootstrapRepo` держало все идентификаторы резолвящимися — страница отозвана, поля схемы оставлены владельцу решением (`docs/measurements/schema-followups.md`). Прочее того же класса: обещание снапшотов redis, которых `export` не делает (обе ветки только печатают «deferred… skipping»); `--check-cron off` как трижды предложенный выход, отвергаемый apiserver'ом; недосказанный радиус `platform rescue` (переустанавливает Cilium, все Application теряют Sync/Healthy); четыре уровня риска миграций из пяти с пропуском САМОГО высокого. **Найден продуктовый дефект, не документационный:** контроллеры VPA в CrashLoopBackOff с момента появления компонента (2.16e) — чарт передаёт `--feature-gates=InPlaceOrRecreate=true` образу 1.7.1, который флаг не принимает; рекомендации считаются, не применяется ничего. Чарт НЕ трогали: снятие флага запустило бы вытеснение подов на живом кластере, это решение владельца. **Один найденный «дефект» отклонён после проверки** — `choosing-the-machine.md` цитирует диагностику дословно и тут же исправляет `kind` на `type` в прозе, что и есть верное обращение. **Тесты были КРАСНЫМИ на master** от собственного фикса прогулки (`bae5559` переписал `app rm` на `app remove`, а сторож пиннит литерал) — поймал сторож, не гейт: `just lint` тестов не запускает, и это записанный пробел композиции. Несёт CLI-патч `v0.2.48`: шаблон scaffold писал в КАЖДЫЙ пользовательский манифест, что схемы вендорятся под `apprafter/cue.mod/pkg/` и `cue vet` сработает — не вендорятся с ADR 0046, команда падает; плюс справка `app add --path` описывала несуществующее правило `discover.find.glob`.

**Acceptance (полный, по §11 спека):** `just lint` падает на каждом из — дрифтнувший CLI-референс, непомеченный fence, негейченный inline-вызов, невалидный CUE-сниппет, неразрешимый `apprafter`-вызов, таблица полей вразрез со схемой, ложное `claims:`, запрещённая фраза, мёртвая ссылка/якорь, отсутствующий frontmatter/`verified-by`, регрессия ратчета. `docs.apprafter.dev` отдаёт сайт на финальных URL'ах, лендинг ведёт на него без «Soon», ни одна внутренняя страница не достижима. `llms.txt`/`llms-full.txt` — абсолютные URL, строка лицензии, описание на каждую страницу. У каждой capability из §5 есть гайд, у каждого гайда — типизированный `verified-by`. У каждой команды есть `about` и ≥1 пример. Закрывающая ручная прогулка зелёная.

**Acceptance закрытых подфаз:** *(a)* `just docs-check` и `just docs-build` зелёные в чистом checkout'е через `nix develop`; ни одной страницы вне nav без записи в allow-list; внутренние страницы не в `site/`. *(b)* `docsgen check` роняет `just lint` на любой правке clap-дерева без `just docsgen-generate`; `commands.json` покрывает все 95 путей; `reference/cli.md` удалён вместе с четырьмя входящими ссылками. *(c)* `cli/docsgen -- gate` роняет `just lint` на дрифтнувшем `apprafter`-вызове, дрифтнувшем schema-идентификаторе или невалидном полном CUE-документе в скоупе; 20 фикстур с негативными контролями пинят каждый исторически найденный дрифт; гейт печатает census-строку (страниц / инвокаций / идентификаторов / opaque-доли / exemptions) на каждый прогон; реальный корпус зелёный (`docsgen gate OK`). *(d)* расфенсенный блок (отступ или `pre`) несёт те же обязательства, что и fence, а незакрытый `pre` — своя находка; несуществующий путь репозитория в спане или в относительной ссылке роняет гейт, глоб — нет; цитата снятого ADR роняет гейт, а `Draft`/`Proposed` и частичное вытеснение — нет; `adr-check-ignore` глушит цитату по каноническому ключу и стареет как остальные исключения; удаление документированной поверхности роняет гейт против коммитнутого `docs/measurements/docs-health.json`, рост проходит молча, движение `exemptions` в ЛЮБУЮ сторону — находка, а отсутствующий/битый census — exit 2, не exit 1; реальный корпус зелёный, и все три новых чека находят на нём ноль. *(e)* сломанная регистрация CUE-лексера роняет `mkdocs build` с внятным сообщением, а не отдаёт зелёную сборку с нехайлайченными fence'ами (пронаблюдено падающим в обе стороны: и «не зарегистрировался», и «зарегистрировался, но токенизирует в одно»); страница без `description` роняет сборку с перечислением страниц; отключённый LLM-хук роняет `scripts/docs-check.sh` с именем недостающего артефакта; каждый URL в `llms.txt` абсолютен и резолвится и в страницу сайта, и в её markdown-двойник по правилу, которое сам `llms.txt` и объявляет; census не уменьшился ни по одному счётчику. *(f)* строгая сборка падает на любой входящей ссылке и на любом заголовочном якоре, сломанном переименованием ВНУТРИ `docs/` (именно она, а не census, — сеть безопасности: счётчики census'а тоталы и переименование от удаления не отличают), а ВНЕ `docs/` — `e2e/`, `cli/`, `operator/`, `platform-stack/`, `scripts/`, `landing/`, `.github/` — единственный инструмент это греп, и он прогоняется тем же коммитом; ни одна страница сайта в скоупе гейта не называет фазу, код подфазы, милестоун, трек или `plan.md` по РАСШИРЕННОМУ паттерну (не по тому, которым свип проверял сам себя), а оставшиеся упоминания `spec.md` — абсолютные URL, объявляющие себя роадмапом в самом предложении; `llms.txt`-счётчики не упали ни от одного из семи переименований; карта редиректов ОТСУТСТВУЕТ намеренно, с записанным замером и названным триггером (первая страница, переехавшая после 2.19g). *(g)* образ, собранный из ГОТОВОГО `site/` (build-стадии в Dockerfile нет), отдаёт четыре пробы плана — `200 text/html` на корне, `200 text/plain` на `llms.txt`, `200` на странице гайда и настоящий **404** на несуществующем URL (перемерено на закрытии, на реально собранном образе, вместе с новой страницей раннбука и её markdown-двойником); отпечатанные ассеты отдаются `immutable`, неотпечатанные под `assets/` — сутки, всё прочее — 300 с, и ни один неотпечатанный URL под `assets/` не запинен на год; `.md` отдаётся как `text/plain`, а не предлагается к скачиванию; релизный workflow гоняет `scripts/docs-check.sh` САМ и собирает опубликованные байты со `--strict`, так что путь, публикующий негейченный сайт, отсутствует; path-фильтр не срабатывает на внутренний changelog и `measurements/`, но срабатывает на `hooks/` и `reference/cli/SUMMARY.md`; `docs-site/` не требует исключения ни в `scan::in_scope`, ни в `exclude_docs` (проверено пробой, а не предположено), а собранный сайт не коммитится; имя образа в workflow и в `Application.cue` сверено как СТРОКА, а не на глаз; манифест проходит `cue vet` и `apprafter app validate` (и проба с заведомо невалидным значением роняет оба — проверка отличает состояния); лендинг-переключатель НЕ лежит коммитом на ветке — `git diff --name-only <base>..HEAD -- 'landing/**'` пуст, `docsUrl` остался `""`, — потому что `landing-autotag.yml` публикует лендинг с любого пуша по `landing/**` и «придержать коммит» невозможно; переключатель записан шагом 5 раннбука вместе с названным механизмом; ШЕСТЬ ручных шагов оператора записаны в ADR и ПЯТЬ из них в раннбук так, что доводятся до конца без плана, включая шаг про приватность GHCR-пакета с пробой, различающей public/private; серверный слой закрыт двумя гейтами, каждый показан срабатывающим и НЕ срабатывающим: `caddy validate` слоем сборки образа (битый Caddyfile ⇒ build exit 1) и `scripts/docs-site-smoke.sh` между build и push (он валит дофиксовый образ на .md-404 и валит образ с блэнкетным `/assets/* immutable` на всех 35 неотпечатанных ассетах); `.md`-404 отдаёт короткий text/plain, а не HTML-страницу под видом plain; манифест читается парсером самого CLI (`expose.port` стал опциональным — до этого `app add` прятал пикер окружений). *(h)* `docsgen check` роняет `just lint` на примере с неразрешимым флагом, с флагом от ЧУЖОЙ команды, на пустой записи, на записи без `apprafter`-вызова и на примере, подшитом не к той команде (все пять показаны падающими, и первые два — на РЕАЛЬНОЙ таблице после `docsgen generate`, то есть при чистой байт-сверке и уже опубликованной ложной странице); каждый из 75 видимых листьев несёт ≥1 пример, а корень, групповые узлы и 3 скрытых листа — ни одного; 187 аргументов и 0 без описания перемерены тем, что печатает clap, а не проекцией `commands.json`, и оба счётчика сошлись; все пять шеллов `clap_complete` отдают скрипт, называющий команды ЭТОГО CLI (ассерт по `Shell::value_variants()`, не по списку в тесте), а рецепты для bash/zsh/fish пройдены руками под изолированным `HOME` до реального дополнения; семь свойств help-rot-стража показаны падающими и восстановленными, шесть из них выводятся из clap-дерева на момент теста, а седьмой (алиасы гайдов) — закоммиченный список, который сам проверяется против корпуса и несёт floor против опустошения; `after_help` цепляется в `run()`, и откат к `Cli::parse()` роняет отдельный тест, а не проходит молча; census не уменьшился ни по одному счётчику; ПЯТЬ пунктов исходного списка (h) НЕ построены и названы поимённо с пробой на каждый, а не списаны. *(i)* инвентарь недостающих гайдов перемерен по дереву команд, а не взят из оценки, и перемер включает разбор того, почему исходная команда давала три разных ответа (ключевание по базовому имени страницы); каждая из пяти страниц лежит в корпусе гейта (`docsgen gate OK` — ни одного нового exemption), стоит в nav `mkdocs.yml` И в индексе своей секции (строгая сборка ловит битую ссылку, но НЕ несвязанную страницу), несёт `description` во frontmatter, попадает в `llms.txt` и в markdown-двойник; ни одна страница не называет фазу, подфазу, роадмап-координату или `plan.md`/`spec.md`; ни один путь репозитория не добавлен (`code_paths` 104 → 104 — где нужен файл, дан абсолютный GitHub-URL); каждая команда, безопасная без кластера, ПРОГНАНА, а каждое значение, которое гейт не проверяет (SKU, режимы, имена полей), сверено с кодом, который его парсит; три команды оставлены без гайда ОСОЗНАННО и записаны с причиной; расхождения между параллельно написанными гайдами сведены поимённо, а не замазаны; коммитнутый census перезаписан `docsgen metrics`, потому что пол отстал от живого замера и не защищал новые страницы.

**Зависит от:** — (машинерия автономна). **Связан с** 2.16h/2.16g/2.12/2.9 (их поверхности — основной контент 2.19i) и с лендингом (2.19g правит трекнутый `docsUrl` релизного пути отдельным коммитом; одноимённый глобал Payload — превью-путь — за аутентифицированным API и остаётся за оператором). 2.19h меняет clap-дерево ⇒ регенерирует референс тем же коммитом.
...
### 2.20 Гайды — это рецепты (ADR 0058) — ЗАКРЫТА 2026-08-30

> 🏁 SR: A · order 3.7b — публичная поверхность как продукт

**Цель:** гайд перестаёт быть переписанным прогоном. Рецептный путь несёт только
`apprafter`-команды и законно-внешние инструменты; всё остальное маршрутизируется по роли.

**Spec:** `docs/superpowers/specs/2026-08-29-guides-are-recipes-design.md`. **ADR:** 0058
(+ амендмент к 0057). **Замер:** 27 страниц, 478 `apprafter` против 325 чужих команд
(VERIFY 153 · TROUBLESHOOT 60 · EXPLAIN 51 · SETUP 61), 96 блоков рвут happy path,
11 из 27 страниц нельзя пройти одними `apprafter`-командами. Четыре худшие страницы —
четыре из пяти, что ссылаются на e2e-прогон.

- [x] **2.20a — ADR-first + два продуктовых бага** (2026-08-29, `36d7e33…6328cdf`)
  ADR 0058; амендмент к 0057 (что измерение опрокинуло + резолюция противоречия в решении 7);
  `spec.md` Appendix B + §1.5 + Revision 16. `import --force` чинит нечитаемый `state.json`
  (`load_or_recover` переименовывает, не удаляет); `app add <git-url>` больше не требует
  манифест в cwd. Третий предложенный фикс (`node_prep.rs:421`) **отозван при проверке** —
  это `SWAPOFF_FAILED_RUNBOOK` на пути неудавшегося отката, а перекат ворклоадов после
  `node prep` реально нужен (inline-запись `memory.swap.max` — временная мера). Spec §5.4.
- [x] **2.20b — гейт `recipe-purity`** (2026-08-29, `4648391`, `bef81ff`, `8ebe68d`)
  **174 находки на 14 страницах, 4 разные команды (kubectl 171, hubble 3), ноль изобретённых.**
  Ранжирование воспроизвело перепись независимо (postgres 40, redis 39, persistent-disk 36,
  egress-policy 21). НЕ подключён к `docsgen gate`: красная проверка в прогоне, который зовут
  lefthook и `just lint`, сделала бы репозиторий незакоммитабельным до конца 2.20c — живёт за
  `docsgen recipe-report`, он же рабочий список для 2.20c. Красность закреплена тестом, включая
  негативный контроль (четыре чистые страницы обязаны молчать) и проверку, что break-glass
...
- [x] **2.20c — перестройка 27 страниц** + слой механики + долг протухания. Гейт зеленеет.
  ...
  - [x] **часть 2 — перестройка страниц** (2026-08-30, `0782e8a…065502f`): **174 → 0**
    находки. Четыре худшие (postgres 40, redis 39, persistent-disk 36, egress-policy 21) были
    прогонами/лабораторией — переписаны в рецепты, механика вынесена в новый слой
    `docs/how-it-works/` (6 страниц). Хвост из 9 страниц — точечные вкрапления, свёрнуты или
    удалены. Адверсариальное ревью переписанного дало **10 подтверждённых дефектов**, включая
    неверную историю удаления на всех 6 страницах.
  - [x] **часть 3** — `recipe-purity` подключён в `Gate::corpus()`; корпус-тест перевёрнут с
    «красный» на «зелёный» + ассерт, что каждое `check=none` типизировано и датировано.
- [x] **2.20d — гейт `behaviour-claim`** (2026-08-30, `e0003fd`): класс, на который слепы
  все остальные проверки — они резолвят ИМЕНА, а протухшая запись о дефекте называет только
  то, что всё ещё существует. **Не список запрещённых фраз** (решение 7 ADR 0057): каждая
  запись привязана к ФАКТУ В КОДЕ (файл + якорь + present/absent), так что таблица говорит,
  за какой фразой следить, а ответ даёт дерево — откати фикс, факт перевернётся, фраза снова
  станет истинной и проверка замолчит сама. Засеян пятью, включая ещё открытый: RBAC без
  глагола `delete` на `resourceclaims`. **Доказан историей, а не собой** —
  `behaviour_history_test.rs` достаёт реальные страницы из git на `894bdfd` и требует, чтобы
  все три исторических клейма были пойманы, а исправленные страницы молчали.

**Вне объёма:** новых существительных CLI нет (`claim`, `app exec`, `app restart` отклонены);
`env-secrets-hash` вынесен отдельно — он тянет релизную цепочку оператора.

### 2.21 Публичный ледгер фич

> 🏁 SR: A · order 3.7b

**Цель:** `/status/` перестаёт быть опубликованным `FEATURE_TRACKER.md` и соединяется с
роадмапом лендинга по стабильному `id` реестра.

**Spec:** `docs/superpowers/specs/2026-08-29-public-feature-ledger-design.md`.

- [x] **Баг подписки** (2026-08-29, `a52ae93`) — кнопка «Get notified when Phase 8+ ships»
  постила `federation` в `interests`, где такой опции нет; Payload отвечал 400, форма
  показывала успех, строка не писалась. Класс закрыт гейтом в `landing-content-gate.test.ts`.
> ✅ **ЗАКРЫТО 2026-09-06** (docs+landing-only, без бампа CLI и монорепо-тега). План:
> `docs/superpowers/plans/2026-09-06-2.21-public-feature-ledger.md` (gitignored, локально).
> Страница 204 → **202** строки; 47 → **64** строки данных; блокквот-проза 965 → **237** слов;
> ноль → **21** ячейка-ссылка. Числа пересчитаны 2026-09-07 после догона 2.21c и разбора аудита —
> первая редакция этого блока (177 строк / 59 строк / 125 слов / 15 ссылок) была верна на коммите
> закрытия и устарела через сутки. Экономия строк ушла на то, что аудит вскрыл: пять новых строк
> с фазой, которую лендинг уже продаёт, и записанные исключения. Обе выкладки `*-check-ignore`
> сняты: нестроенная строка называет **способность**, а не команду и не ключ схемы.

- [x] **2.21a** — пункт ледгера в ADR 0058; решение по `Post-launch` в реестре; чистка
  утечки `Product 1` / `Product 2` в `roadmap.json`.
  - **Решение по `Post-launch` (и заодно по дробной `Phase 4.5`): записи в реестре НЕ будет,
    и ячейки не становятся ссылками.** Оба варианта спеки неверны на доказательствах.
    Свернуть в `managed` — ложь: обе строки (T1→T2 миграция, approval-UI в портале) это
    OSS-core на кластере клиента, а `managed` — «Managed offering launch». Отдельная запись —
    ссылка в никуда: `Roadmap.astro` рисует карточку на запись **контента CMS**, а не реестра,
    и кнопка подписки живёт на карточке; запись без карточки даёт якорь, которого страница
    никогда не эмитит. Это ровно правило §2.2 самой спеки — «ледгер не обещает контрол,
    которого нет» — применённое к случаю, который спека пометила как блокер.
  - Утечка закрыта и **зафенсена** тестом в `landing-content-gate.test.ts`; живой CMS несёт те
    же две строки и вынесен в `CMS_PROD_CHANGES.md` (действие владельца).
- [x] **2.21b** — переписывание страницы: убрать колонки `plan.md` и `order/CP`, продуктовые
  группы вместо `## order N`, схлопнуть блокквот-прозу, колонка «Документация»,
  расширенный словарь отметок для политик и архитектурных свойств.
  - Второй словарь: **`◆` в силе сегодня · `◇` ещё нет**. `◇` — это ровно поправка §4.2:
    свойство утверждается только когда построено то, на чём оно стоит.
  - Колонка «Документация» ссылается на гайд, который покрывает фичу; `code-path` её резолвит,
...
- [x] **2.21c** — перенос матрицы killer features под правилами редактуры; девять
  ...
  - Редактурное правило спеки соблюдено буквально: **лог редактуры не едет вместе с
    отредактированным выводом** — колонки `notes` нет, слова «redacted» нет.
- [x] **2.21d** — соединение: ячейки Phase несут стабильный `id` реестра в href при видимом
  лейбле в тексте; три теста в файле, который уже читает реестр (каждая ссылка резолвится в
  `id`; фаза с контролом подписки — ссылка, без контрола — нет; лендинг ссылается назад);
  обе стороны проверены мутацией. Обратная ссылка живёт в **компоненте**, а не в CMS-поле
  `closing`, поэтому существует без правки прода.

### 2.22 Day-2 дефекты: разбор `day2-followups.md` D4–D14

> 🏁 SR: A · order 3.7c — то, что 2.19j/2.20c задокументировали вместо того, чтобы починить

**Цель:** закрыть одиннадцать продуктовых дефектов, найденных при переписывании гайдов и
разобранных по одному с владельцем 2026-08-30/31. Детали, доказательства и фиксы против кода —
в `docs/measurements/day2-followups.md` (записи **D4–D14**); здесь только порядок, группировка
и релизные цепочки. **Плоский порядок D4→D14 отвергнут:** дефекты кластеризуются по ОБЩЕМУ
фиксу, и поштучный проход трогал бы один и тот же код по три раза и гнал бы три релиза
оператора там, где хватает одного.

**Три сквозных урока разбора** (держать при исполнении каждой подфазы):
1. **Утверждение, которое удовлетворяется вырожденным случаем, хуже отсутствующего.** Три раза
   подряд: `[ "$REQ" = "$RECO" ]` при обеих сторонах 32Mi (D10), тест, закреплявший устаревший
   `nodes[0].kind` (D3), и 420-секундный дедлайн, узаконивший аварию ACL (D5).
2. **Владение ≠ время жизни.** `ownerReference` отвечает «что будет, когда родитель умрёт», и
   никогда — «родитель больше не хочет этого ребёнка» (D4/D12/D13).
3. **Форма доставки определяет доступный контроль.** Императивный сеал отревьюить нельзя →
   остаётся раскрытие; диff манифеста ревьюится → можно гейтить (D14, `§∞.10`).

- [x] **2.22a — преflight и таксономия ошибок CLI** (D11) · CLI-only, без цепочки оператора
  Самое дешёвое из тяжёлых и единственное, на что владелец напоролся лично.
  - [x] `preflight_tool(...)` в начале КАЖДОЙ команды, которая спавнит внешний бинарь — до
        промпта, до похода в кластер, до платного шага. Преflight уже написан
        (`backup.rs:2142`, у него есть ветка `NotFound`) и зовётся ровно из одной команды.
  - [x] Порядок, худшее первым: `restore --reprovision` (гейтит пароль НАМЕРЕННО, с
        комментарием, и не гейтит бинарь → отсутствующий restic стоит оплаченного поднятого
        кластера), `bootstrap-all` (helm нужен в фазе 3/3, после платных ресурсов),
        `repo creds add` (боевой PAT собирается до проверки дубликата/кластера; `rotate` в том
        же файле делает правильно), `backup create`/`list`, `prune`/`check`/`unlock` (созданы
        работать БЕЗ кластера, но ходят в него первыми).
  - [x] Тест на команду: до преflight'а нет ни промпта, ни вызова провайдера.
  - [x] `ExternalToolNotFound { tool, needed_by, purpose, install }` + фраза, которая важна
        после промпта: **ничего никуда не отправлено и кредл не использован**. `min_version`
        НЕ добавлен: «установлен ли» и «достаточно ли новый» падают по-разному и заслуживают
        разных сообщений — вторым вопросом остаётся владеть `preflight_restic_version`.
  - [x] `Kubectl` + `Restic` с чистыми классификаторами stderr (`cli_core::diagnose`):
        недоступен / запрещено / kind не обслуживается, и неверный пароль ≠ отсутствующий
        репозиторий ≠ залипший лок. Вкручены **в узкие места** (7 хелперов `k8s_helpers`,
        оба спавна `restic_runner`), а не в 39 мест вызова — запись прямо говорит, что 584
        не надо гнать к нулю. Консервативны: нераспознанное → `Other` и исходный текст.
  - [x] `doctor` несущий: FAIL вместо WARN для бинаря, без которого CLI не работает (сейчас
        отсутствующий kubectl даёт exit 0 и «Ready to go», **и это закреплено тестом**);
        добавить `restic` (8 спавнов, нигде не проверяется, нет даже в предусловиях quickstart)
        и `git`; половину окружения гонять БЕЗУСЛОВНО (сейчас без активного таргета
        first-run-пользователь — аудитория из докстринга — не узнаёт ничего).
  - **Не гнать 584 к нулю.** Catch-all — правильный дом для разового отказа и неправильный для
    шести команд на пути аварийного восстановления.

- [x] **2.22b — владение: оператор удаляет то, что породил** (D4 + D12 + D13) · оператор + RBAC + chart
  > **Walk GREEN** (`e2e/needs-removal-walk.sh`, 2026-08-31, kind+podman). Четыре прогона: он
  > **до своего предмета дошёл только с четвёртого раза**, а по дороге нашёл **D15** —
  > гейт разрушающих изменений был полностью инертен для base-only приложений, в CRD И в
  > вебхуке, с юнит-тестом, охранявшим вторую половину. Плюс harness-дыра: фаза 1b
  > применяла branch-CRD, но не branch-RBAC, поэтому prune молча 403-ил.
  Один релиз оператора на три дефекта: обобщённый диff owned-детей их все покрывает.
  - [x] LIST детей этого Application + удаление тех, кого нет в желаемом наборе. Предикат —
        UID владельца (лейблов на клеймах нет), тот же, что уже гоняет `.owns()`-watch и
        `list_resource_claims_for_app`. Ближайший внутрикрейтовый образец —
        `delete_all_key_plans_except` (`lib.rs:1974-1995`, чистый фильтр `:2304-2324`).
  - [x] **НЕ «третья prune-ветка в той же форме»**: `has_needs` (`lib.rs:504`) гейтит весь блок
        по НОВОЙ спеке, поэтому удаление ПОСЛЕДНЕГО need'а пропускает блок целиком и любой
        `else` внутри не выполнится. Prune живёт СНАРУЖИ гейта.
  - [x] Глагол `delete` на `resourceclaims` в `rbac.yaml` — **в том же изменении**, что и код
        (повторный урок: ADR 0048 и фикс GC 0.2.31).
  - [x] Порядок несущий: удаление только ПОСЛЕ апрува плана и применения новой спеки, и
        финализатор провизионера обязан отработать → снапшот пишется, 7-дневное окно
        открывается как задокументировано.
  - [x] D12: `else`-ветка для Service (`lib.rs:726`) и CNP (`:736-740`). У Service `delete` в
        RBAC есть с самого начала. Проверить `is_destructive` для снятия `expose`.
  - [x] D13 — **форму рендера менять не нужно, это была ложная развилка** (владелец, 2026-08-31).
        `ownerReferences` — СПИСОК, и GC удаляет зависимый объект, только когда исчезли ВСЕ
        владельцы: это подсчёт ссылок, исполняемый apiserver'ом. Копия несёт по одной
        non-controller ownerRef на каждое потребляющее Application (тот же namespace, несколько
        владельцев, `blockOwnerDeletion: false` — удаление приложения не должно ничем блокироваться).
        Field manager **пер-аппный**: SSA владеет `ownerReferences` как map по `uid`, поэтому
        записи сосуществуют; один общий менеджер прунил бы ту, которую не отправил в этот раз,
        и два приложения выселяли бы друг друга по очереди.
        Вторая зависимость — от PAT — кросс-неймспейсная, а Kubernetes такие ownerRef запрещает
        (это уже знали: докстринг финализатора и `rbac.yaml:432` говорят об этом прямо). Её
        закрывает уже существующий финализатор: копии проставлен `apprafter.io/source-credential`,
        а `gc_derived_secrets` подметает **кластерно по лейблу**, а не по двум зашитым namespace —
        namespace'ы приложений оттуда не вычислимы, а удаление PAT это ровно тот момент, когда
        отозвать надо ВСЁ производное.
        UX удаления, о котором был вопрос, **уже есть**: `repo creds remove` отказывается, когда
        под префиксы креденшла попадают Argo-приложения, перечисляет их и требует `--force`.
  - [x] Walk: приложение с ДВУМЯ needs, убрать ОДИН → RetainedClaim появился, второй клейм не
        тронут. Ни один из четырёх needs-walk'ов этого не делал — все удаляют приложение
        целиком, поэтому все гейты и проходили. **Поставлено `e2e/needs-removal-walk.sh`**
        (фазы 3–6: два именованных pg-клейма, снятие одного, RetainedClaim + `retainUntil`,
        сосед сверен ПО UID, негативный контроль — клейм чужого Application). GREEN
        многократно, последний раз 2026-09-01 (11 фаз, 40 `ok`).
  - [x] Walk: удалить приложение → в его namespace не осталось `dockerconfigjson`-Secret'а.
        Это утверждение поймало бы все три.

- [x] **2.22c — секреты: раскрытие, индекс, распространение** (D14 + D7 + D6)
  > **ЗАКРЫТА 2026-08-31.** Порядок пришлось перевернуть: план ставил docs-половину первой как
  > «бесплатную», но раздел траблшутинга нёс `kubectl`-обход под исключением `known-broken`
  > ровно потому, что D7 не был починен — писать гайд раньше кода значило бы описывать
  > несуществующее (урок ADR 0058). Код → потом один проход по документации, который и разделил
  > страницу по читателю, и снял исключение. Ратчет перезаписан тем же коммитом (11 → 10).
  - [x] **Сначала бесплатное (docs-only):** три поставки из поправки к ADR 0007 —
        сказать это на каждой поверхности (`apprafter secret` = операторский инструмент T1,
        замена OpenBao, без гранулярного доступа и аудита); перенести
        `docs/dev-guide/secrets.md` (половина про привязку остаётся у разработчика, workflow
        сеаливания уезжает в операторский гайд).
  - [x] Оператор, две строки: `check_env_secret_refs` (`lib.rs:1553-1578`) РАЗЛИЧАЕТ «нет
        Secret'а» и «нет ключа» и схлопывает в один `bool` — назвать реальную причину и
        namespace (он в скоупе строкой выше).
  - [x] `apprafter secret list` — имя, namespace, **имена ключей, никогда значения**, флаг
        all-namespaces. Пробел зафиксирован ТРИЖДЫ независимо (амендмент 2.19h к ADR 0057
        предсказал, walk 2.19j увидел, 2.20c напоролся).
  - [x] Индекс в ОБЕ стороны из одного прохода (LIST Applications + скан `env` на
        `EnvValue::Ref(EnvRef::Secret(_))`): app→секреты в `app status` (D7, диагностика),
        секрет→apps в точке сеаливания (D6, радиус) и в раскрытии (D14).
  - [x] Хеш разрешённого конфига на pod-темплейте + показ дрейфа в `app status`.
        **Автоматический роллаут НЕ дефолт на T1** (неизвестный радиус: несколько приложений
        на один секрет; и `rollout: manual` негде жить — у секретов нет декларативной
        поверхности). Роллаут — явное действие того, кто видит, что он затронет.
  - [x] Аннотация автора/времени на сеал — для инсайдерской ошибки и форензики, не как контроль.
  - **Отклонено письменно:** гейт на сеал (апрув непрозрачного блоба — театр, и облагает
    налогом единственную операцию, которая обязана быть быстрой). Автоматика — вопрос T2+
    (`autoRestartAppsOnEnvChanges`), где ревизии наблюдаемы и актор проаудирован.
  - **Предпосылка для 2.16a:** его acceptance-критерий («приложение перечитает Secret») ложен —
        `secretKeyRef` резолвится один раз при старте пода.

- [x] **2.22d — сигналы, которые никому не доходят** (D8 + D10) · оператор + chart + CLI
  > **Код готов, ПРОГОНОВ НЕТ — оба долга явные.** `e2e/vpa-walk.sh` переписан (второе
  > приложение с реальным следом от самого образа, утверждение на `containerStatuses`,
  > hard-fail), но **не запускался**: in-place resize требует настоящего Hetzner, kind/k3d
  > его не умеют. Шага прохода для баннера дискового давления (D8) **не существует вовсе** —
  > запись «Ships with» была планом, а не поставкой. Оба идут в батч прогонов на закрытии 2.22.
  >
  > По дороге исправлены два собственных ошибочных утверждения: (1) «у оператора нет SQL-клиента,
  > значит размер БД непрактичен» — экспортер CNPG снимает вопрос целиком и лучше клиента
  > (своё соединение, не ест слот из `max_connections` арендаторов); (2) «читать in-place
  > condition у VPA» — **такого условия нет**, VPA не пишет в свой CR об этом ничего, факт
  > заявляет kubelet на поде (`PodResizePending`). Для redis байты недостижимы по-настоящему
  > (проверено по исходникам v1.37.0 и `main`) → честная цифра — счётчик ключей, так и подписан.
  - [x] D8: `CapacityWarning` — это диск НОДЫ (`node_free_fraction` = `/node/fs/*`), но
        застамплен на `SharedVolume` → кластер без единого SharedVolume не предупреждается
        вообще, хотя на той же ФС живут owned-диски, данные CNPG, снапшоты Dragonfly, образы
        и логи. Увести на node-scoped носитель (синглтон `PlatformStack`).
  - [x] D8: предупреждать на КАЖДОЙ команде, жёлтым, по форме
        `commands::version_check::maybe_warn_about_newer_version()` (один вызов, `lib.rs:92`).
        Best-effort — недоступный кластер не превращает предупреждение в ошибку.
  - [x] D8: отпорожить по-томовый семпл, который УЖЕ берётся (`pvc_usage` → `status.capacity`,
        но `is_capacity_warning` принимает только node-долю) и показать в `volume status`.
  - [x] D8: колонка состояния в таблице клеймов `app status` (`print_resource_claims`,
        `app.rs:1605` — сейчас NAME/PROVIDER/READY/SCHEDULED/SECRET). Начать с цифр томов и
        дисков, которые уже есть; зонд размера БД — follow-on, не блокирует.
  - [x] D10: сделать walk способным упасть — засеять приложение ВЫШЕ пола, утверждать на
        `status.containerStatuses[].resources` (не на `spec`), hard-fail вместо лога.
  - [x] D10: оживить `notApplied` (единственный прод-вызов хардкодит `infeasible: false`,
        `lib.rs:870`; CRD-поле, тип и рендер CLI построены и недостижимы).
  - [x] D10: преflight k8s-пререквизита так же, как это делает путь смены ноды
        (`node_prep.rs`, `k8s_ge_134`). Отдельный вопрос — пинить ли версию Kubernetes вообще
        (третья поправка к ADR 0054 его не решает).

- [x] **2.22e — откат приложения по скользящему тегу** (D9) · оператор + chart + CLI
  - [x] Хранить предыдущий дайджест: `StatusImage` уже несёт `{tag, resolved, resolvedAt}` →
        `previous` аддитивен. `status.lastAppliedSpec` НЕ помощник (в нём тег, а тег тот же).
  - [x] **Откат обязан пиновать** — иначе следующий реконсиляй перерезолвит тег и накатит
        плохую сборку обратно внутри 60-секундного requeue. Значит это смена РЕЖИМА.
  - [x] Глагол возврата на скользящую модель — **обязателен**, иначе `rollback` дверь в один
        конец из главной фичи.
  - [x] Где живёт пин: `PlatformStack` кластерный (гит не оспаривает) — поэтому `platform pin`
        это одна merge-patch; CR приложения рендерится CMP из репозитория пользователя, и синк
        снесёт запись. Кандидат — аннотация на CR под выделенным SSA-менеджером (ADR 0048 дал
        эмпирику: SSA-аннотация переживает синк без `ignoreDifferences`), она же исчезает
        вместе с приложением. Альтернатива — карта пинов на `PlatformStack`, но это app-scoped
        данные на объекте платформы + своя сборка мусора = дефект D4 на новом месте.
  - [x] `--to sha256:…` = дайджест образа, всё прочее = гит-ревизия (флаг уже занят ревизией).
  - [x] Видимость: жёлтым в `app status` с версией удержания; список запиненных в
        `platform status`; ветка в `resource.customizations.health.apprafter.io_Application`
        (`component_argocd.cue:142`) → **`Suspended`, не `Degraded`**. Здесь health
        АГРЕГИРУЕТСЯ (CR — managed-ресурс приложения, в отличие от заякоренного плана из
        ADR 0048), поэтому меняет здоровье пользовательского Argo-приложения → проверять живым
        прогоном, а не чтением правил (класс, на котором A5 был опровергнут).
  - [x] Walk: пуш второго образа в тот же тег → откат вернул первый **и держится минимум два
        реконсиляя** (наивный фикс проходит первое утверждение и валится на втором).
  > **Walk GREEN** (`e2e/image-rollback-walk.sh`, 2026-08-31, kind+podman, 30 утверждений). ADR 0059. Разведка из 5 страндов + состязательная
  > проверка каждого несущего допущения окупилась: два из четырёх не устояли.
  > (1) «У VPA есть in-place condition» — **такого условия нет** вообще (это было D10,
  > но той же природы). (2) **`Suspended` РЕАЛЬНО вешает синк** — не-hook задача
  > оседает только на Healthy/Degraded, и операция паркуется в Running при multiStep.
  > `CreateNamespace=true` его НЕ взводит (проверяющий сам снял мою тревогу), взводят
  > waves/hooks/managedNamespaceMetadata — которых у нашей формы нет, поэтому это
  > **инвариант**, а проход утверждает `operationState.phase == Succeeded`.
  > (3) Плитка условна: `Suspended` — ВТОРОЙ по здоровью код, перебивает только
  > `Healthy`, поэтому ветка ключуется на маркере пина и стоит ВЫШЕ fall-through.
  > (4) Выживание SSA-аннотации на CR **измерено на живом apiserver** (kind 1.35,
  > наш CRD), а не выведено — по-ключевое владение внутри `metadata.annotations`
  > у CR было честным поводом сомневаться.
  >
  > Попутно найдены ДВА отдельных дефекта. (a) Пять builder'ов статуса **стирали
  > `status.image`** на любой паузе (гейт миграции, ожидание клейма, EnvSecretMissing,
  > невалидный spec) — безвредно, пока поле никто не читал; фатально, когда в нём цель
  > отката. (b) **D18**: `kubectl` вырезает `managedFields` из `get -o json` без
  > `--show-managed-fields`, поэтому ДВА guard'а владения (egress из 2.10 и новый pin)
  > никогда не могли сработать — юнит-тесты строили вход, который в проде не приходил.
  > Оба переведены на новый getter + тест на класс (проверен на отказ).
  >
  > Три бага харнесса по дороге, один мой процессный: я **редактировал скрипт, пока он
  > выполнялся** — bash читает по смещениям, они сдвинулись, и он выполнил мусор.
- [x] **2.22f — долговечность ACL Dragonfly** (D5) · ADR-first
  - [x] **Сначала поправка к ADR 0042**: он отклонил файловые ACL с условием «revisit if
        file-based ACLs are officially confirmed». Условие ВЫПОЛНЕНО (проверено против
        upstream v1.37.0: `LoadToRegistryFromFile` разбирает строки тем же `ParseAclSetUser`,
        что и рантаймовый `ACL SETUSER` — одно определение, два call-site; `$N`, `~*`,
        `&user:*` работают).
  - [x] Билдер файловой формы + байтовое сравнение с `SETUSER`-argv. Осторожно:
        `MaterializeFileContents` режет по ОДИНОЧНОМУ пробелу и требует, чтобы строка
        начиналась с литерала `USER` и несла ≥4 токена — сравнивать надо с ФАЙЛОВОЙ формой.
  - [x] Per-instance Secret + `spec.aclFromSecret`. **Ловушка:** `optional` принимается схемой
        CRD и молча игнорируется (`SecretVolumeSource.Optional` не выставляется) → CR на
        несуществующий Secret даёт под, который не стартует. Secret пишется ДО поля.
  - [x] Строка `default` обязательна и её ошибка запирает провизионер снаружи его же инстанса
        — единственное здесь, что нельзя выпускать без живой проверки.
  - [x] **Один писатель**: файлом владеет петля ресинка; `provision`/`gc` продолжают
        императивные `SETUSER`/`DELUSER` и Secret не трогают. Провизионер и петля — РАЗНЫЕ
        tokio-задачи, и фикс гонки dbnum (`7890d9e`) был СЕРИАЛИЗАЦИЕЙ (`.concurrency(1)`),
        которая их друг с другом не сериализует. И доминирующее окно — не устаревший снапшот:
        `claims_to_repin` фильтрует по `status.ready`, который приходит ПОСЛЕ создания ACL, так
        что параллельная сборка теряет строку даже при LIST строго после чужой записи.
  - [x] Ужесточить walk: Phase 9/10 `needs-redis-walk.sh` УЖЕ рестартят инстанс (`:1203`,
        `:1307`) и УЖЕ проверяют переаутентификацию — но с дедлайном 420с под комментарием,
        который аварию узаконивает. Дельта — одно слово: «восстановился за 420с» →
        «не терял аутентификацию вовсе».
  - [x] Отдельно: образ сервера Dragonfly платформой НЕ пинится (тег берётся из
        скомпилированного дефолта оператора) — все ACL-факты выше про тег, который мы не
        контролируем.
  > **Walk GREEN** (`e2e/needs-redis-walk.sh`, 2026-08-31, kind+podman, 106 утверждений;
  > решающее — вход с ПЕРВОЙ попытки после перезапуска при операторе, погашенном в 0).
  > Первый прогон нашёл ПРОДУКТОВЫЙ дефект, которого не предвидели ни разбор, ни дизайн:
  > петля добавляла `aclFromSecret` живому CR → оператор катил StatefulSet → первый клейм
  > на новом инстансе получал рабочее соединение и тут же перезапуск. Починено рождением
  > CR сразу с полем + засев файла провизионером (create-if-absent).
  > ADR 0042 §10 + §11 ратифицированы. Разведка опровергла
  > два моих собственных утверждения из реестра, оба в опасную сторону.
  > (1) Строка `default` обязательна **не потому**, что её ошибка запирает провизионер:
  > файл БЕЗ неё даёт активного `nopass +@all ~* &*` и **выключает аутентификацию**
  > на инстансе, обслуживающем всех арендаторов кластера. Проверено живьём: `PING` без
  > пароля → `PONG`, `SET` → `OK`. Запирание тоже реально, но от другого — строки
  > `default` без `on`. (2) Правило про одиночные пробелы неверно: `SkipEmpty` их
  > допускает, апстрим это тестирует намеренно. Плюс `ACL LOAD` **запрещён вовсе** —
  > он чистит реестр и рвёт соединения всех арендаторов, причём даже когда файл
  > идеально синхронен.
  > Аргумент безопасности, который стоит помнить: битый файл грузит НОЛЬ пользователей,
  > и сервер всё равно стартует на `requirepass`-дефолте — то есть деградирует ровно
  > до сегодняшнего поведения, не хуже.
  >
  > D17 решён владельцем: снять `+info` И добавить `-pubsub`. `PUBSUB CHANNELS` не
  > фильтруется по `&{user}:*` и возвращает **имена namespace и приложений** соседей —
  > утечка крупнее той, ради которой всё затевалось. Цена одна именная: BullMQ нужен
  > `skipVersionCheck: true`.
  >
  > Записан, но НЕ починен: `CLIENT` выдан и не скоупится по подкомандам на этой версии,
  > поэтому любой арендатор может перечислить и **убить** соединения соседа. Это больше,
  > чем закрывает §11.
- [x] **2.22g — расписание бэкапа: заменить поверхность, а не валидировать** (D2) · CLI + schema + chart
  Независим от a–f, можно вытянуть вперёд. Docs-половина уехала в 2.19j, CLI-половина — нет,
  и этот раскол ровно то, как запись забывается: симптом перестал быть виден, дефект остался.
  - [x] `--at 03:00` вместо `--cron`. Таймзона по умолчанию — **машины оператора**, `--timezone`
        для оверрайда. Время без зоны — не время, а зона, которую оператор имеет в виду, это его
        собственная; UTC разумно просить и плохо предполагать. CLI собирает cron сам и ставит
        `spec.timeZone` на CronJob → ни то, ни другое не забота оператора.
  - [x] `backup status` печатает расписание обратно **в той же зоне**, в которой его задали.
  - [x] Частота — продуктовое решение (ежедневно бэкап, еженедельно проверка). Если понадобится
        второе значение, это `--every day|week`, а не поле cron.
  - [x] `--check off` настоящей веткой (`checkSchedule: ""` → omit в чарте), чтобы отключение
        перестало требовать даты, которая не наступает: страница до 2.19j учила
        `--check-cron "0 6 31 2 *"` — 31 февраля — как подставку под отсутствующий выключатель.
  - [x] **Таймзона сейчас не выставлена и не названа нигде** — ни в чарте, ни в схеме, ни в CLI,
        и ни одна страница не говорит «UTC». CronJob без неё крутится в зоне
        kube-controller-manager. Это ловушка корректности, а не эргономики, и никакая
        валидация синтаксиса её не ловит.
  - [x] Сырой cron как escape hatch допустим позже, но не единственной поверхностью и не той,
        которой учит гайд.
  > **Готово, гейты зелёные** (CRD провалидирован против настоящего apiserver, рендер чарта
  > проверен четырьмя вариантами, 654 юнит-теста CLI). Разведка опровергла несущее допущение
  > НЕ там, где ждали: опасность не в том, что `CronJob.spec.timeZone` вырежет старый
  > Kubernetes (GA с 1.27), а в том, что **наш собственный CRD** структурный — оператор
  > старее поля даёт 200, сохраняет все известные поля и молча роняет это одно. Команда
  > при этом ПОЛУУСПЕШНА: бэкапы включены и идут не в той зоне, а CLI рапортует зону,
  > которую якобы поставил. Поэтому `enable` теперь ЧИТАЕТ ПОЛЕ ОБРАТНО и падает громко.
  > Попутно: `| default` в шаблоне проверки НЕ надо было удалять — измерено на настоящем
  > helm, что достаточно и необходимо только обёрнуть блок в `{{- if $b.checkSchedule }}`.

**Релизная политика 2.22 (решение владельца 2026-08-31):** резать один релиз по завершении
ВСЕЙ 2.22, а не по подфазе. Оговорка, принятая вместе с решением: **walk'и гонять по мере
готовности каждой подфазы, а не копить валидацию на конец.** Причина — 2.22b качественно
отличается от остальных: это единственная подфаза, после которой оператор начинает УДАЛЯТЬ то,
чего раньше не удалял (всё прочее добавляет сигналы, глаголы и сообщения). В общем релизе её
ошибка стоит данных, а история показывает, что и меньшие релизы приходилось янкать (0.2.15 →
хотфикс → янк, 0.2.19 → янк по needs.redis). Живой прогон по подфазе даёт точку отката без
бисекта по большому релизу.

- [x] **2.22h — отказ реконсиляции должен быть виден без чтения логов** (D16) · оператор + CLI
  > ✅ **ЗАКРЫТО (2026-09-01).** Реализация: `operator-core/problems.rs` (`ProblemLedger` —
  > in-memory, `std::sync::Mutex`, cap 5 причин на объект / 512 объектов) + `status.recentProblems[]`
  > + жёлтые строки в `apprafter app status`. Прогон `e2e/needs-removal-walk.sh` фаза 9 — **GREEN**
  > (kind+podman, 11 фаз, 38 `ok`, 0 ошибок).
  >
  > **Главная поправка к плану, сделанная до написания кода: инцидент, ОТКРЫВШИЙ D16, вообще не
  > доходит до `error_policy`.** `prune_orphaned_claims` пишет `warn!` и возвращает `Ok(())` —
  > и правильно делает (ADR 0048: декоративный отказ не имеет права рвать реконсиляцию). Поэтому
  > пункт плана «событие из `error_policy` — единственное узкое место» **неверен**: дизайн,
  > повешенный только на `error_policy`, закрыл бы D16, не покрыв сам D16. Тот же силуэт ещё у
  > четырёх мест `warn!`-and-continue. Реестр вызывается из ОБОИХ классов: `error_policy`
  > синхронен и не может `await`, но мутировать память может — а это ровно то, чем реестр и является.
  >
  > **Второе отклонение: Events не используются, RBAC не менялся.** План предлагал Events ради
  > бесплатных dedup/count/TTL; взамен — поле на объекте, потому что часовой TTL событий короче
  > интервала редко повторяющегося отказа, а условие на объекте переживает GC событий, которым
  > читатель не управляет. Даром достался и нулевой RBAC-дифф (памятка «глагол едет с кодом» —
  > здесь новых глаголов нет; `applications/status` уже имеет `patch`+`update`).
  >
  > **Третье отклонение — ЗАКРЫТО 2026-09-01 по запросу владельца.** `platform status` получил
  > обзор по кластеру: «какие приложения сейчас с проблемами», иначе это выяснялось запуском
  > `app status` по каждому приложению. Три сознательных инверсии относительно соседнего
  > pin-обзора (тот декоративный, этот — сигнал здоровья): печатает и когда всё хорошо (иначе
  > отсутствие секции неотличимо от «проверка не отработала»), громко сообщает о неудачном
  > чтении (RBAC-отказ, отрендеренный как «всё чисто», — единственный вывод, которого эта
  > секция не имеет права давать), и называет приложение **логическим** именем, которое
  > принимает `app status`, а не `metadata.name` CR. Незарегистрированное в Argo приложение
  > всё равно перечисляется с пометкой — именно они чаще всего и сломаны. Обе поверхности
  > делят ОДИН фильтр рендера (`live_problems`), и тест на нерасхождение это фиксирует;
  > проверен вводом расхождения. В отличие от `app status`, обзор **покрыт проходом**
  > (позитивная + негативная ветки).
  >
  > **Долг прохода, явный:** рендер `apprafter app status` в проходе НЕ покрыт — команда
  > резолвит приложение через регистрацию в Argo (`status.resources[]`), а этот проход строит
  > голые CR. Подделывать `status.resources` отказался: поле принадлежит Argo и перезаписывается,
  > а проверка, подпёртая поддельной фикстурой, — это ровно тот проход по null-случаю, от
  > которого файл прохода предостерегает дважды. Проход печатает `NOT COVERED HERE` на каждом
  > зелёном прогоне. Покрыто юнит-тестами (`format_problem_lines`, 6 случаев).
  >
  > **Два зноба ретенции** (`APPRAFTER_PROBLEM_TTL_SECS`, `APPRAFTER_PROBLEM_REFRESH_FLOOR_SECS`,
  > сгруппированы в `ProblemTuning`) — оба логируются на старте, чтобы «состарилось» и «не
  > записалось» различались. Порог обновления сделан настраиваемым **потому что проверка
  > накопления `count` иначе неисполнима внутри прохода**, а неисполнимая проверка не может упасть.
  >
  > **Девять прогонов, и продуктовый код был верен с первого.** Все восемь падений — харнесс, и
  > все одного вида: проверка, неспособная наблюдать то, что заявляет. Разобраны в `bbf7973`;
  > самое поучительное — `kubectl logs | grep -q` под `set -o pipefail`: `grep` закрывает канал
  > по первому совпадению, `kubectl` умирает от SIGPIPE, и **совпадение читается как промах**.
  > Три прогона обвиняли продукт. Назвала причину диагностика, добавленная в падающую ветку.
  >
  > Побочно: фаза 6 получила проверку самого гейта (прогон 2 встал на пять минут и упал тремя
  > шагами ниже — на отсутствующем RetainedClaim), а общий дамп `e2e/lib.sh` теперь печатает
  > MigrationPlan'ы, заявленные `needs` и ownerRef-uid клеймов. Зависание записано как **D20**
  > честно, как невоспроизведённое: окно лога не покрыло одобрение, причина не доказана, фикс
  > не угадан.
  Найдено на walk'е 2.22b: три прогона подряд оператор 403-ил на удалении клейма каждые 30
  секунд, и видно это было **только** в `kubectl logs`.
  - [x] Важное различие: **спроектированные** отказы surface'ятся хорошо
        (`AwaitingResourceClaim`, `EnvSecretMissing`, `ImageResolved`, `MigrationPending` — все
        в `status.conditions` и в `app status`). Исчезает **непредусмотренный** — тот, что
        доходит до `error_policy` через `?`. То есть ровно тот, которого никто не ждал.
  - [x] `error_policy` (`application/src/lib.rs:1018-1031`) целиком: `warn!` + два счётчика +
        requeue. **48** мест `?`/`return Err` втекают в него; `recorder.publish` во всём
        контроллере — **2**, и оба про design-time уведомления, ни одного про отказ.
  - [~] ~~Событие из `error_policy` — это **единственное узкое место**~~ — **посылка опровергнута
        до написания кода, реализовано иначе.** Инцидент, открывший D16, до `error_policy` не
        доходит вовсе: `prune_orphaned_claims` пишет `warn!` и возвращает `Ok(())` (ADR 0048).
        Ещё четыре места того же силуэта. «Узкое место» покрыло бы 48 путей через `?` и НЕ
        покрыло бы сам D16. Events не используются: часовой TTL событий короче интервала редко
        повторяющегося отказа, а поле на объекте переживает GC событий. RBAC-дифф нулевой.
  - [x] Ограниченный `status.recentProblems[]` рядом: TTL событий (час по умолчанию) короче
        интервала медленно повторяющегося отказа, и условие на объекте переживает GC событий,
        которым читатель не управляет. Ограниченный по длине, с `lastSeen`, стареет сам.
  - [~] Печатать в `app status` — **сделано** (жёлтым, до строки VPA). В `platform status` —
        **НЕ сделано, долг явный:** реестр per-Application, у платформенного контроллера своя
        поверхность, и сводить одно к другому без отдельного решения не стал.
  - [x] Две ловушки: повторяющаяся ошибка обязана **дедуплицироваться, а не спамить** (иначе
        30-секундные ретраи пишут статус дважды в минуту вечно), и переставший повторяться
        отказ обязан **стареть сам**, иначе поверхность превращается в список того, что когда-то
        было сломано, и его учатся не читать.
  - [x] Walk: намеренно вызванный отказ (403 — очевидная фикстура, воспроизводится удержанием
        одного глагола RBAC) виден в `app status` без чтения лога.

**Вне объёма 2.22:** `∞.10` (inline sealed env value) — бэклог; 2.16a (периодическая ротация
кредлов клеймов) — ждёт хеш из 2.22c как предпосылку.

### 2.23 Разбор презентационного прохода по лендингу, доке и CLI

> 🏁 SR: A · order 3.7d — источник: `actual_bugs_findings.md` (владелец, 2026-09-05)

**Цель:** закрыть 26 находок прохода владельца по трём публичным поверхностям. Порядок из
файла не сохраняется: находки сгруппированы по **общему фиксу и релизной цепочке**, как в 2.22,
иначе один и тот же quickstart правился бы четыре раза, а CLI релизился бы дважды.

**Сквозная установка владельца, держать при исполнении каждой подфазы:** *дока должна стать
проще и понятнее.* Флоу оператора (1–9) и разработчика (1–10) из файла — это **порядок чтения
существующего материала**, а не заявка на новые страницы. Подфаза, которая увеличивает объём
доки, не выполняет задачу. Счёт по 2.23e, пересчитанный по дереву: **две** страницы уходят
из операторского setup-пути (node-prep в day-2, publish-the-docs-site в Contributing), **четыре**
новых файла, три из которых — распил существующих. Бюджет строк: 6 страниц / 2 080 строк →
10 страниц / ≤1 995. Сокращение **не берётся из распила** — cut-and-paste нейтрален по строкам;
оно берётся ровно из одного места, и это решение, а не следствие: `choosing-the-machine.md:305-445`
документирует ту же процедуру смены машины второй раз целиком (141 строка, и сам текст на `:368`
признаёт пересечение). Если её не сложить в новую страницу ранбука — подфаза выходит в **плюс**
по строкам.

**Три решения владельца, не подлежащие пересмотру при исполнении:**
1. `up` — каноническое имя команды, `bootstrap-all` уезжает в алиас.
2. `platform` управляет платформой. Ошибок приложений в `platform status` быть не должно.
3. Знак `+` в упоминаниях фаз убирается.

- [x] **2.23a — CLI: `up` каноническое, `status` несущий, `platform` только про платформу**
      · одна релизная цепочка CLI, **голова очереди** (2.23d/e правят текст под эти имена)
  - [x] `cli.rs:235` — поменять местами `name`/`alias`: `#[command(name = "up", alias =
        "bootstrap-all")]`. Алиас остаётся — ничего у пользователя не ломается.
  - [x] `commands/status.rs` сегодня — скелет из первого коммита CLI (`8f4267e`): читает
        локальный state и печатает `would show status …`. Переписать как **композицию поверх
        уже написанных читателей**, нового чтения кластера не добавляется:
        заголовок таргета (`resolve_state_paths` + `State::load_or_default`, то, что печатается
        сейчас, одной строкой) → версия/доступный апгрейд PlatformStack → все условия
        `!= True` → проблемные приложения → запиненные приложения → неодобренные MigrationPlan'ы.
  - [x] `platform.rs:70` `status()` — снять обе прикладные секции: `print_pinned_applications`
        (`:236`) и `print_problem_applications` (`:165`) вместе с двумя `kubectl_get_json_by_selector`
        (`:107`, `:108`). `problem_app_rows` / `pinned_app_rows` / `problem_heading` уже
        `pub(crate)` — это перенос вызовов, а не новый код. `platform status` остаётся тем, чем
        назван: канал/пин/autoUpgrade/тир, current/target/available, таблица условий, история,
        componentVersions.
  - [x] Деградация вместо ошибки: `status` при недоступном кластере обязан напечатать локальный
        заголовок таргета и пометить кластерные секции недостижимыми — образец рядом,
        `node_prep.rs:1201` с метками `[api]`/`[ssh]`. Оператору, у которого кластер лёг, эта
        команда нужна именно тогда.
  - [x] Тест на нерасхождение двух поверхностей, который 2.22h уже завёл для `live_problems`,
        переезжает вместе с секциями — он и есть гарантия, что перенос ничего не потерял.
  - [x] Цепочка обязательного: `just docsgen-generate` (меняются четыре байт-сравниваемых
        артефакта — `reference/cli/{up,status}.md`, `index.md`, `commands.json`, `SUMMARY.md`),
        `help_rot_test.rs:681` GuideAlias переворачивается, `aliases_test.rs:122`
        переименовывается, `cli_smoke.rs:177`, `examples.rs:466`. Bump `cli/Cargo.toml` **и
        регенерация `cli/Cargo.lock` в том же коммите** — иначе `--locked` в release-cli.yml.
  - [x] ADR 0057 (`:1968`) фиксирует прямо обратное решение («`apprafter status` осознанно не
        назван: это скелет»). Правка отменяет его — **сначала поправка к ADR, потом код.**

- [x] **2.23b — установка, которая работает** · лендинг + дока, без bump CLI
  - [x] Раздел скачивания на лендинге, чтобы не гнать посетителя на GitHub (`:24`): секция
        `#download` на `/` + тонкая страница `/download`, чтобы у доки был стабильный URL.
  - [x] `landing/web/public/install.sh` — Astro отдаёт `public/` как есть, значит
        `https://apprafter.dev/install.sh` без дополнительной проводки. Определение
        `uname -s`/`uname -m` → три публикуемых триплета, честный отказ на linux aarch64
        (`release-cli.yml:53` его не собирает), проверка `.sha256` до установки.
  - [x] **Резолв версии — не деталь реализации, а суть.** Монорепо публикует пять серий
        релизов, поэтому `/releases/latest` возвращает свежайший **по всем** и может выдать
        `operator/v0.1.134`. Сломанных мест **четыре**, а не три: `README.md:97` и оба quickstart
        (`operator-guide/quickstart.md:41`,
        `dev-guide/quickstart.md:38`) берут версию именно так. CLI уже умеет правильно —
        `version_check.rs:160` `pick_canonical_cli_tag`), плюс `dev-guide/quickstart.md:46` —
        `gh release download` **без тега**, что резолвит тот же отравленный «latest» другим путём.
        Осторожно: rust-фильтр НЕ годится для установщика дословно — `canonicalise_cli_tag`
        (`:179`) снимает `v` и парсит semver, а semver **принимает** prerelease, поэтому реальный
        тег `v0.1.0-mvp` (его собирает тот же `v0.*`-триггер, у него есть ассеты) проходит.
        `install.sh` берёт ту же логику серий, но сужает до `^v[0-9]+\.[0-9]+\.[0-9]+$`, и это
        расхождение записывается во всех трёх копиях.
  - [x] Версия на странице не запекается в `dist/`: резолвится на клиенте тем же фильтром.
        Захардкоженная версия на лендинге протухает молча — `landingHero.json:6` уже такой
        случай, оба гейта сверяют его с CMS, а не с реальностью.
  - [x] Плюс третье место с той же поломкой, которого в находке не было: баннер апгрейда
        самого CLI (`version_check.rs:70`) вёл на `/releases/latest` — тот же отравленный URL,
        только для человека. Правка версии фетча в v0.1.151 починила половину бага.
  - [x] Из доки вести на лендинг: `dev-guide/quickstart.md:28`, `operator-guide/quickstart.md:40`,
        `README.md:96`. Ручной путь через GitHub Releases остаётся второй вкладкой — это
        единственный путь, не зависящий от доступности apprafter.dev.

- [x] **2.23c — лендинг: упоминания фаз** · один контент-проход + один гейт
  - [x] `PhaseChip.astro:30` — `width: fit-content` + `justify-self: start`. Чип `inline-flex`
        внутри grid-элемента блокифицируется во `flex` и растягивается на всю дорожку
        (`BoringTech.astro:90`, `grid-template-columns: 200px 1fr`). Соседние два чипа
        (`Advantages.astro:85`, `TierLadder.astro:119`) уже несут `align-self: flex-start` —
        конвенция в репозитории есть, до `PhaseChip` не доехала.
  - [x] Ссылка на фазу в turnkey-ячейке (`comparison.json:60`) не выделена ничем: `global.css:25`
        снимает все признаки ссылки, а scoped-стили Astro **не достают до `set:html`**.
        Значит чип для CMS-контента обязан жить в `global.css` — класс `.phase-ref`.
  - [x] Sticky-заголовок наезжает на кнопку подписки: `.roadmap-waitlist` авторазмещается в
        строку 2 колонки 1 — ровно ту, по которой едет sticky `.phase-meta`, и позиционированный
        бокс красится поверх статичного. Чинить **разметкой** (обернуть `<ul>` + кнопку в
        `.phase-body`), не `z-index` — z-index прячет столкновение, а не убирает его.
  - [x] Контент-проход: лейблами оформлены 3 упоминания фазы из ~20. Остальные — проза в CMS-полях.
        `.phase-ref` даёт механизм, дальше — проход по `fallback/*.json` + гейт в
        `landing-content-gate.test.ts` (у него уже есть блок «fallback ⊆ registry»), иначе
        регрессирует на следующей правке копирайта.
  - [x] **Знак `+` убирается.** Он значил «эта фаза или позже» и приехал из внутреннего
        design brief, где нигде не объяснён посетителю; применён вдобавок непоследовательно
        (у 3 и 4 его нет, у 5/6/8 есть, а `Phase 4.5+` — третий смысл и вне реестра).
        **21 строка / 22 вхождения**, не 18 — и четыре пропущенных как раз те, что кусаются:
        `web.smoke.test.ts:458` (`toContain('Phase 8+')` — ЖЁСТКОЕ утверждение, падает в момент
        правки `roadmap.json`), `web.smoke.test.ts:240` (негативный гард протухает),
        `cms/payload-types.ts:680` (генерируемое зеркало `Roadmap.ts:34` — регенерировать тем же
        коммитом), `waitlist-payload.test.ts:26` (комментарий). Плюс `advantages.json:9` несёт
        ДВА вхождения в одной строке. И одна поверхность вне лендинга: `docs/index.md:86` и `:88`
        печатают подписи реестра дословно (`[Phase 5+]`, `[Phase 6+]`) — без правки дока протухнет.
        Основной список:
        `phases.json:26,33,40` и `roadmap.json:35,46,54` — **в одном коммите**, они связаны по
        подписи (`Roadmap.astro:17`), дальше tierLadder / advantages / comparison / transparency /
        waitlistCopy / `waitlistInterestOptions.ts` / `cms/globals/Roadmap.ts:34`.
  - [x] **Правка контента лендинга — правка в двух местах.** Прод рендерится из Payload,
        `fallback/*.json` работает только под `LANDING_USE_FALLBACK=1`, а `seedGlobals.ts`
        сеет if-empty. Коммит в репозиторий сам по себе прод не меняет: правка админки/сид —
        действие владельца, с записью в `CMS_PROD_CHANGES.md`.

- [x] **2.23d — правда на страницах** · docs-only, без переездов файлов; **после 2.23a**
  - [x] `migration-plans.md` — admonition (`:67-76`, считая пустую строку на `:67`: именно она
        обрывает таблицу, удаление одного `:68-76` её не чинит) вставлен **внутрь** таблицы: она
        обрывается, и последние 8 из 13 строк вываливаются сырым pipe-текстом, а заявление
        «Thirteen edits gate» (`:57`) становится ложным. Удалить блок целиком — и таблица
        срастается, и утверждение снова верно.
  - [x] Тот же «Known gap: approving a `needs.*` removal does not release the data»
        **неактуален**: 2.22b (`a4cc7ef`, operator/v0.2.45) научил оператор удалять незаявленные
        клеймы. Текст живёт на четырёх страницах — `migration-plans.md:62`, `postgres.md:143`,
        `redis.md:152`, `persistent-disk.md:136`. ADR 0051 (`:30`, `:108`) и `spec.md` (`:555`,
        `:575`) править **не надо** — вопреки тому, что я написал в первой редакции: `651c0db`
        их не трогал, они по-прежнему говорят «garbage-collected», и post-2.22b это снова правда.
        Проверку зафиксировать явно, а не молча пропустить.
  - [x] Блоки про более ранние билды не нужны до 1.0 (`:16`): `quickstart.md:305`
        (`spec.operator.enabled: false`), `choosing-the-machine.md:20`, `troubleshooting.md:276`,
        `node-prep.md:23` (вместе с его front-matter исключением `cli-check-ignore`),
        `backup-restore.md:461-479` (весь блок `### Two defects the validation runs found`, а не
        одна строка `:464`). Удаление осиротит ещё актуальный абзац про stock restic на
        `:481-489` — ему нужен свой заголовок, а не удаление.
  - [x] Фраза, дезавуирующая Amazon упоминанием Amazon (`backup-restore.md:568`, повтор `:614`).
        Сказать положительно: канонические ключи называют протокол, а не вендора.
        `:528` не трогать — там AWS S3 в фактическом перечне провайдеров.
  - [x] `Infrastructure.cue` в quickstart (`:126-151`): убрать 20-строчный `infra.cue` и
        формулировку «the right place to pin the substrate», оставив ссылку на
        `choosing-the-machine.md` §3. **Ни одного из двух вариантов владельца в CLI сегодня нет**
        (в кластере — `reference/index.md:60` фиксирует «not a cluster object»; отдельный
        инфра-репо — отозван ADR 0025 / `79c3307`), поэтому это переформулировка, а любой из
        двух вариантов = ADR-first, отдельной подфазой.
  - [x] Пикер машины: quickstart подаёт `--server-type` обязательным флагом и шлёт за выбором в
        `apprafter target machine`, хотя интерактивный `target add` показывает ту же матрицу
        последним шагом мастера (`target_wizard.rs:121`, и `choosing-the-machine.md:138` это уже
        говорит). Ведущим рецептом становится `apprafter target add prod`; флаговая форма —
        второй фенс «для CI без TTY». То же в `dev-guide/quickstart.md:200`.
  - [x] `doctor` — в преквизиты, до шага 1. Он **уже** годится: после 2.22a (`18d378d`)
        окружение проверяется безусловно, «нет таргета» — WARN и exit 0. Второй прогон на шаге 3
        остаётся: он добавляет шесть таргетных проверок.
  - [x] Проход по корпусу `bootstrap-all` → `up` и снятие ложного утверждения про
        `apprafter status` в `quickstart.md:243`. Осторожно: `help_rot_test.rs:788` требует
        сохранения конкретных литералов на конкретных страницах.

- [x] **2.23e — дока проще: порядок под флоу и распил разросшегося** · docs-only
  - [x] `mkdocs-redirects` включается **первым** и отдельно. Плагин лежит в `flake.nix:75`
        именно под это, но в `mkdocs.yml` `plugins:` его нет — сегодня любой переезд страницы
        молча 404-ит опубликованный URL.
  - [x] `backup-restore.md` — 989 строк, вдвое длиннее следующей операторской страницы. Оба
        простых пути закопаны: локальный бэкап на 89-й строке, включение S3 — на 571-й из 989,
        между ними 356 строк внутренностей restore и 248-строчный ранбук апгрейда субстрата.
        Страница росла приписыванием в конец, поэтому хронология стала порядком чтения — точная
        инверсия частоты нужды. Разбор: страница остаётся под своим именем (все входящие ссылки
        живы) и начинается с двух простых путей — «копия на моей машине» и «копия в S3»;
        restore и DR уезжают в `restore.md`; ранбук смены машины — в отдельную страницу.
        `### Two defects the validation runs found` (`:461-489`) удаляется: это запись changelog,
        она уже есть в `plan-history.md:232`.
  - [x] **Решение владельца (2026-09-05): смена машины — отдельная страница.** Операция редкая и
        не на критическом пути, поэтому она не имеет права стоять ни в `backup-restore.md`, ни
        внутри `choosing-the-machine.md`. Отсюда же берётся сокращение строк, и другого источника
        у него нет: `choosing-the-machine.md:305-445` описывает **ту же** процедуру второй раз
        целиком (141 строка, текст сам признаёт пересечение на `:368`) — эти строки переезжают в
        новую страницу, а на их месте остаётся указатель. Без этого переезда распил
        line-нейтрален и подфаза выходит в плюс.
  - [x] Заодно, раз эти секции всё равно переписываются: `backup enable --endpoint/--prefix`
        (`cli.rs:1579`) в доке **отсутствует**, и «простой путь S3» задокументирован в самой
        неудобной форме — ручной сборкой URL. Плюс `backup create --staging-mode`.
  - [x] `Node preparation` — из setup-пути в day-2. На хэппи-пасе не нужен: резервирования и
        swap кладёт cloud-init при провижене (ADR 0055, решение 7). Страница остаётся на месте
        (переезд файла = редиректы), меняется позиция в nav и вводный абзац: «это ретрофит».
  - [x] `Publish the documentation site` — из операторского гайда в Contributing. Это внутренний
        ранбук публикации **нашей** доки, включая шаг, правящий `landing/` этого репозитория;
        комментарий в nav (`mkdocs.yml:197`) защищает его тремя шагами из пяти, а два умалчиваемых
        — ровно проектные. Перечисление в другой группе nav без переезда файла: восемь входящих
        ссылок живы, `audience` в `llms.txt` переключается сам.
  - [x] nav обоих гайдов под флоу владельца. Сегодня он упорядочен по семействам артефактов:
        бэкап 15-й из 19 под заголовком «Day 2, and getting out of trouble», хотя страница сама
        говорит «Set this up before you need it»; объяснение скользящего тега — на последней
        позиции гайда разработчика, хотя это шаг 4 его флоу.
  - [x] `image-iteration.md` несёт две разные точки чтения: «что платформа делает с моим тегом»
        (до написания манифеста) и «мой деплой плохой, откатись» (после). Распил на этой границе
        закрывает шаги 4 и 10 флоу разработчика без единой новой темы.
  - [x] Две дыры флоу разработчика, где содержимого нет **вообще**: докеризация (шаг 3) и рецепт
        CI со скользящим тегом (шаг 4). Это одна непрерывная история «исходник → образ → тег»,
        поэтому одна страница, а не две; контракт из ADR 0010 плюс один настоящий пример через
        `pymdownx.snippets` (настроен в `mkdocs.yml:158`, не использован ни одной страницей),
        а не пять Dockerfile'ов, которые придётся тащить за пятью экосистемами.
  - [x] **Блокер снят, и он был настоящий:**
        `scripts/docs-artefacts-check.py:1202` (`lost_block`) требует, чтобы тело каждого фенса из
        КОММИТНУТОГО исходника нашлось среди отрендеренных блоков страницы. Инклюд `--8<--` — это
        ровно тело, подменённое между markdown и HTML, поэтому **первый же** инклюд в корпусе
        красит гейт. Плюс `docs/hooks/llm_export.py:99` сам пишет, что близнец унесёт строку
        инклюда вместо файла, и называет это «academic: no published page uses one». Обе правки
        идут отдельной задачей ПЕРЕД страницей, которая инклюд использует.
  - [x] Всё, что решено **не** писать сейчас, уезжает в честный список «Not documented yet»
        (`operator-guide/index.md:98`) — у гайда разработчика такого списка нет, завести.
  - [x] **Замер, а не намерение.** Было: `backup-restore.md` 972 + дублирующая процедура в
        `choosing-the-machine.md` = 1 459 строк на двух страницах. Стало: 296 + 212 + 213 + 342
        + 346 = 1 409 на пяти. Минус 50 строк, и — что важнее — страница первого чтения ужалась
        с 972 до 296, а оба простых пути стоят на 22-й и 58-й строке вместо 89-й и 571-й.
  - [x] `--strict` поймал пять оборванных якорей — ровно тот класс, о котором предупреждал план:
        секция уезжает на другую страницу и уносит свой `#fragment`. `mkdocs-redirects` тут не
        помогает (он не делает якоря), поэтому чинятся ссылающиеся строки.
  - [x] Что блокер был настоящим — проверено возвратом проверки к прежнему поведению: без
        резолвера `lost_block` падает на `dev-guide/build-and-push/` с сообщением, которое само
        называет вторую, тихую половину бага — «*с LLM-артефактами, собранными из исходника до
        рендера, всё ещё ручающимися за них*». То есть близнец нёс бы строку инклюда, и об этом
        не сообщил бы никто.

- [x] **2.23f — хром сайта документации** · docs-only
  - [x] Пугающий баннер на старте — это не наш дев-сервер, а протестное уведомление
        mkdocs-material 9.7.6 про MkDocs 2.0 (`material/templates/__init__.py:37`), и печатается
        оно **на каждом** вызове mkdocs, включая `docs-build` и CI-лог. Гасится одной переменной
        в devShell `flake.nix` — она накрывает все три места вызова разом.
  - [x] **Дыра в доставке, найденная при разборе:** `overrides/**` не входит в path-фильтры ни
        `.github/workflows/docs.yml`, ни `release-docs.yml`. Изменение под `overrides/` сегодня не
        гоняет ни одного гейта и не публикуется — в этой дыре уже лежит `overrides/main.html`.
        Чинится до партиала Plausible, иначе партиалу некуда доехать.
  - [x] Лого и favicon: `mkdocs.yml` не задаёт ни `theme.logo`, ни `theme.favicon`, поэтому в
        шапке стоит стоковый значок темы. Единственный отдельный знак в `docs/` захардкожен в
        `#1e293b` и на индиговой шапке нечитаем — нужен светлый вариант.
  - [x] 23 ссылки на **10** файлах (не 12) показывают имя файла с `.md` как видимый текст, хотя href
        давно без него. Заменить на заголовки страниц из nav и **завести класс в docsgen**
        (`gate.rs:249` рядом с `CODE_PATH`) — иначе вернётся следующей правкой. Чинить
        **после** переездов 2.23e: девять попаданий — на страницах, которые переедут.
  - [x] Plausible как на лендинге — **решение владельца (2026-09-05): подключать**. Штатный путь
        material — партиал `overrides/partials/integrations/analytics/` + `extra.analytics.provider`.
        Ловушка: `navigation.instant` (`mkdocs.yml:85`) переключает страницы через `pushState`,
        поэтому либо подписка на `location$`, либо history-режим в самом Plausible, **но не оба** —
        иначе каждый внутренний переход считается дважды.
  - [x] Политика приватности правится **вместе** с подключением, и правится по существу, а не
        ради галочки. Сегодня `legalPrivacy.json` описывает только `apprafter.dev` и первым
        пунктом «чего мы не делаем» говорит «No third-party trackers» — формально это уже неверно
        про лендинг. Формулировка владельца: Plausible **не трекер** в том смысле, который этот
        пункт отрицает — он не ставит куки, не строит профиль и не следит за посетителем между
        сайтами. Значит текст должен говорить это ЯВНО: cookieless-аналитика без персональных
        данных и без межсайтового отслеживания, названный процессор, оба хоста в области действия.
        Правка контента лендинга — в двух местах (репозиторий + Payload), прод — действие
        владельца с записью в `CMS_PROD_CHANGES.md`.
  - [x] Мелкая блок-схема на `dev-guide/application-cue/#field-reference`: `flowchart LR` из
        6 узлов с 212 символами подписей → естественная ширина ~2000px против ~736px колонки,
        и mermaid уменьшает **весь** рисунок вместе с текстом. Лечится в источнике (`TD` +
        короткие подписи), а не CSS: `useMaxWidth: false` покупает горизонтальный скролл.
        Тот же класс, мягче — `operator-guide/quickstart.md:193` (одна подпись в 142 символа).
  - [x] `docs/status.md` переписать. Две строки застряли в состоянии ветки («awaiting push»,
        «not yet publicly served»), команда названа легаси-именем, нет ни одной строки про всю
        программу 2.20/2.22 (девять закрытых подфаз), а три ячейки таблицы — это 1–3 килобайта
        релизной ретроспективы, которой место в `plan-history.md`. Строки ставить честно:
        ✅ только доставлено И проверено.
  - [x] Вопрос, на который разведка ответить не могла, закрыт замером:
        `https://docs.apprafter.dev/` отвечает **200**, лендинг-смоук зелёный — обе строки
        `🚧` в `docs/status.md` переведены в `✅`, а заметки про состояние ветки («awaiting
        push», «not yet publicly served») удалены. Такие состояния истекают сами, но истекать
        им нечем.
  - [x] Строка Backstage понижена `✅`→`🚧`: она и `reference/cli/open.md` не могли быть верны
        обе, и подвинулась та, что забежала вперёд продукта. Портал ставится опциональным
        компонентом чарта, но пути к нему нет — `apprafter open` умеет только Argo CD и прямо
        это говорит.
  - [x] Новый класс `link-text-path` **родился зелёным** (24 замены до него) и тут же поймал
        родственную форму, которой в находке не было: путь репозитория как подпись ссылки —
        ещё 9 мест. Плюс поймал собственную иллюстрацию в `documentation-gate.md`: страница,
        объясняющая правило, обязана назвать неправильную форму, поэтому пример переписан
        инлайн-кодом, а не живой ссылкой.
  - [x] Дыра в доставке закрыта в обеих workflow: `overrides/**` не входил ни в один
        path-фильтр, то есть изменение партиала не гоняло ни одного гейта и не публиковалось —
        а именно туда и кладётся партиал Plausible.

**Итог 2.23a–h:** восемь подфаз закрыты. Релизы `v0.2.60` и `v0.2.61` (локальные теги).
Правки живого Payload перечислены в `CMS_PROD_CHANGES.md` §C — без них половина 2.23c и
политика приватности из 2.23f на прод не доедут.

- [x] **2.23g — CI следит за версиями внешних компонентов** · CI-only, без bump
  - [x] Автоматики нет никакой: ни `dependabot.yml`, ни Renovate, ни одной из **28** workflow,
        проверяющей апстрим. ~60 внешних пинов в **восьми** синтаксисах (cargo TOML, package.json,
        Dockerfile `FROM`/`ARG`, workflow `uses:`, входы flake, CUE `version:`/`chartVersion:`,
        workflow `with: version:`, литералы `curl .../releases/download/`), большая часть — осень
        2024 в дереве от 2026-09.
  - [x] **Решение владельца (2026-09-05): свой скрипт, а не бот.** Формулировка владельца —
        «если можно бесплатными тулзами, то лучше ими; если они вообще нужны — так-то мы можем
        скриптом проверять новые версии трекуемых пакетов и пытаться по ним прогнать тесты
        (хотя бы юнит и смоук)». Renovate **отклонён**, и не по цене: он закрывает нативно 4
        класса из восьми, а остальные четыре (CUE-пины чартов, входы flake, `with: version:`,
        литералы `curl .../releases/download/`) всё равно требуют регулярок, которые мы пишем
        и сопровождаем сами. То есть мы платим за третью сторону в организации (или за PAT
        в secrets) ровно тем же объёмом кода, что и без неё, и вдобавок отдаём формат коммита,
        расписание и модель PR инструменту, который придётся отговаривать от `build(deps):`,
        от правки первопартийных версий в `platform-stack/cue/component_*.cue` и от драки
        с publish-workflow. Dependabot не рассматривается: те же 4 класса и без регулярок вовсе.
  - [x] Форма: **один инвентарь, один читатель, один отчёт.** Инвентарь пинов — данные
        (по одной записи на пин: файл, как достать текущее значение, где спросить апстрим),
        рядом с ними скрипт, который читает дерево, спрашивает апстрим и печатает таблицу
        «текущее → доступное». Плановый workflow гоняет его и держит **одну** живую issue,
        переписывая её тело, а не плодя по issue на пакет. Ноль PR на первом этапе — именно это
        и просила находка.
  - [x] Второй этап — то, ради чего это вообще стоит писать самим: **попытаться прогнать тесты
        по обновлённой версии**. Скрипт правит пин в рабочей копии, гоняет юнит и смоук того
        стека, которому пин принадлежит, и записывает в отчёт исход — «новее есть, тесты
        зелёные» против «новее есть, тесты красные» — потому что второе и есть настоящая
        стоимость обновления, и её сегодня не знает никто. Это ровно то, что бот дать не может:
        он открывает PR и ждёт, что человек прочитает CI.
  - [x] **Не заводить override версий через env.** `cli-providers/build.rs` осознанно делает CUE
        единственным источником правды на этапе сборки; правильная форма — скрипт правит пин
        в дереве и гоняет существующие гейты по правленому дереву.
  - [x] Границы: **ноль автомержа и ноль автокоммитов**. Отчёт — вход для человека. Первопартийные
        версии (`platform-stack/cue/component_*.cue`, `argocd-cue-cmp/version.cue`,
        `operator/charts/*/Chart.yaml`) в инвентарь не входят вовсе: их двигают релизные workflow,
        и «апстрима» у них нет.
  - [x] **Первый прогон нашёл то, чего не видит ни один существующий гейт.** 49 записей,
        35 отстают: cert-manager `v1.16.2→v1.21.1`, Argo CD `7.7.7→10.8.0`, Cilium
        `1.16.5→1.20.1`, Gateway API `v1.2.1→v1.6.2`, cue `v0.10.0→v0.17.1`, kubectl
        `v1.31.0→v1.37.0`, nixpkgs на 122 дня. Плюс **два дрейфа**, независимо перепроверенных:
        `azure/setup-helm` — `v3.16.4` в `release-operator.yml:311` против `v3.16.2` в
        одиннадцати других, и `ALPINE_VERSION=3.20` в `argocd-cue-cmp/Dockerfile` против
        `alpine:3.21` в `cli/apprafter-backup/Dockerfile`. Дрейф — всегда баг, независимо от
        того, есть ли апстрим новее, поэтому у него своя секция, а не строка в общей таблице.
  - [x] Правило «никаких тихих проходов» проверено ломанием, а не чтением: убитый regex у
        `chart-cert-manager` даёт `UNREADABLE` и exit 1, исчерпанный анонимный лимит GitHub
        даёт 29 записей `UNREACHABLE`, выведенных первыми, и exit 1. Ложного «всё в порядке»
        не бывает ни в одном из двух случаев.
  - [x] `scripts/*.py` добавлен в `check-spdx-headers.sh` — иначе заголовок в новом скрипте
        был бы декоративным.

- [x] **2.23h — четыре дефекта, найденные первым живым прогоном `status`** · cli v0.2.62
  Сводка из 2.23a впервые прогнана на реальном кластере. Ни один из четырёх не был виден
  юнит-тестам, гейтам и ревью, которые её пропустили.
  - [x] **Два здоровых условия сообщались как проблемы.** Чистый кластер печатал
        «2 condition(s) not healthy» про `Ready=True` и `UnauthorizedSourceModification=False`.
        Таблица полярности знала шесть типов условий, а оператор объявляет **восемь**.
  - [x] **Тест полноты, который существовал ровно ради этого, поймать не мог**: его список из
        шести взялся из того же grep'а, что и таблица, — то есть он утверждал, что сборка
        классифицирует то, что сборка уже классифицирует. Теперь читает объявления `COND_*`
        самого оператора, с полом по количеству, чтобы переименование префикса не оставило его
        судить пустой список. Проверено откатом фикса — падает.
  - [x] `status` печатал весь стор над своим же отчётом: `info!(?state, …)`, унаследованный от
        скелета, где эта структура и БЫЛА выводом. Смоук-тест теперь пинит отсутствие дампа.
  - [x] `Node disk:` мог печататься вечно из отравленного кэша: свежесть считалась как
        `now.saturating_sub(fetched_at)`, а для метки из будущего это ноль — внутри любого TTL.
        На машине лежала метка 5138 года, и баннер `Node disk: warm` печатался над каждой
        командой двое суток на ноде с 69% свободного места.
  - [x] Тир читался `(unset)` на кластере, у которого он есть: `State.tier` пишет `init`,
        а кластер, поднятый документированным путём (`target add` → `up`), через него не
        проходит. Фолбэк на тир таргета, но состояние по-прежнему выигрывает — у таргета это
        умолчание для следующего провижена, у состояния — то, что реально поднято.

- [x] **2.23i — правило записывается, иначе свип — это вкус** · ADR-first, блокер для 2.23n/o
  ADR 0058 **не говорит про размещение ADR-цитат ничего**: в нём есть строка `Explain → a separate
  mechanism layer`, а слова `citation`/`cite` не встречаются вовсе. То есть «ADR-ссылки только в
  how-it-works» — ваше расширение решения, а не его текст, и сейчас оно нигде не записано. Свип по
  ненаписанному правилу нечем судить и нечем защищать.
  - [x] Поправка к ADR 0058 (не переписывание — сам ADR требует именно поправок, `:240`): пятая
        строка маршрутизации **Cite** → назначение `how-it-works/` и корпус ADR; форма — гайд
        ссылается на страницу механики, страница механики цитирует ADR.
  - [x] Зеркало в `docs/contributing/documentation-gate.md` (таблица классов `:55-69`), чтобы
        контрибьютор встретил правило там же, где остальные.
  - [x] **Тест на абзац, применяемый по порядку, первое попадание выигрывает** — в
        `docs/contributing/documentation.md`. Разведка вывела рабочую форму: (0) ролевой гейт
        ADR 0058 `:139-144`; (1) **тест переимплементации** — «если завтра заменить реализацию,
        сохранив каждый наблюдаемый контракт, придётся ли переписать абзац?» да → механика;
        (2) **тест подлежащего** — грамматическое подлежащее читатель/его артефакт → рецепт,
        компонент платформы («оператор», «провизионер», «финализатор») → механика; годится как
        дешёвый поиск кандидатов, но сам по себе перестреливает; (3) **тест фальсификации** —
        как читатель проверит, что это правда: командой из гайда → рецепт, открыв `operator/**`
        или ADR → механика.
  - [x] **Решение владельца (2026-09-06): правило на весь файл**, а не только на счастливый путь.
        Хвостовые `Related` тоже: цитата в подвале рецепта — всё равно приглашение читать запись
        решения вместо того, чтобы доделать задачу. Корпус был сам с собой не согласен ровно здесь
        (четыре переработанные страницы зависимостей убрали даже хвостовые ссылки, а
        `egress-policy.md:109` и `dev-guide/resources-and-autoscaling.md:383` сохранили), и строгое
        чтение стало правилом.
  - [x] **И вторая половина того же решения, которая меняет маршрутизацию по существу:** ADR — это
        не более глубокое объяснение того, **как** работает, а запись того, **почему решено, что
        будет работать так**. Это на слой дальше механики и адресовано тому, кто может решение
        изменить, а не тому, кто под ним работает. Значит цитата, найденная в гайде, чаще всего
        подлежит **удалению**, а не переносу: читателю рецепта не нужны ни механика, ни
        обоснование. Три поверхности, три читателя, каждая в одном клике от предыдущей.
  - [x] Поправка к ADR 0058 написана (пятая строка маршрутизации **Cite**), тест на абзац —
        в `docs/contributing/documentation.md` («Which surface does this paragraph belong on?»)
        с рабочим примером из `postgres.md` до переработки 2.20c; зеркало в таблице классов
        `documentation-gate.md`, включая два ещё не построенных класса.

- [x] **2.23j — точечные правки, ни от чего не зависящие** · docs-only + три комментария в коде
  - [x] Дубль в руководстве по бэкапу: фраза «The backup distinguishes user material from platform
        material…» стоит дважды подряд в голове `## What a backup captures` — это моя склейка при
        распиле 2.23e, интро-предложение и первая строка перенесённого куска.
  - [x] `## What a backup captures` перечисляет объекты Kubernetes и redis и **не говорит, что
        база и том захватываются вообще** — а спрашивают именно это. Плюс не говорит, какие типы
        `needs` молча пропускаются. Сверить с `cli/backup-core/src/extract.rs`; там же поправить
        цитату строки кода, которая сейчас указывает на закрывающую скобку.
  - [x] `image-iteration.md` → `## Private images` объясняет, что нужен `SourceCredential`, и
        цитирует ADR вместо ссылки на страницу, которая показывает, как его завести. На всей
        странице ссылки на `private-repos-and-registries.md` нет ни одной.
  - [x] `dev-guide/application-cue.md` — последняя ссылка подписана «Operator quickstart», а ведёт
        на quickstart разработчика в том же каталоге.
  - [x] Три протухших комментария, все три — запись, а не сборка: обоснование в `gate.rs` для
        сканирования цитат внутри фенсов называет цитату в `postgres.md`, которой больше нет
        (фенсовых ADR-цитат в корпусе теперь **ноль**); `cb2208c` осиротил докстринг —
        однострочник `page_directory` уехал на `link_texts_naming_a_page`; комментарий в
        `docsgen/src/main.rs` всё ещё говорит, что recipe-purity держат вне `docsgen gate`,
        хотя его вкрутили в 2.20c.
  - [x] Секция «What a backup captures» переписана не подрезкой, а сменой порядка: **сначала
        данные за `needs`** — таблица по типу дependency с тем, что реально попадает в
        репозиторий (`pg_dump` базы, tar тома, whole-instance снапшот Dragonfly) и что не
        попадает, — и только потом объекты Kubernetes. И явное предупреждение, которого не было
        нигде: `jetstream`, `clickhouse`, `s3` и `notifications` **пропускаются молча**
        (`extract.rs:133`), то есть кластер, использующий любой из них, бэкапом покрыт не
        полностью, и об этом ничто не сообщает во время бэкапа.

- [x] **2.23k — порядок операторского nav и распил target-store** · docs-only
  - [x] Порядок подтверждён неверным по всем трём пунктам. `Secrets` **вверх с 16 на 9**:
        `backup-restore.md:122` уже отсылает к нему, а `:90` запечатывает SealedSecret **до** того,
        как читатель встретил запечатывание; `postgres.md` и `redis.md` тоже предполагают, что
        секреты уже есть. `Choosing the machine` **вниз с 3 на 19** — quickstart прямо говорит, что
        мастер уже сделал этот выбор и «искать заранее нечего», страница осталась справочником
        исключений. `Moving to a bigger machine` **вниз с 10 на 20** — она разрывает блок бэкапа.
  - [x] `Target store reference` не одна вещь: примерно 40% — механика (раскладка на диске,
        сериализация, цепочка разрешения кредов), 60% — рецепт (add / rotate / show / rename /
        remove). Переносить целиком нельзя, надо **делить**.
  - [x] **Рецептная половина сохраняет имя файла** `operator-guide/target-store.md` (заголовок
        меняется на «Managing targets»), и это не эстетика: `mkdocs-redirects` **не умеет якоря**
        (`mkdocs.yml:153` это прямо говорит), а `#destroy-scope` — самая опасная глубокая ссылка
        в гайде, на неё смотрят `quickstart.md`, `troubleshooting.md` и `recovery.md`. При
        сохранении имени `redirect_maps` остаётся `{}`, и ни одна из трёх ссылок не правится.
  - [x] Механическая половина → `how-it-works/the-target-store.md`, вместе с обеими цитатами
        ADR 0030. Побочно: докстринг `docsgen/src/adr.rs:114` ссылается на ту цитату как на
        источник правила склейки строк — обновить, иначе провенанс правила протухнет.
  - [x] Гейт `docs-artefacts-check.py` третий раз подряд падал голым `KeyError` на новой
        неотслеженной странице — `source` строится из `git ls-files`, поэтому собранная, но не
        добавленная в индекс страница ему невидима. Теперь он это и говорит, вместо трейсбека,
        который читается как поломка самого гейта.

- [x] **2.23l — гайд разработчика перестаёт дублировать операторский** · docs-only
  - [x] `dev-guide/quickstart.md` §1 «Register a target and bring the cluster up» пересказывает
        шаги 1–3 операторского quickstart втрое короче, и сокращение **портит порядок**: `doctor`
        оказывается после того, как платный сервер уже создан — ровно та ошибка, которую 2.23d
        починила на операторской стороне. Секция становится ссылкой.
  - [x] То же в §5 про домен: пересказ `connect-a-domain.md` и `cloudflare-origin-cert.md` с
        потерей шага DNSSEC, ожидания статуса зоны Active, параметров сертификата и способа
        выбраться из блокировки фаерволом.
  - [x] Проверено и снимает опасение: `GUIDE_ALIASES` в `help_rot_test.rs` **не пинит** ни одного
        литерала к этой странице, так что обе секции свободно превращаются в ссылки.
  - [x] Обе секции теперь ссылки, и §1 честно называет, почему копия была не просто лишней:
        сокращение уже разошлось с оригиналом — `doctor` в нём стоял **после** того, как платный
        сервер создан, ровно та ошибка, которую 2.23d починила на операторской стороне.
  - [x] §5 разделена по владельцу, а не сокращена: кластерная половина (фаервол, сертификат,
        регистрация зоны, DNS) уходит целиком в операторские ранбуки, у разработчика остаётся
        одно поле `expose` и проверка. Плюс предупреждение, которого не было: переход в `public`
        и добавление хоста — гейтящиеся правки.

- [x] **2.23m — MigrationPlan доезжает до разработчика** · docs-only
  - [x] Строка `MigrationPlan` не встречается в `docs/dev-guide/` ни разу, а все тринадцать
        app-scope триггеров — это правки, которые разработчик вносит в **свой собственный
        манифест**. Из-за этого три страницы гайда разработчика активно врут о том, что читатель
        увидит после правки, которая гейтится.
  - [x] Важное уточнение, чтобы не искать несуществующий долг: в плане 2.23e этого **не было** —
        `D10` там означает шаг 10 флоу (rollback), и он построен. Это неспланированный пробел, а
        не невыполненный пункт. (Единственное невыполненное из 2.23e — секция «Not documented yet»
        в `dev-guide/index.md`.)
  - [x] Новая короткая страница (60–80 строк) в конце блока «опиши своё приложение», до шагов
        отгрузки: таблица тринадцати, переписанная как **правки читателя** («вы убираете
        `needs.pg`», «вы ставите `network: public`»), а не как имена триггеров; что он увидит
        (`AwaitingMigrationApproval`, плитка Degraded в Argo CD); что reject для app-scope нет и
        обратный ход — revert в Git; кто одобряет. Механику одобрения не дублировать — ссылка на
        операторскую страницу.
  - [x] Таблица тринадцати переписана как **правки читателя**, разбита на «что-то убирают» и
        «что-то добавляют, и это расширяет доступ» — вторая группа и есть неожиданная. Плюс явный
        список того, что НЕ гейтится, потому что «поменял тег» — самая частая правка вообще.

- [x] **2.23n — сам свип: механика уезжает в how-it-works** · docs-only, самый большой; **после 2.23i**
  - [x] Масштаб измерен: **53 из 90** ADR-цитат корпуса (59%) сидят в двух рецептных деревьях, на
        32 страницах; 34 из них — на счастливом пути, 19 — в хвостовых списках; три страницы дают
        24 из 53. Механика вкраплена во все страницы `dev-guide/` кроме `build-and-push.md`;
        `application-cue.md` — примерно 40% механики по строкам, `image-iteration.md` с неё
        начинается.
  - [x] Шесть новых страниц `how-it-works/`, в порядке «дешевле и изолированнее — раньше»:
        резервирования и swap ноды; shared volumes; и — отдельно — **`gitops-and-the-cue-cmp.md`,
        которой сейчас нет вовсе**: именно она забирает пассаж про shell-пробу CMP из
        `application-cue.md` → `## The canonical filename`. От секции остаётся одно повелительное
        предложение и ссылка. Вместе с пассажем переезжает и front-matter исключение
        `schema-check-ignore` — оставленное на странице, которая больше не называет
        `spec.source.path`, оно станет мёртвым, а `exemptions` в цензе сверяется в обе стороны.
  - [x] Маршрутизация всех 53 цитат разобрана поимённо: **5** — механика, переезжает целиком;
        **13** — простое упоминание, снимается (осторожно: `migration-plans.md:52` — это H3, снятие
        меняет якорь); **35** — настоящий указатель, становится ссылкой на страницу механики, но
        **у 29 из них назначения ещё не существует**. Это и есть порядок работ: страницы прежде
        ссылок.
  - [x] **Ловушка снята первой, отдельным коммитом:** `behaviour-claim` — единственный класс
        гейта, проверяющий не имена, а истинность утверждений, — **не выполняется на
        `docs/how-it-works/`**. Мы собираемся перенести туда каждое поведенческое утверждение
        корпуса, то есть в единственное дерево, которого эта проверка не видит. Чинится
        перестановкой двух строк в `gate.rs`: `is_guide` продолжает гейтить recipe-purity (это
        верно — в how-it-works kubectl уместен), а цикл `behaviour::falsified` поднимается выше
        охранника.
  - [x] Блокер снят вместе с распилом node-prep: `kubectl rollout restart` учит `kubectl rollout restart` на
        рецептном пути без page-level исключения — recipe-purity выстрелит, как только кто-нибудь
        прогонит гейт красным. Либо схлопнуть, либо записать как разрешённый ADR 0058 `:151`
        случай.
  - [x] **Ход свипа.** Готовы три страницы механики из шести: GitOps + CUE-плагин (её не было
        вовсе — именно она забрала пассаж про shell-пробу), резервирования и swap ноды, и —
        в 2.23k — хранилище таргетов. Цитаты: **91 → 67**, то есть 24 снято или перенесено.
        Из оставшихся семи в гайдах шесть ждут своей страницы механики (shared volumes, бэкап,
        апгрейд платформы ×3, per-env deploy, digest-резолвинг), а `migration-plans.md` с её
        одиннадцатью — отдельный кусок: страница на 60% механика.
  - [x] **Свип закрыт.** Восемь страниц механики вместо запланированных шести: GitOps + CUE-плагин,
        резервирования и swap, хранилище таргетов, гейт одобрения, апгрейд платформы, cross-app
        shared volumes, порядок восстановления, per-env deploy, digest образа. Цитат в гайдах
        **ноль**, кроме двух на `publish-the-docs-site.md` — а она сама переезжает в how-it-works
        в 2.23p, где цитаты законны. По корпусу 91 → 63.
  - [x] Восемь распилов, и ни один не потерял якорь незамеченным: `--strict` ловил их четырежды
        (target-store ×2, restore, per-env, shared-volumes ADR-путь), и каждый раз это была
        секция, уехавшая вместе со своим `#fragment` — тот самый класс, который
        `mkdocs-redirects` не покрывает и покрыть не может.
  - [x] **Три верхних бокса закрыты 2026-09-07 после проверки по дереву, а не по памяти** (они
        описывали работу, которая была сделана, но так и остались неотмеченными). Проверено:
        девять страниц механики на месте; исключение `schema-check-ignore` на `spec.source.path`
        действительно уехало вместе с пассажем и живёт во front matter
        `gitops-and-the-cue-cmp.md` — то есть мёртвым не стало; в обоих деревьях гайдов **ноль**
        цитат `ADR NNNN`. Оговорка про H3 `migration-plans.md` тоже отработала — на её якорь
        ничего не ссылалось.
  - [x] **Предел слова «закрыт», записанный честно.** Свип закрыл **маршрутизацию цитат** — это то,
        что он и мерил. Механика как таковая из гайдов уехала **не вся**: состязательный аудит
        закрытий (2026-09-07) нашёл пять гайдов, которым некуда сослаться, потому что их страницы
        механики не существует, плюс целиком записанное решение в `recovery.md` и самую плотную
        оставшуюся механику в операторском quickstart. Этот долг — **2.25**, и он именно
        продолжение этого свипа, а не новая тема.
  - [x] Второй пустой заголовок того же класса, найденный тем же аудитом: `## Acting on a plan`
        в `migration-plans.md` стоял без тела, сразу за ним `## CLI surface`. `--strict` такое не
        видит: якорь существует, ссылок на него не было, а читатель приходил в пустоту. Заголовок
        получил своё предложение, `CLI surface` ушёл под него.

- [x] **2.23o — два класса гейта, оба рождены зелёными** · после 2.23n
  - [x] Измерено: 89 цитат, из них **49 ссылок, 37 голым текстом, 3 в бэктиках**, ноль внутри
        фенсов. `adr.rs` голый текст **видит** (это его основная грамматика), но форму отбрасывает —
        `AdrRef` несёт только строку и номер, — поэтому правило «цитата обязана быть ссылкой»
        расширением `adr-reference` не делается: рядом с ним встал отдельный сканер `adr_citations`,
        который написание сохраняет.
  - [x] `adr-citation-form` — 26 голых цитат превращены в ссылки одним коммитом. Литеральный
        показ — фенс и инлайн-код — из класса исключён: там страница **демонстрирует** написание,
        и ссылка сломала бы демонстрацию (список классов на странице гейта — ровно этот случай).
        От `adr-reference` класс на фенсах расходится сознательно, и это записано на месте вызова:
        тот спрашивает, стоит ли решение до сих пор (верно и внутри фенса), эти два — может ли
        читатель по нему перейти.
  - [x] `adr-citation-placement` — **родился зелёным, вопреки плану**. План закладывал recipe-purity
        образец (класс есть, в `just lint` не входит) под 29 цитат, которые некуда переносить.
        Слой механики из 2.23n забрал их все, а последние две в `publish-the-docs-site.md`
        перенаправлены на `how-it-works/the-image-digest.md` и на разобранный пример 2.23p — так что
        отдельная подкоманда отчёта не понадобилась, и оба класса сразу в `just lint`.
  - [x] Шесть тестов, обе стороны каждого правила; размещение проверено мутацией (`is_guide` →
        `false` роняет ровно его тест). Перепись пересчитана в том же коммите: `adr_references`
        63 → 59. Две цитаты ушли из гайда на страницы механики; ещё две — повторы того же номера
        абзацем ниже, которые при переносе строк свернулись в «тот же ADR несёт и это».

- [x] **2.23p — деплой как разобранный пример** · docs-only
  - [x] Ранбук публикации — пять шагов, из которых четыре родовые («приложение на поддомене») и
        только пятый специфичен для этого проекта; пятый уже выполнен, то есть страница сейчас —
        запись о завершённом деплое, а не инструкция.
  - [x] Две новые страницы в `how-it-works/`: как задеплоен сайт документации и как задеплоены
        лендинг с CMS. Именно туда, а не в Contributing: (а) это единственное дерево, где ваше
        правило разрешает ADR-цитаты, а обеим страницам они нужны; (б) `is_guide` — префикс пути,
        поэтому страницы там вне recipe-purity и могут показать настоящие `docker build`, `curl` и
        `kubectl`, которых пример требует, без исключений.
  - [x] Каждая страница: что именно деплоится и где живёт; **таблица «что за что отвечает и откуда
        берётся»**, по строке на звено цепи, в каждой ячейке — реальный путь; `--8<--` инклюд
        настоящих `Application.cue` и `Caddyfile` вместо копий (механизм заработал в 2.23e);
        схема триггеров; **и осознанные отсутствия** — чего в манифесте нет и почему.
  - [x] Цепочка лендинга — пять потоков образов, четыре workflow, три манифеста и **ни одной
        публичной страницы**; единственные сквозные описания это `landing/DEPLOY.md` и два
        gitignored русских файла в корне. Отсюда три фактические нестыковки, которые страница
        унаследует, если писать её с них: `DEPLOY.md` описывает host-деплой, который кластер уже
        заменил; preview-манифест запинен на неверный тег; и код-путь, и контент-путь пишут
        `:latest`. Их надо решить до написания, а не после.
  - [x] Список редакции — обязателен: креды, внутренние хостнеймы, всё, что запрещает правило
        гигиены `docs/`.
  - [x] Две из трёх нестыковок оказались **продуктовыми, а не текстовыми**, и записаны как
        `D30`/`D31` в `docs/measurements/day2-followups.md` вместо того, чтобы попасть на
        публичную страницу: preview-манифест лендинга запинен на `:latest` — тот же тег, что
        смотрит прод, — то есть хост предпросмотра сегодня показывает то, что уже опубликовано,
        и шаг инспекции, ради которого построена вся цепочка промоушена, инспектирует живое;
        и `landing/DEPLOY.md` описывает systemd+podman деплой, который кластер заменил.
        Обе — правки живого деплоя, поэтому за владельцем.
  - [x] Третья чинилась безопасно и починена: комментарий в манифесте CMS утверждал
        «DATABASE_URL auto-injected via needs.pg claim» прямо под строкой, которая связывает его
        явно, — автоинъекцию убрали в 2.12. Страница включает этот манифест дословно, так что
        публикация унесла бы и противоречие.

- [x] **2.23q — вернуть 2.21 то, чего у неё нет: план** · передача, не исполнение
  - [x] **План написан:** `docs/superpowers/plans/2026-09-06-2.21-public-feature-ledger.md`
        (дерево gitignored — файл только на этой машине). Пять задач = 2.21a–d + закрытие.
        Три решения, которые спека оставила открытыми, приняты в плане с доказательствами:
        **D1** `Post-launch` не получает записи в реестре и его ячейки не становятся ссылками —
        оба варианта спеки неверны (свернуть в `managed` = ложь: обе строки OSS-core на кластере
        клиента; отдельная запись = ссылка в никуда, потому что `Roadmap.astro` рисует карточку
        на запись CMS-контента, а не реестра, и кнопка живёт на карточке). **D2** второй словарь
        меток — `◆` в силе сегодня / `◇` ещё нет. **D3** две строки (#17, #18) выражены целиком
        внутренним словарём, публичная формулировка предложена и вынесена на приёмку.
  - [x] **Поправка к посылке:** переписывание статусной страницы принадлежит **2.21b**, а не 2.22 —
        2.22 про эту страницу не говорит вообще. То, что вы помните как правку, — 2.23f, и она
        была проходом на честность, а не переписыванием.
  - [x] Спека **жива и полна**: `docs/superpowers/specs/2026-08-29-public-feature-ledger-design.md`,
        290 строк, с §5 «The work», разбитым ровно на 2.21a–d. **Плана по ней нет** — подфаза
        остановилась на границе spec → plan. Осторожно: дерево `docs/superpowers/` gitignored,
        спека существует только на этой машине.
  - [x] Состояние страницы измерено и не изменилось структурно с базовой линии FEATURE_TRACKER:
        205 строк, 11 таблиц, 47 строк данных, **те же пять колонок в каждой таблице**, 8 секций
        `## order N`, **965 слов прозы в блоквотах** в 11 блоках, **ноль строк со ссылкой или
        якорем**. Корневой `FEATURE_TRACKER.md` был сокращён до пятистрочного указателя тем же
        коммитом, который опубликовал страницу, — то есть `/status/` буквально и есть
        опубликованный трекер.
  - [x] Честный учёт того, что 2.23f сделала здесь **хуже**: она добавила десять строк в старой
        пятиколоночной форме и свежее описание внутреннего процесса — то есть слегка увеличила
        объём работы 2.21b.
  - [x] Шаг 2.23: не переписывать страницу здесь, а **написать план по выжившей спеке** и поднять
        2.21. Заодно 2.21d сейчас нестартуема как записана: с лендинга на `/status/` не ведёт ни
        одна ссылка, ни одна ячейка Phase не является якорем, и метка `Post-launch` в реестре
        по-прежнему без записи.

### 2.24 Гайд не отправляет читателя в исходники

> 🏁 SR: A · order 3.7b — правило владельца от 2026-09-06, поверх ADR 0058

> ✅ **ЗАКРЫТО 2026-09-06** (docs-only + класс гейта; без бампа CLI и монорепо-тега).

**Правило (владелец):** «в гайдах такие ссылки должны вести только на страницы из How it works,
а там уже можно на исходники ссылаться, но лучше на ADR, т.к. код может меняться». Записано
поправкой к ADR 0058 «where the repository may be named».

- [x] **2.24a — ссылки.** 11 ссылок на 7 страницах гайдов. Семь удалены, а не перенаправлены:
      пять указывали на `schemas/v1alpha1/application.cue` как «authoritative field list», куда
      переносить **нечего** — `docs/reference/index.md` прямо говорит, что генерируемого field
      reference нет, а таблица полей самого гайда и есть опубликованный список; хуже, схема
      объявляет `jetstream`/`clickhouse`/`s3`/`notifications`, которые статусная страница держит
      вне объёма, то есть ссылка приглашала написать ключ, который скомпилируется и никогда не
      провизится. Две ссылки на `spec.md` — роадмап, намеренно не опубликованный, — отправляли
      оператора к непостроенным возможностям.
- [x] **2.24b — кодовые спаны.** Второй проход нашёл ещё 16 в формах, которые правило про ссылки
      не видит. Худшие: путь в чарт **с номером строки**; целая публичная секция
      «The executable version» про e2e-скрипт, который читатель не может запустить и который
      тратит реальные деньги; два раст-типа как авторитет для цепочки, где соседние слова уже
      называли те же ступени по-человечески.
- [x] **2.24c — класс `code-reference-placement`.** Две поверхности, судимые по-разному:
      **ссылка** — адресат, которого у сайта вне `docs/` нет, поэтому запрещена любая;
      **спан** — имя, которое читатель грепает, и часть таких имён законно его
      (`docs-site/Dockerfile` в ранбуке публикации). Спан ловится, только если открывается на
      дереве реализации, а это множество = git-листинг минус allowlist читательских деревьев,
      то есть новая директория покрыта по умолчанию. Одиннадцать тестов; обе половины проверены
      мутацией.
- [x] **2.24d — ссылки на рабочие примеры.** Вторая просьба владельца в том же сообщении.
      Семь мест, каждое привязано к моменту, когда читатель пишет манифест: таблица полей,
      гайд по окружениям, скаффолд и первый деплой, пара declare-and-bind в pg-гайде, три формы
      `env` в гайде по секретам, блок `expose` в гайде по домену, и раздел в индексе dev-гайда.
      Ещё 14 кандидатов отклонены — в основном потому, что ни один пример не объявляет то,
      о чём та страница.
- [x] **Дефект на самой странице примера,** найденный тем же проходом: оба манифеста лендинга
      несут блок `environments`, а страница о них молчала, при этом её единственная проза про
      «вторую копию приложения» — про отдельный preview-манифест. Читатель мог прочесть
      `environments.dev` как preview-хост. Страница теперь их разделяет.

**Порядок 2.23i–q.** `2.23i` первая и одна: без записанного правила свип нечем судить.
`2.23j`, `2.23k`, `2.23l`, `2.23m` ни от чего не зависят и идут когда угодно. `2.23n` — после
`2.23i`, и ловушку с `behaviour-claim` снимает внутри себя, а не после. `2.23o` — после `2.23n`,
потому что классу размещения нужны существующие назначения. `2.23p` независима. `2.23q` —
передача, исполнение уходит в 2.21.

### 2.26 `e2e-gateway-nightly` — красный 55 ночей подряд

> 🏁 SR: A · order 3.7c — найдено 2026-09-10 по указанию владельца

> ✅ **ЗАКРЫТО 2026-09-10** (e2e + docs; без бампа CLI и монорепо-тега). Прогон **зелёный
> локально дважды** в rootful microVM: 12м59с на первой редакции, повторно — после правки
> четырёх дефектов, которые состязательная проверка нашла в моём же коде.

- [x] **Диагноз: продукт был прав, прогон устарел.** Фаза 9 патчит `expose.network`
      `public → internal` и сразу ждёт прунинга HTTPRoute. Но 2.16b (коммит `90664d7`,
      **17 июля**) научил оператора гейтить деструктивную правку: он заводит `MigrationPlan`,
      встаёт на паузу — и маршрут **правильно** остаётся. Последний зелёный прогон — 17 июля,
      первое падение — 18-го. **55 падений за 55 дней.** Ни одна ночная джоба ни о чём не
      уведомляет, поэтому это никого не разбудило.
- [x] **Фаза 9 переписана и теперь покрывает больше, чем до поломки:** гейт срабатывает
      (`AwaitingMigrationApproval` + `MigrationPending=True`); **один** план гейтит **оба**
      кандидата (`domain-change` + `network-visibility-change` — один патч, два триггера,
      заголовок выигран по алфавиту, а не по важности); маршрут **держится и продолжает
      отдавать 200** всё окно удержания; одобрение потребляет план; и только тогда маршрут
      прунится, `endpointURL` переворачивается точным сравнением, а анти-луп проверяет, что
      гейт не перевзвёлся.
- [x] **Четыре ловушки, которые проверка сняла до прогона:** утверждение про исчезновение
      `PublicRouteReady` стало бы **тавтологией** — пауза пишет ровно два условия и заменяет
      весь массив, так что оно исчезает в момент гейта, а не прунинга; ожидание
      `.status.phase == pending-approval` на свежем плане **зависло бы на весь таймаут** (у
      плана вообще нет status, фаза дефолтится при чтении); имя плана нельзя собрать — в нём
      unix-таймстамп, ищется по лейблам, а прогресс обязан идти в stderr, иначе он попадает в
      `$(...)`; и `apprafter migration approve` здесь не работает — CLI резолвит kubeconfig
      через target-store, которого этот прогон не создаёт.
- [x] **Четыре дефекта в уже применённом коде,** найденные тем же ревью: сравнение
      `endpointURL` могло пройти на двух пустых строках; проба удержанного маршрута была
      однократной там, где фаза 7 ретраит до 18 раз — любой транзиент печатал бы «перестал
      отдавать, пока правка лишь удерживалась», то есть ложное падение, читающееся как
      регрессия продукта, в той самой ночной джобе, которую 55 раз прочли неправильно; проба
      не проверяла HTTP-код; и мой же комментарий обещал защиту от SIGPIPE, которой код не
      имел — `printf | grep -q` та же форма конвейера. Исправлено, включая идентичное место в
      фазе 7.
- [x] **Break-glass в гайде молча не работал.** `docs/operator-guide/migration-plans.md`
      документирует одобрение без `--subresource=status`. CRD объявляет статус-субресурс, поэтому
      apiserver **отбрасывает** stanza, `kubectl` печатает `patched` и выходит 0, план не
      двигается — а контроллер припаркован на `await_change()` и без мутации не просыпается.
      Путь последней надежды, отказывающий бесшумно. Плюс дописано, что нужен cluster-admin:
      не-админу разрешён только переход `pending-approval → approved`, а у свежего плана
      старая фаза пустая, то есть первый же write ему недоступен.
- [x] **Заголовок workflow врал про сборку** — утверждал «No Rust toolchain, no cargo», тогда
      как фаза 6 собирает оператор и вебхук и это половина времени прогона.
- [x] **Комментарий про GatewayClass винил «ненадёжность Cilium».** Механизм другой и
      детерминированный: Cilium 1.16.5 вендорит gateway-api v1.1.0 и пишет `supportedFeatures`
      строками в **том же** status-апдейте, что несёт условие Accepted; закреплённые CRD v1.2.1
      требуют объектов, apiserver отклоняет апдейт **атомарно**, и Accepted не может лечь
      никогда. Ошибка в логе cilium-operator ожидаема и к диагностике падений отношения не
      имеет — теперь так и написано, потому что она стоила этому расследованию времени.

- [ ] **Ночные прогоны никого не уведомляют** — за это 55 красных ночей прошли молча. Минимальная
      починка (джоба заводит/обновляет issue при падении) требует `issues: write`, а заголовок
      этого workflow отдельно фиксирует `permissions: contents: read` как минимум и несёт security-
      note. Расширение прав — решение владельца, не агента. **Вынесено, не сделано.**
- [ ] **Кнопка Approve в Argo CD, вероятно, не работает ни в одном scope** — и ни один прогон её
      не проверяет. У свежего плана `old_phase` пустая, а RBAC-биндинг к `cluster-admin` не даёт
      `system:masters`, поэтому короткого замыкания `is_operator_or_admin` не будет. Проверяется
      одной командой на живом кластере с висящим планом (клик → `kubectl get migrationplan -o
      jsonpath='{.status.phase}'` + лог вебхука): фаза не двинулась и в логе отказ — проблема в
      identity/FSM; фаза не двинулась и тишина — Argo вообще не трогает субресурс. **За владельцем:
      к живым кластерам агент не ходит.**

### 2.25 Долг, вскрытый состязательным аудитом закрытий

> 🏁 SR: A · order 3.7b — открыто 2026-09-07

**Как найдено.** После закрытия 2.21/2.23/2.24 запущен состязательный аудит: пять критиков,
каждый пытается **опровергнуть** то, что закрытия утверждают, плюс независимый проход
подтверждения. 94 находки, 77 подтверждены, 1 опровергнута. Сильнее всего сработал критик,
который шёл по документации **как посторонний, желающий задеплоить приложение**, а не как
ревьюер утверждений — ровно тот проход, который гейты структурно сделать не могут.

- [x] **Закрыто сразу** (коммиты `6d4143f`, `33a14eb`, `f361031`, `6df56d1`): мина в
      `check-cli-version-bump.sh` (сломала бы пуш); ссылки Phase 5/6/8, **сломанные на проде**
      из-за джойна по лейблу; шесть пунктов, которые лендинг продаёт, а ледгер звал
      незапланированными; строки-свойства с кнопкой подписки вопреки собственной легенде;
      блокквот про Backstage, обе половины которого опровергнуты кодом; README с `Phase 5+`;
      смоук-рецепт, который не мог сработать; отсутствие гейта одобрения в §2.5; пустой якорь,
      на который вела глубокая ссылка; три оборванных маршрута гайд → механика.
- [x] **Слой механики достроен.** Пять новых страниц — `sealing-a-secret`, `source-credentials`,
      `the-public-route`, `what-cluster-bootstrap-installs`, `backup-retention-and-checks` — плюс
      раздел про ёмкость, уехавший в `cross-application-shared-volumes`. **Маршрут цел в обе
      стороны:** 22 страницы механики, ни одной недостижимой из гайдов, ноль гайдов без выхода в
      слой механики.
  - **Как писалось.** Черновик каждой страницы делал агент, обязанный подкрепить **каждое**
    несущее утверждение файлом и строкой и отдельно перечислить, чего доказать не смог. Затем
    другой агент пытался опровергнуть, открывая файлы. Вердикты: шесть из шести
    «применить с исправлениями», 33 исправления суммарно. Это окупилось: страница публичного
    маршрута противоречила спецификации Gateway API, на которую опиралась (`*.example.com`
    матчит и `a.b.example.com` — верно то, что wildcard никогда не матчит apex); страница
    бэкапов говорила «два потолка» в трёх местах и перечисляла три; страница бутстрапа
    ссылалась на нумерацию, которой ниже нет.
  - **Найденные попутно противоречия в самом продукте:** справка `secret seal` утверждала, что
    `upgrade-tier` переносит запечатанное, тогда как команда печатает, что апгрейды тиров не
    реализованы; **ADR 0049 §6** описывал `CapacityWarning` как долю свободного места на **ноде**,
    хотя 2.22d перенаправил его на использование самого тома, а сигнал ноды — в `NodeDiskPressure`.
    Оба починены в тех же коммитах.
  - **Одно предложение верификатора отклонено.** Правка снимала типизированный, датированный и
    посчитанный в цензе маркер `check=none` и возвращала тот же `kubectl` инлайн-прозой, которую
    `recipe-purity` не видит по собственному решению. Это обмен проверяемого исключения на
    известную слепую зону. `kubectl` убран из гайда совсем: «когда началось» — это диагностика,
    а страница механики называть `kubectl` вправе.
- [x] **`recovery.md` несёт целиком записанное решение** — заведён **ADR 0060** «no emergency root
      password on a Tier-1 node». В ADR ушли и два варианта, которых гайд не называл (пароль,
      выдаваемый по требованию через API провайдера; и полное отсутствие rescue-пути), и риск,
      который он всегда подразумевал: на T3/T4 «пересобрать за три минуты» перестаёт быть правдой.
      Гайд оставил то, что нужно человеку в беде: консоль не пустит, это намеренно, вход — rescue,
      и консоль всё равно стоит открыть, потому что boot-вывод приходит раньше SSH. Цитаты в гайде
      нет: гайд утверждает, а не аргументирует.
- [x] **`operator-guide/quickstart.md` — самая плотная механика из оставшихся** — уехала в
      `what-cluster-bootstrap-installs.md` вместе с разделением, на которое у quickstart не было
      места: что CLI ставит сам, а что приезжает компонентом чарта под Argo CD. Таблица
      закреплённых версий не воспроизведена, а заменена тем, **откуда версия берётся**, — таблица
      версий на странице для читателя, которому нужны три команды, гниёт по определению.
- [x] **Секция ёмкости в `shared-volumes.md`** — перенесена в
      `cross-application-shared-volumes.md`, и обратная передача темы («читайте про сигнал
      ёмкости в гайде») снята: теперь оба конца указывают в правильную сторону. Секция вдобавок
      была устаревшей — описывала `CapacityWarning` как долю свободного места на ноде.
- [x] **`## Prerequisites` стоит предпоследним** — перенесён выше первого шага на трёх страницах
      зависимостей. Формулировка «verification blocks above» переехала вместе с блоком и стала
      «below». (`connect-a-domain.md` не входила: у неё подготовка кластера и так первым шагом.)
- [x] **Директивы `<!-- docs: … -->` уезжают в публикацию** — сняты в обеих независимых
      реализациях (`llm_export.py` и `docs-artefacts-check.py` выводят правило порознь, потому что
      побайтовое сравнение — «исходник против ожидания», а общий хелпер был бы хуком, который
      подтверждает сам себя). Правило: **целая строка вне фенса**. Половину про фенс написала не
      предусмотрительность — первая версия вычистила строку внутри фенса на странице, которая
      этому синтаксису учит, и артефакт-чек поймал это за один прогон. На живой сборке осталось
      два вхождения, оба — демонстрации на самой обучающей странице.
- [x] **`READER_FACING_TREES` слишком щедр** — измерено: `docs-site/`, `landing/`, `scripts/`,
      `.github/` использует **только** эта страница, все девять спанов на ней. Четыре дерева,
      открытые по всему корпусу ради одного файла, — правило, переставшее значить то, что говорит.
      Список сужен до того, к чему читатель гайда действительно прикасается, а страница
      исключена **по имени** и только из этого класса. Файл не переносили: 2.23e решила это
      сознательно, чтобы внешние ссылки жили.
- [x] **Лендинг продаёт как Phase 3 то, что уже отгружено** — это не маркетинговая правка, а
      исправление ложного утверждения: ледгер держит ту же строку как `Shipped ✅`, и читатель,
      пришедший по ссылке, получал два ответа на один вопрос. В fallback снято; живой CMS выписан
      в `CMS_PROD_CHANGES.md`. Automatic DNS остаётся Phase 3 — он и правда не отгружен.

**Вне объёма 2.23:** `Infrastructure` как объект кластера и GitOps-репозиторий инфраструктуры
(оба = ADR-first, см. 2.23d); полный перенос трёх `needs`-рецептов в гайд разработчика (переезд
файлов ради ролевой чистоты — не то, что просил проход); автомерж любых bump'ов.

---

### 2.27 Манифест — это бандл: CLI видит все воркloadы пакета

> 🏁 SR: A · order 3.7d — открыто 2026-09-14 по замечаниям владельца (`cli-fixes.md`)

**Как найдено.** Владелец положил два приложения в один `Application.cue`, поставил в кластер,
увидел в кубе поды обоих — а в `apprafter app list` одно. Разбор показал, что рендер и оператор
multi-app держат полностью (у cue-cmp даже есть фикстура `inject-fixture-multi`), а схлопывает
**только CLI**, в трёх местах «первый побеждает». Причём это не гипотеза: `landing/web/apprafter/`
в этом репозитории **уже** двухворкloadный бандл (`landingWeb` + `landingWebPreview`), и по
порядку имён prod выигрывает каждый из трёх путей — значит `landing-web-preview` недостижим из
CLI вообще.

**ADR:** 0062 (бандл как единица), 0063 (discovery + проверки консистентности), 0064 (`app restart`).
**Планы:** `docs/superpowers/plans/2026-09-14-2.27{a..f}-*.md` — шесть независимых подсистем,
которые **релизятся одним паком**, но планируются и исполняются порознь.

- [x] **a. cue-cmp: discovery по repo-relative пути + контенту, перечисление каталогов-пакетов
      вместо жадного `cd`, отказ на двусмысленную регистрацию и на неконсистентный бандл.**
      Измерено: четыре раскладки сегодня либо тихо прунят
      приложение, либо краснеют, либо рендерят 1 документ из 2 без предупреждения. Гейт на
      абсолютном `$PWD` запрещён — базу рабочего каталога CMP задают `ARGOCD_CMP_WORKDIR` и
      `TMPDIR`. Плюс `scripts/check-cue-cmp-mirror.sh`: зеркало `plugin.yaml` в
      `component_argocd.cue` монтируется **поверх** копии в образе, и ничто их не сверяло.
      Поставлено в argocd-cue-cmp 0.1.26 / platform-stack 0.2.72 (ADR 0063): вложенный каталог
      сворачивается в объемлющий пакет и называется на stderr, сиблинги под одной регистрацией —
      отказ; publish-workflow чарта теперь ждёт образ сайдкара, который чарт пинит.
      `apprafter app validate` двойников этих четырёх проверок пока не несёт — это `b`, поэтому
      отказ сознательно на него не ссылается. test-discover 5 → 20 кейсов, test-inject 43 → 105
      утверждений, всё проверено ещё и на busybox ash 1.36.1 (`alpine:3.20`).
- [x] **b. CLI: бандл-модель.** `parse_applications`, плюральный резолвер вместо
      `find_apprafter_app_name` (шим в итоге удалён, а не оставлен рядом), `app_index`, единая
      адресация `--workload <name>` при позиционном аргументе-регистрации (ADR 0062), переработка
      `list`/`status`/`remove`/`logs`/`open`/`rollback`/`unpin`, кардинальная pin-aware ячейка
      HEALTH, чтение `status.conditions[]` Argo в `app status`, локальный двойник проверок бандла
      в `app validate`, отказ на `--path` в файл.
      Плюс фикс `argo_app_selector` в `restore.rs` — он сравнивает имя CR с лейблом, чей смысл
      «имя регистрации», из-за чего `restore --data-only` грузит Postgres под живыми подами.
      Поставлено тремя планами: b-1 → `v0.2.67`, b-2 → `v0.2.68`, b-3 → `v0.2.69`.
- [x] **c. `app restart`** (ADR 0064). Прежний отказ был рамкой объёма подфазы 2.20 и уже отозван
      поимённо в `day2-followups.md` для случая ротации кредов. Измерено на kind: аннотация
      pod-template от CLI переживает SSA оператора и не откатывается Argo.
      Поставлено: частичный SSA только на `spec.template.metadata.annotations`, запись — на CR
      (расщепление ADR 0059). Три отказа: пауза миграции НЕ блокирует (роллится уже применённый
      шаблон, протолкнуть загейченное изменение нельзя — печатается строка об этом); ноль реплик
      отказывает, а не рапортует успех; регистрация, чей git рендерит Deployment напрямую,
      отказывает (Argo CD владеет объектом и откатил бы аннотацию).
- [x] **d. Namespace.** `secret seal` предлагает создать с подтверждением (молча — никогда,
      в том числе под `--yes`: блоб привязан к namespace необратимо); `secret remove` перестаёт
      печатать `✓ Removed` для того, чего не было; `secret list -n` и `volume list/status/remove`
      перестают читать отсутствующий namespace как пустой.
      **`app add` уже поднимал namespace сам** — Argo CD делает это через `CreateNamespace=true`
      в syncOptions, пункт был записан из неверной посылки. Реальный дефект оказался соседним:
      destination namespace и `metadata.namespace` манифеста никем не сверялись, а Argo CD создаёт
      первый и применяет воркloadы во второй, не примиряя их, — то есть при расхождении появляется
      пустой namespace, а приложение едет в тот, которого никто не создавал. Визард их
      синхронизировал, флаговый путь — нет; теперь расхождение отвергается с обеими починками.
      Проба namespace отвечает «отсутствует» только на однозначный 404: `Namespace` — объект
      cluster-scoped, и отказ RBAC не должен читаться как отсутствие, иначе namespace-ограниченный
      читатель теряет команды, на которые имеет право.
- [x] **e. `apprafter top`** — capacity / allocatable / requested / schedulable / in-use / free по
      памяти, CPU и диску, с разбивкой на платформу, интегрированные сервисы и приложения.
      IN-USE измеряется: `metrics.k8s.io` для CPU и памяти, kubelet Summary API для диска; где
      источника нет — ячейка `—` и предложение-ошибка кластера под таблицей, но НИКОГДА не `0`.
      Мультинодовость с самого начала; общий итог отсутствует, а не занижен, если хоть одна нода
      не измерилась. Диск — `ephemeral-storage`, то есть корневая ФС целиком, поэтому IN-USE ноды
      законно больше суммы колонки `DISK USE` (только writable layer, логи и emptyDir подов);
      вывод говорит это прямым текстом, а не оставляет вычесть. Строка `other` в норме пуста и
      существует, чтобы сдвинувшийся классификатор объявлял себя, а не терял поды молча.
- [x] **f. Слой отображения.** `cli_core::timefmt` (абсолютное + относительное) и
      `cli_core::quantity` (парсер и человекочитаемый рендер величин) — оба в `cli-core`, потому
      что формат был приватным для `platform.rs`, и именно поэтому не расходился.
      Переведены: `target domain list` (`addedAt`), `secret list` (`SEALED`), история ревизий и
      строка пина в `app status`, таймстамп в тегах `backup list`. Пункты `cli-fixes.md`: домен
      приложения и читаемые размеры VPA в `app status`, сортировка `backup list` от нового к
      старому. Namespace в `app list` и multi-app закрыты ещё в `b`; времени в `app list` больше
      нет вовсе — колонка ушла вместе с PROJECT/REV.
      Попутно: подсказка VPA про uncapped жёстко называла `resources.limits.memory` **изнутри
      цикла по ресурсам**, то есть при упёршемся CPU советовала поднять память.
      Аудит нашёл меньше мест, чем оценка «восемь»: `resolvedAt` уже рендерился относительным
      возрастом, `platform status` уже шёл через общий формат, а чтения `creationTimestamp`
      в `cluster-bootstrap` — арифметика, а не вывод.

> **Ручной прогон по треку 2.27 ещё не делался.** `c`–`f` поставлены и зелены на юнит-уровне
> (включая мутационные тесты на каждый нетривиальный гард), но ни одна из команд не исполнялась
> против кластера, а `top` вдобавок зависит от двух grant'ов, которые подтверждаются только живьём:
> `metrics.k8s.io` и `nodes/proxy`. Строки в `docs/status.md` до прогона — `🚧`, не `✅`.

> **Долг из 2.27a, едет со следующим функциональным изменением cue-cmp (не отдельным релизом).**
> `bundle_refuse` в `argocd-cue-cmp/entrypoint.sh` сознательно НЕ печатает строку «проверьте
> локально через `apprafter app validate`»: в 2.27a её убрали, потому что локальных двойников
> четырёх bundle-проверок не существовало и команда отвечала `✓ valid` ровно на то, что сайдкар
> отказывал. С b-3 (`v0.2.69`) они есть, и строку надо вернуть. Это правка chart-source ⇒
> принудительный бамп `argocd-cue-cmp` + `platform-stack`, поэтому по правилу «косметику не
> релизим отдельно» она ждёт ближайшую функциональную правку сайдкара — `c`–`f` её не затрагивают,
> так что метка живёт здесь.

**Вне объёма 2.27:** soft-condition `HostnameUnique` в операторе (ADR 0063, строка I5) — это
свойство **кластера**, а не бандла: два бандла из разных репозиториев сталкиваются так же.
Нужен оператор + ветка в Lua-healthcheck чарта, то есть свой план и своя релизная цепочка.
Автоматический ролл по хешу секрета (ADR 0064, alternatives) — отложен к Tier 2+ как opt-in;
он не заменяет глагол, потому что таймер не знает, где кончается последовательность правок.

---

### 2.28 Поверхность манифеста: пробы и тюнинг JetStream

> 🏁 SR: A · order 3.7e — открыто 2026-09-15 по двум находкам владельца в манифесте

> 🚧 **В РАБОТЕ.** Трек A закрыт с зелёным живым прогоном; у трека B нет прогона (см. ниже).
> Релиз не резан — 2.28 и 2.29 едут одним паком, потому что обе меняют CRD `Application`,
> а два CRD-апгрейда подряд на живом кластере — риск без выгоды.

**ADR:** 0065 (два раздела; раздел про jetstream — амендмент к ADR 0061 §6).
**Спек:** `docs/superpowers/specs/2026-09-15-2.28-probes-and-jetstream-tuning-design.md`.
**План трека A:** `docs/superpowers/plans/2026-09-15-2.28a-probes.md`.
**Замеры:** `docs/measurements/2.28-jetstream-2026-09-15.md` (nats-server 2.14.3).

- [x] **a. Пробы.** `#Probes`/`#Probe` на `#ApplicationSpec` и `#ApplicationEnvOverride`; форма
      разводится наличием `path` (есть — HTTP GET, нет — TCP connect), а не k8s-овским вложением
      `httpGet`/`tcpSocket`: та форма несёт пять видов действий, эта — два. Тайминги целыми
      секундами под именами k8s, чтобы `"500ms"` было невыразимо. Новый модуль
      `operator-rendering/src/probes.rs`, `merge_probes` в семью частичных override'ов 2.16c, пять
      правил вебхука, строка эффективной конфигурации в `app status`, блок в шаблоне `app scaffold`,
      фикстура `examples/applications/probes-app.cue`, регрессия на живом apiserver в
      `scripts/validate-crds.sh` (поля не прунятся + паттерн `^/` живой).
      **Два поведения платформа добавляет сверх объявленного** и оба названы в ADR: TCP-readiness по
      уже объявленному `expose.port`, когда readiness не объявлена (иначе каждый роллаут даёт окно
      502), и startup-проба, выведенная из liveness с собственной каденцией 5s/60 — период liveness
      отвечает на вопрос «как быстро поймать зависание», а не «сколько терпеть старт».
      **Прогон `e2e/probes-walk.sh` GREEN 8/8** на kind под rootless podman: readiness проверяется
      на `Endpoints`, а не на condition; liveness перезапускает **заклиненный** процесс, пока
      readiness на другом пути продолжает проходить; у фазы про выведенную startup есть контрольный
      случай, без которого она доказывала бы только «приложение успело».
      **Сознательно вне прогона** (названо в шапке скрипта): admission-webhook (его чарту нужен
      cert-manager) и любая команда CLI (все `app`-глаголы резолвятся через регистрацию Argo CD).
- [x] **b. Тюнинг JetStream + DLQ — код.** 12 полей стрима и 21 поле консьюмера, каждое 1:1 на
      одноимённое поле NACK, с проверенными по CRD написаниями перечислений (`byStartSequence`, не
      `by_start_sequence`). Одиннадцать полей **объявлены ради отказа** (шесть изоляционно-опасных
      + весь push-контур): структурная схема прунит неизвестное поле ДО вебхука, поэтому
      `mirror:` иначе исчезал бы молча. `deadLetter` разворачивается в **обычный declared stream**
      через единый список `materialised_streams`, который читают и `claim_view`, и реконсайлер —
      иначе DLQ существовал бы в NATS, а allow-list, deny-вектор, сумма квоты и детектор захвата о
      нём бы не знали. `stream_object`/`consumer_object` приняли типизированный аргумент вместо
      дюжины позиционных. `e2e/jetstream-nack-shape-check.sh` **GREEN** — все 31 поле принимает и
      хранит настоящий CRD запинённого nack 0.35.0.
      **Замеры изменили две записи ADR:** сообщение в `workqueue` после исчерпания `maxDeliver`
      **не удаляется** (тело достаётся через `STREAM.MSG.GET`, консьюмер идёт дальше), и
      `maxDeliver > len(backoff)` — **строго** больше (10116), где черновик писал «ограничен».
      Третий замер дал второе серверное кросс-правило: `discardPerSubject` требует `discard: new`
      **и** `maxMsgsPerSubject > 0` (10052).
- [x] **c. Живой прогон трека B — GREEN 21/21.** `needs-jetstream-walk.sh` вырос с 17 до 21
      проверки: тюнинг консьюмера и стрима читается с ЖИВОГО сервера, DLQ подтверждается как
      обычный declared stream, собирающий ровно advisory этой пары, и он **наполняется** после
      исчерпания редоставки. Строка в `docs/status.md` поднята до `✅`.
      **Поправка к более раннему утверждению в этом же файле:** я записал, что прогон блокирован
      потребностью в Cilium. Это неверно. `needs-jetstream-walk.sh` поднимает обычный kind с
      kindnet, bootstrap идёт с `APPRAFTER_BOOTSTRAP_SKIP_CILIUM=1`, а CNP там — инертный объект
      под stand-in CRD (сам файл это и говорит, прямым текстом). Ошибка возникла из грепа по
      СПИСКУ файлов на `kind_up_cilium`: совпадение было комментарием, объясняющим обратное, и я
      его не открыл. Прогон запускается на хосте.

> **Прогон нашёл мёртвый ключ, и это главный результат трека.** `consumerLimits` объявлялся,
> хранился и НИЧЕГО не делал: CRD его принимал, провижионер эмитил, NACK отвечал 200, а живой стрим
> показывал `consumer_limits: {}`. Причина не в сервере — она в NACK: запинённый
> `natsio/jetstream-controller:0.24.0` запускается в LEGACY-режиме (`--control-loop` по умолчанию
> `false`), а его строитель мапит сорок с лишним полей стрима и `ConsumerLimits` — ноль раз.
> Реализация есть только в новом controller-runtime, недоступном без флага, который сам лог NACK
> называет экспериментальным. Проверил так же остальную поверхность: **все** прочие поля стрима и
> **все** поля консьюмера в legacy-строителе есть; мёртвый ключ ровно один, и он переведён в
> declare-to-reject с отказом, называющим работающую альтернативу.
>
> **Методологическая половина важнее.** Пропустили три гейта, и все три отвечают на вопрос «принято
> ли поле»: CRD хранит, NACK-CRD хранит (`jetstream-nack-shape-check.sh` ровно это и утверждал),
> apiserver отвечает 200. Ни один не отвечает «читает ли это кто-нибудь». Round-trip — не доставка.
> Shape-check теперь несёт это ограничение в собственной шапке, а проверка №19 инвертирована:
> дешёвая ошибка здесь — вернуть ключ, увидеть, что CR его прекрасно хранит, и больше не посмотреть
> на живой стрим.

> **Попутно закрыты две дыры в самих гейтах**, обе найдены тем, что в них упёрлись.
> `crdgen` молча пропускал вид без Rust-арма (`_ => return None`), из-за чего новый CRD прошёл ось
> Rust↔CUE не будучи проверенным ни разу. `WORKSPACE_SCHEMAS` — рукописный список, чей комментарий
> обещает зеркало каталога `schemas/v1alpha1/`, но ничто это не сверяло; пропущенный файл **не
> ломает сборку**, он ломает docs-гейт сообщением про нашу же внедрённую копию, которое читается
> как сломанный манифест пользователя. Обе теперь с тестом, обе мутационно проверены.

> **Нейминг `probes`, а не `healthcheck`** — поднято владельцем, обсуждено, оставлено. `healthcheck`
> это канон Docker/Compose, `probe` — канон Kubernetes; решает то, что **содержимое блока уже
> k8s-ное** (целые секунды под именами `periodSeconds`/`failureThreshold`), поэтому докеровское
> существительное вокруг кубовых полей поместило бы несогласованность внутрь одного блока. Плюс
> единственное число держало бы три вещи с разными смыслами. Отложенное упоминание
> `spec.healthcheck` в 1.79b Part 4 этим отменяется.

---

### 2.29 Общая база данных и расширения PostgreSQL

> 🏁 SR: A · order 3.7f — открыто 2026-09-15; отдельный ADR и отдельный пункт по решению владельца,
> **релиз общий с 2.28**

> 🚧 **В РАБОТЕ.** Поверхность манифеста, валидация и SQL готовы; рантайма нет — общую базу пока
> ничем не создать.

**ADR:** 0066 (следует ADR 0049 и расходится с ним ровно в двух местах, оба аргументированы).
**Спек:** `docs/superpowers/specs/2026-09-15-2.29-shared-database-and-pg-extensions-design.md`.
**Замеры:** `docs/measurements/2.29-shared-database-2026-09-15.md` (PostgreSQL 18.4, Dragonfly v1.37.0).

- [x] **a. Схема и валидация.** CRD `SharedDatabase` (namespaced, свой жизненный цикл);
      `ref`/`access`/`extensions` на общем `#ServiceNeed` — это ось, которую ADR 0049 §2 сам назвал
      кросс-секущей; `sharedRef`/`access`/`extensions` на `ResourceClaimSpec`. Ссылочный need
      **порождает тот же claim**, что и собственный (в отличие от `needs.disk.ref`, который не
      порождает никакого): у привязки к базе есть что провижионить на потребителя — роль, пароль,
      секрет, — и это оставляет `claim.<type>.*`, egress-правило, гейт готовности и GC без единой
      правки. Правила вебхука: `ref` несовместим с `size`/`persistent`/`selector`, `access` без
      `ref` отклоняется (проигнорированный уровень доступа — это приложение, считающее себя
      read-only и не являющееся им), `ref` на неимплементированном типе отклоняется, `ref` с `/`
      получает отдельное сообщение про неймспейс. Расширения ограничены **allow-list'ом**, а
      требующие `shared_preload_libraries` отказываются **отдельной причиной**.
      В статусе `SharedDatabase` **намеренно нет `connectionSecretRef`** — у каждого потребителя
      свой кред, и общий секрет там был бы ровно тем, против чего CRD затевался; на отсутствие
      есть тест.
- [x] **b. SQL ролевой модели.** Чистые строители `shared_pg.rs` + `e2e/shared-pg-sql-check.sh`,
      который исполняет **их собственный вывод** на живом PostgreSQL 18.4 и заканчивается не на
      «операторы прошли», а на свойстве: таблица, созданная одним rw-потребителем, принадлежит
      **группе**; второй rw-потребитель её читает и пишет; ro читает, а его `INSERT` отклоняется;
      платформенная роль по-прежнему не суперпользователь. **GREEN.**
- [x] **c. Расширения для СОБСТВЕННОЙ базы — замкнуты.** Манифест → allow-list вебхука → claim →
      `Database` CR → проверка доступности на живом сервере → условие. `pgvector` работает через
      `needs.pg.extensions` без всякой общей базы. Список эмитится только когда он непустой: пустой
      список — это утверждение «у этой базы не должно быть расширений», которое подралось бы с тем,
      что пользователь создал руками; отсутствие ключа означает «манифест ими не управляет».
      Проверка доступности спрашивает **живой сервер**, а не выводит из allow-list: существует ли
      `vector` — свойство ОБРАЗА, образ не запинён, и бамп CNPG может унести его из-под живой базы.
      Отказ подключения при этом **не** валит claim — недоступный сервер и отсутствующее расширение
      это разные находки, и ронять claim на сетевом моргании было бы неверно.
- [x] **d. ACL общего кэша + pg-клиент.** Общий канальный префикс (каналы не скоупятся по `$N`,
      поэтому пер-юзерный префикс оставил бы двух потребителей одного кэша без общего pub/sub) и
      явные `-publish -spublish` у ro — потому что **измерено**: `-@write` их не запрещает.
      `pg_client.rs` по образцу redis/nats-швов; ни один вариант ошибки не несёт ни текста
      оператора, ни DSN — строка bind'а содержит пароль, поэтому сбой называется **индексом** в
      батче. `e2e/shared-pg-sql-check.sh` вырос до шести шагов и гоняет клиент против того же
      контейнера: psql не может наблюдать ни секвенирование батча, ни атрибуцию по индексу.
- [x] **e. Рантайм — прогон GREEN 67/67.** Контроллер `SharedDatabase` (обе ветки: pg через
      группы + CNPG `Database`, redis через собственный `$N`), путь привязки claim'а по `sharedRef`
      (обе ветки), CLI `apprafter db create/list/status/rm`, триггер ADR 0052 #17 на первый бинд
      пары + отдельный #17b на эскалацию `ro→rw`, RBAC в том же коммите, фикстуры,
      регрессия `crd-validate`, `e2e/shared-database-walk.sh` — **GREEN, 67 проверок, 0 отказов**.

> **Восемь дефектов, все молчаливые.** Шесть найдены сверкой кода с его же комментариями и
> обещаниями ADR, два — живым кластером. По убыванию последствий: реапер шёл на dwell, чтобы
> удалить CNPG-кластер под живой общей базой (каждая инвентаризация «этим кто-нибудь пользуется»
> написана в терминах claim'ов — аллокатор `$N`, снапшот `RetainedClaim` и реапер; общая база
> невидима всем трём); `drop_groups` не мог отработать никогда, потому что `DROP OWNED BY`
> действует в пределах одной БД, а путь удаления снимал финализатор в том же проходе — группы
> утекали **при каждом** удалении; `unbind_consumer` был написан, покрыт юнит-тестом на форму строк
> и **не имел вызывающего**, так что «отзыв доступа на приложение» был ложью, а когда вызывающий
> появился — падал, потому что `REASSIGN OWNED BY` требует членства, а не admin option (тот же
> класс, что уже исправил §3.1 ADR, одной функцией позже); allow-list расширений не применялся к
> `SharedDatabase` вовсе, и `--extension dblink` доезжал до superuser'ного `CREATE EXTENSION`;
> запись статуса на путях отказа **стирала** backing под SSA, из-за чего удаление во время сбоя
> бэкенда пропускало дроп базы; вебхука на удаление привязанной базы не существовало, хотя ADR §6
> его требует, — объект молча вис в Terminating; на свежем кластере pg-ветка 404-ила на
> CNPG-кластере, который создал бы собственный claim, и докладывала это **отсутствием статуса**.

> **`unbind_consumer` и `drop_groups` не исполнялись нигде** — написаны, покрыты юнит-тестами на
> форму строк, оставлены без вызывающего, и оба оказались сломаны, когда вызывающий появился.
> `e2e/shared-pg-sql-check.sh` вырос до восьми шагов и гоняет теперь и их: цикл диагностики минута
> вместо двенадцати, и именно он назвал обе причины, пока прогон мог сказать только `db error`.

> **Половина потерянных прогонов — дефекты самого прогона, и они дороже багов.** Автосинк Argo CD
> отключался ПОСЛЕ сборок образов и все эти минуты откатывал веточные CRD и ClusterRole; веточный
> `ValidatingWebhookConfiguration` не применялся никогда, так что новое правило вебхука было
> невидимо; `GROUPS=` — встроенная переменная bash, и проверка сверялась с GID; диагностика читала
> лог **умирающего** пода. Каждый симптом читался как баг продукта. Записано в
> `reference_walk_shell_assertion_traps`.

> **Аллокатор `$N` не видел общих баз, и это была потеря данных, а не неточность.** Общий кэш
> держит свой `$N` в СОБСТВЕННОМ статусе — у его потребителей `sharedRef` и никакого `dbnum`, —
> поэтому `used_dbnums`, читавший только claim'ы и снапшоты, выдал бы тот же номер следующему
> собственному redis-claim'у и посадил бы двух несвязанных арендаторов в одно пространство ключей,
> где GC любого из них делает `FLUSHDB` другому. Срез `SharedDatabase` сделан **обязательным**
> параметром, а не дефолтным: компилятор задаёт этот вопрос на каждом будущем месте вызова.

> **Паттерн имени расширения расходился с allow-list'ом вебхука.** Первый черновик запрещал `-`,
> а в allow-list'е двумя файлами дальше лежит `uuid-ossp` — apiserver отказывал в стандартном
> contrib-расширении, которое платформа в другом месте разрешает, и отказывал ошибкой паттерна,
> не называющей причину. Обе стороны теперь проверяются в `crd-validate`.

> **Поднятие pool-инстанса вынесено в одну функцию.** Общий кэш занимает `$N` на ТЕХ ЖЕ
> инстансах, что и собственный claim; две реализации означали бы, что вторая по порядку применит
> `Dragonfly` CR, противоречащий первой, и прокрутит StatefulSet под живым арендатором — ровно тот
> найденный прогоном отказ, из-за которого посев ACL-файла стоит перед созданием CR.

> **Замер опроверг ролевую модель, и это записано в ADR как исправление.** Черновик §3.1 поручал
> `managed.roles` создавать групповые и потребительские роли. Не работает: роль с `CREATEROLE` без
> суперпользователя администрирует только то, что создала сама, поэтому ни
> `ALTER ROLE … SET ROLE`, ни `CREATE ROLE … IN ROLE <группа>` над чужими ролями не проходят;
> а владеть базой от имени группы можно только будучи её участником **с SET**. Итог: CNPG создаёт
> одну роль, остальное — провижионер, и управление паролями потребителей общей базы уезжает из
> CNPG. Уцелела та половина, которую замер подтвердил прямо: участнику группы
> `ALTER DEFAULT PRIVILEGES FOR ROLE <группа>` разрешён.

> **Ещё один — в коде, а не в ADR.** Строители сначала эмитили `PASSWORD $1`. Обе половины
> невозможны: `DO`-блок не принимает параметров, а `CREATE ROLE` нельзя подготовить вообще. Пароль
> интерполируется через `quote_literal`, блок помечен тегом `$apprafter$`, чтобы пароль с `$$` не
> закрыл тело, и в модуле записано, что строка bind'а **несёт кред и не подлежит логированию**.
> Ни один ассерт на содержимое строки этого бы не поймал — поймало исполнение.

> **Вне объёма 2.29** (и записано в спеке): межнеймспейсный шеринг; третий уровень доступа `owner`;
> произвольный SQL из манифеста; пиннинг образа операнда CNPG — он ничего здесь не гейтит, а
> закрывает другую опасность (бамп CNPG может сдвинуть **мажор** под живым каталогом данных), и это
> отдельный пункт, а не довесок к фиче, которая от него не зависит.
...
> 🏁 SR: B · order 5 — backups to external S3

> ✅ **Ядро закрыто ранее в 2.6d-4 (2026-07-17):** scheduled off-site S3 push зашифрованного restic-репо (данные + CR + секреты + manifest) через AppRafter CronJob-restic (`apprafter-backup` runner). **Opt-in** (не «default ON» — zero-forced-purchase, ADR 0050 / [[project_2_6d4_s3_backup]]); Velero отклонён (ADR 0050 Alternatives). Остаток 4.12 = per-компонентные continuous-backup хвосты (ниже), НЕ restic-репо push.

**Поставка (остаток — per-component continuous backup, поверх 2.6d-4 restic-push):**
- [ ] CNPG continuous backup в S3 (Barman PITR — комплемент к logical `pg_dump` в restic-репо).
...
> Ранняя (Tier-1 / early-adopter) реализация этого restore-механизма уже поставлена в **2.6d** (`export` + restic backup + `restore --into <fresh-cluster>`, auto-register + re-seal) — это тот самый restore-путь, который acceptance выше подразумевает. 4.12 добавляет *automated push в external S3 + scheduled runner* поверх него (2.6d-4 = его opt-in предтеча).
...
> **2.6d — движок, который этот CRD обёртывает.** Shipped 2.6d (`export`/`backup`/`restore`, restic-движок, restore-ordering + re-seal) — исполнительный слой; будущий `DisasterRecoveryPlan` декларативно оркестрирует его вызовы под каждый scenario ниже (полная потеря кластера → `restore --into <fresh>`; corruption pg → data-restore в существующий кластер). CRD не переизобретает restore, а сшивает шаблоны + расписания вокруг него.
...
### ∞.10 Inline sealed env value — второй режим доставки секретов на T1 (BACKLOG)

> 🏁 SR: C — defer. Идея хорошая, но это заметный объём доп.работы ради T1; не блокирует launch.
> Контекст: разбор D6/D14 (`docs/measurements/day2-followups.md`) + поправка к ADR 0007 (2026-08-30).

**Суть.** Четвёртая форма значения `env`: не `Literal`, не `claim.<...>`, не `secret:"name/key"`,
а **шифротекст прямо в манифесте приложения**, расшифровать который может только кластер.

**Зачем.** Форма доставки определяет, какой контроль вообще доступен:

| форма | ревьюится? | доступный контроль |
| --- | --- | --- |
| `apprafter secret seal` (императивно) | нет, непрозрачный блоб | только **раскрытие** (поправка к ADR 0007) |
| inline шифротекст в манифесте | да — виден факт и переменная, не значение | **гейт через MigrationPlan** |

Сеал отревьюить нельзя по природе; инлайн-шифротекст — можно, потому что изменение становится
диффом спеки, который машинерия MigrationPlan и так видит на каждом реконсиляе. Ровно ту же
осторожность существующий код уже проявляет: `from`/`to` в `DestructiveChange` несут только
сентинелы, никогда значения.

**Ключевое упрощение (владелец, 2026-08-30): это сахар, а не новый криптопуть.** По сути форма
эквивалентна тому, как если бы в одном манифесте/репо лежали `kind: SealedSecret` с шифротекстом
И апп-манифест, который тут же ссылается на него через `ref`. Значит оператор ничего не
расшифровывает: он рендерит SealedSecret с производным именем + обычный `secretKeyRef`, а дальше
работает штатный sealed-secrets-контроллер. Отсюда — и strict scope сохраняется бесплатно
(производное имя `<app>-env-<VAR>` даёт настоящую пару `(namespace, name)`), и весь существующий
путь (`EnvSecretMissing`, рендерер, резолв ref'ов) не меняется.

**Минус (владелец): таким секретом нельзя пошариться между приложениями.** Владелец ровно один.
Это цена — и одновременно свойство: радиус поражения ровно одно приложение, тогда как именно
неизвестный радиус (несколько апп на один секрет) сделал автоматический роллаут небезопасным в D6.
Для секретов в этой форме автоматический роллаут снова становится обсуждаемым.

**Вторая цена:** ротация = коммит + синк вместо одной команды. Для «утёк ключ» это медленнее.
Поэтому это **второй режим, а не замена** — императивный путь остаётся нужен и для платформенных
кредлов в `apprafter-system`, у которых манифеста приложения нет вообще.

**Поставка (когда дойдут руки):**
- [ ] CUE `#EnvValue` + `EnvRef` — четвёртый вариант (`sealed`), CRD через `crdgen`, webhook-валидация формы.
- [ ] Рендер: производное имя + SealedSecret-объект + `secretKeyRef` (оператор не расшифровывает).
- [ ] CLI: получить шифротекст для вставки в манифест (переиспользовать путь `secret seal --stdout`).
- [ ] `ApplicationMigrationStrategy`: новый триггер, классификация `security-boundary`, симметрично
      `env-secret-ref-retarget`; `from`/`to` — сентинелы, не шифротекст.
- [ ] Per-env: оверрайд каждого окружения несёт свой блоб под своим производным именем.
- [ ] Walk: правка блоба в манифесте → план создан → апрув → под поднялся с новым значением.

**Не делать:** ослаблять scope SealedSecrets до namespace-/cluster-wide ради «инлайновости» —
блоб станет переносимым в чужой манифест, что подорвёт ровно тот гейт, ради которого всё затевается.

---

## История изменений плана

> Вынесено в отдельный файл: [`docs/changelog/plan-history.md`](docs/changelog/plan-history.md) — plan-level ledger на English (одна строка на под(под)фазу / патч; ранее таблица жила здесь). Детализация по релизам — `docs/changelog/history.md`.
