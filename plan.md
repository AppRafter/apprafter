...
- [ ] **2.23b — установка, которая работает** · лендинг + дока, без bump CLI
  - [ ] Раздел скачивания на лендинге, чтобы не гнать посетителя на GitHub (`:24`): секция
        `#download` на `/` + тонкая страница `/download`, чтобы у доки был стабильный URL.
  - [ ] `landing/web/public/install.sh` — Astro отдаёт `public/` как есть, значит
  ...
  - [ ] **Резолв версии — не деталь реализации, а суть.** Монорепо публикует пять серий
  ...
  - [ ] Версия на странице не запекается в `dist/`: резолвится на клиенте тем же фильтром.
  ...
  - [ ] Из доки вести на лендинг: `dev-guide/quickstart.md:28`, `operator-guide/quickstart.md:40`,
