...
- [ ] **2.22a — преflight и таксономия ошибок CLI** (D11) · CLI-only, без цепочки оператора
  Самое дешёвое из тяжёлых и единственное, на что владелец напоролся лично.
  - [ ] `preflight_tool(...)` в начале КАЖДОЙ команды, которая спавнит внешний бинарь — до
  ...
  - [ ] Порядок, худшее первым: `restore --reprovision` (гейтит пароль НАМЕРЕННО, с
  ...
  - [ ] Тест на команду: до преflight'а нет ни промпта, ни вызова провайдера.
  - [ ] `ExternalToolNotFound { tool, needed_by, min_version }` + фраза, которая важна после
        промпта: **ничего никуда не отправлено и пароль не использован**. Затем `Kubectl` с
        классификатором stderr (недоступен / запрещено / нет CRD) и `Restic` (неверный пароль
        ≠ сломанный репозиторий).
  - [ ] `doctor` несущий: FAIL вместо WARN для бинаря, без которого CLI не работает (сейчас
