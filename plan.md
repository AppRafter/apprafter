...
- [ ] **`recovery.md` несёт целиком записанное решение** — проблема, решение, последствия,
      отложенное на T3/T4 — в гайде, без ADR и без страницы механики. Это ADR, который не завели.
...
- [ ] **`## Prerequisites` стоит предпоследним** на четырёх страницах зависимостей — после
      Troubleshooting, — хотя предпосылки несущие: строка 214 из 231 и подобные.
- [ ] **Директивы `<!-- docs: … -->` уезжают в публикацию.** `llm_export.py` снимает только
      front matter, поэтому внутренние директивы гейта попадают в markdown-двойники и
      `llms-full.txt`.
- [ ] **`READER_FACING_TREES` слишком щедр для `publish-the-docs-site.md`** — единственной
      страницы, которая эти деревья использует. Либо сузить до пофайловых исключений, либо
      признать страницу контрибьюторской и увести из гайдов (что уже сделала навигация, и это
      расходится с индексом операторского гайда).
- [ ] **Лендинг продаёт как Phase 3 то, что уже отгружено** — «automatic backups to external S3».
      Правка маркетингового текста — за владельцем.
