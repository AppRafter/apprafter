...
> 🚧 **В РАБОТЕ — из десяти подфаз закрыты ДВЕ (2.19a + 2.19b).** Дизайн: `docs/superpowers/specs/2026-08-17-documentation-system-design.md` (rev 2 — переписан после 4-линзового adversarial-ревью: 8 P0 + 40+ P1, три собственных фактических утверждения спека оказались неверны и исправлены). Ветка `feat/2.19-docs-system`. **ADR ещё НЕ ратифицирован** (спек §8 требует его перед 2.19c) — это первый долг трека. **Ни релиза, ни монорепо-тега:** поведение CLI не изменилось (lib-таргет + правки doc-comment'ов), чарты/оператор не тронуты, сайт всё ещё не публикуется (2.19g).
...
- [ ] **(j) Закрытие** *(= W9)*. Ручная прогулка по КАЖДОМУ гайду; `FEATURE_TRACKER.md` / `plan.md` / `plan-history.md`; ADR ратифицирован.
