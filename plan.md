...
- [ ] **2.22e — откат приложения по скользящему тегу** (D9) · оператор + chart + CLI
  - [ ] Хранить предыдущий дайджест: `StatusImage` уже несёт `{tag, resolved, resolvedAt}` →
        `previous` аддитивен. `status.lastAppliedSpec` НЕ помощник (в нём тег, а тег тот же).
  - [ ] **Откат обязан пиновать** — иначе следующий реконсиляй перерезолвит тег и накатит
        плохую сборку обратно внутри 60-секундного requeue. Значит это смена РЕЖИМА.
  - [ ] Глагол возврата на скользящую модель — **обязателен**, иначе `rollback` дверь в один
        конец из главной фичи.
  - [ ] Где живёт пин: `PlatformStack` кластерный (гит не оспаривает) — поэтому `platform pin`
  ...
  - [ ] `--to sha256:…` = дайджест образа, всё прочее = гит-ревизия (флаг уже занят ревизией).
  - [ ] Видимость: жёлтым в `app status` с версией удержания; список запиненных в
  ...
  - [ ] Walk: пуш второго образа в тот же тег → откат вернул первый **и держится минимум два
