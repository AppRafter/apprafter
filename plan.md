...
- [ ] **2.23i — правило записывается, иначе свип — это вкус** · ADR-first, блокер для 2.23n/o
  ...
  - [ ] Поправка к ADR 0058 (не переписывание — сам ADR требует именно поправок, `:240`): пятая
  ...
  - [ ] Зеркало в `docs/contributing/documentation-gate.md` (таблица классов `:55-69`), чтобы
        контрибьютор встретил правило там же, где остальные.
  - [ ] **Тест на абзац, применяемый по порядку, первое попадание выигрывает** — в
        ...
        или ADR → механика.
  - [ ] Четыре семейства случаев тест не решает и требуют вашего слова, а не правила. Их надо
        вынести отдельно и решить до свипа — иначе 2.23n встанет посередине.
  - [ ] **Область действия правила решается здесь же:** только «счастливый путь» гайда или весь
        файл. Сейчас корпус сам с собой не согласен — четыре переработанные страницы зависимостей
        убрали даже хвостовые `Related`, а `egress-policy.md:109` и
        `dev-guide/resources-and-autoscaling.md:383` их сохранили.
