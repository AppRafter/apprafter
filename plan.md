...
- [ ] **2.23l — гайд разработчика перестаёт дублировать операторский** · docs-only
  - [ ] `dev-guide/quickstart.md` §1 «Register a target and bring the cluster up» пересказывает
  ...
  - [ ] То же в §5 про домен: пересказ `connect-a-domain.md` и `cloudflare-origin-cert.md` с
        потерей шага DNSSEC, ожидания статуса зоны Active, параметров сертификата и способа
        выбраться из блокировки фаерволом.
  - [ ] Проверено и снимает опасение: `GUIDE_ALIASES` в `help_rot_test.rs` **не пинит** ни одного
        литерала к этой странице, так что обе секции свободно превращаются в ссылки.

- [ ] **2.23m — MigrationPlan доезжает до разработчика** · docs-only
  - [ ] Строка `MigrationPlan` не встречается в `docs/dev-guide/` ни разу, а все тринадцать
  ...
  - [ ] Важное уточнение, чтобы не искать несуществующий долг: в плане 2.23e этого **не было** —
  ...
  - [ ] Новая короткая страница (60–80 строк) в конце блока «опиши своё приложение», до шагов
