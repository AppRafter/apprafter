...
> **Попутная находка 2.6d-6 (не чинилась, решение за владельцем):** `find_claim_data_dir`
> (`restore.rs`) распознаёт per-claim-снапшот по каталогам `pg` / `redis` / `disk` — а экстрактор
> пишет тома в `volumes/`. То есть sequential-снапшот, содержащий ТОЛЬКО том, выглядит
> беспэйлоадным, и его данные не восстанавливает никто, при этом restore рапортует успех. В 2.6d-6
> в матчер добавлен `jetstream` (с тестом), `disk`/`volumes` не трогался — это отдельное решение
> со своим регрессионным тестом и, возможно, со своим walk'ом.
