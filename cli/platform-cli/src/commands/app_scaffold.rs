...
pub fn scaffold(opts: ScaffoldOpts) -> Result<()> {
    ...
    // 2.12g (ADR 0046 Decision #7): scaffold NO LONGER vendors the
    // schema (cue.mod/pkg/) into the user's repo. The schema +
    // generated `claim` binding are injected at render time by the
    // cue-cmp and at validate time by `apprafter app validate`, so
    // a vendored copy is pure clutter AND a version-drift vector.
    // A stale pre-2.12 vendored cue.mod/ left over from an earlier
    // scaffold would shadow the injected schema with `inject-wins`
    // anyway, but we proactively remove it so the repo stays clean.
    remove_stale_vendored_module(&apprafter_dir);
    ...
    println!("  2. Validate locally — `apprafter app validate`");
    println!("     (reproduces the cluster's render-time `cue` check, so");
    println!("     `claim.<type>.<field>` / `secret: \"name/key\"` refs resolve");
    println!("     exactly as they will at sync time; requires `cue` on PATH).");
...
/// Remove a stale vendored `cue.mod/` directory left by a
/// pre-2.12 scaffold (when scaffold vendored the AppRafter
/// schema under `apprafter/cue.mod/pkg/apprafter.io/schemas/
/// v1alpha1/`). ADR 0046 Decision #7 stopped that: the schema
/// is now injected at render time (cue-cmp) and validate time
/// (`apprafter app validate`), so the vendored copy is clutter
/// and a version-drift vector. Best-effort — failure to remove
/// (read-only fs, missing dir) is non-fatal: scaffold's primary
/// job is writing Application.cue, and a leftover cue.mod/ does
/// not break rendering (the cue-cmp injects inject-wins).
fn remove_stale_vendored_module(apprafter_dir: &Path) {
    let cue_mod_dir = apprafter_dir.join("cue.mod");
    if cue_mod_dir.is_dir() {
        let _ = std::fs::remove_dir_all(&cue_mod_dir);
    }
}
...
mod tests {
    ...
    #[test]
    fn scaffold_does_not_vendor_cue_mod() {
        // 2.12g (ADR 0046 Decision #7): scaffold stops vendoring the
        // schema. The user's repo carries ONLY apprafter/Application.cue
        // — no cue.mod/ tree. The schema + claim binding are injected at
        // render time (cue-cmp) and validate time (`apprafter app
        // validate`), so a vendored copy is both clutter and a
        // version-drift vector.
        let dir = tempdir().unwrap();
        fs::write(dir.path().join("Cargo.toml"), "[package]\n").unwrap();
        ...
        let apprafter = dir.path().join("apprafter");
        assert!(
            apprafter.join("Application.cue").is_file(),
            "Application.cue must still be written"
        );
        assert!(
            !apprafter.join("cue.mod").exists(),
            "scaffold must NOT vendor a cue.mod/ tree (ADR 0046 Decision #7)"
        );
    }

    #[test]
    fn scaffold_removes_stale_pre_2_12_vendored_cue_mod() {
        // Migration: a repo scaffolded before 2.12 carries a vendored
        // apprafter/cue.mod/. Re-running scaffold (e.g. --force) removes
        // it so the repo converges to the clean, no-vendor layout.
        let dir = tempdir().unwrap();
        fs::write(dir.path().join("Cargo.toml"), "[package]\n").unwrap();
        let apprafter = dir.path().join("apprafter");
        let stale_pkg = apprafter
            .join("cue.mod")
            .join("pkg")
            .join("apprafter.io")
            .join("schemas")
            .join("v1alpha1");
        fs::create_dir_all(&stale_pkg).unwrap();
        fs::write(stale_pkg.join("types.cue"), "// stale vendored\n").unwrap();
        fs::write(apprafter.join("Application.cue"), "// old manifest\n").unwrap();
        ...
        assert!(
            !apprafter.join("cue.mod").exists(),
            "re-scaffold must remove a stale vendored cue.mod/"
        );
        // The manifest was refreshed.
        let content = fs::read_to_string(apprafter.join("Application.cue")).unwrap();
