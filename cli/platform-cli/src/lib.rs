...
//! out-of-process tooling — today `docsgen` — can read the clap
//! definitions and reuse the CUE validation path instead of
//! re-implementing either. Everything else is unchanged: modules
//! under `commands/` keep resolving `crate::cli::…` exactly as
//! before, because `crate` is now this lib.
//!
...
pub mod docs_api {
    ...
    /// pipeline (post-2.12 there is no vendored `cue.mod`, so a bare
    /// `cue vet` of a user manifest fails with "no cue.mod").
    pub use crate::commands::app_validate::validate_manifest;
}

/// Parse argv and run the CLI. `main` is a thin wrapper over this so
/// the dispatch table lives next to the definitions it dispatches on.
pub fn run() -> Result<()> {
...
/// exactly once at the binary boundary ([`run`] above) and the
