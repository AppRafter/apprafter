...
//! `docs/**` holds 32 HTML comments and 28 are the identical docsgen
//! banner on the generated CLI reference (29 and 28 as the tree was
//! found at `dc4c5de`; the four added since are grammar examples in
//! the changelog and the contributor reference). [`parse_marker`]
//! returns `Ok(None)` — not an error — for every comment that does not
//! open with `docs:`, which is what keeps those 28 invisible here.
//!
