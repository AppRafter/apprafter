...
| `--path` | — | `/` | no | Path under the repo to render. Defaults to `/`, which lets the cue-cmp plugin walk the whole repository: its discovery command matches any `.cue` file under an `apprafter/` directory, or any file named `apprafter*.cue` anywhere. Both shapes work — the scaffolded `apprafter/Application.cue` is matched by the first |
