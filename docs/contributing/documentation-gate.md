...
| `adr-citation-placement` | An ADR cited anywhere in an operator- or developer-guide file, **or a link into `docs/adr/` including the index** — a pointer with no number is still the decision surface reached with the mechanism layer skipped. A guide answers *what do I run*; a decision's *why* is two surfaces out. Move the citation to the mechanism page the guide links, and link that. |
| `code-reference-placement` | A guide page reaching into the repository. Any **link** that leaves `docs/` — a GitHub `blob`/`tree` URL, or a relative target resolving out of the tree — because the published site has no such address. And a **code span** naming a path in an implementation tree, which is the same defect written without a link. A span opening on a tree the reader works in (`examples/`, `.devcontainer/`, `manifests/`, …) is left alone; so are the issue tracker, releases and third-party documentation. One page is exempt by name — the runbook for publishing this site is contributor material that the nav lists under Contributing while its file stays put. |
