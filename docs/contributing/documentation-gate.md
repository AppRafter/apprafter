...
| `code-reference-placement` | A guide page reaching into the repository. Any **link** that leaves `docs/` — a GitHub `blob`/`tree` URL, or a relative target resolving out of the tree — because the published site has no such address. And a **code span** naming a path in an implementation tree, which is the same defect written without a link. A span opening on a tree the reader works in (`docs-site/`, `landing/`, `examples/`, `scripts/`, `.github/`) is left alone; the issue tracker, releases and third-party documentation are not code. |
