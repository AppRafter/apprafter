...
Approving the plan starts the same seven-day retention path described below:
the new spec is applied, the claim this application no longer declares is
deleted, and the provisioner's finalizer writes the retention record on its way
out. The data and the credential are kept for those seven days, then flushed.
Nothing is deleted while the plan is still pending.
...
| Every application on the pool starts failing `WRONGPASS` or `NOPERM` at once, shortly after a restart | this should no longer happen: claim credentials are written to a file the instance loads at startup | If it does, the instance is not loading its file. Check that the CR points at one: `kubectl -n dragonfly-system get dragonfly <instance> -o jsonpath='{.spec.aclFromSecret.name}'`, and that the file carries the users: `kubectl -n dragonfly-system get secret <instance>-acl -o jsonpath='{.data.acl}' \| base64 -d`. The platform also re-asserts every credential on a periodic pass, so access returns within about five minutes regardless. |
