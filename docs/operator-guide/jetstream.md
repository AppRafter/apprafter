...
| The claim stays unready with `NatsStorageBudgetExceeded` | the namespaces using JetStream are jointly promised more of the shared message server's volume than it holds | The message names the promise and the budget. Unlike the memory budget above, a smaller declaration does help here: lower a `size` on any jetstream need in the cluster, drop JetStream from a namespace that no longer needs it, or move to a larger tier. The namespace on the claim need not be the one that filled the volume. Namespaces already on the server are unaffected and keep running. |
