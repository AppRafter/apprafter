...
what `apprafter target domain list` reports and what the operator's
`PublicRouteReady` verdict enforces, and both are covered by tests.

The listener pair itself is generated by the platform chart — one apex
listener and one wildcard listener per registered zone, both referencing
the same imported certificate.
