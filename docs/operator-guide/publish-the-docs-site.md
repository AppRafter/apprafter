...
the one imported certificate — which is why the certificate is minted
for `<zone>` and `*.<zone>` together
...
??? note "Checking that yourself rather than taking it on trust"
    ...
    **The wildcard is one level deep, in both layers.**
    ...
    The CLI and the operator each apply that same rule, on opposite sides
    of the cluster boundary:
    [the one-label rule](../how-it-works/the-public-route.md#which-applications-a-zone-covers).
...
the single most useful thing to read when the site does not answer. What
sets it, and what each outcome means, is
[The public route](../how-it-works/the-public-route.md#the-publicrouteready-verdict).
...
!!! warning "Any commit touching `landing/**` publishes the landing"
    ...
    added everything else, and could not be. Any push to the default
    branch whose diff touches `landing/**` bumps the patch of the newest
    `landing-v*` tag, pushes it, and triggers the release, which publishes
...
The one-label rule above is not a convention this page invented, and
neither is the listener pair: both are described against the code that
implements them in
[The public route](../how-it-works/the-public-route.md).
