...
- **ADR 0024** removes standing cluster-admin and replaces it with a defence-in-depth bundle: SPIFFE workload identity, OpenBao secret delivery, Kamaji control-plane separation, a two-person rule and just-in-time short-TTL grants for elevated host access, and an immutable audit pipeline. There is no standing god-mode credential for an agent to inherit.
