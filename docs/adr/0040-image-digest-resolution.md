...
- **Degradation — never block.** If resolution fails (registry unreachable, no covering credential for a private image, malformed reference), the controller renders the **verbatim tag** (today's behaviour) and sets a status condition `ImageResolved=False` with a reason. Resolution is best-effort; it never blocks the rollout.
  ...
  *Amended 2026-09-23.* Rendering the verbatim tag on a failure turned every registry blip into two rollouts of an image that had not changed: the pod template went from `repo@sha256:…` to `repo:tag` on the failed lookup and back again on the next success. A failed lookup now **keeps the digest already running for the same image reference**, and renders the tag as written only when there is none — a reference that was never resolved, or one the spec has just changed to. Which reference a running digest belongs to is recorded on the `Deployment` itself, in an `apprafter.io/image-ref` annotation written in the same apply as the image, so the two cannot disagree. The digest in `status.image` for the same reference is the fallback for a `Deployment` that carries no such record; it cannot be the primary source, because it is written at the end of a reconcile, and a reconcile that rolls out a moved tag and then fails leaves it naming the build from before the move. A kept digest must be a `sha256` reference into the tag's own repository. The rest of the decision stands: a moved tag is only learnt from a successful lookup, which rolls; a digest resolved for one reference is never rendered for another; `resolve: "off"` performs no lookup; and `ImageResolved=False` still reports the failure, now naming the digest it kept.
