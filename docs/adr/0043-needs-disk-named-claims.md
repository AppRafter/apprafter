...
The struct enumerates **all** `#PlatformServiceType` members (pg, jetstream, clickhouse, redis, s3, notifications) as `#ServiceNeed | [...#ServiceNeed]`; `disk` is the only key with the `#DiskClaim` shape. Wiring jetstream/s3/etc. through this format later needs no further change to the `needs` container.
...
On disk-claim delete, the provisioner finalizer snapshots a RetainedClaim (backend `disk`, carrying the PVC ref), retains 7 days, then `gc_backend("disk")` deletes the PVC (idempotent / 404-tolerant). Cancel-on-re-provision (2.4f Fix A) applies unchanged: re-deploying the app cancels the RetainedClaim, and `provision_disk` **reattaches** to the existing PVC (the dbnum-reattach analog, ADR 0042 §8 — but for disk the reattach is *unconditional*, since disk data always exists; the 0042 analogy is 1:1 only with redis's *persistent* branch, as an ephemeral redis claim is freed immediately), so disk data survives delete+redeploy within the grace window. The GC live-guard never drops a PVC whose claim is live again.
...
- No StatefulSet at launch → no headless-Service requirement and no StatefulSet update semantics; the Deployment renderer gains only a volume block + a `Recreate` strategy switch.
...
- **Always StatefulSet when disk present.** Simpler than a conditional pivot but imposes the headless-Service requirement and StatefulSet update semantics on the common single-replica case that a Deployment + RWO PVC handles cleanly.
...
- **Env-var foldability of `name`.** A claim `name` with characters invalid in an env-var segment (a hyphen: `my-cache` → `REDIS_URL_MY-CACHE`) would render an invalid `EnvVar.name`. Mitigation: the webhook validates `name` is env-foldable (DNS-1123 label) and the renderer folds `-` → `_` + uppercases — the same category as the existing `DATABASE_URL` reserved-env guard.
