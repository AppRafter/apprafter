...
## cli v0.2.41 — 2.16g follow-ups (node status via cached kubeconfig, rollback fault-hook) + release-lock fix (2026-08-11)
...
**Added (test-only):** an undocumented `APPRAFTER_NODE_SWAP_FORCE_INVALID_DROPIN` fault hook (out of `--help`) makes `node prep`'s first apply write a syntactically-invalid kubelet drop-in so the k3s restart times out and the whole-step rollback fires (the rollback's own rewrite stays valid, so a still-set hook can't sabotage recovery). This wires `e2e/node-swap-walk.sh` STEP6 (previously a documented SKIP) to exercise the induced-failure rollback on a live node against the OTHER-project retrofit node. The rollback state machine itself is unit-covered (2.16g Q9); the hook is inert unless the env is set. Delivers 2.16g follow-up STEP6.
