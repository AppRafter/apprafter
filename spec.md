...
A documented happy path is a happy path. An operator or developer guide's main flow carries `apprafter` commands and genuinely external tools only, never a raw `kubectl` step; independent verification, mechanism, and failure handling each route to their own place (ADR 0058).
...
- **Per-environment overrides, override-wins (ADR 0044, 2.9).** No template strings, no `{{ .Values.image }}` — the selected `environments[<env>]` merges onto `base` override-wins: `image`/`replicas` replace, `env` merges, **`expose`/`imagePolicy` deep-merge by subfield** (an env carries only the diff — env-invariant subfields such as `expose.port` inherit from `base`), **`resources` deep-merges as a MAP-of-map** (2.16d — `requests`/`limits` merge independently key-by-key, so an env override of `limits.memory` keeps the base `limits.cpu`), and `needs` replaces per key. Inherited `expose.hostname`/`tls` are consumed only under `expose.network: "public"` (inert otherwise). The env is a **deploy-time, per-CR scalar** (`Application.spec.environment`): `apprafter app add --env <env>` deploys the same manifest as one self-contained Argo CD Application `<name>-<env>` in a user-chosen namespace — not a single-CR fan-out (cross-namespace ownerReferences are disallowed).
...
The gated classes map to three consequence models: **`data-migration`** is the only class with real data loss (the backing `ResourceClaim` and its DB/volume are garbage-collected); **`security-boundary`** is an irreversible *exposure* of a repository, secret, or public surface — no data loss, but a rollback does not restore the prior state (a leaked credential stays leaked); **`requires-restart`** is everything else — a restart or availability degradation without data loss and without surface expansion.

**Structural hardening (ADR 0052).** Three defenses ensure the gate cannot be disarmed:

- **Approval is bound to a content hash.** The plan stamps `spec.trigger.approvedSpecHash` over the full detected change set; at consume time the operator re-verifies it, so an approval for one edit cannot be applied to a different edit, and any drift re-gates the change as a fresh pending-approval plan. An app-scope plan **requires a non-empty `approvedSpecHash` to consume**: a missing or empty hash is treated as no match and re-gates (never applies), so a forged or pre-2.16b-sec hashless plan can never apply a destructive change. The approval hash binds the gated candidate-set, not the full effective spec; soft (non-gated) changes may accompany the execution of an approved plan and do not trigger a re-gate — by definition a soft change applies without approval.
...
**Approve authority is separate from write authority (S-2, ADR 0052).** A manifest-write or status-write credential must **not** equal approve authority. The authoritative approve path is the Kubernetes status-subresource patch (`kubectl patch migrationplan … --subresource=status` → `phase: approved`), governed by RBAC on the MigrationPlan status subresource; the Argo CD "Approve" button is a convenience layered on top of Argo CD's own RBAC. The operator's SSA field manager is an **ownership label, not an authentication token** — its integrity rests on scoping `patch applications/status` and the MigrationPlan status write to the operator's ServiceAccount, so that no other subject can present it. Git write access alone never approves a plan. **Today** (Tier 1) the approve subjects are the admin-level CLI (`apprafter migration approve`, a full-kubeconfig status patch) and the Argo CD "Approve" resource-action, whose action writes **only `status.phase`** (F-1b-compatible) under Argo's controller RBAC. A named non-admin approver role (`apprafter:migration-approver`) and approve-attribution (`status.approvedBy`/`approvedAt` from the caller's identity) are **deferred to the managed track**, where a client approves through the hosted UI / MCP / outbound-agent and the real human/agent identity — rather than the Argo controller ServiceAccount the button acts as — becomes audit-critical.
...
1. Resolve the active environment (ADR 0044, 2.9): the env is a deploy-time **per-CR** property (`Application.spec.environment`, injected by the cue-cmp); the operator's `effective_spec` merges the selected `environments[<env>]` onto `base` **override-wins**: `image`/`replicas` replace, `env` merges, `expose`/`imagePolicy` deep-merge by subfield (2.16c — a partial env override inherits env-invariant subfields from `base`), `resources` deep-merges as a MAP-of-map (2.16d — `requests`/`limits` merge independently key-by-key), `needs` replaces per key — a pure-Rust merge for v1alpha1, switchable to CUE FFI once CUE-only constructs appear
...
**Resource governance (ADR 0053, 2.16d).** No pod (platform or application) is BestEffort. QoS is chosen per workload role from a real solo-node measurement baseline: stateful backends (CloudNativePG, Dragonfly) are rendered **Guaranteed** (`requests == limits`) with coherent in-process budgets (`shared_buffers` / `--maxmemory` pinned below the cgroup limit) so they are the last, not the first, evicted under node memory pressure; the app container and every platform component are **Burstable** — the app seed is a low memory request plus a generous-but-present memory limit and a CPU request with no CPU limit (an absent limit would break render determinism / Tier-2 well-formedness / local↔in-cluster parity; a low limit OOMKills common runtimes). An explicit `Application.spec.resources` is honored verbatim. No PriorityClass (the kernel OOM score is QoS-derived, not priority-derived); LimitRange is deferred to the Capsule policy layer where a ResourceQuota makes it mandatory. Because `k3s.service` runs in `system.slice` outside `kubepods`, the CLI/k3s-installer additionally reserves node headroom (`system-reserved` / `kube-reserved` / `eviction-hard` + unit `OOMScoreAdjust=-999`), applied at bootstrap and re-appliable to existing clusters via `apprafter node reserve-headroom` (§4.12).
...
- Replace `kubectl` / `helm` / `k9s` for platform engineers. They remain available, supported, and the interface for break-glass and deep inspection — but they are **not required** to install, configure, tune, or maintain AppRafter. Every routine operation has a first-class `apprafter` command, and the documentation reflects that (ADR 0058).
