...
| Status | Feature | plan.md | Checkpoint |
|---|---|---|---|
| ✅ | MCP-native safety gate: destructive operations are paused (`MigrationPlan` CRD), approve/reject via CLI | 1.72–1.78 condensed, 2.16b, 2.16b-sec | CP1 |
...
| ✅ | Data export + backup/restore (`apprafter export` / `backup` / `restore`, restic engine, local-pull default) | 2.6d | CP2 |
...
> **🚧 — seven of ten subphases closed (a–g), and what a user can see is still nothing.** The machinery half has landed: a validating strict site build (**2.19a**), a generated reference over all 95 command paths that is byte-compared against the clap tree (**2.19b**), a content-detected drift gate over the in-scope guides that resolves documented invocations, schema paths and CUE manifests against what ships (**2.19c**), its xref/health half — repository paths, ADR citations, obligations that survive unfencing a block, and a committed corpus census whose obligation counts may not fall (**2.19d**) — and the two mkdocs build hooks: CUE now renders as code, every page carries a one-line `description`, and the site publishes `llms.txt`, `llms-full.txt` and a markdown twin per page (**2.19e**).
>
...
>
> **2.19g built the whole publication path and the site is still unserved — the distinction is the point.** Shipped: an image serving a prebuilt site out of Caddy (the site is built in the workflow under the one pinned nix toolchain, never in the Dockerfile, so the published bytes and the gated bytes come from the same build), a deployable `Application.cue` on `docs.apprafter.dev`, and a release workflow that runs the documentation gate **itself** rather than trusting that a separate workflow ran on the same commit. What it could not do is anything outside the repository, and that is what stands between here and a reader: the push, a Cloudflare DNS record, one `apprafter app add`, and the CMS half of the landing switch. The tracked half of that switch is a **commit of its own** so it can be held back — landing it before DNS resolves would point the landing's front door at a host that does not answer, which is worse than the badge because it looks shipped.
>
> Remaining, and each of them user-visible in a way the seven above are not: the operator steps that actually put the site online and take the "Soon" badge off (the software for both is done — 2.19g), the CLI-UX examples and completions (2.19h), roughly **fifteen missing guides** for capabilities that exist only in `--help` today (2.19i), and the closing walk over every guide (2.19j). The row flips `✅` at 2.19j, not before: a documentation site nobody can open is not a delivered feature — and 2.19e is the sharpest case of that, since it publishes an LLM-readable corpus to a site no one can yet fetch.
>
> No release and no monorepo tag rides any of a–g. `docsgen` is a build-time crate that ships in no release artefact; a–b changed no shipped behaviour (2.19b added a lib target and a narrow two-item `docs_api` facade; 2.19a and the doc-comment audit touched help text), 2.19c widened the schema bundle `apprafter app validate` embeds, and 2.19d, 2.19e, 2.19f and 2.19g touched the shipped binary not at all — neither 2.19f nor 2.19g touched `cli/platform-cli` at any point, so `apprafter --help` and the byte-compared `commands.json` are unchanged. 2.19g adds a container image, `ghcr.io/apprafter/docs`, versioned by commit SHA rather than by a git tag, so publication introduces no tag stream of its own either.
...
| ✅ | Automatic backups to external S3 (opt-in scheduled off-site push; AppRafter CronJob-restic, `PlatformStack.spec.backup`) | 2.6d-4 / 4.12 | CP3 |
